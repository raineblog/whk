...
\hbadness=10000           % 屏蔽 Underfull \hbox
\vbadness=10000           % 屏蔽 Underfull \vbox
\hfuzz=\maxdimen          % 屏蔽 Overfull \hbox
\vfuzz=\maxdimen          % 屏蔽 Overfull \vbox

% 屏蔽所有 LaTeX 标准警告
\WarningsOff* 
% 屏蔽所有 LaTeX 建议/消息
\ErrorsOff[LaTeX] % 如果你连 LaTeX 核心错误都不想看（不推荐，但很彻底）

\directlua{
    % 屏蔽控制台输出的页码信息，例如 [1] [2]
    callback.register("start_page_number", function() end)
    callback.register("stop_page_number", function() end)
    
    % 屏蔽文件读写过程中的部分日志
    % 注意：完全屏蔽 start_run/stop_run 可能会导致严重错误难以调试，
    % 但这里可以屏蔽字体加载器的冗长日志。
    if luaotfload and luaotfload.log then
        luaotfload.log.set_loglevel(0)
    end
}
