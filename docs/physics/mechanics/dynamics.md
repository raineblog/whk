...
牛顿第一定律表述为：不受外力作用的物体必将作速直线运动或保持静止。

这条定律就其叙述的内容本身提出了力和惯性两个重要概念：牛顿把改变物体运动状态的因素归于物体受到了力，而运动物体自身又具有保持其匀速直线运动或保持静止的属性，并称此属性为惯性。

根据运动的相对性，对于不同的参照系，物体往往有不同的加速度，在一个参照系中作今速运动的物体，在另一个参照系中可能有加速度。
...
$$
\bm F=\lim_{\Delta t\to0}{\Delta\bm p\over\Delta t}
$$
...
\\[1.2em]
a(t)
=\lim_{\Delta{t}\to0}\frac{v(t+\Delta{t})-v(t)}{\Delta{t}}
...
一个物体的惯性质量决定它受力时的加速度。根据牛顿运动第二定律，假设一个物体受到一个力 $F$，其加速度为 $a$，则其惯性质量，
...
设参考系 $S'$ 相对于惯性参考系 $S$ 以不变的加速度 $\bm a_0$ 运动。
...
$$
\bm F=m\bm a=m\bm a'+m\bm a_0
$$

在非惯性系 $S'$，有动力学方程，

$$
m\bm a'=\bm F-m\bm a_0=\bm F+\bm F_惯
$$

其中 $\bm F_惯=-m\bm a_0$，称为惯性力，可以理解为受到与加速度方向相反的大小为 $ma_0$ 的力。

这个方程说明，如果把等式右边看成是质点在平动加速参照系 $S'$ 中受到的合力，那么在 $S'$ 系中仍能在形式上运用牛顿第二定律求解动力学问题。等式右边第一项 $F$ 为质点所受真实力的合力，第二项则是惯性力。

应该强调的是，惯性力并不是其它物体施于质点的真实的力，只是为了能在非惯性系中沿用牛顿第二定律的形式来处理问题而人为引人的虚拟力，也称虚拟力。
...
牛顿还考虑直线加速的一种情况：对于一组物体，无论它们相互之间如何运动，如果它们受到平行方向上的相同加速力，它们会继续保持相对运动状态，就像没有收到这种力一样。
...
$$
T=\dfrac{F_1m_2+F_2m_1}{m_1+m_2}T
$$
...
$$
f={m_1\over m_1+m_2}F-\mu mg\cos\theta
$$

若斜面是水平面（$\theta=0$），那么 $\cos\theta=1$：

$$
f={m_1\over m_1+m_2}F-\mu mg
$$
...
$$
F={m\over M}(m+M)(\mu_1-\mu_2)g\cdot\cos\theta
$$

若斜面是水平面（$\theta=0$），那么 $\cos\theta=1$：

$$
F={m\over M}(m+M)(\mu_1-\mu_2)g
$$

注意此形式下，需要上物体能拉动下午题，拉不动的话就更简单了。
...
\begin{aligned}
l&={1\over2}at^2\\
2R\cos\beta&={1\over2}(g\cos\beta)t^2
\end{aligned}
$$
...
$$
2R={OQ\over\cos\theta}={OH\over\cos^2\theta}={l\cos\alpha\over\cos^2(\alpha/2)}
$$

则：

$$
R={l\cos\alpha\over1+\cos\alpha}
$$

则最短时间：

$$
t=\sqrt{4R\over g}=2\sqrt{l\cos\alpha\over g(1+\cos\alpha)}
$$
...
$$
\begin{cases}
20\text{m}&=\frac{1}{2}a\cdot(2\text{s})^2\\
