...
我们由 $\ln x\le x-1$ 可以推导出来以下几个基本不等式：
...
```md {admonition="note" title="$p(x)=0$"}
...
```md {admonition="note" title="$q(x)=0$"}
...
代入指数形式，替换 $a\gets e^x,b\gets e^y$，得：
...
在前文隐零点我们说过，如果可以简单的分离、代入，那么容易解决，但是有的时候不能这么解决，此时问题就麻烦一点，我们以例题：已知 $f(x)=e^x-\ln(x+2)$，求证 $f(x)>\dfrac{1}{6}$ 恒成立。
...
- 对于构造变量，只需要按照原来的方法，代入不等式，只是解不等式可能有一些复杂。
