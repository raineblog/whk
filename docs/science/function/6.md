---
description: 本文系统梳理高中代数不等式刷题进阶方法，涵盖均值不等式、对勾函数、柯西不等式等核心技巧，详解一正二定三相等、拼凑构造、加权待定等解题思路，配大量例题，助力提升不等式证明与最值求解能力。
---
...
$$
\begin{aligned}
\dfrac{c}{x}+\dfrac{d}{y}&=\dfrac{1}{C}(ax+by)\left(\dfrac{c}{x}+\dfrac{d}{y}\right)\\
...
$$
\begin{aligned}
3x+4y&=\dfrac15(3x+4y)\left(\dfrac3x+\dfrac1y\right)\\
&=\dfrac15\left(13+12\dfrac yx+3\dfrac xy\right)\ge5
\end{aligned}
$$
```
...
$$
\begin{aligned}
\dfrac9x+\dfrac{16}{1-x}&=\left(\dfrac9x+\dfrac{16}{1-x}\right)[(x)+(1-x)]\\
...
$$
\left(\sum_{i=1}^nx_i^2p_i\right)\left(\sum_{i=1}^np_i\right)\ge\left(\sum_{i=1}^nx_ip_i\right)^2
$$
