...
- 若 $a_1 < 0, d > 0$，且此时 $n$ 满足 $\\begin{ a_n \le 0 \\ a_{n+1} \ge 0 }cases\\end{ a_n \le 0 \\ a_{n+1} \ge 0 }$，则 $S_n$ 有最小值；

- 若 $a_1 > 0, d < 0$，且此时 $n$ 满足 $\\begin{ a_n \ge 0 \\ a_{n+1} \le 0 }cases\\end{ a_n \ge 0 \\ a_{n+1} \le 0 }$，则 $S_n$ 有最大值。
...
- 对于首项小于 0 而公差大于 0 的等差数列 $\{a_n\}$ 加绝对值后得到的数列 $\{|a_n|\}$ 求和，设 $\{a_n\}$ 的前 $n$ 项和为 $S_n$，$\{|a_n|\}$ 的前 $n$ 项和为 $T_n$，数列 $\{a_n\}$ 的第 $k$ 项小于 0 而从第 $k+1$ 项开始大于或等于 0，于是有
    ...
    $$
    T_n = \\begin{ -S_n & n \le k \\ S_n - 2S_k & n > k }cases\\end{ -S_n & n \le k \\ S_n - 2S_k & n > k }
    $$
...
- 对于首项大于 0 而公差小于 0 的等差数列 $\{a_n\}$ 加绝对值后得到的数列 $\{|a_n|\}$ 求和，设 $\{a_n\}$ 的前 $n$ 项和为 $S_n$，$\{|a_n|\}$ 的前 $n$ 项和为 $T_n$，数列 $\{a_n\}$ 的第 $k$ 项大于 0 而从第 $k+1$ 项开始小于或等于 0，于是有
    ...
    $$
    T_n = \\begin{ S_n & n \le k \\ 2S_k - S_n & n > k }cases\\end{ S_n & n \le k \\ 2S_k - S_n & n > k }
    $$
