...
class PaperAnalyzer:
    ...
    def analyze(self, paper_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        results["基本信息"] = {
            ...
            "平均每题分值": f"{total_score / len(questions):.1f}分"
            if questions
            else "0分",
            "平均每题时间": f"{time_limit / len(questions):.1f}分钟"
            if questions
            else "0分钟",
        }
    ...
    def _analyze_difficulty_distribution(self, questions: List[Dict]) -> Dict:
        ...
        # 计算平均难度
        avg_difficulty = (
            sum(q.get("difficulty", 0.5) * q.get("score", 0) for q in questions)
            / total_score
            if total_score > 0
            else 0
        )
    ...
    def _evaluate_structure(self, results: Dict) -> Dict:
        """评价试卷结构"""
        evaluation = {"总体评价": "良好", "评分": 0.0, "优点": [], "问题": []}
        ...
        easy_percent = float(
            difficulty.get("容易题(0.7-1.0)", {}).get("占比", "0%").rstrip("%")
        )
        medium_percent = float(
            difficulty.get("中等题(0.4-0.7)", {}).get("占比", "0%").rstrip("%")
        )
        hard_percent = float(
            difficulty.get("较难题(0.2-0.4)", {}).get("占比", "0%").rstrip("%")
        )
        ...
        if 25 <= easy_percent <= 35:
            ...
            evaluation["问题"].append(
                f"容易题比例{easy_percent:.1f}%，建议在25%-35%之间"
            )
            score -= 10
        ...
        if 45 <= medium_percent <= 55:
            ...
            evaluation["问题"].append(
                f"中等题比例{medium_percent:.1f}%，建议在45%-55%之间"
            )
            score -= 10
        ...
        if high_ability >= 40:
            ...
            evaluation["问题"].append(
                f"高阶思维能力考查比例{high_ability:.1f}%，建议至少40%"
            )
            score -= 15
