...
2. **使用 read skill**：使用 `.agents/skills/read-project-tools/` 中的工具来分析文档结构
3. **使用 review skill**：使用 `.agents/skills/review-workflow/` 中的工具来检查排版格式
4. **熟悉项目规范**：阅读并理解 `.agents\rules\style.md` 中的排版规范
...
node .agents/skills/read-project-tools/dir_h1.js dev/supplement
```
...
```bash
# 查看 docs 文件夹的结构
node .agents/skills/read-project-tools/dir_h1.js docs
```
...
```bash
# 查看目标 .md 文件的标题结构
node .agents/skills/read-project-tools/file_headings.js docs/[路径]/[文件名].md

# 查看目标 .md 文件的开头内容
node .agents/skills/read-project-tools/file_prelimit.js docs/[路径]/[文件名].md
```
...
```bash
# 对于超大文件，先进行分割
node .agents/skills/read-project-tools/file_split.js dev/supplement/[大文件名]
...
```bash
# 先读取 .agents\rules\style.md，理解排版要求
node .agents/skills/read-project-tools/file_prelimit.js .agents\rules\style.md

# 再读取 .kilocode/rules/ex-copywriting-guidelines.md（如果存在）
node .agents/skills/read-project-tools/file_prelimit.js .kilocode/rules/ex-copywriting-guidelines.md

# 最后读取其他 ex- 开头的规则文件
node .agents/skills/read-project-tools/dir_h1.js .kilocode/rules
```
...
```bash
# 使用 node .agents/skills/read-project-tools/file_headings.js 查看目标文档的标题结构
node .agents/skills/read-project-tools/file_headings.js docs/[路径]/[文件名].md

# 使用 node .agents/skills/read-project-tools/file_prelimit.js 预览文档开头内容
node .agents/skills/read-project-tools/file_prelimit.js docs/[路径]/[文件名].md
```
...
node .agents/skills/review-workflow/review.js lint -- --file docs/[路径]/[文件名].md
...
# 对整个文件夹进行检查（逐个文件）
node .agents/skills/review-workflow/review.js lint -- --file docs/[路径]/1.md
node .agents/skills/review-workflow/review.js lint -- --file docs/[路径]/2.md
```
...
- **.agents\rules\style.md**：排版规范详细说明
- **.agents/skills/read-project-tools/**：文档读取和分析工具
- **.agents/skills/review-workflow/**：排版检查和修复工具
