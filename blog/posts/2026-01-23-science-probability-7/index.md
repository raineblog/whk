...
> 想获取全文细节，请访问原文链接：[https://raineblog.dpdns.org/whk/science/probability/4/](https://raineblog.dpdns.org/whk/science/probability/4/)
...
$$
\operatorname\{cnt} = (c\_1+1)(c\_2+1)\dots(c\_k+1)
$$
...
$$
\sum a\_\{i}= \bigl\[(a+1)^\{n}\bigr]_\{x=1},\qquad \sum (-1)^\{i}a_\{i}= \bigl\[(a+1)^\{n}\bigr]\_\{x=-1}
$$
...
$$
\sum\_\{k=0}^\{n}\frac\{1}\{k+1}\{n\choose k}= \frac\{2^\{,n+1}-1}\{n+1}
$$

先把 $\dfrac\{1}\{k+1}\{n\choose k}$ 写成 $\dfrac\{1}\{n+1}\{n+1\choose k+1}$，再利用二项式系数求和 $\sum\_\{k=0}^\{n}\{n+1\choose k+1}=2^\{,n+1}-1$。

这些技巧在处理带分母的组合求和时尤为有用。

***

> 想了解完整的推导过程、更多练习题以及细节解释，请访问原文链接：[https://raineblog.dpdns.org/whk/science/probability/4/](https://raineblog.dpdns.org/whk/science/probability/4/)
