...
> **路径计数**  
> 从 [\$A \to B\$]{.arithmatex} 有 2 条路，[\$B \to D\$]{.arithmatex} 有 3 条路；  
> 从 [\$A \to C\$]{.arithmatex} 有 4 条路，[\$C \to D\$]{.arithmatex} 有 5 条路。  
> 求 [\$A \to D\$]{.arithmatex} 的所有路径数。
...
给定整数 [\$N\$]{.arithmatex}，若其唯一素因子分解为

::: {.arithmatex}
\$\$ N = p_1\^{c_1} p_2\^{c_2} \dots p_k\^{c_k} \$\$
:::

则 [\$N\$]{.arithmatex} 的正因数个数为

::: {.arithmatex}
\$\$ \operatorname{cnt} = (c_1+1)(c_2+1)\dots(c_k+1) \$\$
:::

举例：[\$2160 = 2\^4 \times 3\^3 \times 5\$]{.arithmatex}，因此
...
对大小为 [\$N\$]{.arithmatex} 的集合 [\$S\$]{.arithmatex}，其子集数为
...
- 真子集：[\$2\^{N}-1\$]{.arithmatex}
- 非空子集：[\$2\^{N}-1\$]{.arithmatex}
- 非空真子集：[\$2\^{N}-2\$]{.arithmatex}

## 排列数和组合数 {#_6}

### 排列数 {#_7}

排列数 [\$A(n,m)\$]{.arithmatex} 表示从 [\$n\$]{.arithmatex} 件物品中取出 [\$m\$]{.arithmatex} 件并排成一列的方案数，公式为

::: {.arithmatex}
\$\$ A(n,m)=\frac{n!}{(n-m)!} \$\$
:::

当 [\$m=n\$]{.arithmatex} 时，[\$A(n,n)=n!\$]{.arithmatex}；约定 [\$0!=1\$]{.arithmatex}。

### 组合数 {#_8}

组合数 [\$C(n,m)\$]{.arithmatex}（记作 [\$\displaystyle{n\choose m}\$]{.arithmatex}）表示从 [\$n\$]{.arithmatex} 件物品中任选 [\$m\$]{.arithmatex} 件，不计顺序的方案数，公式为
...
每行对应 [\$(a+1)\^{i-1}\$]{.arithmatex} 的系数。

### 一些性质 {#_13}

- 奇偶系数相等（当 [\$n\$]{.arithmatex} 为偶数）  
  \$\$  
...
**例题**：在 [\$(x+y)(x-y)\^{5}\$]{.arithmatex} 的展开式中，求 [\$x\^{n}y\^{6-n}\$]{.arithmatex} 的系数。  
解法利用二项式系数的交替符号，可得
...
1.  **把目标指数转化为系数求和**：如求某项之和，可先代入 [\$x=1\$]{.arithmatex} 与 [\$x=-1\$]{.arithmatex} 形成线性方程组。
2.  **利用对称性**：[\${n\choose i} = {n\choose n-i}\$]{.arithmatex} 常用于化简求和范围。
...
若把 [\$n+1\$]{.arithmatex} 件物品放入 [\$n\$]{.arithmatex} 组，则必有至少一组包含两个或更多物品。推广为

> 把 [\$n\$]{.arithmatex} 件物品划分到 [\$k\$]{.arithmatex} 组，必有一组至少 [\$\left\lceil\frac{n}{k}\right\rceil\$]{.arithmatex} 件。
...
若只关心选取 [\$r\$]{.arithmatex} 项且每种元素无限供应，则组合数为
...
一般 [\$Q(n,m)=\dfrac{n!}{m\\(n-m)!}\$]{.arithmatex}。

### 错位排列 {#_22}

错位排列（或称"错位排列"）指没有任何元素出现在原位的排列，记为 [\$D\_{n}\$]{.arithmatex}。闭式为
...
极限概率 [\$P\_{n}=D\_{n}/n!\$]{.arithmatex} 满足 \$ \displaystyle\lim\_{n\to\infty}P\_{n}=\frac{1}{e}\$。
...
前几项为 [\$1, 1, 2, 5, 14, 42, 132,\dots\$]{.arithmatex}

### Lucas 定理 {#lucas}

若 [\$p\$]{.arithmatex} 为素数，则

::: {.arithmatex}
\$\$ {n\choose m}\equiv \prod\_{i}{a\_{i}\choose b\_{i}}\pmod{p} \$\$
:::

其中 [\$n=\sum a\_{i}p\^{i}\$]{.arithmatex}、[\$m=\sum b\_{i}p\^{i}\$]{.arithmatex} 为 [\$p\$]{.arithmatex} 进制展开。
...
把多项式系数的求和视为代入特定 [\$x\$]{.arithmatex}、[\$y\$]{.arithmatex} 的线性组合，例如
...
- 直接计数 [\$m\^{n}\$]{.arithmatex}（球不同、盒子不同）
- 插板公式 [\${n+m-1\choose m-1}\$]{.arithmatex}（球相同、盒子不同）
- 斯特林数（球不同、盒子相同）
- 其他限制使用容斥或分组计数。
...
利用二项式恒等式 [\${n\choose i}+{n\choose i+1}={n+1\choose i+1}\$]{.arithmatex}，并对上下标做适当移位，可得到所需等式。
...
先把 [\$\dfrac{1}{k+1}{n\choose k}\$]{.arithmatex} 写成 [\$\dfrac{1}{n+1}{n+1\choose k+1}\$]{.arithmatex}，再利用二项式系数求和 [\$\sum\_{k=0}\^{n}{n+1\choose k+1}=2\^{\\n+1}-1\$]{.arithmatex}。
