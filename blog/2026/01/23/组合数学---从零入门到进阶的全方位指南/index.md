...
1. **利用对称性**：({n\\choose i} = {n\\choose n-i}) 常用于化简求和范围。
1. **正难则反**：在计数题中常把“不满足条件”的组合计数后整体相减。
...
\[ \\bigl|!\\bigcup\_{i=1}^{m}A\_{i}\\bigr| = \\sum\_{i}|A\_{i}|

- \\sum\_{i\<j}|A\_{i}\\cap A\_{j}|

* \\sum\_{i\<j\<k}|A\_{i}\\cap A\_{j}\\cap A\_{k}|
