...
关于模式你需要在 plan 模式进行规划和 task 和任务和流程的指定、提示词的构建，确保绝对的正确。你需要阅读各个文件的 headings 等，确定植入的位置。

然后你需要切换到 orchestrator 模式，启动 subagent 字代理进行逐个 supplement 的编写。

最后，你需要切换到 code 模式，进行复核、review 并提交到分支。

如果你发现你无法切换模式，可能是平台的限制，请你立刻终止（或者按照任务完成的标准强制结束），然后请求用户手动切换。
