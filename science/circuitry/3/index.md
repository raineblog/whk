...
阳极上阴离子放电：还原性顺序，活泼电极 [\$\>\ce{S\^2}\>\ce{SO_3\^2-}\>\ce{I-}\>\$]{.arithmatex} [\$\ce{Br-}\>\ce{Cl-}\>\ce{OH-}\>\$]{.arithmatex} 含氧酸根 [\$\>\ce{F-}\$]{.arithmatex}。

阴极上阳离子放电：金属活动顺序，[\$\ce{Ag+ \> Hg+ \> Fe\^{3+} \> Cu\^{2+}}\>\$]{.arithmatex} [\$\ce{H+ \> Pb\^{2+} \> Sn\^{2+} \> Fe\^{2+}}\>\$]{.arithmatex} [\$\ce{Zn\^{2+} \> H+(H2O) \> Al\^{3+} \> Mg\^{2+}}\>\$]{.arithmatex} [\$\ce{Na+ \> Ca\^{2+} \> K+}\$]{.arithmatex}。

实际情况可能有所不同，例如浓度也会影响放电顺序，高浓度可能先放电。

总反应：

- 电解溶质型：[\$\ce{CuCl2 -\>\[通电\] Cu + Cl2}\$]{.arithmatex}。

- 电解各半成酸型：[\$\ce{2CuSO4 + 2H2O -\>\[通电\] 2Cu + O2 + 2H2SO4}\$]{.arithmatex}。

- 电解各半成碱型：[\$\ce{2NaCl + 2H2O -\>\[通电\] H2 + Cl2 + 2NaOH}\$]{.arithmatex}。

- 电解水型：[\$\ce{2H2O -\>\[通电\] 2H2 + O2}\$]{.arithmatex}。
...
标准氢电极（简称 SHE），是构成标准电极电势（[\$E\^0\$]{.arithmatex}）基准的工作电极。
...
这个半反应是在镀有铂黑的处于标准状态（气体压强为 [\$1\$]{.arithmatex} 大气压、[\$c(\ce{H+})=\pu{1mol/L}\$]{.arithmatex}）的铂电极上发生的。

在 [\$\pu{25\^oC}\$]{.arithmatex} 时，它的绝对电位大约为 [\$\pu{4.44\pm0.02V}\$]{.arithmatex}，但为了给所有电极反应的电动势设立一个基准值，在任意温度下氢电极的标准电极电势都定义为零，其他电极的电势都是相对于标准氢电极而确定的。
...
| 电极反应 (还原反应) | 标准电极电势 E° |
|---:|:---|
| [\$\ce{K+ + e- \<=\> K}\$]{.arithmatex} | [\$\pu{-2.92 V}\$]{.arithmatex} |
| [\$\ce{Ca\^{2+} + 2e- \<=\> Ca}\$]{.arithmatex} | [\$\pu{-2.87 V}\$]{.arithmatex} |
| [\$\ce{Mg\^{2+} + 2e- \<=\> Mg}\$]{.arithmatex} | [\$\pu{-2.37 V}\$]{.arithmatex} |
| [\$\ce{Al\^{3+} + 3e- \<=\> Al}\$]{.arithmatex} | [\$\pu{-1.66 V}\$]{.arithmatex} |
| [\$\ce{Zn\^{2+} + 2e- \<=\> Zn}\$]{.arithmatex} | [\$\pu{-0.76 V}\$]{.arithmatex} |
| [\$\ce{Fe\^{2+} + 2e- \<=\> Fe}\$]{.arithmatex} | [\$\pu{-0.44 V}\$]{.arithmatex} |
| [\$\ce{2H+ + 2e- \<=\> H2}\$]{.arithmatex} | [\$\pu{0 V}\$]{.arithmatex} |
| [\$\ce{Cu\^{2+} + 2e- \<=\> Cu}\$]{.arithmatex} | [\$\pu{+0.34 V}\$]{.arithmatex} |
| [\$\ce{Ag+ + e- \<=\> Ag}\$]{.arithmatex} | [\$\pu{+0.80 V}\$]{.arithmatex} |

设 [\$E\_{cell}\^{o}\$]{.arithmatex} 为电池的标准电位，可由阳极和阴极的标准电极电位计算而来：
...
- **标准氢电极**：其标准电极电势定义为 [\$\pu{0V}\$]{.arithmatex}，条件为 [\$\pu{25 °C}\$]{.arithmatex}，氢气压力为 [\$\pu{101 kPa}\$]{.arithmatex}。
- **铜锌原电池**：图中圈出的铜和锌可组成铜锌原电池（丹尼尔电池）。其电动势（电压）计算如下 [\$E\_{\text{cell}} = E\_{\text{cathode}} - E\_{\text{anode}} = (\pu{+0.34 V}) - (\pu{-0.76 V}) = \pu{1.10 V}\$]{.arithmatex}
...
二氧化碳电化学还原反应是使用电化学手段将二氧化碳（[\$\ce{CO2}\$]{.arithmatex}）还原的反应，可以得到特定的有机物或燃料（选择性催化），它是碳捕获与利用的最有前途手段之一。
...
一般以高浓度的羧酸钠盐作原料，在中性或弱酸性环境中进行电解。电极以铂制成，阳极产生烷烃和二氧化碳，阴极产生氢氧化钠和氢气。羧酸的碳数最好适中，一般在 [\$10\$]{.arithmatex} 个左右。反应的副产物有低碳烷烃、酯和醇等。
...
酸性重铬酸根废液处理：铁做阳极，电解产生亚铁离子；亚铁离子被重铬酸根氧化为铁离子，重铬酸根变为正三价，水解成 [\$\ce{Cr(OH)3}\$]{.arithmatex} 沉淀。阴极氢离子放电，总反应
...
- 阳极是金属被氧化、失去电子并形成金属离子的区域（[\$\ce{M -\> M\^n+ + ne-}\$]{.arithmatex}）。
...
- 铁的锈蚀是一个电化学过程，在水和空气存在的情况下，铁的表面形成弱阳极和阴极，最终生成铁锈（水合三氧化二铁，[\$\ce{Fe2O3\*xH2O}\$]{.arithmatex}）。

- 在阳极，铁被氧化，失去电子并形成亚铁离子。这通常发生在应力区域或缺氧的位置。

- **析氢腐蚀**：在酸性环境中，铁表面发生氧化反应，生成亚铁离子（[\$\ce{Fe\^2+}\$]{.arithmatex}）并释放电子，同时氢离子（[\$\ce{H+}\$]{.arithmatex}）在阴极处接受电子生成氢气（[\$\ce{H2}\$]{.arithmatex}）。这种腐蚀通常发生在酸性溶液中，腐蚀速率较快。

- **吸氧腐蚀**：在中性或碱性环境中，铁表面发生氧化反应生成亚铁离子（[\$\ce{Fe\^2+}\$]{.arithmatex}），而氧气在阴极处与水反应生成氢氧根离子（[\$\ce{OH-}\$]{.arithmatex}）。最终，亚铁离子与氢氧根离子结合生成氢氧化亚铁（[\$\ce{Fe(OH)2}\$]{.arithmatex}），进一步氧化形成铁锈。

- 锈的形成是一个涉及铁的氧化和氧的还原的多步过程，在最终稳定的锈形成之前会产生中间产物。

不锈钢：

- 不锈钢的耐腐蚀性源于其表面形成的钝化层，该钝化层主要由氧化铬（[\$\ce{Cr2O3}\$]{.arithmatex}）组成。
...
- **基本原理**：通过化学或热处理方法使金属表面氧化，生成一层致密的氧化膜（如氧化铁 [\$\ce{Fe3O4}\$]{.arithmatex}），从而隔绝空气和水分，减缓腐蚀。
...
- 在有水、氧气和二氧化碳的环境中，产生铜锈（碱式碳酸铜 [\$\ce{Cu2(OH)2CO3}\$]{.arithmatex}），又称铜绿。
