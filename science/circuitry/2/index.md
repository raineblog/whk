...
![alt text](../%E6%BB%91%E5%8A%A8%E5%8F%98%E9%98%BB%E5%99%A8.drawio.avif){width="90%"}
...
晶体三极管是半导体基本元件之一，具有电流放大作用，在控制电路中常用作电子开关。本实验采用三极管配合光敏电阻完成光控开关的任务。三极管由三个电极组成，分别是发射极 []{.arithmatex}$e$、基极 []{.arithmatex}$b$ 和集电极 []{.arithmatex}$c$，有 NPN 型和 PNP 型两种。三极管的一个重要特性是，从基极输入一个较小的电流，就会在集电极获得较大的电流。此外，三极管还具有完成断路和接通的开关作用。

![alt text](../image-37.avif){width="30%"}
...
::: {.grid}
![alt text](../image-12.avif)

![alt text](../image-22.avif)
:::

我们发现，如果将两个表笔短接（即被测电阻 []{.arithmatex}$R_x=0$），在欧姆表内部电路由电源、电源内阻 []{.arithmatex}$r$、表头内阻 []{.arithmatex}$R_g$ 和限流电阻 []{.arithmatex}$R$ 串联组成，则满偏电流为
...
我们记内阻 []{.arithmatex}$r_i=r+R_g+R$，根据电流之比为电阻反比，某一时刻电流表示数为 []{.arithmatex}$I$，则
...
我们根据这一表达式，在表盘上直接表上对应的欧姆刻度，如上右图所示。我们知道这个表盘刻度是不均匀的，测量范围在 []{.arithmatex}$(0,+\infty)$ 欧姆。
...
**检流计** []{.arithmatex}$-\kern{-0.4em}\bigcirc\kern{-1em}{\scriptsize\text{G}}\kern{0.05em}-$：也称为**表头**，被用于测量**微弱电流**。指针式的表头是一只高灵敏度的磁电式灵敏检流计，其工作原理为利用电流的磁效应，当电流通过其导线时，会产生磁场，与永久磁铁互斥而带动指针旋转，与磁电式电流表基本相同。电表的主要性能指标基本上取决于表头的性能。表头的灵敏度是指表头指针满刻度偏转时流过表头的直流电流值，这个值越小，表头的灵敏度愈高，其性能就越好。

::: {.grid}
![alt text](../image-9.avif)

  
![alt text](../image-10.avif)
:::

检流计有内阻 []{.arithmatex}$\Omega$、最大电压 []{.arithmatex}$U$ 和最大电流 []{.arithmatex}$I$，满足欧姆定律，检流计与大电阻串联构成电压表 []{.arithmatex}$-\kern{-0.4em}\bigcirc\kern{-1em}{\scriptsize\text{V}}\kern{0.1em}-$，与小电阻并联构成电流表 []{.arithmatex}$-\kern{-0.4em}\bigcirc\kern{-1em}{\scriptsize\text{ I}}\kern{0.2em}-$，由欧姆定律分压分流规律，可以计算出应该串联、并联多大的电阻。
...
- 与 []{.arithmatex}$y$ 轴交点为电源电动势，与 []{.arithmatex}$x$ 轴交点为短路电流，斜率大小为内阻 []{.arithmatex}$r$。

电阻的 []{.arithmatex}$P-I$ 图像：

![](../IP.avif){width="50%"}

电灯泡 []{.arithmatex}$U-I$ 与电源 []{.arithmatex}$U-I$ 联立：

- 根据短路电流 []{.arithmatex}$I=E/r$ 得出横截距，以 []{.arithmatex}$E$ 为纵截距在 []{.arithmatex}$U-I$ 图上做出下降直线（电源）。

  ![](../UI.avif){width="50%"}

- 电灯泡的 []{.arithmatex}$U-I$ 曲线与电源的直线交点即为连接后的电路状态。

### 物理量总结 {#_11}

| 中文 | 字母（单位） | 公式 |
|:--:|:--:|----|
| 电流 | []{.arithmatex}$I$（[]{.arithmatex}$\pu{A}$） | []{.arithmatex}$I = \frac{Q}{t} = \frac{U}{R}$ |
| 电动势 | []{.arithmatex}$E$（[]{.arithmatex}$\pu{V}$） | []{.arithmatex}$E = \frac{W}{q}$ |
| 电压 | []{.arithmatex}$U$（[]{.arithmatex}$\pu{V}$） | []{.arithmatex}$U = I \cdot R$ |
| 电阻 | []{.arithmatex}$R$（[]{.arithmatex}$\Omega$） | []{.arithmatex}$R = \frac{U}{I} = \rho \frac{l}{S}$ |
| 电功率 | []{.arithmatex}$P$（[]{.arithmatex}$\pu{W}$） | []{.arithmatex}$P = U \cdot I = I^2 R = \frac{U^2}{R}$ |
| 发热量 | []{.arithmatex}$Q$（[]{.arithmatex}$\pu{J}$） | []{.arithmatex}$Q = I^2 R t$ |
| 供电效率 | []{.arithmatex}$\eta$ | []{.arithmatex}$\eta = \frac{U \cdot I}{E \cdot I}$ |

百科版本：

| 单位 | 符号 | 物理量 | 注 |
|:--:|:--:|:--:|----|
| 安培 | []{.arithmatex}$A$ | **电流** | 基本单位 |
| 伏特 | []{.arithmatex}$V$ | **电势**，电势差，电动势 | []{.arithmatex}$=W\cdot A^{-1}$ |
| 欧姆 | []{.arithmatex}$\Omega$ | **电阻**，电抗，阻抗 | []{.arithmatex}$=V\cdot A^{-1}$ |
| 法拉 | []{.arithmatex}$F$ | **电容** |  |
| 亨利 | []{.arithmatex}$H$ | 电感 |  |
| 西门子 | []{.arithmatex}$S$ | **电导**，导纳，磁化率 | []{.arithmatex}$=\Omega^{−1}$ |
| 库仑 | []{.arithmatex}$C$ | **电荷量** | []{.arithmatex}$=A\cdot s$ |
| 欧姆⋅米 | []{.arithmatex}$\Omega\cdot m$ | 电阻率 | []{.arithmatex}$\rho$ |
| 西门子/每米 | []{.arithmatex}$S\cdot m^{-1}$ | 电导率 |  |
| 法拉/每米 | []{.arithmatex}$F\cdot m^{-1}$ | 电容率；介电常数 | []{.arithmatex}$\varepsilon$ |
| 反法拉 | []{.arithmatex}$F^{−1}$ | 电弹性 | []{.arithmatex}$=F^{−1}$ |
| 伏安 | []{.arithmatex}$VA$ | 交流电功率，视在功率 |  |
| 无功伏安 | []{.arithmatex}$\mathit{var}$ | 无功功率，虚功 |  |
| 瓦特 | []{.arithmatex}$W$ | **电功率**，有功功率，实功 | []{.arithmatex}$=J\cdot s^{-1}$ |
| 千瓦⋅时 | []{.arithmatex}$kW⋅h$ | **电能** | []{.arithmatex}$=3.6\,MJ$ |
...
- 计算法：将预估 []{.arithmatex}$R_x$ 与 []{.arithmatex}$\sqrt{R_AR_B}$ 比较，作为大小电阻的依据。

- 试触法：将电笔轻轻接触，然后迅速拿开。将电压表的一端分别在电流表的内外试触，如果电压表示数变化较大，则说明电流表分压较大，使用外接法；如果电流表示数变化较大，说明电压表分流较大，使用内接法。

  具体来说，我们可以定量判断 []{.arithmatex}$\vert{\dfrac{U_外-U_内}{U_内}}$ 和 []{.arithmatex}$\vert{\dfrac{I_外-I_内}{I_内}}$ 的关系，确定哪一个更接近真实值。

### 伏阻法 {#_14}

伏阻法，又称伏伏法，如图所示：

![alt text](../image-26.avif){width="80%"}

::::::::: {.grid}
::::: {.admonition .note}
如左图

由已知电阻 []{.arithmatex}$R_0$ 推导出 []{.arithmatex}$R_x$ 两端电压。
...
若考虑电流表分压，则测量值比真实值偏大，测量值为待测电阻和电流表内阻的和，如果 []{.arithmatex}$A_2$ 用内阻已知的电流表，则可以消除系统误差。
:::::
...
![alt text](../image-27.avif){width="80%"}
...
此处可以认为 []{.arithmatex}$R_0$ 充当了电流表，相当于外接，测量值比真实值偏小。
:::::
:::::::::
...
![alt text](../image-28.avif){width="40%"}

1.  闭合开关 []{.arithmatex}$S$ 和 []{.arithmatex}$S_1$，调节滑动变阻器，使电流表满偏。

2.  再闭合开关 []{.arithmatex}$S_2$，保持滑动变阻器位置不动，调节电阻箱示数为 []{.arithmatex}$R$，使得电流表半偏。

3.  记录电阻箱示数 []{.arithmatex}$R$，则 []{.arithmatex}$R=R_x+R_A$。

通常情况下，限流式半偏法通常直接用于测量电流表内阻 []{.arithmatex}$R_A$，此时不接入 []{.arithmatex}$R_x$，电阻箱示数即为电流表的内阻。
...
- 限流式半偏法适合测量小电阻（例如电流表），因为电阻小、闭合 []{.arithmatex}$S_2$ 后总电阻变化也就越小，误差越小。

- 为了减小误差，我们需要电路总电阻变化较小，容易想到需要滑动变阻器接入阻值较大，因此，应当选择大的电源电动势和大的滑动变阻器。

分压式半偏法：

![alt text](../image-29.avif){width="40%"}

1.  电阻箱归零，闭合开关，调节滑动变阻器，使得电压表满偏。

2.  保持滑动变阻器不变，调节电阻箱示数为 []{.arithmatex}$R$，使得电压表半偏。

3.  记录电阻箱示数 []{.arithmatex}$R$，则电阻大小 []{.arithmatex}$R_V=R$。
...
注意，如果开始的时候忘记归零，则可以通过写出电压不变的方程，解出来 []{.arithmatex}$R_V$ 的值。

### 替代法 {#_17}

替代法是一种类似于限流式半偏法的方法，如图所示：

![alt text](../image-30.avif){width="40%"}

1.  闭合开关 []{.arithmatex}$S$ 和 []{.arithmatex}$S_1$，适当调整滑动变阻器，记录此时电流表示数。

2.  断开开关 []{.arithmatex}$S_1$，闭合开关 []{.arithmatex}$S_2$，调整电阻箱，使得电流表示数和上一步相同。

3.  则电阻 []{.arithmatex}$R_x$ 即为电阻箱的示数 []{.arithmatex}$R$，这个方法是没有系统误差的。

### 电桥法 {#_18}

惠斯通电桥是一种测量工具，用来精确测量未知电阻器的电阻。

将待测电阻 []{.arithmatex}$R_x$ 和 []{.arithmatex}$R_3$ 串联，可变电阻 []{.arithmatex}$R_2$ 和 []{.arithmatex}$R_1$ 串联。再将这两个串联电路并联，在各路中点间接入检流计 []{.arithmatex}$V_G$。其中 []{.arithmatex}$V_G$ 可以认为是测量电势差的，或是测量电流的，因为这是等价的。

::: {.grid}
![alt text](../image-2.avif){width="90%"}

![alt text](../image-3.avif){width="90%"}
:::
...
证明：[]{.arithmatex}$D$ 和 []{.arithmatex}$B$ 等势的时候灵敏电流计无示数，结论显然。
...
![alt text](../image-31.avif){width="60%"}

理想情况下，电流表测外电流、电压表测外电压，因为要测两个量，我们最常见的减小偶然误差的方式就是图像法，容易想到我们会指出伏安（[]{.arithmatex}$U-I$）图形，两种方法
...
此时我们如果忽略 []{.arithmatex}$R_A$ 容易知道，**电动势测量准确**，**内阻测量值偏大**。
...
此时我们如果忽略 []{.arithmatex}$R_V$ 容易知道，**电动势测量值偏小**，**内阻测量值偏小**。
...
![](../%E7%94%B5%E6%BA%90%E6%B5%8B%E9%87%8F.svg){width="50%"}
...
![alt text](../image-32.avif){width="60%"}
...
因此绘制出 []{.arithmatex}$\dfrac{1}{I}-R$ 图，根据斜率和截距计算即可。

因为我们忽略了电流表的内阻分压，因此所测的的电源内阻大于真实值，真实值比测量值大了 []{.arithmatex}$R_A$。
:::::::::
...
因此绘制出 []{.arithmatex}$\dfrac{1}{I}-\dfrac{1}{R}$ 图，根据斜率和截距计算即可。
...
若两电源的电动势不相等，即 []{.arithmatex}$\varepsilon_0 \neq \varepsilon_x$，则回路中必有电流，检流计的指针发生偏转。如果 []{.arithmatex}$\varepsilon_0 = \varepsilon_x$，则回路中没有电流，检流计指示为零，这时称电路达到补偿状态。这样可以根据已知电动势 []{.arithmatex}$\varepsilon_0$ 的值，求出待测电动势 []{.arithmatex}$\varepsilon_x$ 的值，这种测电源电动势的方法称为补偿法。

![alt text](../image-13.avif){width="50%"}

工作电源 []{.arithmatex}$E$、限流电阻 []{.arithmatex}$R$、开关 []{.arithmatex}$K_1$、粗细均匀的电阻丝 AB 串联成一闭合电路，称为辅助电路。

- 闭合 []{.arithmatex}$K_1$，调节 []{.arithmatex}$R$，使 AB 中有一恒定的工作电流 []{.arithmatex}$I_0$。[]{.arithmatex}$\varepsilon_0$ 为标准电源，[]{.arithmatex}$\varepsilon_x$ 为待测电源。

- 开关 []{.arithmatex}$K_2$ 倒向标准电源的一边与 []{.arithmatex}$\varepsilon_0$ 连通检流计发生偏转，移动触点 C，使检流计中的电流 []{.arithmatex}$I_g=0$，这时有 []{.arithmatex}$\varepsilon_0=U_{AC}$。

- 同理，[]{.arithmatex}$K_2$ 倒向待测电源一侧时，做相应的调节，使检流计回到零位，则有 []{.arithmatex}$\varepsilon_x = U_{AC'}$。

比较 []{.arithmatex}$\varepsilon_0$ 与 []{.arithmatex}$\varepsilon_x$，有
...
因此只需测出 []{.arithmatex}$L_0$ 与 []{.arithmatex}$L_x$ 的长度比并乘以已知标准电源的电动势即可得到被测电源的电动势。此方法的优点是避免了直接测量小电压时电压表带来的误差。
...
![alt text](../image-34.avif){align="right" width="40%"}
...
通过前面的学习可以知道，光敏电阻、热敏电阻、电阻应变片等电阻式传感器的工作共性是通过测量电阻的变化来确定外界非电学量的变化。与之类似，电容器的电容 []{.arithmatex}$C$ 决定于极板的正对面积 []{.arithmatex}$S$、极板间的距离 []{.arithmatex}$d$ 以及极板间的电介质这三个因素。如果某个物理量的变化能引起上述某个因素的变化，从而引起电容的变化，那么，通过测定电容器的电容就可以确定这个物理量的变化，由此可以制成电容式传感器。电容式传感器有非常广泛的应用。
...
![alt text](../image-36.avif){width="85%"}
:::

除导体外，半导体也能产生霍尔效应，而且半导体的霍尔效应要强于导体。在一个很小的矩形半导体（如砷化铟）薄片上制作四个电极，它就成了一个霍尔元件。在电极 E、F 间通入恒定的电流，同时外加与薄片垂直的磁场，则在电极 M、N 间可出现霍尔电压。

通过分析可知，霍尔电压与磁感应强度有线性关系，因此利用霍尔元件可以测量磁感应强度的大小和方向。霍尔元件能够把磁感应强度这个磁学量转换为电压这个电学量。霍尔元件除了可以检测磁场及其变化，还可以在各种与磁场有关的场合中使用。利用霍尔元件进行微小位移，在两块磁感应强度相同、同极相对放置的磁体缝隙中放入霍尔元件，当霍尔元件处于中间位置时，磁感应强度为零，霍尔电压为零，可将该点作为位移的零点。当霍尔元件沿着 []{.arithmatex}$z$ 轴方向移动时，则有霍尔电压输出，且电压大小与位移大小成正比，从而能够实现微小位移的测量。

### 电磁继电器 {#_30}

::: {.grid}
![alt text](../image-38.avif){width="70%"}

![alt text](../image-39.avif){width="90%"}
:::
