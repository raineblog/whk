...
假设从容量为 []{.arithmatex}$N$（很大）的总体中抽取容量为 []{.arithmatex}$n$ 的样本，我们可以按下列步骤进行系统抽样：

1.  先将总体的 []{.arithmatex}$N$ 个个体编号。

2.  确定分段间隔 []{.arithmatex}$k$，对编号进行分段。

    - 当 []{.arithmatex}$\dfrac{N}{n}$ 是整数时，取 []{.arithmatex}$k=\dfrac{N}{n}$。

    - 当 []{.arithmatex}$\dfrac{N}{n}$ 不是整数时，假设余数为 []{.arithmatex}$r$（[]{.arithmatex}$0<r<n$），可随机地从 []{.arithmatex}$N$ 个个体中剔除余数 []{.arithmatex}$r$ 个个体，此时取 []{.arithmatex}$k=\dfrac{N-r}{n}$。

3.  在第一段用简单随机抽样确定第 []{.arithmatex}$1$ 个个体的编号 []{.arithmatex}$\ell$（[]{.arithmatex}$\ell\le k$）。

4.  将编号为 []{.arithmatex}$\ell, \ell+k, \ell+2k, \dots, \ell+(n-1)k$ 的个体抽出。

我们知道 []{.arithmatex}$\ell, \ell + k, \ell + 2k, \dots, \ell + (n - 1)k$ 是以 []{.arithmatex}$\ell$ 为首项、以 []{.arithmatex}$k$ 为公差的等差数列，设第 []{.arithmatex}$n$ 段抽到的编号为 []{.arithmatex}$a_n$，则 []{.arithmatex}$a_n = \ell + (n - 1)k$。故系统抽样也叫等距抽样。
...
1.  **分层**：将总体分成互不交叉的层。
2.  **确定抽样比**：总体 []{.arithmatex}$N$，样本容量 []{.arithmatex}$n$，则抽样比例为 []{.arithmatex}$\dfrac{n}{N}$。
3.  **分层抽样**：在各层中按抽样比例 []{.arithmatex}$\dfrac{n}{N}$ 独立地进行简单随机抽样。
4.  **汇合样本**：将各层抽取的样本合并为最终样本。

例如：第一层 []{.arithmatex}$X$ 有 []{.arithmatex}$n$ 个元素，第二层 []{.arithmatex}$Y$ 有 []{.arithmatex}$m$ 个元素。
