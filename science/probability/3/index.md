...
为什么说概率「很可能」是 \$0\$？考虑这样的随机变量 \$X\$：它以 \$½\$ 的概率取 \$0\$，以 \$½\$ 的概率服从开区间 \$(0, 1)\$ 上的均匀分布。显然 X 满足连续型随机变量的定义。对任何实数 \$r \in (0, 1)\$，不难得到 \$P{ X = r } = 0\$，但同时有 \$P{ X = 0 } = ½\$。
...
\$\$ f(n,k,p)={\frac {(n\cdot p)^{k}}{e^ \$\$}\cdot k!}
...
\$\$ f_n(k)={n \choose k}p^{k}(1-p)^\quad (k=0,1,\ldots ,n) \$\$
