...
**方法二（最优解）**：均值不等式 由题可知，20 件产品中恰有 2 件不合格品的概率为 f(p) = C\_{20}^2 p^2 (1-p)^{18}. f(p) = C\_{20}^2 p^2 (1-p)^{18} = \\frac{190}{81} (9p)(9p)(1-p)^{18} \\le \\frac{190}{81} \\left[ \\frac{9p+9p+18(1-p)}{20} \\right]^{20} = \\frac{190}{81} \\left( \\frac{18}{20} \\right)^{20}，当且仅当 9p = 1-p，即 p = \\frac{1}{10} 可得所求．
