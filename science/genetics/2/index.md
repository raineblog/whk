...
| -------- | ----------------- | --------------------------------------- | ----------------------------------------------------------- | ----------------------------------------------------------- |
| *F\*\*n* | $\\dfrac{1}{2^n}$ | $1-\\dfrac{1}{2^n}=\\dfrac{2^n-1}{2^n}$ | $\\dfrac{1}{2}-\\dfrac{1}{2^{n+1}}=\\dfrac{2^n-1}{2^{n+1}}$ | $\\dfrac{1}{2}+\\dfrac{1}{2^{n+1}}=\\dfrac{2^n+1}{2^{n+1}}$ |
...
|          | Aa                  | AA                                          |
| -------- | ------------------- | ------------------------------------------- |
| *F\*\*n* | $\\dfrac{2}{2^n+1}$ | $1-\\dfrac{2}{2^n+1}=\\dfrac{2^n-1}{2^n+1}$ |

#### 自由交配比例

杂合子 Aa 自由交配：比例恒为 1:2:1。

配子法：一个区域内 AA:Aa=1:2，求自由交配一代后性状及其比例。

找出配子：A:a=2×1+1×2:1×2=2:1，列出表格：

得出答案：基因型 AA:Aa:aa=4:4:1，形状分离比为 8:1。

杂合子 Aa 自由交配，逐代淘汰 aa 隐性个体：

|          | AA                | Aa                |
| -------- | ----------------- | ----------------- |
| *F\*\*n* | $\\dfrac{n}{n+2}$ | $\\dfrac{2}{n+2}$ |

#### 合子致死问题

形式为，AA,Aa,aa 中有基因型导致致死，或者部分致死。

在 1:2:1 的基础上，减掉一部分数即可，例如 aa 有 50% 致死：
...
因此，n 对等位基因 AaBbCc… 分离的现象：

|             | 1 对相对性状      | 2 对相对性状                               | *n* 对相对性状            |
| ----------- | ----------------- | ------------------------------------------ | ------------------------- |
| *F*1 配子   | 有 2 种 1 : 1     | 有 22 种 1 : 1 : 1 : 1                     | 有 2*n* 种 (1 : 1)*n*     |
| *F*2 表现型 | 有 2 种 3 : 1     | 有 22 种 9 : 3 : 3 : 1                     | 有 2*n* 种 (3 : 1)*n*     |
| *F*2 基因型 | 有 2 种 1 : 2 : 1 | 有 22 种 1 : 2 : 1 : 2 : 4 : 2 : 1 : 2 : 1 | 有 2*n* 种 (1 : 2 : 1)*n* |
...
我们知道 Ab 雄配子一定会与 1:1:1:1 的 AB,Ab,aB,ab 的配子结合后死亡。
...
- 例如 AAa 染色体的配子产生为：(AA,a);(Aa,A);(Aa,A)，即 AA:Aa:A:a=1:2:2:1。
...
举例：果蝇的有翅 A\_ 无翅 aa，与长翅 B\_ 短翅 bb。

#### 显性抑制作用 13 : 3 型

其中一对基因对另一对其抑制作用且本身不控制性状的表现，称其为抑制基因，或者显性时性状抵消、与双隐性相同。

(A_bb);(A_B\_,aaB\_,aabb)
