...
![发酵工程流程图](/static/image/image.5f01b842fd.avif){width="60%"}
...
>
> - **来源**：蔬菜自带硝酸盐 $to$ 杂菌（大肠杆菌等）将其还原为有毒的亚硝酸盐。
...
$1$ 葡萄糖 $to$ $2$ 乳酸 $+ \ 2$ \[ATP]\{.abbr title="Adenosine Triphosphate (三磷酸腺苷)"}
...
$1$ 葡萄糖 $to$ $1$ 乳酸 $+$ $1$ 乙醇 $+$ $1$ $ce\{CO2}$ $+$ $1$ \[ATP]\{.abbr title="Adenosine Triphosphate (三磷酸腺苷)"}
...
工作菌种库（甘油管，$pu\{-80 ^\{\circ}C}$）$to$ 摇瓶（$pu\{250 mL}$）$to$ 种子罐（$pu\{500 L}$）$to$ 发酵罐（$pu\{50000 L}$）
...
>
> - 养的是细胞 $to$ 目的是防污染（它是保镖）。
> - 养的是真菌 $to$ 目的是除杂菌（它是除草剂）。
> - 养的是大肠杆菌 $to$ 目的是筛选（它是考官，只有带"准考证/质粒"的才能活）。
...
- **中心法则**：\[DNA]\{.abbr title="Deoxyribonucleic Acid (脱氧核糖核酸)"} $to$ \[RNA]\{.abbr title="Ribonucleic Acid (核糖核酸)"} $to$ 蛋白质。发酵工程本质上是操纵这个流程。
