...
这种缩放向量并相加的思想很重要，我们称 *ı̂*, *ȷ̂* 为 *x\*\*y* 坐标系的**基向量**。
...
根据上面基向量的思想，我们只需要记录 *ı̂*, *ȷ̂* 的变换位置即可。

感性理解，我们可以根据变化的 *ı̂*, *ȷ̂* 推断出述任意向量位置。

有一个性质，若一向量可以表示为，

*v⃗* = *a\*\*ı̂* + *b\*\*ȷ̂*

那么在 *ı̂*, *ȷ̂* 变换后的 *ı̂*′, *ȷ̂*′ 中，在原坐标系中，有，
...
左边一列右边一列（称为矩阵的列）分别表示变换之后的 *ı̂*, *ȷ̂* 基，(*a*, *b*), (*c*, *d*)。
...
在变换的时候，可以先对 *ı̂* 变换，再对 *ȷ̂* 变换，可以方便一点。

如果变换的 *ı̂*, *ȷ̂* 是线性相关的，那么就会丢失一个维度，使张成空间成为一个直线。
...
具体的，正常情况下，*ȷ̂* 在 *ı̂* 的左侧，因此如果反过来了，就意味着空间被翻转。
...
食指、中指分别指向 *ı̂*, *ȷ̂*，此时若拇指指向 *k̂*，则行列式为正，反之为负。
...
则解方程的过程，相当于找到一个向量 *x⃗* 在经过 *A* 的变换后，恰好等于 *v⃗*。

对于 det *A* ≠ 0 的情况，显然解是唯一的，我们可以通过找到 *A* 的逆的方式来求解。
...
意味着把 *ı̂* 变换到 (3, 4, 5)，把 *ȷ̂* 变换到 (1, 1, 9)。
