...
\$\$ \Delta_2=\det Q=[AC]{.abbr title="Alternating Current (交流电)"}-\dfrac{B\^2}{4} \$\$
...
\$\$ \lambda^2-(A+C)\lambda+([AC]{.abbr title="Alternating Current (交流电)"}-B^2/4)=0 \$\$
...
\$\$ e={\sqrt {\frac {2{\sqrt {(A-C)^{2}+B^{\eta (A+C)+{\sqrt {(A-C)}}}^{2}+B^ \$\$}}}}}
...
切点弦方程：设过椭圆外一点 \$A=(x_0,y_0)\$ 有椭圆的切线 \$AB,[AC]{.abbr title="Alternating Current (交流电)"}\$，其中 \$B=(x_1,y_1),C=(x_2,y_2)\$ 为切点，则 \$BC\$ 的方程也为

\$\$ \dfrac{x_0x}{a^2}+\dfrac{y_0y}{b^2}=1 \$\$

证明：根据 \$AB,[AC]{.abbr title="Alternating Current (交流电)"}\$ 是切线，列出 \$AB,[AC]{.abbr title="Alternating Current (交流电)"}\$ 的直线方程
...
（2011 江苏）已知椭圆 \$\dfrac{x\^2}{4} + \dfrac{y\^2}{2} = 1\$，过坐标原点的直线交椭圆于 \$P, A\$ 两点，其中点 \$P\$ 在第一象限，过点 \$P\$ 作 \$x\$ 轴的垂线，垂足为 \$C\$，连接 \$[AC]{.abbr title="Alternating Current (交流电)"}\$ 并延长交椭圆于点 \$B\$，设直线 \$PA\$ 的斜率为 \$k\$，证明：对任意 \$k \> 0\$，求证：\$PA \perp PB\$。

注意到本题中，点 \$P\$ 与点 \$A\$ 关于原点对称，点 \$C\$ 是点 \$P\$ 在 \$x\$ 轴上的射影。而且我们知道求出来的点 \$P\$ 的横坐标比较复杂（需要开方），所以干脆不去求，直接设 \$P(m, km)\$。则立即能知道 \$A(-m, -km), C(m, 0)\$。因而可以求得直线 \$[AC]{.abbr title="Alternating Current (交流电)"}\$ 的斜率为 \$k\_{[AC]{.abbr title="Alternating Current (交流电)"}} = \dfrac{km}{2m} = \dfrac{k}{2}\$。此时可设直线 \$[AC]{.abbr title="Alternating Current (交流电)"}\$ 的方程为 \$y = \dfrac{k}{2}(x - m)\$。联立直线 \$[AC]{.abbr title="Alternating Current (交流电)"}\$ 和椭圆方程，消去变量 \$y\$ 并整理可得

\$\$ \begin{cases} \dfrac{x\^2}{4} + \dfrac{y\^2}{2} = 1 \\ y = \dfrac{k}{2}(x - m) \end{cases} \Rightarrow (k\^2 + 2)x\^2 - 2mk\^2x + k^2m^2 - 8 = 0 \$\$

由韦达定理得 \$x_B + x_A = \dfrac{2mk^2}{k^2 + 2}\$，因为 \$x_A = -m\$。因此有 \$x_B = \dfrac{2mk^2}{k^2 + 2} + m = \dfrac{3mk\^2 + 2m}{k\^2 + 2}\$，代入直线 \$[AC]{.abbr title="Alternating Current (交流电)"}\$ 得
...
\$\$ AB\cdot CD+AD\cdot BC=[AC]{.abbr title="Alternating Current (交流电)"}\cdot BD \$\$

广义托勒密定理指出，任意凸四边形左式大于等于右式，取等当且仅当为圆内接。

\$\$ AB\cdot CD+AD\cdot BC\ge [AC]{.abbr title="Alternating Current (交流电)"}\cdot BD \$\$
...
如右图，假设点 \$P\$ 位于三角形 \$ABC\$ 的外接圆上。在外接圆上选择一点 \$B\'\$，使得直线 \$PB\'\$ 垂直于 \$[AC]{.abbr title="Alternating Current (交流电)"}\$。那么 \$BB\'\$ 平行于 \$P\$ 的西姆松线。
...
\$\$ \bm X=\bm L\times\bm L\'=\vmatrix{\bm i&\bm j&\bm k\\t&1&2t-3\\1&0&2}=(2,3,1) \$\$
