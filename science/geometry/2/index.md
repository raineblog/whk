...
$$
k=\dfrac\{y\_2-y\_1}\{x\_2-x\_1},x\_1\neq x\_2
$$
...
$$
\boxed\{y-y\_0=k(x-x\_0)}\tag\{\scriptsize\text\{点斜式}}
$$
...
$$
\boxed\{y-y\_1=\dfrac\{y\_2-y\_1}\{x\_2-x\_1}(x-x\_1)}\tag\{\scriptsize\text\{两点式}}
$$

可以认为是把点斜式和斜率的公式带入而已，也可以认为是：

$$
\boxed\{\dfrac\{y-y\_1}\{x-x\_1}=\dfrac\{y\_2-y\_1}\{x\_2-x\_1}}\tag\{\scriptsize\text\{两点式}}
$$
...
$$
\begin\{cases} x&=x\_0+\lambda\cos\alpha\ y&=y\_0+\lambda\sin\alpha \end\{cases}
$$
...
特殊的，如果直线过原点，则 $C=0$；对于斜率不存在的，直线方程就为 $x=x\_0$。
...
$$
\boxed\{(x-x\_1)(x-x\_2)+(y-y\_1)(y-y\_2)=0}
$$
...
$$
\begin\{cases} \ell\_1:\&Ax+By+C\_1=0\ \ell\_2:\&Ax+By+C\_2=0 \end\{cases}
$$

写出点到直线距离公式，将另一直线带入分子：

$$
\boxed\{\text\{dis}=\dfrac\{|C\_2-C\_1|}\{\sqrt\{A^2+B^2}}}
$$
...
- 直线和圆的位置关系：相离、相切、相交。
  ...
  1. 圆心到直线的距离，与圆的半径比较。
     ...
     $$
     d=\dfrac\{Ax\_O+By\_O+C}\{\sqrt\{A^2+B^2}}
     $$
...
带入圆的参数方程（$x\_0=r\cos\theta,y\_0=r\sin\theta$）：
...
$$
\boxed\{\dfrac\{(x-x\_a)^2+(y-y\_a)^2}\{(x-x\_b)^2+(y-y\_b)^2}=k^2}
$$
...
$$
2x^2-2(x\_1+x\_2)x+(x\_1^2+x\_2^2)+2y^2-2(y\_1+y\_2)y+(y\_1^2+y\_2^2)-C=0
$$
...
此时，如果 $A,B$ 都在曲线上，则可以通过韦达定理转化的方法，例如若有 $x\_2=\lambda x\_1$ 则

$$
\begin\{aligned} \lambda+\dfrac\{1}\{\lambda}&=\dfrac\{x\_1}\{x\_2}+\dfrac\{x\_2}\{x\_1}\ &=\dfrac\{(x\_1+x\_2)^2}\{x\_1x\_2}-2 \end\{aligned}
$$
