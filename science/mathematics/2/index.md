...
- 子集：[]{.arithmatex}$A \subseteq B$ 或 []{.arithmatex}$B \supseteq A$，表示 []{.arithmatex}$A$ 中的任意元素都属于 []{.arithmatex}$B$

  []{.arithmatex}$A \subseteq A, \varnothing \subseteq A$

  []{.arithmatex}$A \subseteq B$ 且 []{.arithmatex}$B \subseteq C \implies A \subseteq C$

  []{.arithmatex}$A \subseteq B$ 且 []{.arithmatex}$B \subseteq A \implies A=B$

- 真子集：[]{.arithmatex}$A \subsetneqq B$ 或 []{.arithmatex}$B \supsetneqq A$，表示 []{.arithmatex}$A \subseteq B$ 且 []{.arithmatex}$B$ 中至少有一元素不属于 []{.arithmatex}$A$

  []{.arithmatex}$A \subsetneqq B$ 且 []{.arithmatex}$B \subsetneqq C \implies A \subsetneqq C$

- 空集（[]{.arithmatex}$\varnothing$）是任何集合的子集，是任何非空集合的真子集。

- 有 []{.arithmatex}$n$ 个元素的集合，有 []{.arithmatex}$2^n$ 个子集，[]{.arithmatex}$2^n-1$ 个真子集，[]{.arithmatex}$2^n-1$ 个非空子集，[]{.arithmatex}$2^n-2$ 个非空真子集。

- 空集只有一个子集，没有真子集、非空子集、非空真子集。

集合的表示：

- 列举法：[]{.arithmatex}$\{a,b\}$（[]{.arithmatex}$a\neq b$）。

- 描述法：[]{.arithmatex}$\{x\mid x=f(t), p(t)\}$。

- 符号法：[]{.arithmatex}$\R$ 实数集，[]{.arithmatex}$\C$ 复数集，[]{.arithmatex}$\Q$ 有理数集，[]{.arithmatex}$\N$ 自然数集，[]{.arithmatex}$\Z$ 整数集，[]{.arithmatex}$\P$ 质数集。右上角加星号（[]{.arithmatex}$*$）表示去零，右下角加正、负号表示取正、负。

- 图示法：Venn（维恩）图。

### 集合的运算 {#_4}

表示某一元素属于某个集合时，用 []{.arithmatex}$\in$，例如 []{.arithmatex}$1 \in \{1, 2, 3\}$。若不属于则用 []{.arithmatex}$\notin$。

1.  并集：[]{.arithmatex}$A = \{2, 3, 4\}, B = \{1, 2, 3\}, A \cup B = \{1, 2, 3, 4\}$。

2.  交集：[]{.arithmatex}$A = \{2, 3, 4\}, B = \{1, 2, 3\}, A \cap B = \{2, 3\}$。

3.  补集：[]{.arithmatex}$U = \{1, 2, 3\}, A \subseteq U$, 若 []{.arithmatex}$A = \{1\}$, 则 []{.arithmatex}$\complement_U A = \{2, 3\}$。
...
- 原命题：若 []{.arithmatex}$p$ 则 []{.arithmatex}$q$。

- 逆命题：若 []{.arithmatex}$q$ 则 []{.arithmatex}$p$。

- 否命题：若非 []{.arithmatex}$p$ 则非 []{.arithmatex}$q$。

- 逆否命题：若非 []{.arithmatex}$q$ 则非 []{.arithmatex}$p$。

容易知道，原命题与逆否命题同真同假，互为充要条件。

| 量词 | 命题 | 否命题 |
|:--:|:--:|:--:|
| 全称量词 | []{.arithmatex}$\forall x\in M,p(x)$ | []{.arithmatex}$\exist x\in M,\neg p(x)$ |
| 存在量词 | []{.arithmatex}$\exist x\in M,p(x)$ | []{.arithmatex}$\forall x\in M,\neg p(x)$ |
