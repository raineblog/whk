...
设 \$d=\gcd(m,n)\$，可选 \$u,v\>0\$，使 \$mu-nv=d\$. 因此 \$D\|(a^m-1),D\|(a^-1)\$，扩展开来 \$D\|(a^{mu}-a^)=a^{nv}(a^d-1)\$，故 \$D\|(a\^d-1)\$. 综合上述原命题得证。
...
- 算数基本定理：\$N = p_1^{c_1}p_2^}\dots p_n\^{c_n}\$，\$N\$ 的正约数个数 \$=\red{\boxed{\displaystyle\prod\_{i=1}\^{n}(c_i+1)}}\$，\$N\$ 的所有正约数的和 \$=\displaystyle\prod\_{i=1^{n}(\displaystyle\sum\_{j=0}^(p_i)^j)=\red{\boxed{\prod\_{i=1}^n\frac{p_i\^{c_i+1}-1}{p_i-1}}}\$.
...
记 \$\displaystyle a_n=1+10^4+\dots+10^=\frac{10^{4n}-1}{10^4-1}\$，接下来分 \$n\$ 为奇偶讨论即可。
...
\$\$\begin{aligned}n^4+4^n&=n\^4+4\cdot 4^{2k}=n^4+4\cdot (2k)^4=n^4+4n^2\cdot(2^k)^2+4\cdot(2k)^4-4n^2\cdot(2^k)^2\\=(n^2+2\cdot 2^{2k})^2-(2n\cdot 2^k)^2=(n^2+2^n+2^{2k+1})(n^2-2^{k+1}n+2^\$\$})\end{aligned
...
提示：\$x(x^2-1)=y^k\implies a^kb^k=(ab)^k,1=a^-b^k=(a^2)^k-b^k\$，导出矛盾。
...
\$\$(x+1)^{x+1}\>x^+(x+1)x^x\>2x^x\implies y,z\leq x\$\$
...
若 \$N = p_1^{c_1}p_2^\right)\$}\dots p_n\^{c_n}\$，则 \$\displaystyle\varphi(N)=N\times\frac{p_1-1}{p_1}\times\frac{p_2-1}{p_2}\times\dots\times\frac{p_n-1}{p_n}=N\cdot\displaystyle\prod\_{质数p\|N}\left(1-\frac{1}{p
...
9.  设 \$f(n)=\displaystyle\sum\_{d\mid n}\varphi(d)\$，利用 \$\varphi\$ 是积性函数，得到：\$\\ 若 \$n,m\$ 互质，则 \$f(nm)=\displaystyle\sum\_{d\mid nm}\varphi(d)=\displaystyle\sum\_{d\mid n}\varphi(d)\cdot\displaystyle\sum\_{d\mid m}\varphi(d)=f(n)f(m)\$，即 \$f(n)\$ 是积性函数。\$\\ 对于单个质因子有：\$\begin{aligned}f(p\^m)&=\displaystyle\sum\_{d\mid p^m}\varphi(d)=\displaystyle\sum\_{i=0}^\varphi(p^i)=\varphi(1)+\varphi(p)+\varphi(p^2)+\varphi(p^3)+\dots+\varphi(p^m)\\= 1+(p-1)+(p-1)p+(p-1)p^2+\dots+(p-1)p^\\=1+(p-1)+(p^2-p)+(p^3-p^2)+\dots+(p^m-p^{m-1})=p^m\end{aligned} \\ 所以 \$f(n)=\displaystyle\prod\_{i=1}^{m}f(p_i^})=\displaystyle\prod\_{i=1^{m}p_i^=n\$
...
积性函数：\$\varphi(n),\sigma *k(n)=\displaystyle\sum*d\^k,\sigma \_0(n)\$ 通常简记为 \$d(n)\$ 或 \$\tau(n)\$，\$\sigma \_1(n)\$ 通常简记为 \$\sigma(n)\$。
...
例如可证明 \$a,b,c,d\in\N^\*,a^\$ 能被 \$240=2\^4\times 3\times 5\$ 整除。}-a\^{4c+d

例 1：设 \$a,b,c\in\Z,a+b+c=0,d=a^{1999}+b^\$，证明 \$d\$ 可被 \$6\$ 整除。}+c\^{1999
...
\$\$a^b\equiv\begin{cases}a^ m)\$\$} \varphi(m)}&\text{if }\gcd(a,m)=1\\ a\^b&\text{if }\gcd(a,m)\neq 1,b\<\varphi(m)\\ a\^{b \mathrm{mod} \varphi(m)+\varphi(m)}&\text{if }\gcd(a,m)\neq 1,b\geq\varphi(m)\end{cases} (\mathrm{mod
...
若 \$p\$ 是质数，则对于模 \$p\$ 意义下的 \$n\$ 次整系数多项式 \$f(x)=a_nx^n+a\_{n-1}x^ p)\$ 至多有 \$n\$ 个不同的解。}+\dots+a_0(p\nmid a_n)\$ 同余方程 \$f(x)\equiv 0(\mathrm{mod
