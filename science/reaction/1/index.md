...
对于溶液 []{.arithmatex}$\ce{(aq)}$ 或气体 []{.arithmatex}$\ce{(g)}$，定义：
...
单位通常为 []{.arithmatex}$\pu{mol/(L*s)}$ 或 []{.arithmatex}$\pu{mol/(L*min)}$，[]{.arithmatex}$1\pu{mol/(L*s)}=60\pu{mol/(L*min)}$。

在高中阶段，反应速率只是反应在某一个阶段的平均速率，瞬时速率高中通常不考虑；对于瞬时反应速率则是考虑 []{.arithmatex}$t$ 经过微小时间的导数。

我们知道：在同一个方程式中 []{.arithmatex}$v$ 正比于其化学计量数，即：
...
其本质就是，速率大小的比较，理论上需要化为一个物质的速率（归一法），而我们假设方程式中存在一个化学计量数恒为 []{.arithmatex}$1$ 的物质，用其来反应反应的速率大小即可。
...
我们在热化学中介绍了反应进度的问题，如果你不知道，可以将 []{.arithmatex}$\xi$ 理解为，对于一个特定的反应（方程式扩倍影响反应）发生了多少摩尔，因此，当我们把反应系数扩大到 []{.arithmatex}$k$ 倍，对于的 []{.arithmatex}$\xi$ 应当变为 []{.arithmatex}$1/k$ 的大小。
...
表示单位时间，单位体积内发生的反应进度。参照 whk wiki，[]{.arithmatex}$\dfrac{\xi}V$ 称为「反应进度浓度」，对于的反应速率称为「反应进度浓度速率」。

### 其它反应速率 {#_5}

化学反应速率也有别的定义方式，如：

- 单位时间内**物质的量**变化量，即 []{.arithmatex}$v_n=\dfrac{|\Delta n|}t$。

- 单位时间内**物质质量**变化量，即 []{.arithmatex}$v_m=\dfrac{|\Delta m|}t$。

- 不同反应速率的性质不同，如 []{.arithmatex}$\dfrac{|\Delta n|}t$ 定义的反应速率适用于任何形态的物质。
...
| 浓度（[]{.arithmatex}$\pu{mol/L}$） | []{.arithmatex}$\ce{\alpha A}$ | []{.arithmatex}$\ce{+}$ | []{.arithmatex}$\ce{\beta B}$ | []{.arithmatex}$\ce{<=>}$ | []{.arithmatex}$\gamma C$ |
|:--:|:--:|:--:|:--:|:--:|:--:|
| **起**始浓度 | []{.arithmatex}$a_1$ |  | []{.arithmatex}$b_1$ |  | []{.arithmatex}$c_1$ |
| **转**化浓度 | []{.arithmatex}$-a_2$ |  | []{.arithmatex}$-b_2$ |  | []{.arithmatex}$+c_2$ |
| **平**衡浓度 | []{.arithmatex}$a_3$ |  | []{.arithmatex}$b_3$ |  | []{.arithmatex}$c_3$ |
...
- 气体体系反应中的气体均为理想气体，满足 []{.arithmatex}$pV = nRT$（详见热力学）。
...
|  | 相关性 | 说明 |
|:--:|:--:|----|
| 温度 []{.arithmatex}$T$ | 正相关 | 温度每升高 []{.arithmatex}$\pu{10K}$，速率提升 []{.arithmatex}$2\sim4$ 倍 |
| 催化剂 []{.arithmatex}$\mathrm{Cat}$ | 正相关 | 降低反应速率的称为抑制剂或负催化剂 |
| 浓度 []{.arithmatex}$c$ | 正相关 | 固体、纯液体除外 |
| 压强 []{.arithmatex}$p$ | 正相关 | 有气体参与的参与 |
| 接触面积 []{.arithmatex}$m$ | 正相关 | 固体与液体、气体的接触 |
| 其他 |  | 射线、光等 |
...
- **恒温恒容**的容器中，有一气体为基础的反应正在进行，现在向容器充入惰性气体，反应速率如何变化？
  ...
  解析：虽然 \$p_总 \$ 升高，\$c_总 \$ 升高，然而每个参与反应的气体物质的量不变，容积不变，因此物质的量浓度不变。于是，反应速率不变。或者说，[]{.arithmatex}$c=n/V$，根据气体的体积就是容器的容积，因此物质的量浓度并没有改变。

- **恒温恒压**的容器中，有一气体为基础的反应正在进行，现在向容器充入惰性气体，反应速率如何变化？

  答案：变小。

  解析：充入气体后，根据 []{.arithmatex}$pV = nRT$，[]{.arithmatex}$n$ 增多，[]{.arithmatex}$p$ 不变，因此 []{.arithmatex}$V$ 变大。每个参与反应的气体物质的量不变，容积变大，因此物质的量浓度均变小。于是，反应速率变小。这里因为每个气体反应物的物质的量浓度均变小，因此分压也均变小，失去的这一部分总压被充入的惰性气体的分压弥补，**总压强不变**。
...
每一步反应都成为**基元反应**。这里的 []{.arithmatex}$\ce{I.}$ 是带有单电子的原子，这种带有单电子的原子或原子团称作**自由基**。自由基的反应活性极强，稳定性极差，寿命极短。
...
其「反应进度浓度瞬时速率」[]{.arithmatex}$v$ 与 []{.arithmatex}$c^a(\ce A)c^b(\ce B)$ 成正比，即
...
其中的 []{.arithmatex}$k$ 称作**反应速率常数**，与反应物浓度无关，与温度、催化剂、固体表面积性质等有关，不同化学反应也有不同的反应速率常数。

对于一个非基元反应（复杂反应），速率模型可能非常复杂（例如酶促反应的米氏动力学模型等），简单的模型可能认为其与 []{.arithmatex}$c^n(\ce A)c^m(\ce B)$ 成正比，其中 []{.arithmatex}$n,m$ 为实验测量结果；又或者加入生成物的贡献。一般情况下根据题目给出的模型计算即可。

注意到，如果我们对一个可逆反应用正逆两次质量作用定律，以及两次阿伦尼乌斯方程，就可以得到平衡常数 []{.arithmatex}$K=\dfrac{k_正}{k_逆}$。

### 阿伦尼乌斯方程 {#_16}

在质量作用定律​中，我们定义了一个速率常数 []{.arithmatex}$k$，阿伦尼乌斯方程指出：
...
- []{.arithmatex}$A$ 称作指前因子（阿伦尼乌斯常数），是一个常数。

- []{.arithmatex}$\operatorname{exp}(x)$ 的含义为 []{.arithmatex}$e^x$。

- []{.arithmatex}$E_a$ 为反应的活化能，单位为焦尔每摩尔（[]{.arithmatex}$\pu{J/mol}$）或千焦每摩尔（[]{.arithmatex}$\pu{kJ/mol}$），在温度变化范围不大时被视为常数。

- []{.arithmatex}$R$ 为理想气体常数。

- []{.arithmatex}$T$ 为绝对温标下的温度，单位为开尔文（K）。

该经验公式表明：

- 因为 []{.arithmatex}$E_a>0$ 是朴素的，因此温度越高，反应速率常数增大，反应速率通常随之提高。

- 活化能越高，温度升高时反应速率增加得越快，反应速率对温度越敏感；高温对活化能高的反应有利，低温对活化能低的反应有利。

- 对于一个 []{.arithmatex}$E_{a 正}> E_{a 逆}$ 的反应（吸热反应），若温度升高，正逆反应的速率均增大，但是 \$k_正 \$ 增大的幅度更大，所以 \$v_正 \$ 增大的倍数也更大，所以反应正向移动，同时化学平衡常数 []{.arithmatex}$K$ 也增大。
