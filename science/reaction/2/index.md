...
|                       前提                      | 描述                                                                            |
| :-------------------------------------------: | ----------------------------------------------------------------------------- |
| `<nowrap>`\{=html}**恒温恒容**`</nowrap>`\{=html} | 体系内部压强与气体物质的量成正比，气体总物质的量增大，容器总压强也增大。                                          |
...
$$
\dfrac\{|\Delta n(\ce A)|}\{\nu\_A} = \dfrac\{|\Delta n(\ce B)|}\{\nu\_B} = \dfrac\{|\Delta n(\ce C)|}\{\nu\_C} = \dots = \xi
$$
...
之所以写 $K\_c$，是因为高中大部分平衡常数都指浓度平衡常数。

**反应商**，是反映平衡进行的物理量，用符号 $Q\_c$ 表示。
...
**平衡常数**，是可逆反应达到平衡时其反应商，用符合 $K\_c$ 表示：

- 当一个可逆反应达到平衡状态时：

  $$
  K\_c=\dfrac\{\[c(C)]^c\[c(D)]^d}\{\[c(A)]^a\[c(B)]^b}
  $$
...
|   相加  | 相加（$\Delta H\_1 + \Delta H\_2$） | 相乘（$K\_1 \cdot K\_2$） |
...
- 当 $Q\_\{c}\<K\_\{c}$ 时：平衡向右移动。

- 当 $Q\_\{c}=K\_\{c}$ 时：达到平衡状态。

- 当 $Q\_\{c}>K\_\{c}$ 时：平衡向左移动。

### 压强平衡常数 $K\_p$ \{#k\_p}

压强平衡常数，用符号 $K\_p$ 表示，类似浓度平衡常数：
...
- 对于反应：
  ...
  $$
  K\_p=\dfrac\{\{P\_C}^p\times \{P\_D}^q}\{\{P\_A}^m \times \{P\_B}^n}
  $$
...
- 告诉**恒温恒压**，并告诉开始时的压强，需要列四段式，求出每个物质的量分数 $n%$，然后利用 $P\_分 = P\_总 \times n%$ 计算即可。

- 告诉**恒温恒容**，压强会随着反应气体计量数变化而变化，需要根据初始压强 $P\_平 $ 及 $\dfrac\{P\_平}\{P\_0}=\dfrac\{n\_平}\{n\_0}$ 计算出 $P\_平 $，然后求出 $n%$，再按照上一种情况进行计算。

- 列四段式计算出每个物质的量分数 $n%$ 后，最好通过将计算出所有物质的量分数相加看是否等于 $1$ 来进行验算，避免计算时出现错误。

- 按照 $K\_p$ 的计算公式计算平衡常数时，对于总压可以直接观察并计算出上下约分后剩下的总压次数，无需每次计算。 对于平行反应的 $K\_p$ 计算，对应的分压带的是主副反应的总分压，而非单独主反应。求解某物质单独分压时，可设数列方程求解。
...
$$
K\_p=K\_n\cdot\paren\{\dfrac\{p\_0}\{n\_\{\text\{总}}}}^\{\Delta n\_\{g}}
$$

$$
K\_p=K\_c\cdot\paren\{\dfrac\{p\_0}\{c\_\{\text\{总}}}}^\{\Delta n\_\{g}}
$$

若恒温恒容，则

$$
p\_A=\dfrac\{n\_A}\{n\_0}\cdot p\_0
$$
...
- $v\_正 > v\_逆 $：平衡向正反应方向移动；
- $v\_正 = v\_逆 $：反应达到平衡状态，不发生平衡移动；
- $v\_正 \< v\_逆 $：平衡向逆反应方向移动；
...
等效平衡下，正逆反应的反应物转化率 $\alpha\_正 + \alpha\_逆 = 1$。
...
- 减容：假设在 $T,V$ 环境下，二氧化氮的浓度为 $c\_0$，那么，恒温 $V/2$，$c\_0\<c\_x\<2c\_0$，容器内气体变浅。
