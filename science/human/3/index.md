...
![alt text](../%E7%BB%86%E8%83%9E%E5%A4%96%E6%B6%B2.avif){width="50%"}
...
- 血浆中约 []{.arithmatex}$90\%$ 为水；其余 []{.arithmatex}$10\%$ 分别是：蛋白质（[]{.arithmatex}$7\% \sim 9\%$），无机盐（约 []{.arithmatex}$1\%$），以及血液运输的其他物质，包括各种营养物质（如葡萄糖）、激素、各种代谢废物等。
...
- **渗透压**是指溶液中溶质微粒对水的吸引力。
  ...
  在组成细胞外液的各种无机盐离子中，含量上占有明显优势的是 []{.arithmatex}$\ce{Na+}$ 和 []{.arithmatex}$\ce{Cl-}$。细胞外液渗透压的 []{.arithmatex}$90\%$ 以上来源于 []{.arithmatex}$\ce{Na+}$ 和 []{.arithmatex}$\ce{Cl-}$。在 []{.arithmatex}$\pu{37^oC}$ 时。人的血浆渗透压约为 []{.arithmatex}$\pu{770 kPa}$，相当于细胞内液的渗透压。

  我们将会在水盐平衡部分详细讨论渗透压的稳态。

- 正常人的血浆近中性，pH 为 []{.arithmatex}$7.35 \sim 7.45$。血浆的 pH 之所以能够保持稳定，与其中含有的 []{.arithmatex}$\ce{HCO3-,H2CO3}$ 等物质有关。

  模拟生物体维持 pH 的稳定：在溶液中加入酸或碱，缓冲对（如 []{.arithmatex}$\ce{HPO4^2- / H2PO4-}$）能使溶液 pH 的变化减弱；与自来水相比，生物组织匀浆更类似于缓冲液。

  ![alt text](../image-4.avif){width="60%"}

  人体内环境中也有很多缓冲对，其中最重要的是 []{.arithmatex}$\ce{HCO3- / H2CO3}$，其次还有 []{.arithmatex}$\ce{HPO4^2- / H2PO4-}$ 等。当一定量的酸性或碱性物质进入后，内环境的 pH 仍能维持在一定范围内。

- 人体细胞外液的温度一般维持在 []{.arithmatex}$\pu{37^oC}$ 左右（此处的体温是一个总的概念，并不是通过测量皮肤的温度就可以得出的体温）。

  正常情况下，不同人的体温，会因年龄、性别等的不同而存在微小差异；同一个人的体温在一日内也有变化，但变化幅度一般不超过 []{.arithmatex}$\pu{1^oC}$。尽管气温波动范围较大，但健康人的体温始终接近 []{.arithmatex}$\pu{37^oC}$。
...
<td colspan="2" class="tg-c7ow"><span style="font-weight:400">验证或探究某种激素的生理作用</span></td>
</tr>
<tr>
<td class="tg-c7ow"><span style="font-weight:400">实验设计</span></td>
<td class="tg-3f42"><span style="font-weight:400">②不摘除内分泌腺（手术但不摘除）</span><br />
<span style="font-weight:400">③对摘除内分泌腺的实验组植入内分泌腺（或补充激素）</span></td>
<td class="tg-3f42"><span style="font-weight:400">②不饲喂动物激素（饲喂等量正常饲料）</span><br />
<span style="font-weight:400">③施加动物激素抑制剂</span></td>
<td class="tg-3f42"><span style="font-weight:400">②不注射动物激素（注射等量生理盐水）</span><br />
<span style="font-weight:400">③施加动物激素抑制剂</span></td>
</tr>
</tbody>
</table>
...
- 作为信使传递信息：激素种类多、量极微，既不组成细胞结构，又不提供能量，也不起催化作用，而是随体液到达靶细胞，使靶细胞原有的生理活动发生变化。激素是调节生命活动的信息分子，一经靶细胞接受并起作用后就失活了。因此，体内需要源源不断地产生激素，以维持激素含量的动态平衡。
  ...
  激素一受体复合物进入细胞核后结合于染色质的非组蛋白的特异位点上，启动或抑制该部位的 [DNA]{.abbr title="Deoxyribonucleic Acid (脱氧核糖核酸)"} 转录过程，进而促进或抑制 [mRNA]{.abbr title="messenger RNA (信使 RNA)"} 的形成，结果是诱导或减少某种蛋白质（主要是酶）的合成，引起相应的生物效应。
  :::
  :::::
...
除激素外，其他一些化学物质，如组胺、某些气体分子（[]{.arithmatex}$\ce{NO}$、[]{.arithmatex}$\ce{CO}$ 等）以及一些代谢产物（如 []{.arithmatex}$\ce{CO2}$），也能作为体液因子对细胞、组织和器官的功能起调节作用。[]{.arithmatex}$\ce{CO2}$ 是调节呼吸运动的重要体液因子。体液中 []{.arithmatex}$\ce{CO2}$ 浓度变化会刺激相关感受器，从而通过神经系统对呼吸运动进行调节。临床上给患者输入 []{.arithmatex}$\ce{O2}$ 时，往往采用含有 []{.arithmatex}$5\%$ 左右的 []{.arithmatex}$\ce{CO2}$ 的混合气体，以达到刺激呼吸中枢的目的。
...
![alt text](../image-19.avif){width="90%"}
...
![alt text](../raineblog.dpdns.org_whk_science_human_3_.avif)
...
- 图中黄色部分：胰岛 B 细胞（[]{.arithmatex}$\beta$ 细胞），分泌胰岛素。

- 图中绿色部分：胰岛 A 细胞（[]{.arithmatex}$\alpha$ 细胞），分泌胰高血糖素。

- 图中紫色部分：胰岛 D 细胞（[]{.arithmatex}$\delta$ 细胞），分泌生长抑素。
...
糖皮质激素（Glucocorticoids, GCs）是一类由肾上腺皮质（束状带）分泌的甾体激素，其中对人类最重要、活性最强的是**皮质醇（Cortisol）**。它们之所以被称为"糖"皮质激素，最初是因为它们对糖代谢有显著调节作用，但实际上它们几乎影响人体所有的器官系统。其作用机制主要分为**基因组效应**（结合细胞内受体，调节基因转录，起效慢）和**非基因组效应**（直接作用于细胞膜或线粒体，起效快）。

1.  糖代谢

    - **作用：**升高血糖。
    - **通路与机制：**
      1.  **促进糖异生：**在肝脏中，诱导糖异生关键酶（如磷酸烯醇式丙酮酸羧激酶 PEPCK）的基因转录，将氨基酸、甘油等非糖物质转化为葡萄糖。
      2.  **抑制外周利用：**抑制肌肉和脂肪组织对葡萄糖的摄取和利用（减少 GLUT4 转运体移向细胞膜）。
    - **效果：**保证在饥饿或应激状态下大脑有足够的葡萄糖供应。
    - **失调副作用（过量）：类固醇性糖尿病**。长期高水平会导致血糖持续升高，诱发胰岛素抵抗。

2.  蛋白质代谢

    - **作用：**促进分解，抑制合成。
    - **通路与机制：**
      1.  激活泛素-蛋白酶体途径（Ubiquitin-proteasome pathway），加速骨骼肌蛋白分解为氨基酸（为肝脏糖异生提供原料）。
      2.  抑制氨基酸进入肌细胞。
    - **效果：**消耗肌肉储备以应对紧急能量需求。
    - **失调副作用（过量）：**
      - **肌肉萎缩、肌无力**（四肢纤细）。
...
3.  脂肪代谢
    ...
    - **作用：**脂肪重新分布（Lipid Redistribution）。
    - **通路与机制：**
      1.  **四肢：**增强脂肪酶活性，促进脂肪分解。
      2.  **躯干/面部：**这里的脂肪细胞对胰岛素更敏感（胰岛素因高血糖而代偿性分泌增加），从而促进脂肪合成。
    - **效果：** 改变身体脂肪储存模式。
    - **失调副作用（过量）：向心性肥胖**（Cushing 综合征典型特征），表现为满月脸（Moon face）、水牛背（Buffalo hump）、腹部肥胖，但四肢相对瘦小。

4.  抗炎与免疫抑制作用：这是糖皮质激素在临床药理上最主要的应用领域。
    ...
    - **通路与机制：**
      ...
      2.  **抑制转录因子NF-κB**：直接结合并抑制 NF-κB，从而阻断多种促炎细胞因子（如TNF-[]{.arithmatex}$\alpha$, IL-1, IL-6, IL-8）的基因转录。
      3.  **诱导淋巴细胞凋亡：**直接诱导 T 细胞和 B 细胞的凋亡，降低抗体生成。
      4.  **稳定溶酶体膜：**防止溶酶体破裂释放水解酶损伤组织。
    - **效果：**红、肿、热、痛迅速消退，过敏反应停止，自身免疫病缓解。
    - **失调副作用（过量）：**
      - **严重的免疫抑制**：极易并发细菌、病毒、真菌感染。
      - **结核病复发**：体内潜伏的结核杆菌可能因免疫力下降而扩散。
      - **掩盖感染症状**：因不发热、无炎症反应，导致感染加重却未被发现。

5.  心血管系统调节

    - **作用：**维持血压，允许作用（Permissive Action）。
    - **通路与机制：**
      1.  **允许作用：**糖皮质激素本身不直接收缩血管，但它能增加血管平滑肌对儿茶酚胺（肾上腺素、去甲肾上腺素）的敏感性（上调 []{.arithmatex}$\alpha1$ 受体表达）。如果缺乏GC，血管对升压药会失去反应。
...
6.  骨骼与钙磷代谢
    ...
    - **作用：**调节钙平衡（通常表现为负钙平衡）。
    - **通路与机制：**
      1.  **肠道：**拮抗维生素 D 的作用，抑制肠道对钙的吸收。
      2.  **骨骼：**直接抑制成骨细胞（造骨）的功能，并诱导其凋亡；同时通过继发性甲状旁腺激素（PTH）升高，间接激活破骨细胞（溶骨）。
    - **效果：**血液中钙来源减少，不得不从骨头中"借"钙。
    - **失调副作用（过量）：**
      - **骨质疏松症**：极易发生压缩性骨折。
      - **股骨头无菌性坏死**：这是大剂量激素冲击治疗的严重并发症之一，机制涉及骨内血管脂肪栓塞和成骨细胞死亡。
      - **儿童生长发育迟缓**。
...
7.  神经系统与应激反应
    ...
    - **作用：**调节情绪、认知和兴奋性。
    - **通路与机制：**
      - 糖皮质激素受体（GR）广泛分布于海马体和杏仁核。适量的激素是维持正常神经兴奋性所必需的。
    - **效果：**提高机体对有害刺激（创伤、寒冷、惊恐）的耐受力（即"应激反应"）。
    - **失调副作用（过量）：**
      - **中枢兴奋**：失眠、欣快感、激动。
      - **类固醇性精神病**：严重的可能出现幻觉、躁狂或重度抑郁。
      - **记忆力减退**：长期高皮质醇对海马体神经元有毒性作用。
...
8.  血液系统
    ...
    - **通路与机制：**
      1.  刺激骨髓造血功能。
      2.  改变细胞在血管内的分布（边缘池 vs 循环池）。
    - **效果：**
      - **增加：**红细胞、血红蛋白、血小板、**中性粒细胞**（注意：虽然数量增加，但游走和吞噬功能是下降的，这是因为激素让中性粒细胞从血管壁脱落进入血液循环，即"边缘池"释放）。
      - **减少：**淋巴细胞、嗜酸性粒细胞、嗜碱性粒细胞。
    - **失调副作用（过量）：** 红细胞增多症，血栓风险增加（因血小板升高）。

激素过多（库欣综合征 Cushing\'s Syndrome）：如果体内糖皮质激素长期过量（无论是自身肿瘤分泌还是药物摄入），患者会呈现典型的**满月脸、水牛背、向心性肥胖、皮肤紫纹、高血压、高血糖、骨质疏松**。

激素缺乏（艾迪生病 Addison\'s Disease）：如果肾上腺皮质功能减退，患者会出现**低血糖、低血压（休克风险）、消瘦、乏力、皮肤色素沉着（因为 ACTH 分泌增多导致黑色素生成）**。

医源性肾上腺皮质萎缩（停药反应）：

- **机制：**长期外源性补充糖皮质激素，会通过负反馈机制强烈抑制下丘脑-垂体-肾上腺轴（HPA轴），导致患者自身的肾上腺皮质萎缩，停止分泌皮质醇。
- **后果：**如果突然停药，患者自身的肾上腺无法立刻恢复工作，会导致急性肾上腺皮质功能衰竭（肾上腺危象），表现为严重休克甚至死亡。
- **对策：**长期用药者必须**缓慢减量（Tapering）**，不可骤停。
...
![alt text](../%E6%97%A0%E6%A0%87%E9%A2%98-2025-09-14-2104.svg){width="80%"}

血糖平衡的调节，也就是调节血糖的来源和去向，使其处于平衡状态。

![alt text](../raineblog.dpdns.org_whk_science_human_3_%281%29.avif)
...
![alt text](../image-20.avif){align="right" width="40%"}
...
- 钠离子 []{.arithmatex}$\ce{Na+}$ 的主要来源是食盐，几乎全部由小肠吸收，主要经肾随尿排出，排出量几乎等于摄入量。

- 钾离子 []{.arithmatex}$\ce{K+}$ 不仅在维持细胞内液的渗透压上起决定性作用，而且还具有维持心肌舒张、保持心肌正常兴奋等重要作用。
...
- 炎炎夏季，在高温环境中从事体力劳动的人，最容易发生中暑。中暑是指高温引起机体体温调节功能紊乱所表现出的一系列症状，如高热、皮肤干燥、恶心、呕吐、食欲不振、心悸、头痛等。严重中暑会使体温超过 []{.arithmatex}$\pu{40^oC}$，病死率比较高。
