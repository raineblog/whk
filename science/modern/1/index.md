...
分子势能：分子间存在着相互作用力，可以证明分子间的作用力所做的功与路径无关，分子组成的系统具有分子势能。分子势能的大小由分子间的相对位置决定，这说明分子势能 *E\*\*p* 与分子间距离 *r* 是有关系的。设两个分子相距无穷远，我们可以规定它们的分子势能为 0。让一个分子 A 不动，另一个分子 B 从无穷远处逐渐靠近 A。在这个过程中，分子间的作用力做功，分子势能的大小发生改变。

当分子 B 向分子 A 靠近，分子间距离 *r* 大于 *r*0 时，分子间的作用力表现为引力，力的方向与分子的位移方向相同，分子间的作用力做正功，分子势能减小。当分子间距离 *r* 减小到 *r*0 时，分子间的作用力为 0，分子势能减到最小。越过平衡位置 *r*0 后，分子 B 继续向分子 A 靠近，分子间的作用力表现为斥力，力的方向与分子的位移方向相反，分子间的作用力做负功，分子势能增大。

可见，分子势能的大小是由分子间的相对位置决定的。由以上分析可知，如果选定分子间距离 *r* 为无穷远时的分子势能 *E\*\*p* 为 0，分子势能 *E\*\*p* 随分子间距离 *r* 的变化有最小值，即当 *r* = *r*0 时，分子势能最小。物体的体积变化时，分子间距离将发生变化，因而分子势能随之改变。可见，分子势能与物体的体积有关。
...
- **功（*W*）**：外界对系统所做的机械量贡献。在准静态过程中，体积功为 *W* = −∫*p*d*V*。
- **热量（*Q*）**：因温度差而引起的能量转移。
...
对于任何热机，从高温热库吸收热量 *Q*1，对外做功 *W*，向低温热库排出热量 *Q*2，其效率 *η* 定义为：
...
这是热机效率的理论极限。它告诉我们，要提高发动机效率，要么提高燃烧温度（提高 *T*H），要么优化散热（降低 *T*L）。
...
体系的一个状态函数或者几个状态函数发生了改变时，体系的状态发生变化，称体系经历了一个热力学过程，简称过程。体系发生变化前的状态称作始态，变化后的状态称作终态。体系变化的始态和终态一旦确定，各状态函数的改变量也随之确定。状态函数的改变量用 *Δ* 表示，如始态温度 *T*1，终态温度 *T*2，我们有状态函数改变量 *Δ\*\*T* = *T*2 − *T*1。有一些过程具有值得研究的特殊性，如：
...
**功的计算**：在 P-V 图上，气体对外做功或外界对气体做功，可以通过曲线下的面积来表示。 根据热力学第一定律中功的定义 *W* = −∫*p*d*V*：
...
其中 *p*ext 是外界施加在系统边界上的压强，而非系统内部的压强。功的本质是“系统克服外部阻力推动边界位移“。只有在准静态（可逆）过程中，外压与内压始终相差无穷小，才能用内部压强 *p* 代替 *p*ext 来积分。
...
这说明“体积增大“不是做功的充分条件。同样，有压强也不等于在做功——刚性容器里加热气体，压强升高、温度升高，但体积不变（d*V* = 0），因此 *W* = 0。必须同时有压强和边界位移，才有体积功。
...
其中 *γ* = *C\*\*p*/*C\*\*V* 称为**绝热指数**。这一公式解释了为什么喷漆罐在使用时会变冷，以及柴油机通过快速压缩空气来点火的原理。
...
回顾压强的相关概念，大气压强是 $\\pu{1atm}\\approx\\pu{1bar}=\\pu{E5Pa}$，液体压强会用 *ρ**g**h* 表达，而在很多情境下，我们通常会用毫米汞柱（$\\pu{mmHg}$）来表示压强，大气压强约为 $\\pu{76mmHg}$。

气体压强的微观解释：从分子动理论的观点来看，气体对容器的压强源于气体分子的热运动，当它们飞到器壁时，就会跟器壁发生碰撞（可视为弹性碰撞），就是这个撞击对器壁产生了作用力，从而产生了压强。选择一个与器壁发生正碰的气体分子为研究对象，由于是弹性碰撞，所以气体分子与器壁碰撞前后的动量大小为 *m\*\*v*，方向相反，气体分子受到的冲量为
...
不妨设单位体积内有 *n* 个气体分子，气体分子的速度为 *v*，单个字体分子的平均质量为 *m*、平均动能为 *ϵ̄\*\*k*，再结合温度与平均平动动能的关系，可以得到：
...
- **体积修正**：常数 *b* 修正分子固有体积，约等于 $\\pu{1 mol}$ 分子体积的 4 倍。分子本身占据体积，使分子自由活动的空间减小为 *V* − *n\*\*b*。
- **压强修正**：常数 *a* 修正分子间引力，反映分子间引力强度。容器内部的分子受到周围分子的引力作用，使得实际压强比理想压强小，因此在内压强上增加一项。

密闭容器中理想气体在**单位时间内、单位面积上与容器壁的碰撞次数**（在物理学中通常记为碰撞频率 *Z* 或 *Γ*），与你提到的**压强、温度、分子密度都有关**，此外还与**气体分子的质量**有关。不过，由于理想气体状态方程（*P* = *n**k**T*）将压强、温度和分子密度联系在了一起，因此这三个量并不是完全独立的。
...
而分子的平均速率 *v̄* 与**温度 *T*** 和 **分子质量 *m*** 有关：
...
根据理想气体状态方程微观形式 *P* = *n**k**T*，我们可以把分子密度 *n* 替换为 *P*/(*k\*\*T*)，代入上面的公式中，可以得到碰撞次数与压强的关系式：
...
- **气体分压定律**（道耳顿分压定律）：容器中混合气体对器壁所产生的压强等于在同样温度、体积条件下组成混合气体各成分单独存在时的分压强之和。
  ...
  下面是基于理想气体状态方程的推导。由 *p* = *c**R**T*，我们容易推得：
  ...
  某组分气体分压等于容器总压乘以该组分气体的物质的量分数。对于物质的量，使用 *p* = *n**k**T* 即可，推导步骤类似，略。
...
波尔茨曼常数 *k*，理想气体常数 *R*，阿伏伽德罗常数 *N\*\*A*，满足：
...
- **循环过程**：对于一个封闭的循环路径（回到初态），内能的变化量 *Δ\*\*U* = 0。由热力学第一定律 *Δ\*\*U* = *Q* + *W* 可知，*Q* = −*W*。也就是说，循环吸收的净热量等于系统对外的净功（图像包围的面积）。
...
对于汞液柱问题，通常用 $\\pu{cmHg}$ 作为压强的单位，因为这样可以把 *m**g**h* 直接写成 *h*（液面的高度差），与 $\\pu{Pa}$ 不统一。
...
**例题**：一竖直放置的气缸由两个粗细不同的圆柱形筒组成，气缸中活塞Ⅰ和活塞Ⅱ之间封闭有一定量的理想气体，两活塞用一轻质弹簧连接，气缸连接处有小卡销，活塞Ⅱ不能通过连接处。活塞Ⅰ、Ⅱ的质量分别为 2*m*、*m*，面积分别为 2*S*、*S*，弹簧原长为 *l*。初始时系统处于平衡状态，此时弹簧的伸长量为 0.1*l*，活塞Ⅰ、Ⅱ到气缸连接处的距离相等，两活塞间气体的温度为 *T*0。已知活塞外大气压强为 *p*0，忽略活塞与缸壁间的摩擦，气缸无漏气，不计弹簧的体积。

缓慢加热两活塞间的气体，求当活塞Ⅱ刚运动到气缸连接处时，活塞间气体的压强和温度。

**分析**：本题的难点在于气缸上下横截面积不同（上方 2*S*，下方 *S*），存在一个面积为 *S* 的台阶面。这导致很多人在“整体分析法“中产生困惑：台阶面对气体有力的作用吗？整体分析还成立吗？
...
*p*0 ⋅ 2*S* + 2*m\*\*g* + *m\*\*g* = *p*0 ⋅ *S* + *p* ⋅ *S*

化简得：

$$ p = p_0 + \\frac{3mg}{S} $$

可见压强 *p* 并非 *p*0，但它是**由常数（重力、大气压）决定的固定值**。只要活塞没有卡住，在缓慢加热过程中，这个压强始终不变——这就是许多参考答案说“气体压强不变“的由来。
...
台阶面正是维持气体自身平衡的关键。若不考虑台阶面，仅看活塞对气体的力（向下 2*p\*\*S*，向上 *p\*\*S*），气体确实受力不平衡。一旦将台阶面的反作用力纳入分析，缺口就被补足了。

如果把气体也纳入“整体“，那么外力就必须包含台阶面对气体的支持力，此时“证明压强不变“就陷入了循环论证——你需要先证明台阶力不变才能证明压强不变。因此，正确的做法是**机械系统和气体分开分析**。

#### 推广：非直柱体容器

上述台阶面的分析可以推广到任意形状的容器。以一个上窄下宽的圆台形容器装满水为例：水深为 *h*，底面积为 *S*底，水的总重力为 *G*水。液体压强给出容器底部的压力：
...
**充气问题**：设想一个容器，初始状态压强为 *p*1、体积为 *V*、温度为 *T*1，充入同种气体后，末状态压强为 *p*2、体积仍为 *V*、温度为 *T*2。

设充入气体的物质的量为 *Δ\*\*n*，这些气体在充入前处于某个状态（比如大气压 *p*0、温度 *T*0、体积 *V*0）。根据物质的量守恒：
...
这就是充气问题的基本方程。如果充入的是大气（*p*0 = *p*atm，*T*0 = *T*环境），则只需确定充入气体的体积 *V*0 即可。

**放气问题**：类似地，容器放气时，设放出气体的物质的量为 *Δ\*\*n*：
...
其中 *p*0、*T*0、*V*0 是放出气体在环境中的状态参量。

**一个实用的技巧**：如果充气或放气过程中温度保持不变（*T*1 = *T*2 = *T*），上述方程可以简化为：
...
**打气问题的特例**：用打气筒给容器打气，每次打入压强为 *p*0、体积为 *V*0 的气体，共打 *n* 次。设容器体积为 *V*，最终压强为 *p*，温度不变。
...
**抽气问题的特例**：用抽气机从容器中抽气，每次抽出体积为 *V*0 的气体。设容器体积为 *V*，初始压强为 *p*0，抽 *n* 次后压强为 *p\*\*n*，温度不变。

每次抽气时，容器内气体膨胀到体积 *V* + *V*0（假设抽气机与容器连通），然后抽气机将气体排出。根据玻意耳定律，第一次抽气后：
...
例如，一个钢瓶装有压强为 *p*0 的气体，每次放出气体的物质的量为 *Δ\*\*n*（在标准状态下体积为 *V*std），共放 *n* 次，求最终压强。
...
**宏观层面：压强平衡。** 水银停在水平管中不动，是因为左右两侧的力完全平衡。以一个典型场景为例，L 形拐角处的压强来自上方开口处的大气压（如 $P_0 = \\pu{75 cmHg}$）加上竖直水银柱（如 $\\pu{5 cm}$）产生的压强，即水银柱右侧受到的向左压强为 $\\pu{80 cmHg}$。左端封闭气体的压强 *P\*\*A* 恰好等于 $\\pu{80 cmHg}$，维持平衡。
...
设大气压强为 *p*0，则地球表面受到的总压力为：

*F* = *p*0*S* = 4*π\*\*p*0*R*2

其中，近似的取 *p*0 = 1.013 × 105Pa，地球半径 *R* = 6.37 × 106m。
...
已知水在 $\\pu{14 ^\\circ C}$ 时的饱和蒸气压为 $\\pu{12.0 mmHg}$，试近似计算在 $\\pu{100 ^\\circ C}$ 和 $\\pu{14 ^\\circ C}$ 时，单位时间内通过单位面积水面的蒸发变为水蒸气的分子数之比 *n*100 : *n*14。
...
设每个容器体积为 *V*，设 *p*0 = 1atm 表示大气压，*T*0 = 273K。

封入 *n*(mol) 的气体，稳定后两个容器分别有 *n*1, *n*2(mol) 的气体，温度 *T*1 = 273K, *T*2 = 373K。
...
在容积为 *V* 的容器中，有气体，压强为 *p*1，连容器重 *G*1。

缓慢放掉一部分气体，测得压强为 *p*2，连容器重 *G*2。
...
一定质量的理想气体，压强为 *p*1、温度为 *T*1，与同体积的压强为 *p*1、温度为 *T*2 的同种气体混合。
...
有 3mol 理想气体，自体积 *V*0 膨胀到 3*V*0，其压强 *P* 随体积 *V* 的变化规律如，

$$ P=P_0\\left(2-{V\\over4V_0}\\right) $$

其中 *p*0 为初始状态的气压，求该气体在此过程中能达到的最高温度。
...
当 *V* = 4*V*0 时，*T* 最大；但是因为 *V*0 ≤ *V* ≤ 3*V*0，因此取 *V* = 3*V*0，
...
1. **内能变化量**：两球完全相同，初始温度相同，都升高了相同的温度 *Δ\*\*T*，且膨胀后的体积相等。材质、初态和末态的温度与体积完全一致，因此两球的内能变化量相等，即 *Δ**U**A* = *Δ**U**B*。
1. **对外做功量**：小球在液体中膨胀，需要推开周围的液体，克服液体的压强做功。液体内部的压强为 *p* = *p*0 + *ρ**g**h*，其中 *p*0 为大气压，*ρ* 为液体密度，*h* 为深度。由于水银的密度远大于水的密度（*ρ*水银 ≫ *ρ*水），两球被固定在同一深度 *h*，因此在整个膨胀过程中，水银中小球所承受的外部压强始终大于水中小球的外部压强。两球膨胀增加的体积相同，在压强更大的水银中膨胀需要做更多的功，即 *W\*\*B* > *W\*\*A*。
1. **结论**：将上述两点代入热力学第一定律公式。由于 *Δ\*\*U* 相同且 *W\*\*B* > *W\*\*A*，因此 *Q\*\*B* > *Q\*\*A*。即**水银中的小球吸收的热量更多**——为了达到相同的最终状态，水银中的小球需要额外吸收更多的热量来克服更大的外部压强做功。

#### 例题八

一根两端封闭的粗细不均匀玻璃管，上粗下细，竖直放置。管内有一段水银柱将理想气体分为 *A*（上方）和 *B*（下方）两部分。现缓慢升高温度，试判断水银柱的移动方向，并比较压强变化量 *Δ**p**A* 与 *Δ**p**B*、压力变化量 *Δ**F**A* 与 *Δ**F**B* 的大小关系。
...
由于两部分气体初始温度 *T* 相同、升高的温度 *Δ\*\*T* 相同，压强增加量 *Δ\*\*P* 与初始压强 *P* 成正比。

根据初始状态的受力平衡，下方气体的压强需要支撑上方气体压强加上水银柱压强，即 *P\*\*B* = *P\*\*A* + *ρ**g**h*，因此初始压强 *P\*\*B* > *P\*\*A*。

若体积不变，下方气体压强增加量更大（*Δ**P**B*′ > *Δ**P**A*′），水银柱受到向上的净力，将**向上移动**。
...
因为 *h*′ \< *h*，所以 *P\*\*B*′ − *P\*\*A*′ \< *P\*\*B* − *P\*\*A*，移项得 *Δ**P**B* \< *Δ**P**A*，即上方气体压强增加量更大。

**第三步：比较压力变化量（本题难点）。**

上方气体对液面压力（向下）*F\*\*A* = *P\*\*A* ⋅ *S\*\*A*，下方气体对液面压力（向上）*F\*\*B* = *P\*\*B* ⋅ *S\*\*B*。
...
由于管子上粗下细（类似漏斗状），倾斜管壁对水银产生垂直于管壁斜向上的弹力，其竖直分量 *F*壁 向上。平衡条件为：
...
取变化量，重力 *G* 不变（*Δ\*\*G* = 0），因此：

*Δ**F**A* − *Δ**F**B* = *Δ\*\*F*壁

温度升高后，管内气体压强增大，水银向外挤压管壁的力变大，根据牛顿第三定律，管壁的托力 *F*壁 也变大，即 *Δ\*\*F*壁 > 0。

代回等式得 *Δ**F**A* − *Δ**F**B* > 0，即 *Δ**F**A* > *Δ**F**B*。
