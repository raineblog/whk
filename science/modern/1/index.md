...
分子势能：分子间存在着相互作用力，可以证明分子间的作用力所做的功与路径无关，分子组成的系统具有分子势能。分子势能的大小由分子间的相对位置决定，这说明分子势能 $E\_p$ 与分子间距离 $r$ 是有关系的。设两个分子相距无穷远，我们可以规定它们的分子势能为 $0$。让一个分子 A 不动，另一个分子 B 从无穷远处逐渐靠近 A。在这个过程中，分子间的作用力做功，分子势能的大小发生改变。

当分子 B 向分子 A 靠近，分子间距离 $r$ 大于 $r\_0$ 时，分子间的作用力表现为引力，力的方向与分子的位移方向相同，分子间的作用力做正功，分子势能减小。当分子间距离 $r$ 减小到 $r\_0$ 时，分子间的作用力为 $0$，分子势能减到最小。越过平衡位置 $r\_0$ 后，分子 B 继续向分子 A 靠近，分子间的作用力表现为斥力，力的方向与分子的位移方向相反，分子间的作用力做负功，分子势能增大。

可见，分子势能的大小是由分子间的相对位置决定的。由以上分析可知，如果选定分子间距离 $r$ 为无穷远时的分子势能 $E\_p$ 为 $0$，分子势能 $E\_p$ 随分子间距离 $r$ 的变化有最小值，即当 $r = r\_0$ 时，分子势能最小。物体的体积变化时，分子间距离将发生变化，因而分子势能随之改变。可见，分子势能与物体的体积有关。
...
对于任何热机，从高温热库吸收热量 $Q\_1$，对外做功 $W$，向低温热库排出热量 $Q\_2$，其效率 $\eta$ 定义为：

$$
\eta = \frac\{W}\{Q\_1} = \frac\{Q\_1 - Q\_2}\{Q\_1} = 1 - \frac\{Q\_2}\{Q\_1}
$$
...
体系的一个状态函数或者几个状态函数发生了改变时，体系的状态发生变化，称体系经历了一个热力学过程，简称过程。体系发生变化前的状态称作始态，变化后的状态称作终态。体系变化的始态和终态一旦确定，各状态函数的改变量也随之确定。状态函数的改变量用 $\Delta$ 表示，如始态温度 $T\_1$，终态温度 $T\_2$，我们有状态函数改变量 $\Delta T = T\_2 - T\_1$。有一些过程具有值得研究的特殊性，如：
...
- **等温过程**：温度 $T$ 恒定。对于理想气体，$pV = nRT = \text\{const.}$。功 $W = -nRT \ln(V\_2/V\_1)$。在 P-V 图上为双曲线。
...
其中 $\gamma = C\_p / C\_\{V}$ 称为**绝热指数**。这一公式解释了为什么喷漆罐在使用时会变冷，以及柴油机通过快速压缩空气来点火的原理。
...
- **体积分数** $\varphi = V\_i / V\_\{total}$
- **摩尔分数** $x = n\_i / n\_\{total}$
- **压强分数**（即分压比总压） $P\_i / P\_\{total}$
...
![alt text](/static/image/image-4.608b31a58f.avif)
...
- **气体分压定律**（道耳顿分压定律）：容器中混合气体对器壁所产生的压强等于在同样温度、体积条件下组成混合气体各成分单独存在时的分压强之和。
  ...
  $$
  \begin\{aligned} p&=cRT\ &=(c\_1+c\_2+\dots)RT\ &=c\_1RT+c\_2RT+\dots\ &=p\_1+p\_2+\dots \end\{aligned}
  $$

  同时，设温度相同，则：

  $$
  \dfrac\{p\_分}\{p\_总} = \dfrac\{c\_分}\{c\_总} = \dfrac\{n\_分}\{n\_总} = x\_分
  $$
...
波尔茨曼常数 $k$，理想气体常数 $R$，阿伏伽德罗常数 $N\_A$，满足：
...
$$
\dfrac\{p\_1V\_1}\{T\_1}=\dfrac\{p\_2V\_2}\{T\_2}
$$
...
![alt text](/static/image/image-5-313d0d9.2aa00c3fea.avif)
...
$$
\Delta p=\dfrac\{n\_0R}\{V\_0}\Delta T
$$

不妨联立原式：

$$
p\_0=\dfrac\{n\_0RT\_0}\{V\_0}
$$

因此容易发现：

$$
\Delta p=\dfrac\{p\_0}\{T\_0}\Delta T
$$

或者，

$$
\dfrac\{\Delta p}\{p\_0}=\dfrac\{\Delta T}\{T\_0}
$$
...
**充气问题**：设想一个容器，初始状态压强为 $p\_1$、体积为 $V$、温度为 $T\_1$，充入同种气体后，末状态压强为 $p\_2$、体积仍为 $V$、温度为 $T\_2$。

设充入气体的物质的量为 $\Delta n$，这些气体在充入前处于某个状态（比如大气压 $p\_0$、温度 $T\_0$、体积 $V\_0$）。根据物质的量守恒：
...
$$
\frac\{p\_2V}\{T\_2} = \frac\{p\_1V}\{T\_1} + \frac\{p\_0V\_0}\{T\_0}
$$

这就是充气问题的基本方程。如果充入的是大气（$p\_0 = p\_\{\text\{atm}}$，$T\_0 = T\_\{\text\{环境}}$），则只需确定充入气体的体积 $V\_0$ 即可。
...
$$
\frac\{p\_1V}\{T\_1} = \frac\{p\_2V}\{T\_2} + \frac\{p\_0V\_0}\{T\_0}
$$

其中 $p\_0$、$T\_0$、$V\_0$ 是放出气体在环境中的状态参量。

**一个实用的技巧**：如果充气或放气过程中温度保持不变（$T\_1 = T\_2 = T$），上述方程可以简化为：

$$
p\_2V = p\_1V + p\_0V\_0
$$

或者

$$
p\_1V = p\_2V + p\_0V\_0
$$

这说明，在恒温条件下，充气问题本质上是"压强与体积的乘积"的加法运算。

**打气问题的特例**：用打气筒给容器打气，每次打入压强为 $p\_0$、体积为 $V\_0$ 的气体，共打 $n$ 次。设容器体积为 $V$，最终压强为 $p$，温度不变。

从物质的量守恒的角度，相当于把 $n$ 份气体一次性充入：

$$
pV = p\_1V + n \cdot p\_0V\_0
$$

若初始容器内为真空（$p\_1 = 0$），则：

$$
p = \frac\{nV\_0}\{V} \cdot p\_0
$$

**抽气问题的特例**：用抽气机从容器中抽气，每次抽出体积为 $V\_0$ 的气体。设容器体积为 $V$，初始压强为 $p\_0$，抽 $n$ 次后压强为 $p\_n$，温度不变。

每次抽气时，容器内气体膨胀到体积 $V + V\_0$（假设抽气机与容器连通），然后抽气机将气体排出。根据玻意耳定律，第一次抽气后：
...
$$
p\_n = p\_0 \cdot \left(\frac\{V}\{V + V\_0}\right)^n
$$
...
例如，一个钢瓶装有压强为 $p\_0$ 的气体，每次放出气体的物质的量为 $\Delta n$（在标准状态下体积为 $V\_\{\text\{std}}$），共放 $n$ 次，求最终压强。

设钢瓶体积为 $V$，温度不变。物质的量守恒给出：

$$
n\_0 = n\_n + n \cdot \Delta n
$$

即：

$$
\frac\{p\_0V}\{RT} = \frac\{p\_nV}\{RT} + n \cdot \frac\{p\_\{\text\{std}}V\_\{\text\{std}}}\{RT\_\{\text\{std}}}
$$
...
![alt text](/static/image/image.8fc3e76f93.png)
...
不妨记试管总长度 $L$，设温度为 $T\_x$ 时，某一平衡点水银柱剩下 $x$，那么气柱的总长度就是 $L-x$，在这个状态下：

$$
\dfrac\{(p\_0+x)(L-x)}\{T\_x}=C
$$

注意到，我们把它化为整式，就是一个二次方程，即：

$$
x^2+(L-p\_0)x+CT\_x-p\_0L=0
$$

我们发现，如果这个方程无解，那么说明不存在平衡点，也就是说只要大于等于这个温度，水银就会被气体本身的能力彻底推出试管，这个温度就是：

$$
\Delta=(L-p\_0)^2-4(CT\_x-p\_0L)=0
$$

即：

$$
T\_x=\dfrac\{(L-p\_0)^2+4p\_0L}\{4C}
$$
...
设大气压强为 $p\_0$，则地球表面受到的总压力为：

$$
F=p\_0S=4\pi p\_0R^2
$$

其中，近似的取 $p\_0=1.013\times10^5\mathrm\{Pa}$，地球半径 $R=6.37\times10^6\mathrm m$。
...
设每个容器体积为 $V$，设 $p\_0=1\mathrm\{atm}$ 表示大气压，$T\_0=273\mathrm K$。

封入 $n(\mathrm\{mol})$ 的气体，稳定后两个容器分别有 $n\_1,n\_2(\mathrm\{mol})$ 的气体，温度 $T\_1=273\mathrm K,T\_2=373\mathrm K$。
...
$$
V=\frac\{nRT\_0}\{2p\_0}\tag\{5}
$$

由 $(2)(4)$，

$$
pV=(n-n\_2)RT\_1\tag\{6}
$$

由 $T\_1\cdot(3)+T\_2\cdot(6)$，

$$
pV(T\_1+T\_2)=nRT\_1T\_2\tag7
$$

由 $(5)(7)$，

$$
p=\frac\{nR}\{V}\cdot\frac\{T\_1T\_2}\{T\_1+T\_2}=\frac\{2p\_0}\{T\_0}\cdot\frac\{T\_1T\_2}\{T\_1+T\_2}
$$

再根据 $T\_0=T\_1=273\mathrm K,T\_2=373\mathrm K$，

$$
p=\frac\{2p\_0}\{1+T\_1/T\_2}=1.15p\_0
$$
...
在容积为 $V$ 的容器中，有气体，压强为 $p\_1$，连容器重 $G\_1$。

缓慢放掉一部分气体，测得压强为 $p\_2$，连容器重 $G\_2$。
...
$$
p\_1V=\{G\_1/g-M\_0\over\mu}RT\ p\_2V=\{G\_2/g-M\_0\over\mu}RT
$$

联立，得，

$$
(p\_1-p\_2)V=\{G\_1-G\_2\over\mu g}RT
$$

即，

$$
\mu=\{RT\over gV}\cdot\{G\_1-G\_2\over p\_1-p\_2}
$$
...
$$
\rho=\{M\over V}=\{\mu p\over RT}=\{p\over gV}\cdot\{G\_1-G\_2\over p\_1-p\_2}
$$
...
一定质量的理想气体，压强为 $p\_1$、温度为 $T\_1$，与同体积的压强为 $p\_1$、温度为 $T\_2$ 的同种气体混合。
...
$$
\{p\_1V\_0\over T\_1}=n\_1R\ \{p\_2V\_0\over T\_2}=n\_2R
$$

联立，得，

$$
\{p\_1/T\_1\over p\_2/T\_2}=\{n\_1\over n\_2}\tag1
$$

设终态温度为 $T$，与外界无热量交换，能量守恒，

$$
Q\_1=Q\_2\ cn\_1\mu(T\_1-T)=cn\_2\mu(T-T\_2)\ n\_1(T\_1-T)=n\_2(T-T\_2)\tag2
$$

联立 $(1)(2)$，

$$
\{p\_1/T\_1\over p\_2/T\_2}=\{T-T\_2\over T\_1-T}\\\[0.5em] T=\{(p\_1+p\_2)T\_1T\_2\over p\_1T\_2+p\_2T\_1}
$$

#### 例题六 \{#\_32}

有 $3\mathrm\{mol}$ 理想气体，自体积 $V\_0$ 膨胀到 $3V\_0$，其压强 $P$ 随体积 $V$ 的变化规律如，

$$
P=P\_0\left(2-\{V\over4V\_0}\right)
$$

其中 $p\_0$ 为初始状态的气压，求该气体在此过程中能达到的最高温度。

列出方程，

$$
P=P\_0\left(2-\{V\over4V\_0}\right)\ PV=3RT
$$

联立，

$$
\{3RT\over P\_0}=V\left(2-\{V\over4V\_0}\right)=-\{1\over4V\_0}V^2+2V
$$

当 $V=4V\_0$ 时，$T$ 最大；但是因为 $V\_0\le V\le3V\_0$，因此取 $V=3V\_0$，

$$
T\_\{\max}=\{5P\_0V\_0\over4R}
$$
