...
- 内能与物质及其状态、温度压强决定，考虑热力学第一定律 *Δ\*\*U* = *Q* + *W*。
...
- 焓为人为定义的一种系统的属性，有公式 *H* = *U* + *p\*\*V*，与温度压强等有关。
...
- 当压强不变时：
  ...
  *Δ*(*p\*\*V*) = *Δ**p**V* + *p**Δ**V* + *Δ**p**Δ\*\*V* = *p**Δ**V*

- 根据热力学第一定律：

  *Δ**Q**p* = *Δ\*\*U* − *W* = *Δ\*\*U* + *Δ*(*p\*\*V*) = *Δ\*\*H*
...
- 化学反应导致的焓变一般用 *Δ*r*H* 表示（r 表示 reaction）。
...
- 高中阶段对「摩尔反应焓变」概念的简化处理高中阶段将「摩尔反应焓变」这个概念称呼为「焓变」，同时用 *Δ\*\*H* 表示焓变；然而事实上，这两个量的量纲和意义都显然不同。
...
- 状态：用 g 表示气态（gas），l 表示液态（liquid），s 表示固态（solid），aq 表示溶解在水溶液中的溶质（aqueous）；有些物质的聚集状态为固体时，还需注明晶形（因为晶形不同，内能不同，焓值也不同）。
- 数值：正负号 ± 表示吸放热，化学计量数表示摩尔数，可以为分数（但是最好比较整）；同一反应的化学计量数等比例扩倍时，摩尔反应焓变也会扩倍。常见单位有 $\\pu{J/mol, kJ/mol}$ 等，表示每摩尔反应所吸收的热量。
- 高中阶段特有的简化处理：不用 *Δ*r*H*m 表达摩尔反应焓变，而直接用 *Δ\*\*H* 表达。
...
- 熵变：反应前后体系熵的变化，符号为 *Δ\*\*S*。
- 反应熵变（*Δ\*\*S*）= 生成物总熵 − 反应物总熵。
- 常见的熵增加的过程：固体的溶解过程、气体的扩散过程、水的气化过程及墨水的扩散过程都是体系混乱度增大的过程，即熵增加的过程。
- 产生气体或气体物质的量增大的反应，熵变通常都是正值，为熵增加反应。
- 反应熵变（*Δ\*\*S*）是与反应进行的方向有关的因素之一，但不是决定反应能否自发进行的唯一因素。但在一定条件下，反应也能自发进行。说明熵判据也具有一定的局限性。
...
- 焓变（*Δ\*\*H*）和熵变（*Δ\*\*S*）都与反应的自发性有关，却又都不能独立地作为反应自发性的依据。要判断反应进行的方向，必须综合考虑体系的焓变和熵变。
...
- 吉布斯能变（$\\Delta G,\\pu{kJ/mol}$）可以用来评估一个反应是否具有自发性，容易知道，在等温情况下：
  ...
  *Δ\*\*G* = *Δ\*\*H* − *T**Δ**S*
  ...
  它不仅与 *Δ\*\*H*、*Δ\*\*S* 有关，还与温度 *T* 有关，其表达式为

- 研究表明，在**等温**、**等压**及除了体积功以外不做其他功的条件下，其规律是：

  | 吉布斯自由能变                       | 反应进行的方向 |
  | ------------------------------------ | -------------- |
  | *Δ\*\*G* = *Δ\*\*H* − *T**Δ**S* \< 0 | 正反应自发进行 |
  | *Δ\*\*G* = *Δ\*\*H* − *T**Δ**S* = 0  | 反应到平衡状态 |
  | *Δ\*\*G* = *Δ\*\*H* − *T**Δ**S* > 0  | 逆反应自发进行 |
...
| ------------------------------ | --------------------------------------------------------- | --------------------------------------------------------- |
| *Δ\*\*S* > 0 **（熵增反应）**  | *Δ\*\*G* 恒小于零 反应总是自发的                          | 温度**高于**特定值，反应为自发 温度低于特定值，反应不自发 |
| *Δ\*\*S* \< 0 **（熵减反应）** | 温度**低于**特定值，反应为自发 温度高于特定值，反应不自发 | *Δ\*\*G* 恒大于零 反应总是不自发                          |
...
- 因为 *R\*\*T* 恒正，我们得出 *Q* \< *K* 即为自发过程，同理 *Q* > *K* 为不自发，*Q* = *K* 为平衡状态。
...
- 对限定条件不同的热力学过程，热力学自由能有不同表达形式。最常见的有吉布斯自由能 *G* 和亥姆霍兹自由能 *A*（曾用 *F* 记号）。
  ...
  - 等温等容过程用亥姆霍兹自由能 *A* = *U* − *T\*\*S* 作为自发性判据；
  - 等温等压过程用吉布斯自由能 *G* = *H* − *T\*\*S* 作为自发性判据。

- 根据焓的定义，两者间存在 *G* = *A* + *p\*\*V* 关系。
