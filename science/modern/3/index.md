...
光在同种均匀介质中沿直线传播，在真空中光的传播速度为 c=\\pu299792458m/s，约 \\pu3×108m/s，不同颜色的光在真空中的传播速度相同，在其他介质中的传播速度不同。正在发光的物体叫做光源。
...
1. **隐含条件**：发生全反射时，通常默认透明介质外部（即孔壁与介质之间微小的间隙，或者作为理想物理模型）的折射率为空气的折射率，即 n外​≈1。

推导过程：

**第一步：求出光线的最大入射角**

物体高度为 L，小孔正好在 L/2 的高度处（即小孔对准物体的中心），物体到小孔的距离为 s。从物体最顶端（或最底端）发出的光线，进入小孔时的入射角 θ1​ 最大。
...
sinθ1​=s2+(L/2)2​L/2​=4s2+L2​L​

**第二步：求光进入介质后的折射角**

设光线进入折射率为 n 的透明材料后，折射角为 θ2​。根据折射定律：
...
光线在透明材料内部射向侧壁时，入射角为 90∘−θ2​。要使所有的光都能出射，即便是入射角最大（折射角 θ2​ 也最大）的边缘光线，也必须在侧壁发生全反射。

全反射的临界角 θc​ 满足 sinθc​=n1​。因此，光线在侧壁的入射角必须大于等于临界角：
...
在示意图中，一束光线 PO 在点 O 处射到了一面竖直镜子上，反射光线是 OQ 。在点 O 处作出一条假想的与镜面垂直的直线，叫做法线（normal）。有了法线，我们可以测量出入射角 θi​，和反射角 θr​。反射定律表述为 θr​=θi​，换句话说，反射角等于入射角，同时反射光线，入射光线，法线在同一平面内，反射光线与入射光线分局法线两侧。
...
在金属中，没有结合能的电子被称作自由电子。当这些电子随着入射光振动时，它们的辐射的场和入射光的场之间的相位差是 π，所以向前的辐射终止了入射光，而向后的辐射就是反射光。
...
其中 λ 是真空中光的波长，λ 是这束光进入这种介质后的波长，也就是说

λ=nλ0​​

我们知道，光从空气射到水面时，一部分光射进水中，另一部分光返回到空气中。

入射光线与法线的夹角 θ1​ 称为入射角，折射光线与法线的夹角 θ2​ 称为折射角，折射定律（斯涅尔定律）指出：

sinθ2​sinθ1​​=v2​v1​​=λ1​λ1​​=n12​

式中 n12​ 是**相对折射率**，它与入射角、折射角的大小无关，只与两种介质的性质有关，

n12​=n1​n2​​=sinθ2​sinθ1​​
...
如果一束光从一种介质射入另一种介质，我们称折射率较小的为光疏介质，折射率较大的为光密介质。存在一个临界角 θc​ 使得入射角大于这个角，则不会发生折射现象，所有关系全部向内面反射，称为**全反射**或全内反射。

在不考虑耗损的情况下：

sinθc​=n1​n2​​

其中 n2​ 是光疏介质的折射率，n1​ 是光密介质的折射率，当光疏介质是真空或者空气时
...
1. 在线段 AO 上竖直地插上两枚大头针 P1​、P2​。
1. 从玻璃砖 bb′ 一侧透过玻璃砖观察大头针 P1​、P2​ 的像，调整视线方向直到 P1​ 的像被 P2​ 的像挡住。再在 bb′ 一侧插上大头针 P3​、P4​，使 P3​ 能挡住 P1​、P2​ 的像，P4​ 能挡住 P1​、P2​ 的像及 P3​ 本身。
1. 移去玻璃砖，在拔掉 P1​、P2​、P3​、P4​ 的同时分别记下它们的位置，过 P3​、P4​ 作直线 O′B 交 bb′ 于 O′。连接 O、O′，OO′ 就是玻璃砖内折射光线的方向。∠AON 为入射角，∠O′ON′ 为折射角。
1. 用量角器量出入射角和折射角的度数。查出它们的正弦值，并把这些数据填入记录表格里。
1. 用上述方法分别求出入射角是 15∘、30∘、45∘、60∘ 和 75∘ 时的折射角，查出入射角和折射角的正弦值，记录在表格里。
1. 算出不同入射角时 sinrsini​ 的值，比较一下，看它们是否接近一个常数。求出几次实验测得的 sinrsini​ 的平均值，这就是这块玻璃的折射率。
...
1. 插针 P1​ 与 P2​、P3​ 与 P4​ 的间距要适当大些，以减小确定光路方向时出现的误差。
1. 实验时入射角不能太小（接近零度），否则会使测量误差加大；也不能太大（接近 90∘），否则会不易观察到 P1​、P2​ 的像。
...
所谓**半波损失**，是指波从**波疏介质**（波速大、折射率小）射向**波密介质**（波速小、折射率大）时，反射波的相位会发生 π 的突变。这相当于波在反射点“平白无故”多跑（或少跑）了 **半个波长 (*λ*/2)**，因此被称为半波损失。

为了直观理解这个抽象的光学概念，我们可以借用机械波的\*\*“固定端反射”\*\*模型：

- **固定端反射（波疏 → 波密）**： 想象一根轻绳的一端被固定在墙上（墙壁质量极大，可视为“无限波密”）。当你抖动绳子产生一个向上的脉冲传向墙壁时，你会发现**反射回来的脉冲是向下的**。

  - **物理本质**：界面处的位移必须为 0（波节），入射波和反射波必须在此处相互抵消，因此相位必须相反（差 *π*）。
  - **结论**：发生半波损失。

- **自由端反射（波密 → 波疏）**： 如果绳子的一端套在一个光滑的竖直杆上（可以自由上下移动）。当你传来一个向上的脉冲，**反射回来的脉冲依然是向下的**？不，是**向上的**！
...
其中 n1​ 为入射介质折射率，n2​ 为折射介质折射率。

- **当 *n*1 \< *n*2（波疏 → 波密）**： 分母恒正，分子 n1​−n2​\<0，导致 r 为**负值**。 在数学上，振幅变负等价于相位增加了 π（因为 sin(ωt+π)=−sin(ωt)）。**这就是半波损失的数学本质。**
- **当 *n*1 > *n*2（波密 → 波疏）**： 分子 n1​−n2​>0，r 为**正值**，波形不倒相，无半波损失。

核心考点：薄膜干涉中的光程差。在处理**薄膜干涉**（如肥皂泡、增透膜、劈尖干涉）问题时，必须先判断反射面是否发生了半波损失，再列**光程差**公式。

δ=2nd+Δλ

- 若上下表面**都**发生（或都**不**发生）半波损失：*Δ\*\*λ* = 0。
- 若**只有一个**表面发生半波损失：$\\Delta \\lambda = \\pm \\dfrac{\\lambda}{2}$。

**典型案例**：空气中的肥皂膜（折射率 n≈1.33）。

- **前表面**（空气 → 膜，光疏 → 光密）：有半波损失。
- **后表面**（膜 → 空气，光密 → 光疏）：无半波损失。
- **结论**：此时的光程差公式必须加 *λ*/2。这就是为什么极薄的肥皂膜（*d* → 0）在反射光下看起来是**暗**的（光程差 *λ*/2，干涉相消）。
...
其中，d 为双缝间距，ℓ 为挡板与屏幕之间的举例，而 Δx 表示的是相邻的两个两条纹，取其中间部位的间距。我们用**测量头**来测量这个 Δx 因为它通常是较小的，难以像 d,ℓ 一样用刻度尺测量。测量头的读数类似于螺旋测微器，我们不再进行讲解。

如果你观察干涉图样（那一排明暗相间的条纹）：

- **中央亮纹（*n* = 0）：** 正中间最亮的那条线。
- **一级亮纹（*n* = ±1）：** 中央亮纹两侧紧邻的第一个亮纹（左边一个，右边一个）。
- **二级亮纹（*n* = ±2）：** 在一级亮纹之外，再往两侧数第一个亮纹（**即距离中心点第二远的亮纹**）。

在实际操作中，为了演示更加清晰，我们基本上都是使用红色光进行实验，因为 Δx 与 λ 是成正比的，红光波长比较长，因此间距较大，更容易读出。类似的，我们知道，光的波长一般是比较小的，为了让 Δx 较大，我们通常会让 ℓ 稍大一些（或者说 d 通常是很小的），这样读数会更加方便。
...
- **双缝间距 *d***：直接决定条纹间距。*d* 越小 *Δ\*\*x* 越大，条纹越稀疏（便于测量）；*d* 越大 *Δ\*\*x* 越小，条纹越密集。
...
劳埃德于 1834 年提出了一种更简单的观察干涉的装置。如图 1 所示，MN 为一块平玻璃板，用作反射镜，S1​ 是一狭缝光源，从光源发出的光波，一部分掠射（即入射角接近 90∘）到玻璃平板上，经玻璃表面反射到达屏上；另一部分直接射到屏上。

这两部分光也是相干光，它们同样是用分波阵面得到的。反射光可看成是由虚光源 S2​ 发出的。S1​ 和 S2​ 构成一对相干光源，对干涉条纹的分析与杨氏实验也相同。中画有阴影的区域表示相干光在空间叠加的区域。这时在屏上可以观察到明暗相间的干涉条纹。

应该指出，在劳埃德镜实验中，如果把屏幕移近到和镜面边缘 N 相接触，即图 1 中 E′ 的位置，这时从 S1​ 和 S2​ 发出的光到达接触处的路程相等，应该出现明纹，但实验结果却是暗纹，其他的条纹也有相应变化。这一实验事实说明了由镜面反射出来的光和直接射到屏上的光在 N 处的相位相反，即相位差为 π。由于直射光的相位不会变化，所以只能认为光从空气射向玻璃平板发生反射时，反射光的相位跃变了 π。

进一步的实验表明：光从光疏介质射到光密介质界面反射时，在掠射（入射角 i=90∘ 或正入射，即 i=0）的情况下，反射光的相位较之入射光的相位有 π 的突变，这一变化导致了反射光的波程在反射过程中附加了半个波长，故常称为半波损失。今后在讨论光波叠加时，若有半波损失，在计算波程差时必须计及，否则会得出与实际情况不符的结果。
...
> **进阶知识：布儒斯特角与双折射**
>
> - **布儒斯特角（Brewster’s Angle）**：当光以特定角度从一种介质射向另一种介质时，反射光会变成**完全偏振光**，且偏振方向垂直于入射面。此时反射光线与折射光线互相垂直 (tan *θ\*\*B* = *n*2/*n*1)。摄影师旋转偏振镜消除水面反光，找的就是这个角度。
