...
光在同种均匀介质中沿直线传播，在真空中光的传播速度为 []{.arithmatex}$c=\pu{299 792 458 m/s}$，约 []{.arithmatex}$\pu{3\times10^8m/s}$，不同颜色的光在真空中的传播速度相同，在其他介质中的传播速度不同。正在发光的物体叫做光源。
...
在示意图中，一束光线 []{.arithmatex}$PO$ 在点 []{.arithmatex}$O$ 处射到了一面竖直镜子上，反射光线是 []{.arithmatex}$OQ$ 。在点 []{.arithmatex}$O$ 处作出一条假想的与镜面垂直的直线，叫做法线（normal）。有了法线，我们可以测量出入射角 []{.arithmatex}$\theta _{i}$，和反射角 []{.arithmatex}$\theta _{r}$。反射定律表述为 []{.arithmatex}$\theta _{r}=\theta _{i}$，换句话说，反射角等于入射角，同时反射光线，入射光线，法线在同一平面内，反射光线与入射光线分局法线两侧。
...
在金属中，没有结合能的电子被称作自由电子。当这些电子随着入射光振动时，它们的辐射的场和入射光的场之间的相位差是 []{.arithmatex}$\pi$，所以向前的辐射终止了入射光，而向后的辐射就是反射光。
...
这个比值即为这种介质的**绝对折射率**，简称折射率，其中 []{.arithmatex}$v$ 为光在这种介质中的传播速度。自然界中的物质折射率通常是大于 []{.arithmatex}$1$ 的（人造材料可以实现折射率对于一定频率范围内的电磁波是负值，称为负折射率超材料）。

| 介质 | 真空 | 空气 | 水 | 水晶 | 玻璃 |
|:--:|:--:|:--:|:--:|:--:|:--:|
| 折射率 | []{.arithmatex}$1$ | []{.arithmatex}$1.00028$ | []{.arithmatex}$1.33$ | []{.arithmatex}$1.55$ | []{.arithmatex}$1.5$ |
...
其中 []{.arithmatex}$\lambda$ 是真空中光的波长，[]{.arithmatex}$\lambda$ 是这束光进入这种介质后的波长，也就是说
...
入射光线与法线的夹角 []{.arithmatex}$\theta_1$ 称为入射角，折射光线与法线的夹角 []{.arithmatex}$\theta_2$ 称为折射角，折射定律（斯涅尔定律）指出：
...
式中 []{.arithmatex}$n_{12}$ 是**相对折射率**，它与入射角、折射角的大小无关，只与两种介质的性质有关，
...
如果一束光从一种介质射入另一种介质，我们称折射率较小的为光疏介质，折射率较大的为光密介质。存在一个临界角 []{.arithmatex}$\theta_c$ 使得入射角大于这个角，则不会发生折射现象，所有关系全部向内面反射，称为**全反射**或全内反射。
...
其中 []{.arithmatex}$n_2$ 是光疏介质的折射率，[]{.arithmatex}$n_1$ 是光密介质的折射率，当光疏介质是真空或者空气时
...
2.  在白纸上画一直线 []{.arithmatex}$aa'$ 作为界面，过 []{.arithmatex}$aa'$ 上的一点 []{.arithmatex}$O$ 画出界面的法线 []{.arithmatex}$NN'$，并画一条线段 []{.arithmatex}$AO$ 作为入射光线。

3.  把长方形玻璃砖放在白纸上，并使其长边与 []{.arithmatex}$aa'$ 重合，再用直尺画出玻璃的另一边 []{.arithmatex}$bb'$。

    ![alt text](../image.avif){align="right" width="40%"}

4.  在线段 []{.arithmatex}$AO$ 上竖直地插上两枚大头针 []{.arithmatex}$P_1$、[]{.arithmatex}$P_2$。

5.  从玻璃砖 []{.arithmatex}$bb'$ 一侧透过玻璃砖观察大头针 []{.arithmatex}$P_1$、[]{.arithmatex}$P_2$ 的像，调整视线方向直到 []{.arithmatex}$P_1$ 的像被 []{.arithmatex}$P_2$ 的像挡住。再在 []{.arithmatex}$bb'$ 一侧插上大头针 []{.arithmatex}$P_3$、[]{.arithmatex}$P_4$，使 []{.arithmatex}$P_3$ 能挡住 []{.arithmatex}$P_1$、[]{.arithmatex}$P_2$ 的像，[]{.arithmatex}$P_4$ 能挡住 []{.arithmatex}$P_1$、[]{.arithmatex}$P_2$ 的像及 []{.arithmatex}$P_3$ 本身。

6.  移去玻璃砖，在拔掉 []{.arithmatex}$P_1$、[]{.arithmatex}$P_2$、[]{.arithmatex}$P_3$、[]{.arithmatex}$P_4$ 的同时分别记下它们的位置，过 []{.arithmatex}$P_3$、[]{.arithmatex}$P_4$ 作直线 []{.arithmatex}$O'B$ 交 []{.arithmatex}$bb'$ 于 []{.arithmatex}$O'$。连接 []{.arithmatex}$O$、[]{.arithmatex}$O'$，[]{.arithmatex}$OO'$ 就是玻璃砖内折射光线的方向。[]{.arithmatex}$\angle AON$ 为入射角，[]{.arithmatex}$\angle O'ON'$ 为折射角。

7.  用量角器量出入射角和折射角的度数。查出它们的正弦值，并把这些数据填入记录表格里。

8.  用上述方法分别求出入射角是 []{.arithmatex}$15^\circ$、[]{.arithmatex}$30^\circ$、[]{.arithmatex}$45^\circ$、[]{.arithmatex}$60^\circ$ 和 []{.arithmatex}$75^\circ$ 时的折射角，查出入射角和折射角的正弦值，记录在表格里。

9.  算出不同入射角时 []{.arithmatex}$\dfrac{\sin i}{\sin r}$ 的值，比较一下，看它们是否接近一个常数。求出几次实验测得的 []{.arithmatex}$\dfrac{\sin i}{\sin r}$ 的平均值，这就是这块玻璃的折射率。

**光的折射定律**：折射光线、入射光线、法线在同一平面内，折射光线、入射光线在法线两侧。入射角的正弦跟折射角的正弦成正比，如果用 []{.arithmatex}$n$ 来表示这个比例常数，就有：
...
3.  插针 []{.arithmatex}$P_1$ 与 []{.arithmatex}$P_2$、[]{.arithmatex}$P_3$ 与 []{.arithmatex}$P_4$ 的间距要适当大些，以减小确定光路方向时出现的误差。

4.  实验时入射角不能太小（接近零度），否则会使测量误差加大；也不能太大（接近 []{.arithmatex}$90^\circ$），否则会不易观察到 []{.arithmatex}$P_1$、[]{.arithmatex}$P_2$ 的像。
...
视深问题：垂直向一深度为 []{.arithmatex}$h$ 的水池看去，水的视深是多少？已知水的折射率 []{.arithmatex}$n$。
...
所谓**半波损失**，是指波从**波疏介质**（波速大、折射率小）射向**波密介质**（波速小、折射率大）时，反射波的相位会发生 []{.arithmatex}$\pi$ 的突变。这相当于波在反射点"平白无故"多跑（或少跑）了 **半个波长 ([]{.arithmatex}**$\lambda/2$)，因此被称为半波损失。

为了直观理解这个抽象的光学概念，我们可以借用机械波的**"固定端反射"**模型：

- **固定端反射（波疏 []{.arithmatex}**$\to$ 波密）： 想象一根轻绳的一端被固定在墙上（墙壁质量极大，可视为"无限波密"）。当你抖动绳子产生一个向上的脉冲传向墙壁时，你会发现**反射回来的脉冲是向下的**。

  - **物理本质**：界面处的位移必须为 0（波节），入射波和反射波必须在此处相互抵消，因此相位必须相反（差 []{.arithmatex}$\pi$）。
  - **结论**：发生半波损失。

- **自由端反射（波密 []{.arithmatex}**$\to$ 波疏）： 如果绳子的一端套在一个光滑的竖直杆上（可以自由上下移动）。当你传来一个向上的脉冲，**反射回来的脉冲依然是向下的**？不，是**向上的**！
...
>
> - []{.arithmatex}$n_{\text{入}} < n_{\text{折}}$ （光撞墙）：有 []{.arithmatex}$\lambda/2$ 损失。
> - []{.arithmatex}$n_{\text{入}} > n_{\text{折}}$ （光撞气）：无损失。

菲涅耳公式的数学证明------为什么会出现"相位突变"？本质上是边界条件的数学必然。在大学物理中，我们通过**菲涅耳公式**（Fresnel Equations）来定量描述这一过程。

对于垂直入射的光波，反射波的电矢量振幅 []{.arithmatex}$A'$ 与入射波振幅 []{.arithmatex}$A$ 的比值（反射系数 []{.arithmatex}$r$）为：
...
其中 []{.arithmatex}$n_1$ 为入射介质折射率，[]{.arithmatex}$n_2$ 为折射介质折射率。

- **当 []{.arithmatex}**$n_1 < n_2$（波疏 []{.arithmatex}$\to$ 波密）： 分母恒正，分子 []{.arithmatex}$n_1 - n_2 < 0$，导致 []{.arithmatex}$r$ 为**负值**。 在数学上，振幅变负等价于相位增加了 []{.arithmatex}$\pi$（因为 []{.arithmatex}$\sin(\omega t + \pi) = -\sin(\omega t)$）。**这就是半波损失的数学本质。**

- **当 []{.arithmatex}**$n_1 > n_2$（波密 []{.arithmatex}$\to$ 波疏）： 分子 []{.arithmatex}$n_1 - n_2 > 0$，[]{.arithmatex}$r$ 为**正值**，波形不倒相，无半波损失。
...
- 若上下表面**都**发生（或都**不**发生）半波损失：[]{.arithmatex}$\Delta \lambda = 0$。
- 若**只有一个**表面发生半波损失：[]{.arithmatex}$\Delta \lambda = \pm \dfrac{\lambda}{2}$。

**典型案例**：空气中的肥皂膜（折射率 []{.arithmatex}$n \approx 1.33$）。

- **前表面**（空气 []{.arithmatex}$\to$ 膜，光疏 []{.arithmatex}$\to$ 光密）：有半波损失。
- **后表面**（膜 []{.arithmatex}$\to$ 空气，光密 []{.arithmatex}$\to$ 光疏）：无半波损失。
- **结论**：此时的光程差公式必须加 []{.arithmatex}$\lambda/2$。这就是为什么极薄的肥皂膜（[]{.arithmatex}$d \to 0$）在反射光下看起来是**暗**的（光程差 []{.arithmatex}$\lambda/2$，干涉相消）。
...
>
> 原子发光是断续的，每一次发光只能持续约 []{.arithmatex}$10^{-8}$ 秒，发出一列有限长度的波，称为波列。
>
...
其中，[]{.arithmatex}$d$ 为双缝间距，[]{.arithmatex}$\ell$ 为挡板与屏幕之间的举例，而 []{.arithmatex}$\Delta x$ 表示的是相邻的两个两条纹，取其中间部位的间距。我们用**测量头**来测量这个 []{.arithmatex}$\Delta x$ 因为它通常是较小的，难以像 []{.arithmatex}$d,\ell$ 一样用刻度尺测量。测量头的读数类似于螺旋测微器，我们不再进行讲解。

![alt text](../image-2.avif){width="40%"}

在实际操作中，为了演示更加清晰，我们基本上都是使用红色光进行实验，因为 []{.arithmatex}$\Delta x$ 与 []{.arithmatex}$\lambda$ 是成正比的，红光波长比较长，因此间距较大，更容易读出。类似的，我们知道，光的波长一般是比较小的，为了让 []{.arithmatex}$\Delta x$ 较大，我们通常会让 []{.arithmatex}$\ell$ 稍大一些（或者说 []{.arithmatex}$d$ 通常是很小的），这样读数会更加方便。
...
劳埃德于 1834 年提出了一种更简单的观察干涉的装置。如图 1 所示，[]{.arithmatex}$MN$ 为一块平玻璃板，用作反射镜，[]{.arithmatex}$S_1$ 是一狭缝光源，从光源发出的光波，一部分掠射（即入射角接近 []{.arithmatex}$90^\circ$）到玻璃平板上，经玻璃表面反射到达屏上；另一部分直接射到屏上。

这两部分光也是相干光，它们同样是用分波阵面得到的。反射光可看成是由虚光源 []{.arithmatex}$S_2$ 发出的。[]{.arithmatex}$S_1$ 和 []{.arithmatex}$S_2$ 构成一对相干光源，对干涉条纹的分析与杨氏实验也相同。中画有阴影的区域表示相干光在空间叠加的区域。这时在屏上可以观察到明暗相间的干涉条纹。

![alt text](../7f497ec15f6c0d1f_e77bd7e.svg){width="60%"}

应该指出，在劳埃德镜实验中，如果把屏幕移近到和镜面边缘 []{.arithmatex}$N$ 相接触，即图 1 中 []{.arithmatex}$E'$ 的位置，这时从 []{.arithmatex}$S_1$ 和 []{.arithmatex}$S_2$ 发出的光到达接触处的路程相等，应该出现明纹，但实验结果却是暗纹，其他的条纹也有相应变化。这一实验事实说明了由镜面反射出来的光和直接射到屏上的光在 []{.arithmatex}$N$ 处的相位相反，即相位差为 []{.arithmatex}$\pi$。由于直射光的相位不会变化，所以只能认为光从空气射向玻璃平板发生反射时，反射光的相位跃变了 []{.arithmatex}$\pi$。

进一步的实验表明：光从光疏介质射到光密介质界面反射时，在掠射（入射角 []{.arithmatex}$i=90^\circ$ 或正入射，即 []{.arithmatex}$i=0$）的情况下，反射光的相位较之入射光的相位有 []{.arithmatex}$\pi$ 的突变，这一变化导致了反射光的波程在反射过程中附加了半个波长，故常称为半波损失。今后在讨论光波叠加时，若有半波损失，在计算波程差时必须计及，否则会得出与实际情况不符的结果。
...
电磁波也是一种波，具有波动性的光也会出现多普勒效应，但与机械波不同的是，电磁波的传播不需要介质，因此是电磁波的波源如光源和接收器的相对速度 []{.arithmatex}$V$ 决定接收频率。 法国物理学家斐索独立地对来自恒星的波长偏移做了解释，指出了利用这种效应测量恒星相对速度的办法。光波频率的变化使人感觉到是颜色的变化。
...
式中 []{.arithmatex}$c$ 为波在真空中的波速（光速），可见，如果恒星远离我们而去，则光的谱线就向红光方向移动，称为红移；如果恒星朝向我们运动，光的谱线就向紫光方向移动，称为蓝移。
...
- **偏振光的产生**：自然光在通过偏振片 []{.arithmatex}$P$ 时，只有振动方向与偏振片的透振方向一致的光波才能顺利通过。也就是说，通过偏振片 []{.arithmatex}$P$ 的光波，在垂直于传播方向的平面上，沿着某个特定的方向振动。这种光叫作**偏振光**（polarized light）。
- **偏振片的检偏**：通过偏振片 []{.arithmatex}$P$ 的偏振光照射到偏振片 []{.arithmatex}$Q$ 时：
  - 如果两个偏振片的透振方向**平行**，那么，通过 []{.arithmatex}$P$ 的偏振光的振动方向与偏振片 []{.arithmatex}$Q$ 的透振方向一致，可以透過 []{.arithmatex}$Q$（图 4.6-3 乙）；
  - 如果两个偏振片的透振方向**垂直**，那么，偏振光的振动方向跟偏振片 []{.arithmatex}$Q$ 的透振方向垂直，不能透过 []{.arithmatex}$Q$（图 4.6-3 丙）。
...
> **进阶知识：布儒斯特角与双折射**
>
> - **布儒斯特角（Brewster\'s Angle）**：当光以特定角度从一种介质射向另一种介质时，反射光会变成**完全偏振光**，且偏振方向垂直于入射面。此时反射光线与折射光线互相垂直 ([]{.arithmatex}$\tan \theta_B = n_2/n_1$)。摄影师旋转偏振镜消除水面反光，找的就是这个角度。
...
2.  **平行度好**
    ...
    - **应用**：**精确测距**。对准目标发出一个极短的激光脉冲，测量发射脉冲与收到反射回波的时间间隔，就可以求出目标的距离。利用激光测量地球到月亮的距离（大约为 []{.arithmatex}$\pu{38 万 km}$），准确度可以达到厘米级别。此外，还用于房屋装修测距、武器目标指引等。
