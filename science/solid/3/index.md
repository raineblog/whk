...
1. 若 $ PA=PB=PC $，则 $ O $ 是 $ABC $ 的**外心**。
1. 若 $ PA $, $ PB $, $ PC $ 与平面 $ ABC $ 所成角相等，则 $ O $ 是 $ABC $ 的**外心**。
1. 若 $ P $ 到 $ABC $ 各边的距离相等，则 $ O $ 是 $ABC $ 的**内心**。
1. 若三侧面与底面所成的二面角相等，则 $ O $ 是 $ABC $ 的**内心**。
1. 若 $ PA PB $, $ PA PC $, $ PB PC $，则 $ O $ 是 $ABC $ 的**垂心**。
1. 若 $ PA BC $, $ PB AC $, $ PC AB $，则 $ O $ 是 $ABC $ 的**垂心**。

### 补形补体

1. 若四面体有三条棱两两互相垂直，则可将其放入某个长方体内。
1. 若四面体的四个面均是直角三角形，则可将其放入某个长方体内。
1. 正四面体 $ P-ABC $ 可以补形为正方体且正方体的棱长 $ a = $。
