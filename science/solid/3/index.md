# 空间几何体 {#_1}

## 基础知识 {#_2}

### 分类 {#_3}

<figure>
<p><img src="../%E7%A9%BA%E9%97%B4%E5%87%A0%E4%BD%95%E4%BD%93.avif" alt="空间几何体" /></p>
<figcaption><p>多面体和旋转体</p></figcaption>
</figure>

多面体是指三维空间中由平面多边形、直边和顶点组成的几何形状。多面体表面积通用公式就是将每一个平面多边形的面积相加求和。

旋转体是指平面曲线以同一平面内的一条直线作为旋转轴进行旋转所形成的立体几何图形。

### 斜二测画法 {#_4}

将平面 [\$x\$]{.arithmatex} 轴不变，[\$y\$]{.arithmatex} 轴向右倾斜 [\$45\^\circ\$]{.arithmatex} 并长度缩短到原来的 [\$1/2\$]{.arithmatex}，称为斜二测画法，画出来的图称为直观图。

容易知道，直观图的面积是原图形的 [\$\dfrac{\sqrt2}4\$]{.arithmatex}，原图形的面积是直观图的 [\$2\sqrt2\$]{.arithmatex} 倍。

竖直的 [\$z\$]{.arithmatex} 轴也不变，称所画出的图形称为直观图。

### 祖暅原理 {#_5}

祖暅（gèng）之《缀术》有云：「缘幂势既同，则积不容异」。

::: {.arithmatex}
\$\$ V=\int\_{0}\^HS(h)\mathrm{d}h \$\$
:::

祖暅原理（也称等幂等积定理）指出：所有等高处横截面积相等的两个等高的立体，其体积也必然相等。经典推论是，两个等高的立体，如果其所有等高处横截面积相等，则其体积必然相等。

### 辛普森法则 {#_6}

设拟柱体的高为 [\$H\$]{.arithmatex}，如果用平行于底面的平面去截该图形，所得到的截面面积是与到底面之间距离 [\$h\$]{.arithmatex} 的不超过 [\$3\$]{.arithmatex} 次的函数，那么该拟柱体的体积 [\$V\$]{.arithmatex} 为：

::: {.arithmatex}
\$\$ V=\dfrac{h(a+4b+c)}6 \$\$
:::

其描述的是辛普森积分法（一种数值积分方法）：

::: {.arithmatex}
\$\$ \int\_{a}\^{b}f(x)\mathrm dx\approx {\dfrac {b-a}{6}}\left\[f(a)+4f\left({\dfrac {a+b}{2}}\right)+f(b)\right\] \$\$
:::

### 欧拉公式 {#_7}

欧拉公式表述为：[\$V-E+F=2\$]{.arithmatex}，即顶点数 Vertex [\$-\$]{.arithmatex} 棱数 Edge [\$+\$]{.arithmatex} 面数 Face [\$=2\$]{.arithmatex}。

|  | 面数 | 棱数 | 顶点数 | 每面边数 | 每顶点棱数 | 备注 |
|:--:|:--:|:--:|:--:|:--:|:--:|:--:|
| 正四面体 | [\$4\$]{.arithmatex} | [\$6\$]{.arithmatex} | [\$4\$]{.arithmatex} | [\$3\$]{.arithmatex} | [\$3\$]{.arithmatex} |  |
| 正六面体 | [\$6\$]{.arithmatex} | [\$12\$]{.arithmatex} | [\$8\$]{.arithmatex} | [\$4\$]{.arithmatex} | [\$3\$]{.arithmatex} | 即正方体 |
| 正八面体 | [\$8\$]{.arithmatex} | [\$12\$]{.arithmatex} | [\$6\$]{.arithmatex} | [\$3\$]{.arithmatex} | [\$4\$]{.arithmatex} | 两个正四棱锥上下叠在一起 |
| 正十二面体 | [\$12\$]{.arithmatex} | [\$30\$]{.arithmatex} | [\$20\$]{.arithmatex} | [\$5\$]{.arithmatex} | [\$3\$]{.arithmatex} |  |
| 正二十面体 | [\$20\$]{.arithmatex} | [\$30\$]{.arithmatex} | [\$12\$]{.arithmatex} | [\$3\$]{.arithmatex} | [\$5\$]{.arithmatex} |  |

## 多面体 {#_8}

### 棱柱 {#_9}

![棱柱](../image-1.avif){align="right"}

- 棱柱：上下底面平行（且全等），各侧面都是四边形，侧面的公共边互相平行。

- 棱柱分为直棱柱和**斜棱柱**（右图）。

- 底面是 [\$n\$]{.arithmatex} 边形，叫做 [\$n\$]{.arithmatex} 棱柱。

- 底面为正 [\$n\$]{.arithmatex} 边形，叫做正 [\$n\$]{.arithmatex} 棱柱。

- 侧棱竖直的 [\$n\$]{.arithmatex} 棱柱，叫做直 [\$n\$]{.arithmatex} 棱柱。

- 底面是平行四边形，叫做平行六面体。

根据祖暅原理，任何棱柱都可以转化为一个长方体，从而：

::: {.arithmatex}
\$\$ V=Sh \$\$
:::

### 棱锥 {#_10}

- 侧面都是三角形，有公共顶点。

- 底面是 [\$n\$]{.arithmatex} 边形，有 [\$n\$]{.arithmatex} 个侧面，叫做 [\$n\$]{.arithmatex} 棱锥。

- 底面是正 [\$n\$]{.arithmatex} 边形，且顶点在底面上的投影在底面中心，叫做正 [\$n\$]{.arithmatex} 棱锥。

- 有一条侧棱竖直的 [\$n\$]{.arithmatex} 棱锥，叫做直 [\$n\$]{.arithmatex} 棱锥。

根据祖暅原理，任何棱锥都可以转化为一个正棱锥，从而：

::: {.arithmatex}
\$\$ V=\dfrac{Sh}3 \$\$
:::

证明：

::: {.arithmatex}
\$\$ V=\int\_{0}\^HS(h)\mathrm{d}h \$\$
:::

其中：

::: {.arithmatex}
\$\$ S(h)=S\cdot\dfrac{h\^2}{H\^2} \$\$
:::

注意到：

::: {.arithmatex}
\$\$ \dfrac{\mathrm d(Sh\^3/3H\^2)}{\mathrm dh}=\dfrac{Sh\^2}{H\^2} \$\$
:::

因此：

::: {.arithmatex}
\$\$ V=\dfrac{SH\^3}{3H\^2}=\dfrac{SH}{3} \$\$
:::

这个公式对所有椎体成立。

### 棱台 {#_11}

- 将棱锥用一个平行于底面的平面截下，下半部分叫做棱台。

根据祖暅原理，任何棱台都可以转化为一个圆台，从而根据：

::: {.arithmatex}
\$\$ V=\dfrac{h}{6}\[S+S\'+4\times\dfrac{1}{4}(S+S\'+2\sqrt{SS\'})\]=\dfrac h3(S+\sqrt{SS\'}+S\') \$\$
:::

棱台的体积公式也可以描述为一种希罗平均数：

::: {.arithmatex}
\$\$ H={\dfrac {1}{3}}\left(A+{\sqrt {AB}}+B\right) \$\$
:::

用以计算棱台或圆台的体积等于棱台或圆台的高乘以两底面面积的希罗平均数。

希罗平均数也可以看成算术平均数与几何平均数的加权平均数。

::: {.arithmatex}
\$\$ H={\dfrac {2}{3}}\cdot {\dfrac {A+B}{2}}+{\dfrac {1}{3}}\cdot {\sqrt {AB}} \$\$
:::

## 旋转体 {#_12}

### 圆柱 {#_13}

- 将一个长方形绕着一条边（轴）旋转得到的立体几何图形。

- 侧面的每一条平行于轴的线称为母线。

::: {.arithmatex}
\$\$ V=Sh=\pi r\^2h \$\$
:::

::: {.arithmatex}
\$\$ S\_{表}=2\pi r\^2+2\pi rh=2\pi(r+h) \$\$
:::

显然。

### 圆锥 {#_14}

- 将一个直角三角形绕着一条直角边（轴）旋转得到的立体几何图形。

- 直角三角形的每一个斜边对应的线称为母线。

根据祖暅原理：

::: {.arithmatex}
\$\$ V=\dfrac13Sh=\dfrac13\pi r\^2h \$\$
:::

也可以用定积分算。

表面积可以直接拆分为侧面和底面：

::: {.arithmatex}
\$\$ S=\pi r\^2+\pi rl=\pi r(r+l) \$\$
:::

其中 [\$l\$]{.arithmatex} 表示母线的长度：

::: {.arithmatex}
\$\$ l=\sqrt{r\^2+h\^2} \$\$
:::

### 圆台 {#_15}

- 将一个直角梯形绕着它的直角边（轴）旋转得到的立体几何图形。

- 直角梯形的每一个斜边对应的线称为母线。

- 也可以视为将圆台用一个平行于底面的平面截下，下半部分叫做圆台。

根据辛普森法则：

::: {.arithmatex}
\$\$ V=\dfrac16h\left\[\pi r\^2+\pi\left(\dfrac{r+R}{2}\right)\^2+\pi R\^2\right\]=\dfrac{\pi h}3(r\^2+rR+R\^2) \$\$
:::

表面积同样可以拆分：

::: {.arithmatex}
\$\$ S=\pi\[r\^2+(r+R)l+R\^2\] \$\$
:::

其中 [\$l\$]{.arithmatex} 表示母线的长度：

::: {.arithmatex}
\$\$ l=\sqrt{h\^2+(R-r)\^2} \$\$
:::

## 方法总结 {#_16}

### 投影法 {#_17}

三棱锥 \$ P-ABC \$ 中，点 \$ P \$ 在平面 \$ ABC \$ 中的射影为 \$ O \$：

1.  若 \$ PA=PB=PC \$，则 \$ O \$ 是 \$ \triangle ABC \$ 的**外心**。
2.  若 \$ PA \$, \$ PB \$, \$ PC \$ 与平面 \$ ABC \$ 所成角相等，则 \$ O \$ 是 \$ \triangle ABC \$ 的**外心**。
3.  若 \$ P \$ 到 \$ \triangle ABC \$ 各边的距离相等，则 \$ O \$ 是 \$ \triangle ABC \$ 的**内心**。
4.  若三侧面与底面所成的二面角相等，则 \$ O \$ 是 \$ \triangle ABC \$ 的**内心**。
5.  若 \$ PA \perp PB \$, \$ PA \perp PC \$, \$ PB \perp PC \$，则 \$ O \$ 是 \$ \triangle ABC \$ 的**垂心**。
6.  若 \$ PA \perp BC \$, \$ PB \perp [AC]{.abbr title="Alternating Current (交流电)"} \$, \$ PC \perp AB \$，则 \$ O \$ 是 \$ \triangle ABC \$ 的**垂心**。

### 补形补体 {#_18}

1.  若四面体有三条棱两两互相垂直，则可将其放入某个长方体内。
2.  若四面体的四个面均是直角三角形，则可将其放入某个长方体内。
3.  正四面体 \$ P-ABC \$ 可以补形为正方体且正方体的棱长 \$ a = \dfrac{PA}{\sqrt{2}} \$。
4.  若四面体的三组对棱分别相等，则可将其放入某个长方体内。

### 截面问题 {#_19}

![alt text](../image.avif){width="100%"}

找截面有交线法和平行线法两个常用的方法，本人喜欢先用平行线，然后找交线。

![](../%E6%88%AA%E9%9D%A2%E9%97%AE%E9%A2%98.avif){width="80%"}

1.  连接被截的若干点。

2.  做面上的平行线、延长线。

3.  算长度，勾股判断图形。

### 最值问题 {#_20}

### 翻折问题 {#_21}

### 反射问题 {#_22}

## 球的概述 {#_23}

### 球的定义 {#_24}

一个半圆绕着它的直径（轴）旋转得到的立体几何图形。

球内两个端点在球面上且过球心的线段称为直径。

根据球心 [\$O\$]{.arithmatex} 称一个球为「球 [\$O\$]{.arithmatex}」。

### 球的体积表面积 {#_25}

根据辛普森法则：

::: {.arithmatex}
\$\$ V=\dfrac{1}{6}\times2r\times(0+4\pi r\^2+0)=\dfrac43\pi r\^3 \$\$
:::

根据球的表面积等于其外接圆柱的侧面面积（详细看 3B1B 的视频）：

::: {.arithmatex}
\$\$ S=2\pi r\times2r=4\pi r\^2 \$\$
:::

### 球中的定值问题 {#_26}

画一个圆，画出上下两个椭圆表示两个截面，有勾股定理。

### 球中的最值问题 {#_27}

## 外接球问题 {#_28}

### 方体的外接球 {#_29}

容易发现，长方体外接球直径就是其对角线的长度，也就是：

::: {.arithmatex}
\$\$ R=\dfrac{\sqrt{a\^2+b\^2+c\^2}}{2} \$\$
:::

### 棱锥的外接球 {#_30}

我们尝试求出外接球的半径 [\$R\$]{.arithmatex} 找到三棱锥的一个底面 [\$ABC\$]{.arithmatex}，容易发现外接球的球心 [\$O\$]{.arithmatex} 在底面的投影一定是 [\$ABC\$]{.arithmatex} 的外心（[\$r=AH=BH=CH\$]{.arithmatex}），另外根据 [\$OD=R\$]{.arithmatex}，那么就有：

::: {.arithmatex}
\$\$ R\^2=OH\^2+r\^2=R\^2 \$\$
:::

其中 [\$r\$]{.arithmatex} 为底面外接圆半径，可以通过正弦定理求出。

特殊的，若顶点在底面任意顶点正上方，则外接球球心位与底面外接圆正上方 [\$h=H/2\$]{.arithmatex} 处。

对于 [\$n\$]{.arithmatex} 棱锥也类似。

### 棱柱的外接球 {#_31}

同样是找底面的外接圆圆心，然后向上做高 [\$h\$]{.arithmatex} 即可。

特殊的，如果是直棱柱，[\$h=H/2\$]{.arithmatex} 其中 [\$H\$]{.arithmatex} 是体的高度。

### 三棱锥内切球 {#_32}

注意到可以将三棱锥分为，等面积法：

::: {.arithmatex}
\$\$ V=\dfrac{1}{3}S_1r+\dfrac{1}{3}S_2r+\dfrac{1}{3}S_3r+\dfrac{1}{3}S_4r \$\$
:::

即：

::: {.arithmatex}
\$\$ r=\dfrac{3V}{S_1+S_2+S_3+S_4}=\dfrac{3V}{S_表} \$\$
:::
