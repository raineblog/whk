...
将平面 []{.arithmatex}$x$ 轴不变，[]{.arithmatex}$y$ 轴向右倾斜 []{.arithmatex}$45^\circ$ 并长度缩短到原来的 []{.arithmatex}$1/2$，称为斜二测画法，画出来的图称为直观图。

容易知道，直观图的面积是原图形的 []{.arithmatex}$\dfrac{\sqrt2}4$，原图形的面积是直观图的 []{.arithmatex}$2\sqrt2$ 倍。

竖直的 []{.arithmatex}$z$ 轴也不变，称所画出的图形称为直观图。
...
设拟柱体的高为 []{.arithmatex}$H$，如果用平行于底面的平面去截该图形，所得到的截面面积是与到底面之间距离 []{.arithmatex}$h$ 的不超过 []{.arithmatex}$3$ 次的函数，那么该拟柱体的体积 []{.arithmatex}$V$ 为：
...
欧拉公式表述为：[]{.arithmatex}$V-E+F=2$，即顶点数 Vertex []{.arithmatex}$-$ 棱数 Edge []{.arithmatex}$+$ 面数 Face []{.arithmatex}$=2$。

|  | 面数 | 棱数 | 顶点数 | 每面边数 | 每顶点棱数 | 备注 |
|:--:|:--:|:--:|:--:|:--:|:--:|:--:|
| 正四面体 | []{.arithmatex}$4$ | []{.arithmatex}$6$ | []{.arithmatex}$4$ | []{.arithmatex}$3$ | []{.arithmatex}$3$ |  |
| 正六面体 | []{.arithmatex}$6$ | []{.arithmatex}$12$ | []{.arithmatex}$8$ | []{.arithmatex}$4$ | []{.arithmatex}$3$ | 即正方体 |
| 正八面体 | []{.arithmatex}$8$ | []{.arithmatex}$12$ | []{.arithmatex}$6$ | []{.arithmatex}$3$ | []{.arithmatex}$4$ | 两个正四棱锥上下叠在一起 |
| 正十二面体 | []{.arithmatex}$12$ | []{.arithmatex}$30$ | []{.arithmatex}$20$ | []{.arithmatex}$5$ | []{.arithmatex}$3$ |  |
| 正二十面体 | []{.arithmatex}$20$ | []{.arithmatex}$30$ | []{.arithmatex}$12$ | []{.arithmatex}$3$ | []{.arithmatex}$5$ |  |
...
- 底面是 []{.arithmatex}$n$ 边形，叫做 []{.arithmatex}$n$ 棱柱。

- 底面为正 []{.arithmatex}$n$ 边形，叫做正 []{.arithmatex}$n$ 棱柱。

- 侧棱竖直的 []{.arithmatex}$n$ 棱柱，叫做直 []{.arithmatex}$n$ 棱柱。
...
- 底面是 []{.arithmatex}$n$ 边形，有 []{.arithmatex}$n$ 个侧面，叫做 []{.arithmatex}$n$ 棱锥。

- 底面是正 []{.arithmatex}$n$ 边形，且顶点在底面上的投影在底面中心，叫做正 []{.arithmatex}$n$ 棱锥。

- 有一条侧棱竖直的 []{.arithmatex}$n$ 棱锥，叫做直 []{.arithmatex}$n$ 棱锥。
...
根据球心 []{.arithmatex}$O$ 称一个球为「球 []{.arithmatex}$O$」。
...
我们尝试求出外接球的半径 []{.arithmatex}$R$ 找到三棱锥的一个底面 []{.arithmatex}$ABC$，容易发现外接球的球心 []{.arithmatex}$O$ 在底面的投影一定是 []{.arithmatex}$ABC$ 的外心（[]{.arithmatex}$r=AH=BH=CH$），另外根据 []{.arithmatex}$OD=R$，那么就有：
...
其中 []{.arithmatex}$r$ 为底面外接圆半径，可以通过正弦定理求出。

特殊的，若顶点在底面任意顶点正上方，则外接球球心位与底面外接圆正上方 []{.arithmatex}$h=H/2$ 处。

对于 []{.arithmatex}$n$ 棱锥也类似。

### 棱柱的外接球 {#_31}

同样是找底面的外接圆圆心，然后向上做高 []{.arithmatex}$h$ 即可。

特殊的，如果是直棱柱，[]{.arithmatex}$h=H/2$ 其中 []{.arithmatex}$H$ 是体的高度。
