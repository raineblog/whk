...
**理论上，复杂分子的同系物划分**依然遵循同系物的核心定义：一是结构相似，即含有相同种类、相同数目的官能团，且碳骨架类型相似；二是在分子组成上相差一个或若干个 $\\ce{CH2}$ 原子团。

在复杂分子中，多出来的 $\\ce{CH2}$ 可以插在不同的位置。以 3-(4-甲酰基苯基) 丙酸为例，其下一个同系物可以是：

- 插在脂肪链上：变成 4-(4-甲酰基苯基) 丁酸
- 作为支链长在脂肪链上：变成 3-(4-甲酰基苯基)-2-甲基丙酸
- 长在苯环上：变成 3-(3-甲基-4-甲酰基苯基) 丙酸

以上这些分子在中学或基础有机化学的严格定义下全都是原分子的同系物，因为它们满足「官能团相同」且「相差一个 $\\ce{CH2}$」。

**复杂情况下同系物概念的价值与局限**：

同系物概念在简单分子中非常有意义，因为它们构成线性递变的系列，化学性质几乎完全一样，物理性质呈现完美的规律性。但当分子变得复杂时，这个概念就显示出局限性。

物理性质的规律性被完全打破：在复杂分子中，增加一个 $\\ce{CH2}$，它插入的位置会导致分子的空间构型和分子间作用力发生剧变。例如，如果在靠近羧基的地方加一个甲基，可能会因为空间位阻导致分子无法紧密排列，使得新同系物的熔点不升反降。这就导致无法再通过同系物来预测物理性质。

化学性质不再「极其相似」：虽然官能团种类和数量没变，但 $\\ce{CH2}$ 的位置会引发化学性质的突变。距离效应方面，如果 $\\ce{CH2}$ 插在醛基和羧基之间，原本由于距离近可以发生的「分子内成环反应」可能会因为距离变远而彻底失效。空间位阻方面，在一个复杂的支链结构中增加一个 $\\ce{CH2}$，可能会挡住某个官能团，导致原本能发生的反应突然变得极难发生。

组合爆炸导致「系列」不复存在：在复杂分子中，增加一个 $\\ce{CH2}$ 会产生几十甚至上百种同分异构体。如果再增加一个 $\\ce{CH2}$，同分异构体会呈指数级暴增。这已经不是一个「系列」了，而是一团乱麻的「网络」。
...
新制氢氧化铜（$\\ce{Cu(OH)2}$ 悬浊液）可以用来鉴别甲苯、溴苯、乙醇、乙醛四种物质，利用了物理性质（溶解度与密度）和化学性质（醛基的还原性）。
...
$$ \\ce{CH3CHO + 2Cu(OH)2 + NaOH ->[\\triangle] CH3COONa + Cu2O v(砖红色)
