...
碳原子在有机物中，通常与四个原子直接相连，即配位数为 []{.arithmatex}$4$，与一个碳原子相连的原子既可以是碳原子、也可以是其他原子，碳原子的这种独特的成键能力，是有机化合物多样性的根本原因。
...
碳原子间的共价键可以是：单键（[]{.arithmatex}$\ce{C - C}$）、双键（[]{.arithmatex}$\ce{C = C}$）、三键（[]{.arithmatex}$\ce{C # C}$）。
...
[]{.arithmatex}$\Omega=$ 重键数 []{.arithmatex}$+$ 最简环数

也就是说，一个不饱和度，也就是少了一对氢原子，例如，[]{.arithmatex}$\ce{C=C}$ 不饱和度为 []{.arithmatex}$1$，[]{.arithmatex}$\ce{C#C}$ 为 []{.arithmatex}$2$。

同时，不饱和度 []{.arithmatex}$\Omega$ 也可以通过分子式求出：
...
- 卤素原子 []{.arithmatex}$\ce{X}$ 视为氢原子、硅视为碳原子。

- []{.arithmatex}$\ce{C-O}$ 和 []{.arithmatex}$\ce{C=O}$ 不影响不饱和度，硫也是。

- 一个苯环 []{.arithmatex}$=$ 三个双键一个环，不饱和度为 []{.arithmatex}$4$。
...
|  | 表示方式 | 示例 |
|:--:|----|:--:|
| 分子式 | 用元素符号表示物质的分子组成 | []{.arithmatex}$\ce{CH4}$ |
| 最简式（实验式） | 用元素符号表示化合物中各元素原子个数的最简整数比 | []{.arithmatex}$\ce{CH2O}$ |
| 电子式 | 在元素符号周围用「[]{.arithmatex}$\cdot$」或「[]{.arithmatex}$\times$」表示原子的最外层电子的成键情况 | ![](../CCl4.avif){width="200"} |
| 结构式 | 用短线「[]{.arithmatex}$\ce{-}$」来表示 1 个共价键，将所有原子连接起来 | ![](../2.3.svg){width="200"} |
| 结构简式 | 省略结构式中的单键「[]{.arithmatex}$\ce{-}$」，将与碳原子相连的其他原子写在其旁边，在右下角注明其个数 | []{.arithmatex}$\ce{CH3CH = CH2}$ |
...
同系物：结构相似，在分子组成上相差一个或若干个 []{.arithmatex}$\ce{CH2}$ 原子团的化合物互称为同系物。烷烃就是一个同系物系列。

|     类别     | 通式                                         |
|:------------:|----------------------------------------------|
|    链烷烃    | []{.arithmatex}$\ce{C_n H_{2n+2} (n \ge 1)}$ |
|    单烯烃    | []{.arithmatex}$\ce{C_n H_{2n} (n \ge 2)}$   |
|    环烷烃    | []{.arithmatex}$\ce{C_n H_{2n} (n \ge 3)}$   |
|     炔烃     | []{.arithmatex}$\ce{C_n H_{2n-2} (n \ge 2)}$ |
|    二烯烃    | []{.arithmatex}$\ce{C_n H_{2n-2} (n \ge 4)}$ |
| 苯及其同系物 | []{.arithmatex}$\ce{C_n H_{2n-6} (n \ge 6)}$ |
...
|:--:|:--:|:--:|:--:|:--:|:--:|
| 烯烃 | 碳碳双键 | []{.arithmatex}$\ce{C=C}$ | 炔烃 | 碳碳三键 | []{.arithmatex}$\ce{C#C}$ |
| 卤代烃 | 碳卤键 | []{.arithmatex}$\ce{-X}$ | 醇/酚 | 醇羟基/酚羟基 | []{.arithmatex}$\ce{-OH}$ |
| 醚 | 醚键 | []{.arithmatex}$\ce{-O-}$ | 胺 | 氨基 | []{.arithmatex}$\ce{-NH2}$ |
| 醛 | 醛基 | []{.arithmatex}$\ce{-CHO}$ | 酮 | 酮羰基 | []{.arithmatex}$\ce{-CO-}$ |
| 羧酸 | 羧基 | []{.arithmatex}$\ce{-COOH}$ | 酯 | 酯基 | []{.arithmatex}$\ce{-COOR}$ |
| 酰胺/酰卤 | 酰胺/酰卤基 | []{.arithmatex}$\ce{-CONH2/-COX}$ | 腈 | 氰基 | []{.arithmatex}$\ce{-CN}$ |
...
1.  **母体官能团定为 []{.arithmatex}**$1$ 号位： 例如在"甲苯"中，甲基所在的碳自动定为 []{.arithmatex}$1$ 号，不需要写\"1-甲基\...\"。
2.  **其余取代基位次最小：** 从 []{.arithmatex}$1$ 号位开始，向其余取代基最近的方向编号。
...
现代化的元素分析仪可以测出有机物含有元素，例如 []{.arithmatex}$\ce{C,H,O}$ 的质量比，可以根据这个推算出为止有机物的实验式（最简式）。

有机物完全燃烧的规律：

- 燃烧热：则每个碳原子对燃烧热的贡献大约是 []{.arithmatex}$\pu{400kJ/mol}$，每个氢原子对燃烧热的贡献大约是 []{.arithmatex}$\pu{150kJ/mol}$。也就是说近似的：
  ...
  假设产物水为液态，这个公式对于碳数小于 []{.arithmatex}$20$ 的烷烃是较为准确的，对于烯烃、炔烃、芳香烃等就误差较大了。
...
- 耗氧量：实际上上面的燃烧热的近似公式，也是可以用耗氧量来估算的，有一个经验规律是，烃的燃烧热与其完全燃烧的耗氧量，非常接近一个线性关系。我们不妨先写出燃烧的方程式：
  ...
  对于等物质的量的有机物，[]{.arithmatex}$\paren{x+\dfrac{y}{4}}$ 的值越大，耗氧量越多、燃烧热越大；对于等质量的有机物，[]{.arithmatex}$y/x$ 的值越大，耗氧量越多，燃烧热越大。
...
金刚烷，分子式 []{.arithmatex}$\ce{C10H16}$，它的碳架结构相当于金刚石晶格网络中的一个晶胞，故得名金刚烷。它是一种脂环烃，具有类似樟脑的气味，是无色晶体，容易结晶，其衍生物可以用作药物。
...
::: {.grid}
![](../image-34.avif){width="20%"}[]{.arithmatex}$\ce{+ H2O ->}\quad$
