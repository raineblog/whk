...
| 电子式           | 在元素符号周围用「\\cdot」或「\\times」表示原子的最外层电子的成键情况                      |                   |
...
- 同系物定义中的「结构相似」是指碳链和成键方式相同、官能团相同、官能团数目相同、官能团与其他原子的连接方式相同等。
...
- 含有乙基 \\Rightarrow 必然含有甲基。
- 「不含甲基」 \\Rightarrow 不能有乙基、丙基、丁基等任何带有末端 \\ce{-CH3} 的烷基。
