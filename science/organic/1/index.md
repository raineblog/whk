...
1. 碳的原子半径小，更容易形成双键和三键，增加了有机物的多样性。
1. 碳具有适中的电负性，可以与很多非金属原子形成共价键，使有机物更稳定。
1. 同样，原子半径小，空间位阻小，可以连接多个基团，形成复杂的、紧凑的三维结构。

碳原子间的共价键可以是：单键（\\ceC−C）、双键（\\ceC=C）、三键（\\ce{C # C}）。
...
也就是说，一个不饱和度，也就是少了一对氢原子，例如，\\ceC=C 不饱和度为 1，\\ce{C#C} 为 2。

同时，不饱和度 Ω 也可以通过分子式求出：

Ω(\\ceCx​Hy​)Ω(\\ceCx​Hy​Sia​Xm​)Ω(\\ceCx​Hy​Oq​Sp​)Ω(\\ceCx​Hy​Nb​)​=2(2x+2)−y​=22(x+a)+2−(y+m)​=2(2x+2)−y​=22(x+b)+2−(y+b)​=22x+2−y+b​​

- 卤素原子 \\ceX 视为氢原子、硅视为碳原子。
- \\ceC−O 和 \\ceC=O 不影响不饱和度，硫也是。
- # 一个苯环
  三个双键一个环，不饱和度为 4。
...
| 电子式           | 在元素符号周围用「⋅」或「×」表示原子的最外层电子的成键情况                                 |                     |
| 结构式           | 用短线「$\\ce{-}$」来表示 1 个共价键，将所有原子连接起来                                   |                     |
| 结构简式         | 省略结构式中的单键「$\\ce{-}$」，将与碳原子相连的其他原子写在其旁边，在右下角注明其个数    | $\\ce{CH3CH = CH2}$ |
| 键线式           | 省略结构简式中与碳原子直接相连的氢原子，每个拐点或端点均表示一个碳原子，不足的用氢原子补足 |                     |
| 球棍模型         | 小球表示原子，短棍表示化学键                                                               |                     |
| 空间填充模型     | 用不同体积的小球表示不同大小的原子，相对大小关系应与原子实际相对大小关系一致               |                     |
...
同系物：结构相似，在分子组成上相差一个或若干个 \\ceCH2 原子团的化合物互称为同系物。烷烃就是一个同系物系列。

| 类别         | 通式                             |
| ------------ | -------------------------------- |
| 链烷烃       | $\\ce{C_n H\_{2n+2} (n \\ge 1)}$ |
| 单烯烃       | $\\ce{C_n H\_{2n} (n \\ge 2)}$   |
| 环烷烃       | $\\ce{C_n H\_{2n} (n \\ge 3)}$   |
| 炔烃         | $\\ce{C_n H\_{2n-2} (n \\ge 2)}$ |
| 二烯烃       | $\\ce{C_n H\_{2n-2} (n \\ge 4)}$ |
| 苯及其同系物 | $\\ce{C_n H\_{2n-6} (n \\ge 6)}$ |
...
当有机物变得复杂时，例如含有苯环、醛基、羧基、复杂的脂肪链或支链结构时，同系物的划分需要更加精细的分析。

**理论上，复杂分子的同系物划分**依然遵循同系物的核心定义：一是结构相似，即含有相同种类、相同数目的官能团，且碳骨架类型相似；二是在分子组成上相差一个或若干个 \\ceCH2 原子团。

在复杂分子中，多出来的 \\ceCH2 可以插在不同的位置。以 3-(4-甲酰基苯基) 丙酸为例，其下一个同系物可以是：

- 插在脂肪链上：变成 4-(4-甲酰基苯基) 丁酸
- 作为支链长在脂肪链上：变成 3-(4-甲酰基苯基)-2-甲基丙酸
- 长在苯环上：变成 3-(3-甲基-4-甲酰基苯基) 丙酸

以上这些分子在中学或基础有机化学的严格定义下全都是原分子的同系物，因为它们满足「官能团相同」且「相差一个 \\ceCH2」。

**复杂情况下同系物概念的价值与局限**：

同系物概念在简单分子中非常有意义，因为它们构成线性递变的系列，化学性质几乎完全一样，物理性质呈现完美的规律性。但当分子变得复杂时，这个概念就显示出局限性。

物理性质的规律性被完全打破：在复杂分子中，增加一个 \\ceCH2，它插入的位置会导致分子的空间构型和分子间作用力发生剧变。例如，如果在靠近羧基的地方加一个甲基，可能会因为空间位阻导致分子无法紧密排列，使得新同系物的熔点不升反降。这就导致无法再通过同系物来预测物理性质。

化学性质不再「极其相似」：虽然官能团种类和数量没变，但 \\ceCH2 的位置会引发化学性质的突变。距离效应方面，如果 \\ceCH2 插在醛基和羧基之间，原本由于距离近可以发生的「分子内成环反应」可能会因为距离变远而彻底失效。空间位阻方面，在一个复杂的支链结构中增加一个 \\ceCH2，可能会挡住某个官能团，导致原本能发生的反应突然变得极难发生。

组合爆炸导致「系列」不复存在：在复杂分子中，增加一个 \\ceCH2 会产生几十甚至上百种同分异构体。如果再增加一个 \\ceCH2，同分异构体会呈指数级暴增。这已经不是一个「系列」了，而是一团乱麻的「网络」。

**现代化学中的替代概念**：在现代药物化学、材料化学和高级有机化学中，化学家几乎不再使用「同系物」这个词。取而代之的是衍生物或类似物，例如在研发新药时，如果有一个复杂的有效分子，化学家会在它上面加个甲基、换个卤素、延长一下碳链，这些新分子统称为「类似物」，化学家关心的是「结构-活性关系」，而不是它们在碳数上差了几个。还有药效团或核心骨架，人们只关注分子中真正起作用的那几个官能团的相对空间位置，至于脂肪链怎么长，只被视为调节脂溶性的「附属物」。
...
现代化的元素分析仪可以测出有机物含有元素，例如 \\ceC,H,O 的质量比，可以根据这个推算出为止有机物的实验式（最简式）。

有机物完全燃烧的规律：

- 燃烧热：则每个碳原子对燃烧热的贡献大约是 \\pu400kJ/mol，每个氢原子对燃烧热的贡献大约是 \\pu150kJ/mol。也就是说近似的：
...
- 耗氧量：实际上上面的燃烧热的近似公式，也是可以用耗氧量来估算的，有一个经验规律是，烃的燃烧热与其完全燃烧的耗氧量，非常接近一个线性关系。我们不妨先写出燃烧的方程式：
  ...
  \\ce{C_xH_y + $\\paren{x+\\dfrac{y}{4}}$O2 ->[点燃] xCO2 + \\dfrac{y}{2}H2O(g)}

  对于等物质的量的有机物，(x+4y​) 的值越大，耗氧量越多、燃烧热越大；对于等质量的有机物，y/x 的值越大，耗氧量越多，燃烧热越大。
...
1. **分子中原子间的振动**：通常振动光谱在近红外区和中红外区，一般称为红外光谱（IR）。
1. **分子中电子在不同能级的分子轨道间跃迁**：得到紫外-可见光谱（UV-Vis）。
...
金刚烷，分子式 \\ceC10H16，它的碳架结构相当于金刚石晶格网络中的一个晶胞，故得名金刚烷。它是一种脂环烃，具有类似樟脑的气味，是无色晶体，容易结晶，其衍生物可以用作药物。

金刚烷的化学性质较为复杂，课内不涉及，其性质详见 \[金刚烷

- 维基百科\](https://zh.wikipedia.org/zh-hans/%E9%87%91%E5%88%9A%E7%83%B7#%E5%8C%96%E5%AD%A6%E6%80%A7%E8%B4%A8)。

### 环氧乙烷

从 2-氯乙醇中脱去氯化氢，是实验室中制取环氧乙烷最普遍的方法：

\\ceCl–CH2CH2–OH+NaOH−>[△](CH2CH2)O+NaCl+H2O

氯乙醇可由乙烯与次氯酸反应制备：

\\ceCH2=CH2+HOCl−>HO–CH2CH2–Cl
...
\\ce2CH2=CH2+O2−>[Ag][△]2C2H4O

这是一个经典的工业反应，注意如果是钯/铜催化剂则是生成乙醛，但银催化剂特指生成环氧乙烷。

\\ceC2H4O+H2O−>HO−CH2−CH2−OH
...
如果题目说一个有机物「不含有甲基」，那么这个分子中就绝对不能有乙基。

原因很简单：甲基的结构是 \\ce−CH3，乙基的结构是 \\ce−CH2CH3，乙基的末端就是一个甲基。因此：

- 含有乙基 ⇒ 必然含有甲基。
- 「不含甲基」 ⇒ 不能有乙基、丙基、丁基等任何带有末端 \\ce−CH3 的烷基。

真正不含甲基的有机物通常是：完全由 \\ce−CH2− 构成的环状化合物（如环己烷）、仅含两个碳原子的烯烃或炔烃（如乙烯、乙炔）、芳香烃（如苯）等。

### 新制氢氧化铜鉴别有机物

新制氢氧化铜（\\ceCu(OH)2 悬浊液）可以用来鉴别甲苯、溴苯、乙醇、乙醛四种物质，利用了物理性质（溶解度与密度）和化学性质（醛基的还原性）。

**第一步：常温混合观察**。将四种液体分别加入新制氢氧化铜悬浊液中：

- **分层，有机层在上方**的是**甲苯**（密度约 $\\pu{0.87 g/cm^3}$，比水小）。
- **分层，有机层在下方**的是**溴苯**（密度约 $\\pu{1.50 g/cm^3}$，比水大）。
- **不分层**的是**乙醇**和**乙醛**（均与水互溶）。

**第二步：加热**。对不分层的两支试管加热：

- **产生砖红色沉淀**的是**乙醛**（醛基还原 $\\ce{Cu(OH)2}$ 生成 $\\ce{Cu2O}$）。
- **无砖红色沉淀**的是**乙醇**（无醛基，不能还原 $\\ce{Cu(OH)2}$）。

\\ceCH3CHO+2Cu(OH)2+NaOH−>[△]CH3COONa+Cu2Ov(砖红色)+3H2O
