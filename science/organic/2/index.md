...
1. 氧化的最终产物不同（极其重要）
   ...
   - *应用考点*：如果把甲醛气体通入酸性 (\\ce{KMnO4}) 溶液，溶液褪色且**有气体（(\\ce{CO2})）生成**；而乙醛通入则只褪色，无气体。

1. 与新制 (\\ce{Cu(OH)2}) 反应的极致程度（结合上题）
...
1. 碘仿反应（鉴别两者的终极武器）这是一个非常经典的有机鉴别反应。
...
1. 常温常压下的状态不同
   ...
   - *应用考点*：在计算阿伏伽德罗常数（(N_A)）的题目中，如果说“标况下 (\\pu{22.4 L}) 的乙醛”，这是**错误**的陷阱（因为标况下乙醛不是气体）；但说“标况下 (\\pu{22.4 L}) 的甲醛”则是完全成立的（含 (\\pu{1 mol}) 的 (\\ce{HCHO})）。
...
广义的取代反应（Substitution reaction）定义：分子中的某个原子或基团（(\\ce{-A})）被另一个原子或基团（(\\ce{-B})）所替换的过程。只看**起点和终点，不看过程**。如果你要求有严格的有机机理（狭义的取代），事情就变得非常有趣了。

在脂肪族化合物中，经典的狭义取代反应是 **(S_N1)**（单分子亲核取代）和 **(S_N2)**（双分子亲核取代）。比如氢氧根取代溴乙烷中的溴，是 (S_N2) 机理：亲核试剂从背面进攻，旧键断裂和新键形成**同时发生**（一步完成）。
...
1. \*\*第一步：加成（Addition）\*\*亲核试剂（比如醇里的氧，或者水里的氧）去进攻带有部分正电荷的羰基碳。此时，(\\ce{C=O}) 双键断裂，变成单键，原本的 (sp^2) 平面碳变成了 **(sp^3) 杂化的四面体中间体**。
1. \*\*第二步：消除（Elimination）\*\*这个四面体中间体很不稳定。氧原子上的负电荷（或孤对电子）会重新推回来，**重新形成 (\\ce{C=O}) 双键**。为了维持碳的四价，必须“挤走”一个离去基团（在酯化中挤走水，在水解中挤走醇）。

既然微观过程是“先加成，后消除”，那它到底算不算取代反应？

在高等有机化学中，为了统合“宏观表象”和“微观机理”，给这类反应起了一个专属的名字，叫作： **亲核酰基取代反应（Nucleophilic Acyl Substitution，简称 (S_N Ac)）**。
...
因为高中化学经常问一个物质能否与 (\\ce{NaHCO3}) 反应产生 (\\ce{CO2})，因此我们这里将其作为“酸性强弱”的标准，能通过复分解反应与碳酸氢钠溶液反应产生二氧化碳气体的有机物，**其根本特征只有一个：该有机物在水溶液中的酸性必须强于碳酸的酸性**（强酸制弱酸原理）。具体来说，碳酸的表观酸度常数 (pK_a \\approx 6.38)。因此，**任何 (pK_a < 6.38) 的有机物**，在热力学上都能与碳酸氢钠反应生成二氧化碳。尽管从实验上来讲，还需要考虑动力学因素，如果反应过慢看不到气泡，一般也不视为能反应。
...
1. 除了含有羧基（(\\ce{-COOH})），还有哪些有机物能产生 (\\ce{CO2})？
   ...
   1. 极端的碳负离子酸（$\\ce{C-H} 酸）——氢连在碳上也能变强酸。
      ...
      **特征**：通常连在碳上的氢根本没有酸性（例如甲烷 (pK_a \\approx 48)）。但如果这个碳连了极强的吸电子基团，C-H 键就会断裂释放出 (H^+)。
...
1. 含有羧基（(\\ce{-COOH})），但却不能与 (\\ce{NaHCO3}) 产生 (\\ce{CO2}) 的有机物有没有？
   ...
   1. 氨基酸（内盐效应 / 等电点）——最普遍的例外。
      ...
      **原因**：羧基上的 (\\ce{H+}) 已经“内部转移”给了氨基。此时如果你加小苏打，能够被碱中和的质子实际上在 (\\ce{-NH3^+}) 上。而 (\\ce{-NH3^+}) 的 (pK_a \\approx 9.6)，远弱于碳酸。因此，甘氨酸（以及大多数等电点附近的中性氨基酸）不会与 (\\ce{NaHCO3}) 反应冒泡。
   ...
   1. 极长链高级脂肪酸（物理溶解度限制）。
      ...
      **原因**：如果你把硬脂酸粉末和 (\\ce{NaHCO3}) 溶液混合在常温下，**肉眼几乎看不到任何气泡**。因为硬脂酸极度疏水，反应只能在固体表面极小范围内发生，且生成的硬脂酸钠（肥皂）会包裹在表面阻碍反应。这属于动力学和相态造成的“不能”。（注：如果加热到融化并剧烈搅拌，依然是可以反应的）。
...
**能否产生 (CO_2) 的唯一标尺是 (pK_a < 6.38)**。

- 只要阴离子足够稳定（如磺酸、苦味酸、维生素C、方酸），没有羧基也能冒泡。
- 即使有羧基，如果质子被内部劫持（氨基酸）、被静电荷死死拉住（多元酸酸式盐）、或者被疏水结构/空间位阻严密包裹（硬脂酸、多取代苯甲酸），它也无法与 (NaHCO_3) 产生 (CO_2)。
