...
烷烃的通式：如果链状烷烃中的碳原子数为 *n*，则氢原子数为 2*n* + 2。其分子式可以用通式 $\\ce{C\_{n}H\_{2n+2}}$ 表示。
...
**苯**，为最简单的一种芳香烃，芳香族化合物皆由其衍生而成。苯结构是一个平面正六边形分子，碳原子以 sp2 轨道键结，所以任二键互成 $\\pu{120^o}$。苯的含碳量很高，在空气中燃烧产生明亮并带有浓烟的火焰。
...
苯的同系物：分子里含有一个苯环的烃，通式为 $\\ce{C_nH\_{2n-6}}$（*n* ≥ 6）。
...
广义来说，有芳香性的烃就是芳香烃，而芳香性指的是分子具有特殊的稳定性，这种稳定性来源于完全离域的 *π* 电子体系，一般来说，符合休克尔规则（Hückel 规则）的平面环状共轭体系具有芳香性，即 *π* 电子数为 4*n* + 2（*n* = 0, 1, 2, …）。但是不是所有符合这个简单规律的都是，例如如果环的结构无法形成平面型，所有 *π* 电子云难以肩并肩形成大 *π* 键，或者说，某些物质强行压到平面型，需要的能量远远高于形成离域的稳定结构而减少的，得不偿失，就没有芳香性（例如环癸五烯/[10]-轮烯）。
...
真正的非苯芳烃（只含碳氢的芳香烃）包括：薁（Azulene，由五元环和七元环稠合而成，化学式 $\\ce{C10H8}$）、环丙烯基正离子、环戊二烯基负离子、环庚三烯基正离子等。

### 芳香杂环化合物

苯环中的 $\\ce{CH}$ 基团被杂原子（$\\ce{N}$、$\\ce{O}$、$\\ce{S}$ 等）取代后，形成的含杂原子芳香环统称为**芳香杂环化合物**。常见的有吡啶、吡咯、呋喃、噻吩等。理解杂环芳香性的核心在于判断**杂原子的孤对电子是否参与了芳香大 *π* 键**。

根据杂原子孤对电子的归属，可以将芳香杂环分为两大类。

\*\*吡啶型：孤对电子不参与离域。\*\*代表分子有吡啶（六元环，含 $\\ce{=N-}$）、嘧啶、喹啉等。

以吡啶为例，环中的 $\\ce{N}$ 原子采取 *s\*\*p*2 杂化。其 5 个价电子分配如下：

- 两个 *s\*\*p*2 杂化轨道各贡献 1 个电子，与相邻碳原子形成 *σ* 键。
- 一个电子进入垂直于环平面的未杂化 *p* 轨道，与环上 5 个碳原子的 *p* 电子共同形成含 6 个电子的芳香大 *π* 键，满足 4*n* + 2 规则（*n* = 1）。
- 剩余两个电子（孤对电子）占据第三个 *s\*\*p*2 杂化轨道，该轨道位于环平面内并向环外伸出，**完全不参与离域**。

由于孤对电子独立于 *π* 体系之外，吡啶具有较强的碱性和配位能力。这对孤对电子可以直接提供给金属离子（如 $\\ce{Ni}$）形成配位键，而环的芳香性不受影响。

\*\*吡咯型：孤对电子参与离域。\*\*代表分子有吡咯（五元环，含 $\\ce{-NH-}$）、呋喃（含 $\\ce{-O-}$）、噻吩（含 $\\ce{-S-}$）等。

以吡咯为例，环上有 4 个碳原子，每个碳提供 1 个 *π* 电子，共 4 个。为了满足 6 电子的 Hückel 芳香稳定结构，$\\ce{N}$ 原子必须将其孤对电子（2 个）全部放入 *p* 轨道中参与离域，与碳的 4 个电子共同构成芳香大 *π* 键。为此，$\\ce{N}$ 原子虽在经典画法中看似 *s\*\*p*3 杂化（连有三个单键），实际上会强行变为 *s\*\*p*2 杂化以实现芳香化。

由于孤对电子已经“充公“给了整个环的离域体系，吡咯的 $\\ce{N}$ **几乎没有碱性，也很难作为配体连接金属**。若强行用孤对电子去配位，环的芳香性将被破坏，分子变得极不稳定。

呋喃中的氧原子（第 VIA 族，6 个价电子）有两对孤对电子：一对进入 *p* 轨道参与离域（类似吡咯），另一对位于平面内的 *s\*\*p*2 轨道中（类似吡啶）。理论上平面外的那对电子可以配位，但由于氧的电负性较大，实际配位能力远不如吡啶。

噻吩中硫的原子轨道较大，一对孤对电子参与芳香性，另一对在 *s\*\*p*2 轨道中可一定程度配位，但同样弱于吡啶。
...
- 在溴乙烷分子中，由于**溴原子的电负性比碳原子的大**，使 $\\ce{C-Br}$ 的电子向 $\\ce{Br}$ 原子偏移，进而使碳原子带部分正电荷（*δ*+），$\\ce{Br}$ 原子带部分负电荷（*δ*−），这样就形成一个极性较强的共价键：$\\ce{C^{\\delta^+}-Br^{\\delta^-}}$，其键长大而键能较小。因此在化学反应中，$\\ce{C-Br}$ **较易断裂**，使碳原子与带负电荷的基团结合，$\\ce{Br}$ 原子被其他原子或原子团所取代，生成负离子离去。
...
光卤代拓展：**有机物的光卤代反应（Photo-halogenation）** 是有机化学中非常基础且重要的一类反应。它通常指的是烷烃或带有烷基侧链的化合物，在光照（*h\*\*ν*）条件下，分子中的氢原子被卤素原子（通常是氯或溴）取代的过程。该反应的核心机制是**自由基链式取代反应（Free Radical Chain Substitution）**。

通式：

$$ \\ce{R-H + X2 ->[h\\nu\\text{ or }\\Delta] R-X + HX} $$

其中，*X*2 通常为 *C\*\*l*2 或 *B\*\*r*2。*F*2 反应太剧烈难以控制，*I*2 反应吸热且可逆，通常不发生。如果在反应中生成了手性中心，产物的立体化学特征是什么？
...
- **第一阶段：链引发**。
  ...
  在紫外光（UV）或高温作用下，卤素分子发生**均裂（Homolytic Cleavage）**，生成两个高活性的卤素自由基。*C\*\*l*⋅ 表示带有单电子的氯自由基，极不稳定，反应活性极高。
...
1. \*\*氯 ($\\ce{Cl_2}$)：\*\*反应放热，速度快。$\\ce{Cl\*}$ 活性较高，但选择性中等（既能取代一级氢，也能取代二级、三级氢，得到混合物）。
1. **溴 ($\\ce{Br_2}$)：**反应放热较少（夺氢步骤甚至是吸热的），速度较慢。$\\ce{Br\*}$ 活性较低，但**选择性极高**（倾向于取代最稳定的碳自由基对应的氢）。
1. \*\*碘 ($\\ce{I_2}$)：\*\*夺氢步骤是强吸热的，反应难以正向进行，且生成的 $\\ce{HI}$ 还原性强，会将 $\\ce{R-I}$ 还原回 $\\ce{R-H}$。

当烷烃分子中有不同类型的氢（一级 1∘、二级 2∘、三级 3∘）时，卤素取代哪里**核心原则：** 反应速率取决于**中间体烷基自由基的稳定性**。

- **氯代（低选择性）：**

  - 虽然 3∘ 氢反应最快，但 $\\ce{Cl\*}$ 太活泼（“饿不择食”），加上 1∘ 氢的数量通常更多，所以氯代通常得到各种异构体的混合物。
...
烯丙位/苄位卤代：这是非常有用的反应。因为烯丙基自由基和苄基自由基由于 *p* − *π* 共轭效应而异常稳定。因此，即使在温和条件下（如 N-溴代丁二酰亚胺 NBS，低浓度 $\\ce{Br2}$，光照），也能高选择性地发生在烯丙位或苄位上，而双键或苯环本身不被破坏。例如：用 NBS/CCl4/hv 处理甲苯，可以得到苄基溴。
...
溴与乙醛的反应因其条件不同而有不同的产物。溴（$\\ce{Br2}$）与乙醛（$\\ce{CH3CHO}$）之间的反应是一个非常经典的有机化学反应。由于乙醛分子中同时含有**醛基（$\\ce{-CHO}$）**和 ***α*-甲基（与羰基相连的 $\\ce{-CH3}$）**，溴又具有氧化性和发生亲电反应的能力，因此两者的反应途径**高度依赖于反应条件（如溶剂、酸碱性、温度等）**。
...
1. ***α*-氢的取代反应**
   ...
   **酸性或中性条件**（通常在有机溶剂如 $\\ce{CCl4}$ 或乙酸中）：在酸催化下，乙醛会发生一溴代反应。
   ...
   - 反应过程：
     ...
     1. **亲电加成-消除**：烯醇的富电子双键进攻溴分子（$\\ce{Br2}$），失去一个质子后生成一溴乙醛。

   **碱性条件**：如果在碱性溶液中加入溴（本质上是生成了次溴酸盐 $\\ce{NaOBr}$），反应将迅速进行到底，发生碳碳键断裂。

   1. **快速多卤代**：碱夺取乙醛的 *α*-氢生成烯醇负离子。烯醇负离子进攻 $\\ce{Br2}$ 生成一溴乙醛。由于溴的强吸电子性，使得剩下的 *α*-氢**酸性更强**，更容易被碱拔去。因此，反应不会停留在一步，而是迅速生成**三溴乙醛**（$\\ce{CBr3CHO}$）。
   1. **碳碳键断裂（亲核酰基取代）**：氢氧根离子（$\\ce{OH-}$）作为亲核试剂进攻三溴乙醛的羰基碳。随后，含有三个吸电子溴原子的碳负离子（$\\ce{-CBr3}$）作为一个良好的离去基团脱落。
   1. **质子转移**：离去的 $\\ce{^-CBr3}$ 夺取酸上的质子，生成**溴仿**（$\\ce{CHBr3}$，一种重的液体，有特殊甜味）。
...
与乙醛不同，甲醛（$\\ce{HCHO}$）的分子结构中**没有 *α*-碳原子**，因此也就**没有任何 *α*-氢**。这个结构上的根本差异决定了溴与甲醛的反应途径相对单一：**它不能发生像乙醛那样的 *α*-取代反应或卤仿反应，主要且几乎唯一的反应类型就是氧化反应。**
...
甲醛（$\\ce{HCHO}$）和乙醛（$\\ce{CH3CHO}$）虽然同属醛类，但甲醛由于其**特殊的对称结构（没有烷基，碳直接连着两个氢）**，在整个醛族中是一个绝对的“异类“。除了反应系数不同之外，它们在物理性质和化学性质上有着非常显著的区别。
...
1. 氧化的最终产物不同。
   ...
   - 如果把甲醛气体通入酸性 $\\ce{KMnO4}$ 溶液，溶液褪色且**有气体（$\\ce{CO2}$）生成**；而乙醛通入则只褪色，无气体。

1. 与新制 $\\ce{Cu(OH)2}$ 反应的极致程度。
...
1. 碘仿反应（鉴别两者的经典方法）。这是一个非常经典的有机鉴别反应。
   ...
   - **乙醛**：含有 **$\\ce{CH3-C(=O)-}$**（甲基酮 / 乙醛）结构，能与碘（$\\ce{I2}$）和氢氧化钠（$\\ce{NaOH}$）溶液发生**碘仿反应**，生成**黄色的具有特殊气味的碘仿（$\\ce{CHI3}$）沉淀**。
...
1. 在浓碱中的反应（歧化 vs 缩合）。这是由于\*\*有没有 *α*-H（与羰基相邻的碳上的氢）\*\*决定的。
   ...
   - **甲醛**：**没有 *α*-H**。在浓碱（如浓 $\\ce{NaOH}$）中不能缩合，而是发生**康尼查罗反应（Cannizzaro 反应，即歧化反应）**。两分子甲醛互相氧化还原，一分子变成甲醇，一分子变成甲酸钠。
...
1. 常温常压下的状态不同。
   ...
   - **乙醛**：常温下是**极易挥发的液体**（沸点 $\\pu{20.8^\\circ C}$）。所以在标况下（$\\pu{0^\\circ C}$），乙醛是液体。
   - 在计算阿伏伽德罗常数（*N\*\*A*）的题目中，如果说“标况下 $\\pu{22.4 L}$ 的乙醛”，这是错误的陷阱（因为标况下乙醛不是气体）；但说“标况下 $\\pu{22.4 L}$ 的甲醛”则是完全成立的（含 $\\pu{1 mol}$ 的 $\\ce{HCHO}$）。

甲醛由于其特殊的“两头都是氢“的无烷基结构，使得它：

1. **还原性极强**（出 $\\ce{Cu}$、出 $\\ce{H2}$、能被氧化成 $\\ce{CO2}$）。
1. **缺乏 *α*-H**（不能碘仿、碱中歧化）。
1. **物理性质独特**（常温是气体）。
...
- **$\\alpha-\\ce{H}$ 被取代反应**：羧酸分子中与羧基相邻的 $\\alpha-\\ce{H}$ 较活泼，易被卤素取代。常见如卤素（$\\ce{Cl2}$）在催化剂作用下取代 $\\alpha-\\ce{H}$，生成 *α*-卤代羧酸。
...
从反应机理的角度看，酯化和酯的水解走的并不是经典的 *S\*\*N*1 或 *S\*\*N*2 路径，而是亲核酰基取代（*S**N**A\*\*c*）机理。广义的取代反应（Substitution reaction）定义为：分子中的某个原子或基团（$\\ce{-A}$）被另一个原子或基团（$\\ce{-B}$）所替换的过程，只看**起点和终点，不看过程**。但在脂肪族化合物中，经典的狭义取代反应是 ***S**N*1\*\*（单分子亲核取代）和 ***S**N*2\*\*（双分子亲核取代），比如氢氧根取代溴乙烷中的溴就是 *S\*\*N*2 机理——亲核试剂从背面进攻，旧键断裂和新键形成**同时发生**（一步完成）。

**但是，酯化和酯的水解并不走这种“一步到位“或“碳正离子“的经典路径！**

因为羧基和酯基中的碳是 ***s**p*2 杂化**的（含有一个 $\\ce{C=O}$ 双键）。有机化学中有一个规律：**亲核试剂极难对 \*s**p\*2 杂化碳进行直接的 \*S**N\*2 背后进攻\*\*（因为有极大的空间位阻，且 *π* 键电子云会排斥亲核试剂）。

它的取代采用了一种“先加成、后消除“的机理，分为两步：

1. **第一步：加成（Addition）**

   亲核试剂（比如醇里的氧，或者水里的氧）进攻带有部分正电荷的羰基碳。此时，$\\ce{C=O}$ 双键断裂变成单键，原本的 *s\*\*p*2 平面碳变成了 ***s**p*3 杂化的四面体中间体\*\*。
...
有机物的酸性强弱与其结构密切相关，判断能否与 $\\ce{NaHCO3}$ 反应的关键在于 *p**K**a* \< 6.38。能通过复分解反应与碳酸氢钠溶液反应产生二氧化碳气体的有机物，**其根本特征只有一个：该有机物在水溶液中的酸性必须强于碳酸的酸性**（强酸制弱酸原理）。具体来说，碳酸的表观酸度常数 *p**K**a* ≈ 6.38。因此，**任何 \*p**K**a\* \< 6.38 的有机物**，在热力学上都能与碳酸氢钠反应生成二氧化碳。尽管从实验上来讲，还需要考虑动力学因素，如果反应过慢看不到气泡，一般也不视为能反应。

1. 除了含有羧基（$\\ce{-COOH}$），还有哪些有机物能产生 $\\ce{CO2}$？

   原理：只要有机分子中含有其他能释放质子（$\\ce{H+}$）的官能团，且解离后的阴离子非常稳定（*p**K**a* \< 6.38），就能反应。常见的有以下几类（从常见到极端）：

   1. 磺酸类（$\\ce{-SO3H}$）——最常见的强有机酸。

      **特征**：含有磺酸基，酸性极强，通常 *p**K**a* \< 0（比很多无机酸都强）。

      **代表物**：对甲苯磺酸、苯磺酸、甲磺酸。它们与 $\\ce{NaHCO3}$ 反应非常剧烈。

   1. 含有强吸电子基团的酚类（$\\ce{-OH}$）——被“逼成“强酸的弱酸。

      **特征**：普通的苯酚（石炭酸）*p**K**a* ≈ 10，酸性弱于碳酸，不能与 $\\ce{NaHCO3}$ 反应。但在苯环上引入多个强吸电子基团（如硝基 $\\ce{-NO2}$），会极大地分散阴离子的电荷，使其酸性剧增。

      **代表物**：**2,4-二硝基苯酚**：*p**K**a* ≈ 4.1，可以与 $\\ce{NaHCO3}$ 反应释放 $\\ce{CO2}$。

      **代表物**：**苦味酸（2,4,6-三硝基苯酚）**：*p**K**a* ≈ 0.38，酸性已经接近无机强酸，剧烈反应。

   1. 特殊的烯醇类（$\\ce{C=C-OH}$）。

      **特征**：本身没有羧基，但羟基连在双键碳上，且解离后的阴离子能通过共轭体系高度稳定化。

      **代表物**：**抗坏血酸（维生素 C）**。它的 *p**K**a*1 ≈ 4.17，酸性强于醋酸，泡腾片里经常用它（或者柠檬酸）配小苏打来产生气泡。

   1. 氧碳酸类（Oxocarbons）——高度对称的化合物。

      **特征**：完全由碳和氧构成的环状化合物带有羟基，解离后电荷在高度对称的环上完美平均分配。

      **代表物**：**方酸（Squaric acid，3,4-二羟基-3-环丁烯-1,2-二酮）**。虽然只有羟基，但其 *p**K**a*1 ≈ 1.5，是一种非常强的有机酸，剧烈释放 $\\ce{CO2}$。

   1. 极端的碳负离子酸（$\\ce{C-H}$ 酸）——氢连在碳上也能变强酸。

      **特征**：通常连在碳上的氢根本没有酸性（例如甲烷 *p**K**a* ≈ 48）。但如果这个碳连了极强的吸电子基团，C-H 键就会断裂释放出 $\\ce{H+}$。

      **代表物**：**三硝基甲烷（$\\ce{HC(NO2)3}$）**。由于三个硝基的强大吸电子效应，它的 *p**K**a* ≈ 0.17，不仅能与 $\\ce{NaHCO3}$ 反应冒泡，甚至能腐蚀金属。更极端的还有**五氰基环戊二烯**，它是已知最强的有机酸之一（超酸级别）。

1. 含有羧基（$\\ce{-COOH}$），但却不能与 $\\ce{NaHCO3}$ 产生 $\\ce{CO2}$ 的有机物有没有？

   有。尽管绝大多数羧酸的 *p**K**a* 在 3 ∼ 5 之间（强于碳酸），但在一些特殊或极端的情况下，含有羧基的有机物是**不能**让小苏打冒泡的。原因可以分为“热力学限制“和“动力学 / 物理限制“：
   ...
   1. 氨基酸（内盐效应 / 等电点）——最普遍的例外。
      ...
      **原因**：羧基上的 $\\ce{H+}$ 已经“内部转移“给了氨基。此时如果加入小苏打，能够被碱中和的质子实际上在 $\\ce{-NH3^+}$ 上。而 $\\ce{-NH3^+}$ 的 *p**K**a* ≈ 9.6，远弱于碳酸。因此，甘氨酸（以及大多数等电点附近的中性氨基酸）不会与 $\\ce{NaHCO3}$ 反应冒泡。

   1. 多元酸的酸式盐——最后一个质子的酸性不足。

      **特征**：多元弱酸的前几个氢已经解离（变成盐），分子中依然保留着羧基（$\\ce{-COOH}$），但此时解离难度极大。

      **代表物**：**枸橼酸氢二钠（二碱式柠檬酸钠）**。柠檬酸有三个羧基，第三个羧基解离的 *p**K**a*3 = 6.40。

      **原因**：带有两个负电荷的柠檬酸酸式根，即使它身上还带着一个真实的 $\\ce{-COOH}$ 官能团，它在热力学上也无法再把质子强行塞给 $\\ce{HCO3^-}$ 了，不产生 $\\ce{CO2}$。

   1. 极长链高级脂肪酸（物理溶解度限制）。

      **特征**：含有羧基，酸性在理论上足够（*p**K**a* ≈ 5），但在水里完全不溶。

      **代表物**：\*\*硬脂酸（十八烷酸）\*\*固体。

      **原因**：如果把硬脂酸粉末和 $\\ce{NaHCO3}$ 溶液混合在常温下，**肉眼几乎看不到任何气泡**。因为硬脂酸极度疏水，反应只能在固体表面极小范围内发生，且生成的硬脂酸钠（肥皂）会包裹在表面阻碍反应。这属于动力学和相态造成的“不能“。（注：如果加热到融化并剧烈搅拌，依然是可以反应的）。

   1. 极端空间位阻的羧酸（空间屏蔽效应）。

      **代表物**：**2,4,6-三叔丁基苯甲酸**。

      **原因**：苯环上羧基的两侧被庞大的“叔丁基“完全包裹，像一堵墙一样把 $\\ce{-COOH}$ 保护了起来。碳酸氢根离子（$\\ce{HCO3^-}$）很难靠近羧基去夺取质子，导致反应速率极度缓慢，宏观上观察不到明显的 $\\ce{CO2}$ 气泡产生。
...
固醇属于类固醇的一个子群，固醇是最早发现的类固醇化合物，自然界中分布甚广。广义上的固醇，包括最简单的腺甾醇；而狭义上的固醇，还需在 17 号 $\\ce{C}$ 上有一个约 8 ∼ 10 碳原子的烃侧链。
...
- **加热条件下的极度混乱（焦糖化反应）**
  ...
  - 葡萄糖在达到熔点（约 $\\pu{146^oC}$）后会熔化。如果继续加热，它不会发生像乙醇那样整齐划一的 *A* + *A* → *B* + *H*2*O* 反应。
...
根据氨基连结在羧酸中碳原子的位置，可将氨基酸分为 *α*, *β*, *γ*, *δ*, … 等类型，在生物化学中，若无明示，氨基酸通常默认 *α* 氨基酸，即胺基和羧基直接连接在同一个 $\\ce{-CH-}$ 结构上的氨基酸，其通式是 $\\ce{H2N-CHR-COOH}$。
