...
7.  常温常压下的状态不同
    ...
    - *应用考点*：在计算阿伏伽德罗常数（$N_A$）的题目中，如果说"标况下
      $\pu{22.4 L}$
...
在脂肪族化合物中，经典的狭义取代反应是 **$S_N1$**（单分子亲核取代）和
**$S_N2$**（双分子亲核取代）。比如氢氧根取代溴乙烷中的溴，是 $S_N2$
...
**亲核酰基取代反应（Nucleophilic Acyl Substitution，简称 $S_N Ac$）**。
...
$pK_a \approx 6.38$。因此，**任何 $pK_a < 6.38$
...
1.  除了含有羧基（$\ce{-COOH}$），还有哪些有机物能产生 $\ce{CO2}$？
    ...
    2.  含有强吸电子基团的酚类（$\ce{-OH}$）------被"逼成"强酸的弱酸。
        ...
        **特征**：普通的苯酚（石炭酸）$pK_a \approx 10$，酸性弱于碳酸，不能与
        $\ce{NaHCO3}$ 反应。但在苯环上引入多个强吸电子基团（如硝基
        $\ce{-NO2}$），会极大地分散阴离子的电荷，使其酸性剧增。

        **代表物**：**2,4-二硝基苯酚**：$pK_a \approx 4.1$，可以与
    ...
    3.  特殊的烯醇类（$\ce{C=C-OH}$）。
        ...
        **代表物**：**抗坏血酸（维生素 C）**。它的
        $pK_{a1} \approx 4.17$，酸性强于醋酸，泡腾片里经常用它（或者柠檬酸）配小苏打来产生气泡。
...
2.  含有羧基（$\ce{-COOH}$），但却不能与 $\ce{NaHCO3}$ 产生 $\ce{CO2}$
    ...
    1.  氨基酸（内盐效应 / 等电点）------最普遍的例外。
        ...
        **原因**：羧基上的 $\ce{H+}$
        ...
        $\ce{-NH3^+}$ 上。而 $\ce{-NH3^+}$ 的
        $pK_a \approx 9.6$，远弱于碳酸。因此，甘氨酸（以及大多数等电点附近的中性氨基酸）不会与
        $\ce{NaHCO3}$ 反应冒泡。
...
![alt text|w40%](../Steran_num_ABCD.svg)

腺甾烷的衍生物：

::: {#table1}
| ![alt text](../Gonane.svg) 腺甾烷 | ![alt text](../Androstane.svg) 雄烷 | ![alt text](../Estrane.svg) 雌烷 | ![alt text](../Pregnan.svg) 孕烷 | ![alt text](../Cholane.svg) 胆烷 |
|:--:|:--:|:--:|:--:|:--:|
| ![alt text](../Cholesterol.svg) 胆固醇 | ![alt text](../Testosteron.svg) 睾酮 | ![alt text](../Estradiol.svg) 雌二醇 | ![alt text](../Progesteron.svg) 孕酮 | ![alt text](../Cholic_acid.svg) 胆酸 |
:::
...
![alt text|w50%](../Amino_acid_generic_structure.avif)
...
![alt text|w100%](../Amino_Acids.avif)
...
![alt text|w80%](../Peptidformationball-8dec250.avif)
...
![alt text|w100%](../225_Peptide_Bond-01.avif)
...
![alt text|w90%](../Nucleotides_zh.avif)

- 如果其五碳糖是脱氧核糖，则此单体的聚合物是脱氧核糖核酸 DNA。

- 如果其五碳糖是核糖，则此单体的聚合物是核糖核酸 RNA。

![alt text|w90%](../Difference_DNA_RNA-ZH.svg)
...
![alt text|w90%](../DNA_Structure.avif)
