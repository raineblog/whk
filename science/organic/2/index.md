...
烷烃分子中的碳原子都采取 $text\{sp3}$ 杂化，形成四面体结构；碳原子以 $sigma$ 键与其他碳原子或氢原子结合；每个碳原子形成 $4$ 个共价单键；除甲烷外，烷烃分子中既有极性键，又有非极性键。
...
![alt text](/static/image/image-2.908b85a55b.avif){width="70%"}
**苯**，为最简单的一种芳香烃，芳香族化合物皆由其衍生而成。苯结构是一个平面正六边形分子，碳原子以 $text\{sp2}$ 轨道键结，所以任二键互成 $pu\{120^o}$。苯的含碳量很高，在空气中燃烧产生明亮并带有浓烟的火焰。
...
- _甲酸钠不是"醛"_：甲酸钠里面不是有 -CHO 吗？为什么不叫醛基？为什么不与新制 $Cu(OH)\_2$ 反应？

  这是一个极其经典的结构化学问题。甲酸钠（$ce\{HCOONa}$）的结构简式看起来像 $ce\{\[H-C(=O)-O]-\[Na]+}$。在视觉上，它左边确实有一个 $ce\{H-C(=O)-}$ 结构，这看起来和醛基一模一样。有些老师为了方便记忆，也会通俗地说"甲酸及其盐含有醛基"。**但这在严谨的化学原理上是不准确的。**

  - **普通醛基（$ce\{R-CHO}$）**：羰基（$ce\{C=O}$）是一个强吸电子基团，导致碳氢键（$ce\{C-H}$）的电子云偏向碳，使得氢原子变得活泼，容易被氧化脱去。

  - **甲酸根离子（$ce\{HCOO-}$）**：碳连着一个带负电荷的氧（$ce\{-O^-}$）。这个氧拥有丰富的孤对电子，它会和羰基发生**强烈的大 $pi$ 键共轭效应（$p-\pi$ 共轭）**。氧上的电子云会源源不断地补充给羰基碳，使得整个甲酸根离子极其稳定。

  这种稳定作用导致甲酸根上的那个碳氢键（$ce\{C-H}$）被大大加固了，它的**还原性大幅度下降**。

  虽然甲酸（盐）在极端的条件下（比如用银氨溶液，即多伦试剂，水浴加热）依然能发生银镜反应，因为银氨溶液氧化性相对较强且反应环境不同；但是，**新制 $ce\{Cu(OH)2}$（斐林试剂）的氧化性比银氨溶液弱得多**。甲酸钠因为存在共轭稳定效应，其微弱的还原性已经不足以在常规实验条件下把 $ce\{Cu(OH)2}$ 还原了。因此，它不与新制 $ce\{Cu(OH)2}$ 反应。
...
**溴与乙醛的反应**：溴（$ce\{Br2}$）与乙醛（$ce\{CH3CHO}$）之间的反应是一个非常经典的有机化学反应。由于乙醛分子中同时含有**醛基（$ce\{-CHO}$）**和**$alpha$-甲基（与羰基相连的 $ce\{-CH3}$）**，溴又具有氧化性和发生亲电反应的能力，因此两者的反应途径**高度依赖于反应条件（如溶剂、酸碱性、温度等）**。
...
2. **$alpha$-氢的取代反应**
   ...
   乙醛的 $alpha$-碳原子上连接有三个氢原子，受羰基强烈的吸电子诱导效应影响，这些 $alpha$-氢具有一定的酸性（活泼氢），可以被溴发生取代。取代反应的产物取决于体系的酸碱性。

   **酸性或中性条件**（通常在有机溶剂如 $ce\{CCl4}$ 或乙酸中）：在酸催化下，乙醛会发生一溴代反应。

   - **为什么停留在一溴代？** 引入一个吸电子的溴原子后，碳正离子中间体的稳定性降低，使得进一步的"烯醇化"变得更加困难。因此，在酸性条件下，反应很容易控制在**一溴代阶段**。
   ...
   **碱性条件**：如果在碱性溶液中加入溴（本质上是生成了次溴酸盐 $ce\{NaOBr}$），反应将迅速进行到底，发生碳碳键断裂。

   1. **快速多卤代**：碱夺取乙醛的 $alpha$-氢生成烯醇负离子。烯醇负离子进攻 $ce\{Br2}$ 生成一溴乙醛。由于溴的强吸电子性，使得剩下的 $alpha$-氢**酸性更强**，更容易被碱拔去。因此，反应不会停留在一步，而是迅速生成**三溴乙醛**（$ce\{CBr3CHO}$）。

   2. **碳碳键断裂（亲核酰基取代）**：氢氧根离子（$ce\{OH-}$）作为亲核试剂进攻三溴乙醛的羰基碳。随后，含有三个吸电子溴原子的碳负离子（$ce\{-CBr3}$）作为一个良好的离去基团脱落。

   3. **质子转移**：离去的 $ce\{^-CBr3}$ 夺取酸上的质子，生成**溴仿**（$ce\{CHBr3}$，一种重的液体，有特殊甜味）。

   **特殊意义**：**乙醛是唯一能够发生卤仿反应的醛**（因为只有它含有 $ce\{CH3-C(=O)-}$ 结构）。
...
**甲醛与溴的反应**：与乙醛不同，甲醛（$ce\{HCHO}$）的分子结构中**没有 $alpha$-碳原子**，因此也就**没有任何 $alpha$-氢**。这个结构上的根本差异决定了溴与甲醛的反应途径相对单一：**它不能发生像乙醛那样的 $alpha$-取代反应或卤仿反应，主要且几乎唯一的反应类型就是氧化反应。**

#### 甲醛的特殊性 \{#\_15}

特殊的醛：甲醛。最直观的是，甲醛分子中相当于有两个 $ce\{-CHO}$，因此反应时，消耗的氧化剂数量翻倍。
...
- **甲醛（$ce\{HCHO}$）**：羰基连着**两个氢**。它既没有推电子的烷基，也没有空间位阻，这使得它的**还原性极强**，且**没有 $alpha$-碳原子（自然也没有 $alpha$-氢原子）**。

化学性质的显著差异：

1. 氧化的最终产物不同（极其重要）

   - **乙醛**：被氧化剂（如酸性 $ce\{KMnO4}$、银氨溶液、新制 $ce\{Cu(OH)2}$）氧化后，最终产物是**乙酸（$ce\{CH3COOH}$）**。乙酸极其稳定，无法被继续氧化。

   - **甲醛**：甲醛被氧化后，首先生成**甲酸（$ce\{HCOOH}$）**。甲酸虽然在弱氧化剂下较稳定，但如果遇到强氧化剂（如酸性高锰酸钾、重铬酸钾），甲酸会被继续氧化成碳酸，最终分解为 **$ce\{CO2}$ 和 $ce\{H2O}$**。

   - _应用考点_：如果把甲醛气体通入酸性 $ce\{KMnO4}$ 溶液，溶液褪色且**有气体（$ce\{CO2}$）生成**；而乙醛通入则只褪色，无气体。

2. 与新制 $ce\{Cu(OH)2}$ 反应的极致程度（结合上题）

   - **乙醛**：在加热条件下，只能将 $ce\{Cu^2+}$ 还原到 $+1$ 价，生成砖红色的 **$ce\{Cu2O}$** 沉淀。

   - **甲醛**：因为还原性极强，在碱性条件下不仅能生成 $ce\{Cu2O}$，还能进一步将铜离子彻底还原为**单质铜（$ce\{Cu}$，红色沉淀，不溶于浓盐酸）**，并且伴随着甲醛的催化脱氢，放出**大量氢气（$ce\{H2}$）**。

   - _与银氨溶液的区别_：银氨溶液中氨的最主要作用是提供强碱性环境和减缓反应速度，这也导致银氨溶液的氧化性并没有裴琳试剂强，甲醛与银氨溶液的反应与乙醛基本上相似。

3. 碘仿反应（鉴别两者的终极武器）这是一个非常经典的有机鉴别反应。

   - **乙醛**：含有 **$ce\{CH3-C(=O)-}$**（甲基酮/乙醛）结构，能与碘（$ce\{I2}$）和氢氧化钠（$NaOH$）溶液发生**碘仿反应**，生成**黄色的具有特殊气味的碘仿（$ce\{CHI3}$）沉淀**。

   - **甲醛**：没有 $ce\{-CH3}$ 结构，**绝对不能**发生碘仿反应。用碘和氢氧化钠可以瞬间鉴别甲醛和乙醛。

4. 在浓碱中的反应（歧化 vs 缩合）这是由于\*\*有没有 $alpha$-H （与羰基相邻的碳上的氢）\*\*决定的。

   - **乙醛**：有 $alpha$-H。在碱性条件下，极易发生羟醛缩合反应，生成高分子聚合物（变成黄褐色的**醛树脂**）。

   - **甲醛**：**没有 $alpha$-H**。在浓碱（如浓 $ce\{NaOH}$）中不能缩合，而是发生**康尼查罗反应（Cannizzaro 反应，即歧化反应）**。两分子甲醛互相氧化还原，一分子变成甲醇，一分子变成甲酸钠。
...
7. 常温常压下的状态不同
   ...
   - _应用考点_：在计算阿伏伽德罗常数（$N\_A$）的题目中，如果说"标况下 $pu\{22.4 L}$ 的乙醛"，这是**错误**的陷阱（因为标况下乙醛不是气体）；但说"标况下 $pu\{22.4 L}$ 的甲醛"则是完全成立的（含 $pu\{1 mol}$ 的 $ce\{HCHO}$）。

甲醛由于其特殊的"两头都是氢"的无烷基结构，使得它：

1. **还原性爆表**（出 $ce\{Cu}$、出 $ce\{H2}$、能被氧化成 $ce\{CO2}$）；
...
![](/static/image/CnH2nO2.fbd46e9bd1.avif)
...
<tbody>
<tr>
<td><img src="../Fat-Conformation-Sat.avif" /></td>
<td><img src="../Fat-Conformation-Cis.avif" /></td>
<td><img src="../Fat-Conformation-Trans.avif" /></td>
</tr>
<tr>
<td>硬脂酸：。<br />
软脂酸：。</td>
...
![](/static/svg/油脂.1cebd3c3e6.svg){width="70%"}
...
![](/static/svg/Steran_num_ABCD.1861d55ccc.svg){width="40%"}
腺甾烷的衍生物：

::: \{#table1}

| <img src={image13} /> 腺甾烷 | <img src={image14} /> 雄烷 |  <img src={image15} /> 雌烷 | <img src={image16} /> 孕烷 | <img src={image17} /> 胆烷 |
| :-----------------------: | :----------------------: | :-----------------------: | :----------------------: | :----------------------: |
| <img src={image18} /> 胆固醇 | <img src={image19} /> 睾酮 | <img src={image20} /> 雌二醇 | <img src={image21} /> 孕酮 | <img src={image22} /> 胆酸 |
|            :::            |                          |                           |                          |                          |
...
![](/static/image/Amino_Acids.e03db80b30.avif){width="100%"}
...
![](/static/image/Peptidformationball-8dec250.ce349641b7.avif){width="80%"}
...
![](/static/image/225_Peptide_Bond-01.8c50245e8a.avif){width="100%"}
...
![](/static/image/Nucleotides_zh.75acf58579.avif){width="90%"}
...
![](/static/svg/Difference_DNA_RNA-ZH.0729bcb645.svg){width="90%"}
...
<tbody>
<tr>
<td style="text-align: center;"><img src="../Base_pair_GC.svg" /></td>
<td style="text-align: center;"><img src="../Base_pair_AT.svg" /></td>
</tr>
</tbody>
</table>
...
![](/static/image/DNA_Structure.5262f907c1.avif){width="90%"}