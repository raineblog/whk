...
|          | 描述                         | 通式                         |
| -------- | ---------------------------- | ---------------------------- |
| **烷烃** | 碳原子之间全部以单键结合     | \\ce{C_nH\_{2n+2}}（n\\ge1） |
| **烯烃** | 分子中含有一个或多个碳碳双键 | \\ce{C_nH\_{2n}}（n\\ge2）   |
| **炔烃** | 分子中含有一个或多个碳碳三键 | \\ce{C_nH\_{2n-2}}（n\\ge2） |
...
苯的同系物：分子里含有一个苯环的烃，通式为 \\ce{C_nH\_{2n-6}}（n\\ge6）。
...
广义来说，有芳香性的烃就是芳香烃，而芳香性指的是分子具有特殊的稳定性，这种稳定性来源于完全离域的 \\pi 电子体系，一般来说，符合休克尔规则（Hückel 规则）的平面环状共轭体系具有芳香性，即 \\pi 电子数为 4n+2（n=0,1,2,\\dots）。但是不是所有符合这个简单规律的都是，例如如果环的结构无法形成平面型，所有 \\pi 电子云难以肩并肩形成大 \\pi 键，或者说，某些物质强行压到平面型，需要的能量远远高于形成离域的稳定结构而减少的，得不偿失，就没有芳香性（例如环癸五烯/[10]-轮烯）。
...
苯环中的 \\ce{CH} 基团被杂原子（\\ce{N}、\\ce{O}、\\ce{S} 等）取代后，形成的含杂原子芳香环统称为**芳香杂环化合物**。常见的有吡啶、吡咯、呋喃、噻吩等。理解杂环芳香性的核心在于判断**杂原子的孤对电子是否参与了芳香大 \\pi 键**。

根据杂原子孤对电子的归属，可以将芳香杂环分为两大类。

\*\*吡啶型：孤对电子不参与离域。\*\*代表分子有吡啶（六元环，含 \\ce{=N-}）、嘧啶、喹啉等。

以吡啶为例，环中的 \\ce{N} 原子采取 sp^2 杂化。其 5 个价电子分配如下：

- 两个 sp^2 杂化轨道各贡献 1 个电子，与相邻碳原子形成 \\sigma 键。
- 一个电子进入垂直于环平面的未杂化 p 轨道，与环上 5 个碳原子的 p 电子共同形成含 6 个电子的芳香大 \\pi 键，满足 4n+2 规则（n=1）。
- 剩余两个电子（孤对电子）占据第三个 sp^2 杂化轨道，该轨道位于环平面内并向环外伸出，**完全不参与离域**。

由于孤对电子独立于 \\pi 体系之外，吡啶具有较强的碱性和配位能力。这对孤对电子可以直接提供给金属离子（如 \\ce{Ni}）形成配位键，而环的芳香性不受影响。

\*\*吡咯型：孤对电子参与离域。\*\*代表分子有吡咯（五元环，含 \\ce{-NH-}）、呋喃（含 \\ce{-O-}）、噻吩（含 \\ce{-S-}）等。

以吡咯为例，环上有 4 个碳原子，每个碳提供 1 个 \\pi 电子，共 4 个。为了满足 6 电子的 Hückel 芳香稳定结构，\\ce{N} 原子必须将其孤对电子（2 个）全部放入 p 轨道中参与离域，与碳的 4 个电子共同构成芳香大 \\pi 键。为此，\\ce{N} 原子虽在经典画法中看似 sp^3 杂化（连有三个单键），实际上会强行变为 sp^2 杂化以实现芳香化。

由于孤对电子已经“充公“给了整个环的离域体系，吡咯的 \\ce{N} **几乎没有碱性，也很难作为配体连接金属**。若强行用孤对电子去配位，环的芳香性将被破坏，分子变得极不稳定。

呋喃中的氧原子（第 VIA 族，6 个价电子）有两对孤对电子：一对进入 p 轨道参与离域（类似吡咯），另一对位于平面内的 sp^2 轨道中（类似吡啶）。理论上平面外的那对电子可以配位，但由于氧的电负性较大，实际配位能力远不如吡啶。

噻吩中硫的原子轨道较大，一对孤对电子参与芳香性，另一对在 sp^2 轨道中可一定程度配位，但同样弱于吡啶。
...
溴与乙醛的反应因其条件不同而有不同的产物。溴（\\ce{Br2}）与乙醛（\\ce{CH3CHO}）之间的反应是一个非常经典的有机化学反应。由于乙醛分子中同时含有**醛基（\\ce{-CHO}）**和 **\\alpha-甲基（与羰基相连的 \\ce{-CH3}）**，溴又具有氧化性和发生亲电反应的能力，因此两者的反应途径**高度依赖于反应条件（如溶剂、酸碱性、温度等）**。
...
与乙醛不同，甲醛（\\ce{HCHO}）的分子结构中**没有 \\alpha-碳原子**，因此也就**没有任何 \\alpha-氢**。这个结构上的根本差异决定了溴与甲醛的反应途径相对单一：**它不能发生像乙醛那样的 \\alpha-取代反应或卤仿反应，主要且几乎唯一的反应类型就是氧化反应。**
...
1. 在浓碱中的反应（歧化 vs 缩合）。这是由于\*\*有没有 \\alpha-H（与羰基相邻的碳上的氢）\*\*决定的。
...
1. 常温常压下的状态不同。
   ...
   - 在计算阿伏伽德罗常数（N_A）的题目中，如果说“标况下 \\pu{22.4 L} 的乙醛”，这是错误的陷阱（因为标况下乙醛不是气体）；但说“标况下 \\pu{22.4 L} 的甲醛”则是完全成立的（含 \\pu{1 mol} 的 \\ce{HCHO}）。

甲醛由于其特殊的“两头都是氢“的无烷基结构，使得它：

1. **还原性极强**（出 \\ce{Cu}、出 \\ce{H2}、能被氧化成 \\ce{CO2}）。
1. **缺乏 \\alpha-H**（不能碘仿、碱中歧化）。
1. **物理性质独特**（常温是气体）。
...
从反应机理的角度看，酯化和酯的水解走的并不是经典的 S_N1 或 S_N2 路径，而是亲核酰基取代（S_NAc）机理。广义的取代反应（Substitution reaction）定义为：分子中的某个原子或基团（\\ce{-A}）被另一个原子或基团（\\ce{-B}）所替换的过程，只看**起点和终点，不看过程**。但在脂肪族化合物中，经典的狭义取代反应是 **S_N1**（单分子亲核取代）和 **S_N2**（双分子亲核取代），比如氢氧根取代溴乙烷中的溴就是 S_N2 机理——亲核试剂从背面进攻，旧键断裂和新键形成**同时发生**（一步完成）。

**但是，酯化和酯的水解并不走这种“一步到位“或“碳正离子“的经典路径！**

因为羧基和酯基中的碳是 **sp^2 杂化**的（含有一个 \\ce{C=O} 双键）。有机化学中有一个规律：**亲核试剂极难对 sp^2 杂化碳进行直接的 S_N2 背后进攻**（因为有极大的空间位阻，且 \\pi 键电子云会排斥亲核试剂）。

它的取代采用了一种“先加成、后消除“的机理，分为两步：

1. **第一步：加成（Addition）**

   亲核试剂（比如醇里的氧，或者水里的氧）进攻带有部分正电荷的羰基碳。此时，\\ce{C=O} 双键断裂变成单键，原本的 sp^2 平面碳变成了 **sp^3 杂化的四面体中间体**。

1. **第二步：消除（Elimination）**

   这个四面体中间体很不稳定。氧原子上的负电荷（或孤对电子）会重新推回来，**重新形成 \\ce{C=O} 双键**。为了维持碳的四价，必须“挤走“一个离去基团（在酯化中挤走水，在水解中挤走醇）。

在高等有机化学中，为了统合“宏观表象“和“微观机理“，给这类反应起了一个专属的名字：**亲核酰基取代反应（Nucleophilic Acyl Substitution，简称 S_NAc）**。
...
有机物的酸性强弱与其结构密切相关，判断能否与 \\ce{NaHCO3} 反应的关键在于 pK_a < 6.38。能通过复分解反应与碳酸氢钠溶液反应产生二氧化碳气体的有机物，**其根本特征只有一个：该有机物在水溶液中的酸性必须强于碳酸的酸性**（强酸制弱酸原理）。具体来说，碳酸的表观酸度常数 pK_a \\approx 6.38。因此，**任何 pK_a < 6.38 的有机物**，在热力学上都能与碳酸氢钠反应生成二氧化碳。尽管从实验上来讲，还需要考虑动力学因素，如果反应过慢看不到气泡，一般也不视为能反应。

1. 除了含有羧基（\\ce{-COOH}），还有哪些有机物能产生 \\ce{CO2}？

   原理：只要有机分子中含有其他能释放质子（\\ce{H+}）的官能团，且解离后的阴离子非常稳定（pK_a < 6.38），就能反应。常见的有以下几类（从常见到极端）：

   1. 磺酸类（\\ce{-SO3H}）——最常见的强有机酸。

      **特征**：含有磺酸基，酸性极强，通常 pK_a < 0（比很多无机酸都强）。

      **代表物**：对甲苯磺酸、苯磺酸、甲磺酸。它们与 \\ce{NaHCO3} 反应非常剧烈。

   1. 含有强吸电子基团的酚类（\\ce{-OH}）——被“逼成“强酸的弱酸。

      **特征**：普通的苯酚（石炭酸）pK_a \\approx 10，酸性弱于碳酸，不能与 \\ce{NaHCO3} 反应。但在苯环上引入多个强吸电子基团（如硝基 \\ce{-NO2}），会极大地分散阴离子的电荷，使其酸性剧增。

      **代表物**：**2,4-二硝基苯酚**：pK_a \\approx 4.1，可以与 \\ce{NaHCO3} 反应释放 \\ce{CO2}。

      **代表物**：**苦味酸（2,4,6-三硝基苯酚）**：pK_a \\approx 0.38，酸性已经接近无机强酸，剧烈反应。

   1. 特殊的烯醇类（\\ce{C=C-OH}）。

      **特征**：本身没有羧基，但羟基连在双键碳上，且解离后的阴离子能通过共轭体系高度稳定化。

      **代表物**：**抗坏血酸（维生素 C）**。它的 pK\_{a1} \\approx 4.17，酸性强于醋酸，泡腾片里经常用它（或者柠檬酸）配小苏打来产生气泡。

   1. 氧碳酸类（Oxocarbons）——高度对称的化合物。

      **特征**：完全由碳和氧构成的环状化合物带有羟基，解离后电荷在高度对称的环上完美平均分配。

      **代表物**：**方酸（Squaric acid，3,4-二羟基-3-环丁烯-1,2-二酮）**。虽然只有羟基，但其 pK\_{a1} \\approx 1.5，是一种非常强的有机酸，剧烈释放 \\ce{CO2}。

   1. 极端的碳负离子酸（\\ce{C-H} 酸）——氢连在碳上也能变强酸。

      **特征**：通常连在碳上的氢根本没有酸性（例如甲烷 pK_a \\approx 48）。但如果这个碳连了极强的吸电子基团，C-H 键就会断裂释放出 \\ce{H+}。

      **代表物**：**三硝基甲烷（\\ce{HC(NO2)3}）**。由于三个硝基的强大吸电子效应，它的 pK_a \\approx 0.17，不仅能与 \\ce{NaHCO3} 反应冒泡，甚至能腐蚀金属。更极端的还有**五氰基环戊二烯**，它是已知最强的有机酸之一（超酸级别）。

1. 含有羧基（\\ce{-COOH}），但却不能与 \\ce{NaHCO3} 产生 \\ce{CO2} 的有机物有没有？

   有。尽管绝大多数羧酸的 pK_a 在 3\\sim5 之间（强于碳酸），但在一些特殊或极端的情况下，含有羧基的有机物是**不能**让小苏打冒泡的。原因可以分为“热力学限制“和“动力学 / 物理限制“：
   ...
   1. 氨基酸（内盐效应 / 等电点）——最普遍的例外。
      ...
      **原因**：羧基上的 \\ce{H+} 已经“内部转移“给了氨基。此时如果加入小苏打，能够被碱中和的质子实际上在 \\ce{-NH3^+} 上。而 \\ce{-NH3^+} 的 pK_a \\approx 9.6，远弱于碳酸。因此，甘氨酸（以及大多数等电点附近的中性氨基酸）不会与 \\ce{NaHCO3} 反应冒泡。

   1. 多元酸的酸式盐——最后一个质子的酸性不足。

      **特征**：多元弱酸的前几个氢已经解离（变成盐），分子中依然保留着羧基（\\ce{-COOH}），但此时解离难度极大。

      **代表物**：**枸橼酸氢二钠（二碱式柠檬酸钠）**。柠檬酸有三个羧基，第三个羧基解离的 pK\_{a3} = 6.40。

      **原因**：带有两个负电荷的柠檬酸酸式根，即使它身上还带着一个真实的 \\ce{-COOH} 官能团，它在热力学上也无法再把质子强行塞给 \\ce{HCO3^-} 了，不产生 \\ce{CO2}。

   1. 极长链高级脂肪酸（物理溶解度限制）。

      **特征**：含有羧基，酸性在理论上足够（pK_a \\approx 5），但在水里完全不溶。
