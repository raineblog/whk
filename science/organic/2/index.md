...
烷烃的通式：如果链状烷烃中的碳原子数为 $n$，则氢原子数为 $2n+2$。其分子式可以用通式 $\ce\{C\_\{n}H\_\{2n+2}}$ 表示。
...
|                      石油炼制方法                     | 目的                  | 原理                                              | 原料        |
| :---------------------------------------------: | ------------------- | ----------------------------------------------- | --------- |
| `<no-wrap>`\{=html}常压分馏（物理）`</no-wrap>`\{=html} | 获得以燃料油为常压主的不同石油分馏产品 | 用蒸发和冷凝的方法将原油分成不同沸点范围的馏分                         | 原油        |
...
杂原子指除去 $\ce\{C}$ 和 $\ce\{H}$ 的任何原子。某碳原子与杂原子成几条键，就处于几级氧化程度。 单键算 $1$ 根，双键算 $2$ 根，三键算 $3$ 根，下文中我们简记用 Z0，Z1，Z2，Z3 表示。
...
|        | 描述             |                              通式 |
| :----: | -------------- | ------------------------------: |
| **烷烃** | 碳原子之间全部以单键结合   | $\ce\{C\_nH\_\{2n+2}}$（$n\ge1$） |
| **烯烃** | 分子中含有一个或多个碳碳双键 |   $\ce\{C\_nH\_\{2n}}$（$n\ge2$） |
| **炔烃** | 分子中含有一个或多个碳碳三键 | $\ce\{C\_nH\_\{2n-2}}$（$n\ge2$） |
...
由于密度小于空气，乙烯在实验室中可通过向下排空气法收集。烯烃的化学性质：化学性质主要由碳碳双键（$\ce\{C = C}$）决定。
...
- **氧化反应**：几乎所有的烃都可以燃烧。
  ...
  $$
  \begin\{align} \ce\{C2H4 + 3 O2 &->\[点燃] 2 CO2 + 2 H2O}\ \ce\{C\_xH\_y + $\left(x+\dfrac\{y}\{4}\right)$ O2 &->\[点燃] x CO2 + \dfrac\{y}\{2} H2O} \end\{align}
  $$
...
![alt text|w70%](/static/image/image-2.908b85a55b.avif)
...
苯的同系物：分子里含有一个苯环的烃，通式为 $\ce\{C\_nH\_\{2n-6}}$（$n\ge6$）。
...
除苯环之外的芳香烃：广义来说，有芳香性的烃就是芳香烃，而芳香性指的是分子具有特殊的稳定性，这种稳定性来源于完全离域的 $\pi$ 电子体系，一般来说，符合休克尔规则（Hückel 规则）的平面环状共轭体系具有芳香性，即 $\pi$ 电子数为 $4n+2$（$n=0,1,2,\dots$）。但是不是所有符合这个简单规律的都是，例如如果环的结构无法形成平面型，所有 $\pi$ 电子云难以肩并肩形成大 $\pi$ 键，或者说，某些物质强行压到平面型，需要的能量远远高于形成离域的稳定结构而减少的，得不偿失，就没有芳香性（例如环癸五烯/\[10]-轮烯）。高中阶段不需要知道严谨的理论，只需要知道，如果说某种有芳香性的物质，在推断题中，要看一下有没有提示某种基团（例如吡咯、呋喃等芳香杂环，是芳香族化合物的一种），不要把芳香局限为苯环即可。
...
其中，$X\_2$ 通常为 $Cl\_2$ 或 $Br\_2$。$F\_2$ 反应太剧烈难以控制，$I\_2$ 反应吸热且可逆，通常不发生。如果在反应中生成了手性中心，产物的立体化学特征是什么？
...
1. \*\*氟 ($\ce\{F\_2}$)：\*_反应极度放热，几乎是爆炸性的。$\ce\{F_}$ 极其活泼，选择性极差，通常会打断 $\ce\{C-C}$ 键，实验室很少直接进行光氟代。

2. \*\*氯 ($\ce\{Cl\_2}$)：\*_反应放热，速度快。$\ce\{Cl_}$ 活性较高，但选择性中等（既能取代一级氢，也能取代二级、三级氢，得到混合物）。

3. **溴 ($\ce\{Br\_2}$)：\*_反应放热较少（夺氢步骤甚至是吸热的），速度较慢。$\ce\{Br_}$ 活性较低，但**选择性极高\*\*（倾向于取代最稳定的碳自由基对应的氢）。

4. \*\*碘 ($\ce\{I\_2}$)：\*\*夺氢步骤是强吸热的，反应难以正向进行，且生成的 $\ce\{HI}$ 还原性强，会将 $\ce\{R-I}$ 还原回 $\ce\{R-H}$。
...
- \*\*甲酸钠和裴琳试剂到底反应吗？\*\*答案是肯定的，经过验证，这个反应是可以发生的，只不过反应要求比乙醛要严格一些，强碱性环境、加热，差一点现象就会减弱。而甲酸的反应就更严格了，需要更强的碱性环境，避免甲酸与其中和。很多人说[甲酸钠对裴琳试剂没有现象](http://www.hxjxzzbjb.cn/index.php?m=content\&c=index\&a=show\&catid=76\&id=1551)，这是不严谨的。

- **甲酸和新制 $\ce\{Cu(OH)2}$ 的反应**

  甲酸中既有 $\ce\{-COOH}$ 又有 $\ce\{-CHO}$，因此既能和氢氧化铜发生酸碱中和，又能

- **强氧化反应**：酸性高锰酸钾，以及重铬酸钾等强氧化剂可以将醛，甚至是醇直接氧化到酸。
...
**溴与乙醛的反应**：溴（$\ce\{Br2}$）与乙醛（$\ce\{CH3CHO}$）之间的反应是一个非常经典的有机化学反应。由于乙醛分子中同时含有**醛基（$\ce\{-CHO}$）**和**$\alpha$-甲基（与羰基相连的
$\ce\{-CH3}$）**，溴又具有氧化性和发生亲电反应的能力，因此两者的反应途径**高度依赖于反应条件（如溶剂、酸碱性、温度等）**。

总体来说，溴与乙醛的反应主要分为两类：**氧化反应**和\*\*$\alpha$-氢的取代反应（卤代反应）\*\*。

1. **氧化反应（溴水条件）**

   当乙醛与**溴水**（溴的水溶液）反应时，主要发生的是氧化反应。醛基具有较强的还原性，而溴水是一个温和的氧化剂。

   - **反应条件**：水溶液，通常在室温下进行。

   - **反应现象**：溴水的红棕色逐渐褪去，溶液变为无色。

   - **反应机理**：在水溶液中，溴会发生微弱的歧化反应生成次溴酸（$\ce\{HOBr}$）：

     $$
     \ce\{Br2 + H2O \<=> HBr + HOBr}
     $$

     次溴酸是实际发挥作用的强氧化剂，它将醛基（水合后形成的偕二醇）氧化为羧基，生成乙酸。

   - **应用**：该反应常用于检验醛基的存在（虽然通常更爱用银镜反应或斐林试剂，但溴水褪色也是醛类区别于酮类的一个特征，因为酮一般不被溴水氧化）。

2. **$\alpha$-氢的取代反应**

   乙醛的
   $\alpha$-碳原子上连接有三个氢原子，受羰基强烈的吸电子诱导效应影响，这些
   $\alpha$-氢具有一定的酸性（活泼氢），可以被溴发生取代。取代反应的产物取决于体系的酸碱性。

   **酸性或中性条件**（通常在有机溶剂如 $\ce\{CCl4}$
   或乙酸中）：在酸催化下，乙醛会发生一溴代反应。

   - **为什么停留在一溴代？**
     引入一个吸电子的溴原子后，碳正离子中间体的稳定性降低，使得进一步的"烯醇化"变得更加困难。因此，在酸性条件下，反应很容易控制在**一溴代阶段**。

   - 反应过程：

     1. **烯醇化**：在酸催化下，乙醛发生酮-烯醇互变异构，生成少量的烯醇（$\ce\{CH2=CH-OH}$）。这是决速步。

     2. **亲电加成-消除**：烯醇的富电子双键进攻溴分子（$\ce\{Br2}$），失去一个质子后生成一溴乙醛。

   **碱性条件**：如果在碱性溶液中加入溴（本质上是生成了次溴酸盐
   $\ce\{NaOBr}$），反应将迅速进行到底，发生碳碳键断裂。

   1. **快速多卤代**：碱夺取乙醛的
      $\alpha$-氢生成烯醇负离子。烯醇负离子进攻 $\ce\{Br2}$
      生成一溴乙醛。由于溴的强吸电子性，使得剩下的
      $\alpha$-氢**酸性更强**，更容易被碱拔去。因此，反应不会停留在一步，而是迅速生成**三溴乙醛**（$\ce\{CBr3CHO}$）。

   2. **碳碳键断裂（亲核酰基取代）**：氢氧根离子（$\ce\{OH-}$）作为亲核试剂进攻三溴乙醛的羰基碳。随后，含有三个吸电子溴原子的碳负离子（$\ce\{-CBr3}$）作为一个良好的离去基团脱落。

   3. **质子转移**：离去的 $\ce\{^-CBr3}$
      夺取酸上的质子，生成**溴仿**（$\ce\{CHBr3}$，一种重的液体，有特殊甜味）。

   **特殊意义**：**乙醛是唯一能够发生卤仿反应的醛**（因为只有它含有
   $\ce\{CH3-C(=O)-}$ 结构）。

| 反应条件        | 主要试剂                                | 反应类型                   | 主要产物                               | 现象/备注                    |
| :---------- | :---------------------------------- | :--------------------- | :--------------------------------- | :----------------------- |
| **水溶液**     | 溴水（$\ce\{Br2/H2O}$）                 | 氧化反应                   | 乙酸（$\ce\{CH3COOH}$）$+$ $\ce\{HBr}$ | 溴水红棕色褪去。                 |
| **有机溶剂/酸性** | 纯溴或 $\ce\{Br2/CCl4}$                | $\alpha$-取代（亲电）        | 溴乙醛（$\ce\{BrCH2CHO}$）              | 限于一卤代，释放 $\ce\{HBr}$ 气体。 |
| **碱性水溶液**   | $\ce\{Br2 + NaOH}$（即 $\ce\{NaOBr}$） | 溴仿反应（$\alpha$-取代$+$裂解） | 溴仿（$\ce\{CHBr3}$）$+$ 甲酸钠           | 生成比水重的微黄色/无色液体，有特殊气味。    |

综上所述，探讨溴和乙醛的反应，必须首先明确其物理化学环境。\*\*氧化（溴水褪色）**和**溴仿反应（碱性降解）\*\*是化学教学和实际应用中最核心的两个途径。

**甲醛与溴的反应**：与乙醛不同，甲醛（$\ce\{HCHO}$）的分子结构中**没有
$\alpha$-碳原子**，因此也就**没有任何
$\alpha$-氢**。这个结构上的根本差异决定了溴与甲醛的反应途径相对单一：**它不能发生像乙醛那样的
$\alpha$-取代反应或卤仿反应，主要且几乎唯一的反应类型就是氧化反应。**
...
特殊的醛：甲醛。最直观的是，甲醛分子中相当于有两个
$\ce\{-CHO}$，因此反应时，消耗的氧化剂数量翻倍。

|  氧化过程  | $\ce\{R-CH2OH}$ | $\ce\{\<=>\[氧化]\[还原]}$ | $\ce\{R-CHO}$ | $\ce\{->\[氧化]}$ | $\ce\{R-COOH}$ |
| :----: | :-------------: | :--------------------: | :-----------: | :-------------: | :------------: |
| 相对分子质量 |      $M\_r$     |                        |    $M\_r-2$   |                 |    $M\_r+14$   |

甲醛（$\ce\{HCHO}$）和乙醛（$\ce\{CH3CHO}$）虽然同属醛类，但甲醛由于其**特殊的对称结构（没有烷基，碳直接连着两个氢）**，在整个醛族中是一个绝对的"异类"。除了反应系数不同之外，它们在物理性质和化学性质上有着非常显著的区别。以下为你系统总结（包含你刚才探究的内容），这些也是高考和化学竞赛中极爱考的"易错点"和"区分点"：

核心逻辑：结构决定性质

- **乙醛（$\ce\{CH3CHO}$）**：羰基（$\ce\{C=O}$）连着一个甲基（$\ce\{-CH3}$）和一个氢（$\ce\{-H}$）。甲基是推电子基团。

- **甲醛（$\ce\{HCHO}$）**：羰基连着**两个氢**。它既没有推电子的烷基，也没有空间位阻，这使得它的**还原性极强**，且**没有
  $\alpha$-碳原子（自然也没有 $\alpha$-氢原子）**。

化学性质的显著差异：

1. 氧化的最终产物不同（极其重要）

   - **乙醛**：被氧化剂（如酸性 $\ce\{KMnO4}$、银氨溶液、新制
     $\ce\{Cu(OH)2}$）氧化后，最终产物是**乙酸（$\ce\{CH3COOH}$）**。乙酸极其稳定，无法被继续氧化。

   - **甲醛**：甲醛被氧化后，首先生成**甲酸（$\ce\{HCOOH}$）**。甲酸虽然在弱氧化剂下较稳定，但如果遇到强氧化剂（如酸性高锰酸钾、重铬酸钾），甲酸会被继续氧化成碳酸，最终分解为
     **$\ce\{CO2}$ 和 $\ce\{H2O}$**。

   - _应用考点_：如果把甲醛气体通入酸性 $\ce\{KMnO4}$
     溶液，溶液褪色且**有气体（$\ce\{CO2}$）生成**；而乙醛通入则只褪色，无气体。

2. 与新制 $\ce\{Cu(OH)2}$ 反应的极致程度（结合上题）

   - **乙醛**：在加热条件下，只能将 $\ce\{Cu^2+}$ 还原到 $+1$
     价，生成砖红色的 **$\ce\{Cu2O}$** 沉淀。

   - **甲醛**：因为还原性极强，在碱性条件下不仅能生成
     $\ce\{Cu2O}$，还能进一步将铜离子彻底还原为**单质铜（$\ce\{Cu}$，红色沉淀，不溶于浓盐酸）**，并且伴随着甲醛的催化脱氢，放出**大量氢气（$\ce\{H2}$）**。

   - _与银氨溶液的区别_：银氨溶液中氨的最主要作用是提供强碱性环境和减缓反应速度，这也导致银氨溶液的氧化性并没有裴琳试剂强，甲醛与银氨溶液的反应与乙醛基本上相似。

3. 碘仿反应（鉴别两者的终极武器）这是一个非常经典的有机鉴别反应。

   - **乙醛**：含有
     **$\ce\{CH3-C(=O)-}$**（甲基酮/乙醛）结构，能与碘（$\ce\{I2}$）和氢氧化钠（$NaOH$）溶液发生**碘仿反应**，生成**黄色的具有特殊气味的碘仿（$\ce\{CHI3}$）沉淀**。

   - **甲醛**：没有 $\ce\{-CH3}$
     结构，**绝对不能**发生碘仿反应。用碘和氢氧化钠可以瞬间鉴别甲醛和乙醛。

4. 在浓碱中的反应（歧化 vs 缩合）这是由于\*\*有没有 $\alpha$-H
   （与羰基相邻的碳上的氢）\*\*决定的。

   - **乙醛**：有
     $\alpha$-H。在碱性条件下，极易发生羟醛缩合反应，生成高分子聚合物（变成黄褐色的**醛树脂**）。

   - **甲醛**：**没有 $\alpha$-H**。在浓碱（如浓
     $\ce\{NaOH}$）中不能缩合，而是发生**康尼查罗反应（Cannizzaro
     反应，即歧化反应）**。两分子甲醛互相氧化还原，一分子变成甲醇，一分子变成甲酸钠。

5. 聚合反应的产物不同

   - **乙醛**：容易发生三聚或四聚，生成环状化合物。

   - **甲醛**：容易发生缩聚反应，生成直链的高分子------**聚甲醛**（POM，一种高强度工程塑料）；能与苯酚发生经典的缩聚反应，生成**酚醛树脂（电木）**。这是高中有机必考的高分子反应。

6. 与氨气（$\ce\{NH3}$）的反应

   - **甲醛**：与氨水反应会生成一种结构极其特殊的笼状大分子------**六亚甲基四胺（俗称乌洛托品）**，广泛用于医药和炸药工业。

   - **乙醛**：与氨气反应只是简单的加成缩合，生成乙醛氨。

7. 常温常压下的状态不同

   - **甲醛**：常温常压下是**气体**（标况下也是气体）。它是高中阶段唯一需要记住的**气态含氧衍生物**。我们常说的"福尔马林"是它的
     $35%\sim 40%$ 的水溶液。

   - **乙醛**：常温下是**极易挥发的液体**（沸点
     $\pu\{20.8^oC}$）。所以在标况下（$\pu\{0^oC}$），乙醛是液体。

   - _应用考点_：在计算阿伏伽德罗常数（$N\_A$）的题目中，如果说"标况下
     $\pu\{22.4 L}$
     的乙醛"，这是**错误**的陷阱（因为标况下乙醛不是气体）；但说"标况下
     $\pu\{22.4 L}$ 的甲醛"则是完全成立的（含 $\pu\{1 mol}$ 的
     $\ce\{HCHO}$）。

甲醛由于其特殊的"两头都是氢"的无烷基结构，使得它：

1. **还原性爆表**（出 $\ce\{Cu}$、出 $\ce\{H2}$、能被氧化成
   $\ce\{CO2}$）；
2. **缺乏 $\alpha$-H**（不能碘仿、碱中歧化）；
3. **物理性质独特**（常温是气体）。

掌握了这三条主线，关于甲醛的所有探究题和易错题你都能游刃有余地破解！
...
广义的取代反应（Substitution
reaction）定义：分子中的某个原子或基团（$\ce\{-A}$）被另一个原子或基团（$\ce\{-B}$）所替换的过程。只看**起点和终点，不看过程**。如果你要求有严格的有机机理（狭义的取代），事情就变得非常有趣了。

在脂肪族化合物中，经典的狭义取代反应是 **$S\_N1$**（单分子亲核取代）和
**$S\_N2$**（双分子亲核取代）。比如氢氧根取代溴乙烷中的溴，是 $S\_N2$
机理：亲核试剂从背面进攻，旧键断裂和新键形成**同时发生**（一步完成）。

**但是，酯化和酯的水解，并不走这种"一步到位"或"碳正离子"的经典路径！**

因为羧基和酯基中的碳是 **$sp^2$ 杂化**的（含有一个 $\ce\{C=O}$
双键）。有机化学中有一个规律：**亲核试剂极难对 $sp^2$ 杂化碳进行直接的
$S\_N2$ 背后进攻**（因为有极大的空间位阻，且 $\pi$
键电子云会排斥亲核试剂）。

那么它是怎么发生取代的呢？它采用了一种\*\*"曲线救国"\*\*的机理，分为两步：

1. \*\*第一步：加成（Addition）\*\*亲核试剂（比如醇里的氧，或者水里的氧）去进攻带有部分正电荷的羰基碳。此时，$\ce\{C=O}$
   双键断裂，变成单键，原本的 $sp^2$ 平面碳变成了 **$sp^3$
   杂化的四面体中间体**。

2. \*\*第二步：消除（Elimination）\*\*这个四面体中间体很不稳定。氧原子上的负电荷（或孤对电子）会重新推回来，**重新形成
   $\ce\{C=O}$
   双键**。为了维持碳的四价，必须"挤走"一个离去基团（在酯化中挤走水，在水解中挤走醇）。

既然微观过程是"先加成，后消除"，那它到底算不算取代反应？

在高等有机化学中，为了统合"宏观表象"和"微观机理"，给这类反应起了一个专属的名字，叫作：
**亲核酰基取代反应（Nucleophilic Acyl Substitution，简称 $S\_N Ac$）**。

也就是说：

1. **从净结果（Net Result）来看**，它就是一个取代反应。
2. **从微观机理（Mechanism）来看**，它是一个\*\*加成-消除（Addition-Elimination）\*\*的过程。
...
因为高中化学经常问一个物质能否与 $\ce\{NaHCO3}$ 反应产生
$\ce\{CO2}$，因此我们这里将其作为"酸性强弱"的标准，能通过复分解反应与碳酸氢钠溶液反应产生二氧化碳气体的有机物，**其根本特征只有一个：该有机物在水溶液中的酸性必须强于碳酸的酸性**（强酸制弱酸原理）。具体来说，碳酸的表观酸度常数
$pK\_a \approx 6.38$。因此，**任何 $pK\_a \< 6.38$
的有机物**，在热力学上都能与碳酸氢钠反应生成二氧化碳。尽管从实验上来讲，还需要考虑动力学因素，如果反应过慢看不到气泡，一般也不视为能反应。

1. 除了含有羧基（$\ce\{-COOH}$），还有哪些有机物能产生 $\ce\{CO2}$？

   原理：只要有机分子中含有其他能释放质子（$\ce\{H+}$）的官能团，且解离后的阴离子非常稳定（$pK\_a \< 6.38$），就能反应。常见的有以下几类（从常见到极端）：

   1. 磺酸类（\ce\{-SO3H}）------最常见的强有机酸。

      **特征**：含有磺酸基，酸性极强，通常
      $pK\_a \< 0$（比很多无机酸都强）。

      **代表物**：对甲苯磺酸、苯磺酸、甲磺酸。它们与 $\ce\{NaHCO3}$
      反应非常剧烈。

   2. 含有强吸电子基团的酚类（$\ce\{-OH}$）------被"逼成"强酸的弱酸。

      **特征**：普通的苯酚（石炭酸）$pK\_a \approx 10$，酸性弱于碳酸，不能与
      $\ce\{NaHCO3}$ 反应。但在苯环上引入多个强吸电子基团（如硝基
      $\ce\{-NO2}$），会极大地分散阴离子的电荷，使其酸性剧增。

      **代表物**：**2,4-二硝基苯酚**：$pK\_a \approx 4.1$，可以与
      $\ce\{NaHCO3}$ 反应释放 $\ce\{CO2}$。

      **代表物**：**苦味酸（2,4,6-三硝基苯酚）**：$pK\_a \approx 0.38$，酸性已经接近无机强酸，剧烈反应。

   3. 特殊的烯醇类（$\ce\{C=C-OH}$）。

      **特征**：本身没有羧基，但羟基连在双键碳上，且解离后的阴离子能通过共轭体系高度稳定化。

      **代表物**：**抗坏血酸（维生素 C）**。它的
      $pK\_\{a1} \approx 4.17$，酸性强于醋酸，泡腾片里经常用它（或者柠檬酸）配小苏打来产生气泡。

   4. 氧碳酸类（Oxocarbons）------高度对称的怪胎。

      **特征**：完全由碳和氧构成的环状化合物带有羟基，解离后电荷在高度对称的环上完美平均分配。

      **代表物**：**方酸（Squaric
      acid，3,4-二羟基-3-环丁烯-1,2-二酮）**。虽然只有羟基，但其
      $pK\_\{a1} \approx 1.5$，是一种非常强的有机酸，剧烈释放
      $\ce\{CO2}$。

   5. 极端的碳负离子酸（$\ce\{C-H} 酸）------氢连在碳上也能变强酸。

      **特征**：通常连在碳上的氢根本没有酸性（例如甲烷
      $pK\_a \approx 48$）。但如果这个碳连了极强的吸电子基团，C-H
      键就会断裂释放出 $H^+$。

      **代表物**：**三硝基甲烷（$\ce\{HC(NO2)3}$）**。由于三个硝基的强大吸电子效应，它的
      $pK\_a \approx 0.17$，不仅能与 $\ce\{NaHCO3}$
      反应冒泡，甚至能腐蚀金属。更极端的还有**五氰基环戊二烯**，它是已知最强的有机酸之一（超酸级别）。

2. 含有羧基（$\ce\{-COOH}$），但却不能与 $\ce\{NaHCO3}$ 产生 $\ce\{CO2}$
   的有机物有没有？

   有。尽管绝大多数羧酸的 $pK\_a$ 在 $3\sim5$
   之间（强于碳酸），但在一些特殊或极端的情况下，含有羧基的有机物是**不能**让小苏打冒泡的。原因可以分为"热力学限制"和"动力学/物理限制"：

   1. 氨基酸（内盐效应 / 等电点）------最普遍的例外。

      **特征**：分子中同时含有氨基（碱性）和羧基（酸性）。

      **代表物**：\*\*甘氨酸（Glycine）**的晶体或水溶液。在固态和中性水溶液中，甘氨酸实际上是以**两性离子（$\ce\{^+H3N-CH2-COO-}$）\*\*的形式存在的。

      **原因**：羧基上的 $\ce\{H+}$
      已经"内部转移"给了氨基。此时如果你加小苏打，能够被碱中和的质子实际上在
      $\ce\{-NH3^+}$ 上。而 $\ce\{-NH3^+}$ 的
      $pK\_a \approx 9.6$，远弱于碳酸。因此，甘氨酸（以及大多数等电点附近的中性氨基酸）不会与
      $\ce\{NaHCO3}$ 反应冒泡。

   2. 多元酸的酸式盐------最后一颗子弹太弱了。

      **特征**：多元弱酸的前几个氢已经解离（变成盐），分子中依然保留着羧基（$\ce\{-COOH}$），但此时解离难度极大。

      **代表物**：**枸橼酸氢二钠（二碱式柠檬酸钠）**。柠檬酸有三个羧基，第三个羧基解离的
      $pK\_\{a3} = 6.40$。

      **原因**：带有两个负电荷的柠檬酸酸式根，即使它身上还带着一个真实的
      $\ce\{-COOH}$ 官能团，它在热力学上也无法再把质子强行塞给
      $\ce\{HCO3^-}$ 了，不产生 $\ce\{CO2}$。

   3. 极长链高级脂肪酸（物理溶解度限制）。

      **特征**：含有羧基，酸性在理论上足够（$pK\_a \approx 5$），但在水里完全不溶。

      **代表物**：\*\*硬脂酸（十八烷酸）\*\*固体。

      **原因**：如果你把硬脂酸粉末和 $\ce\{NaHCO3}$
      溶液混合在常温下，**肉眼几乎看不到任何气泡**。因为硬脂酸极度疏水，反应只能在固体表面极小范围内发生，且生成的硬脂酸钠（肥皂）会包裹在表面阻碍反应。这属于动力学和相态造成的"不能"。（注：如果加热到融化并剧烈搅拌，依然是可以反应的）。

   4. 极端空间位阻的羧酸（空间屏蔽效应）。

      **代表物**：**2,4,6-三叔丁基苯甲酸**。

      **原因**：苯环上羧基的两侧被庞大的"叔丁基"完全包裹，像一堵墙一样把
      $\ce\{-COOH}$
      保护了起来。碳酸氢根离子（$\ce\{HCO3^-}$）很难靠近羧基去夺取质子，导致反应速率极度缓慢，宏观上观察不到明显的
      $\ce\{CO2}$ 气泡产生。

**能否产生 $CO\_2$ 的唯一标尺是 $pK\_a \< 6.38$**。

- 只要阴离子足够稳定（如磺酸、苦味酸、维生素C、方酸），没有羧基也能冒泡。
- 即使有羧基，如果质子被内部劫持（氨基酸）、被静电荷死死拉住（多元酸酸式盐）、或者被疏水结构/空间位阻严密包裹（硬脂酸、多取代苯甲酸），它也无法与
  $NaHCO\_3$ 产生 $CO\_2$。
...
- 也称磷脂质，是含有磷酸的脂类，属于复合脂。磷脂为两性分子，一端为亲水的含氮或磷的头，另一端为疏水（亲油）的长烃基链。
  ...
  <img alt="alt text|w50%" src={image9} />
...
- 糖脂是通过糖苷键连接的碳水化合物的脂质，它们的作用是保持膜的稳定性并促进细胞识别。在所有真核细胞膜的表面上发现这些碳水化合物。
  ...
  <img alt="alt text|w50%" src={image10} />
...
![alt text|w70%](/static/svg/油脂.1cebd3c3e6.svg)
...
![alt text|w40%](/static/svg/Steran_num_ABCD.1861d55ccc.svg)
腺甾烷的衍生物：
...
以前所有分子式可写成 $\ce\{C\_m(H2O)\_n}$ 的化学物质皆被称为**碳水化合物**，但是现在生物化学理解上的糖类是指除了碳数不为一和二的「碳水化合物」。
...
![alt text|w100%](/static/image/Amino_Acids.e03db80b30.avif)
...
![alt text|w80%](/static/image/Peptidformationball-8dec250.ce349641b7.avif)
...
![alt text|w100%](/static/image/225_Peptide_Bond-01.8c50245e8a.avif)
...
![alt text|w90%](/static/image/Nucleotides_zh.75acf58579.avif)
- 如果其五碳糖是脱氧核糖，则此单体的聚合物是脱氧核糖核酸 DNA。

- 如果其五碳糖是核糖，则此单体的聚合物是核糖核酸 RNA。

![alt text|w90%](/static/svg/Difference_DNA_RNA-ZH.0729bcb645.svg)
...
<thead>
<tr>
<th style="text-align: center;">胞嘧啶、鸟嘌呤<br />
之间生成三条氢键</th>
<th style="text-align: center;">腺嘌呤、胸腺嘧啶<br />
之间生成两条氢键</th>
</tr>
</thead>
<tbody>
...
![alt text|w90%](/static/image/DNA_Structure.5262f907c1.avif)