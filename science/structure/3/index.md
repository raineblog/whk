...
- 通常来说，只有第二周期元素之间才能生成经典的 \\pi 键，例如硫酸中氧硫键为反馈 \\pi 键。
  ...
  - **\\delta 键**：过渡金属之间（如 \\ce{Re-Re}、\\ce{Cr-Cr}）可以用四瓣形的 d 轨道（如 d\_{xy}）进行「面对面」重叠，形成电子云分布在四个区域的 \\delta 键。\\delta 键是比 \\pi 键更高一阶的键型，几乎只存在于过渡金属—金属键中。两个过渡金属原子之间可同时形成 1 个 \\sigma 键、2 个 d\\pi-d\\pi 键和 1 个 \\delta 键，构成罕见的**四重键**，例如 \\ce{[Re2Cl8]^2-} 中的 \\ce{Re}{}^{\\large =}\_{\\large =}\\ce{Re}。
...
\\ce{AgCl} 溶于氨水，但 \\ce{AgBr} 微溶，\\ce{AgI} 完全不溶于氨水。原因在于配合物的稳定性不足以打破 \\ce{AgI} 极小的 K\_{sp}。
...
- 题目给出了未知的 K\_{\\text{稳}}（稳定常数）数据——必定考查配位平衡，处理方法与 K\_{sp} 或酸的 K_a 类似。
