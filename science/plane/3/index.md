...
公式特征注意：用 [\$\cos2\alpha\$]{.arithmatex} 联系 [\$\sin\alpha,\cos\alpha\$]{.arithmatex}。
...
根据上面的可以推出 [\$\alpha,\beta\$]{.arithmatex} 和差角的正余弦。

- 两式平方后作和、平方后作差。

- 两式相乘。

也可以换元，设 [\$t\$]{.arithmatex} 为 [\$\sin,\cos\$]{.arithmatex} 等，将原式化为关于 [\$t\$]{.arithmatex} 的二次函数或分式。

注意求区间上的值域，也可以结合不等式相关内容。

### 一角一函数 {#_4}

一角一函数，可以简单的理解为 [\$y=A\sin(\omega x+\varphi)\$]{.arithmatex} 的形式。
...
当一个式子中的函数次数仅为奇数后者偶数的时候，可以补充 [\$\sin\^2+\cos\^2=1\$]{.arithmatex} 齐次。

也可以结合弦化切，除以一个 [\$\sin\^2+cos\^2\$]{.arithmatex} 的若干次方。
...
整体角，即将 [\$y=A\sin(\omega x+\varphi)\$]{.arithmatex} 中的 [\$\omega x+\varphi\$]{.arithmatex} 设为单独的变量如 [\$t\$]{.arithmatex} 后进行解决的思想。

化为一角一函数后，用整体角结合三角函数性质进行快速解决。

根据整体角的范围，画出函数图像或者列出关于 [\$k,k\in\mathbb Z\$]{.arithmatex} 的方程。
...
然后如果可以求出 [\$\omega\$]{.arithmatex} 的一些其他条件（比如奇偶性），直接求出来。

### 已知起点终点 {#_12}

思想：复合函数、还原。

对于 [\$f(x)=A\sin(\omega x+\varphi)\$]{.arithmatex}，我们令 [\$t=\omega x+\varphi\$]{.arithmatex}。

注意 [\$\omega\$]{.arithmatex} 的正负性，得出 [\$t\$]{.arithmatex} 的取值范围，进行进一步求解。
...
未知 [\$\varphi\$]{.arithmatex}：根据已知点或特殊信息（对称轴、对称中心）带入，列出方程组求出 [\$\varphi\$]{.arithmatex}。

### 未知起点终点 {#_13}

可以将问题再分为：正正、正负。

- 对于正负的，通常区间内存在一个已知点（[\$\omega=0\$]{.arithmatex}），画图解题。

- 对于正正的，通常先用周期长度限制，然后列关于 [\$k\$]{.arithmatex} 的式子。

特殊的，如果 [\$\omega\$]{.arithmatex} 的正负不确定，应当讨论 [\$\operatorname{sgn}(\omega)\$]{.arithmatex}。

### [\$\varphi\$]{.arithmatex} 未知求 [\$\omega\$]{.arithmatex} {#varphi-omega}

这一类问题通常比较难：

- 各种条件，先转化为区间长度，初步限制 [\$\omega\$]{.arithmatex} 的范围。

- 根据特殊信息，限制 [\$\omega\$]{.arithmatex} 的奇偶性等性质。

- 在独立的 [\$\omega\$]{.arithmatex} 取值中，一次判断是否满足条件。

比较难算。

## 三角形中的三角函数 {#_14}

条件，在三角形中，有 [\$x+y+z=\pi\$]{.arithmatex}（三角形内角和），那么：
...
根据 [\$\displaystyle\tan\alpha={1\over\cot\alpha}\$]{.arithmatex} 展开正切的形式一即可。
...
根据 [\$\displaystyle\tan\alpha={1\over\cot\alpha}\$]{.arithmatex} 展开正切的形式二即可。
...
已知 [\$\tan\beta\$]{.arithmatex} 有意义，且 [\$\sin(\alpha+\beta)={1\over2}\$]{.arithmatex}，[\$\sin(\alpha-\beta)={1\over3}\$]{.arithmatex}，求 [\$\dfrac{\tan\alpha}{\tan\beta}\$]{.arithmatex}。
...
已知 [\$\theta\in\[0,2\pi)\$]{.arithmatex} 且 [\$\sin(x+\theta)\$]{.arithmatex} 是偶函数，求 [\$\theta\$]{.arithmatex}。
...
因为 [\$x\in\mathbb R\$]{.arithmatex}，所以 [\$\cos\theta=0\$]{.arithmatex}，即： [\$\displaystyle\theta={\pi\over2}/{3\pi\over2}\$]{.arithmatex}。

知识点：偶函数、和差恒等式。

### 例题三 {#_30}

求函数 [\$\displaystyle g(x)=\sin\^2\left(x+{\pi\over12}\right)+\sin\^2\left(x+{\pi\over4}\right)\$]{.arithmatex} 的值域。
...
然后就忒简单了，答案是，函数 [\$g\$]{.arithmatex} 的值域为 [\$\displaystyle\left\[1-{\sqrt3\over2},1+{\sqrt3\over2}\right\]\$]{.arithmatex}。

知识点：函数、和差恒等式、降次公式、辅助角公式。

另外：最后辅助角公式的应用中，[\$\arctan b/a\$]{.arithmatex} 可以不用算出来，因为 [\$x\$]{.arithmatex} 属于实数域，[\$\sin\$]{.arithmatex} 函数里面一定是任何一个实数都取得到，直接取 [\$r=\sqrt{a\^2+b\^2}=2\sqrt3\$]{.arithmatex} 即可得出答案。
