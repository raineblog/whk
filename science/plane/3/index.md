...
公式特征注意：用 cos 2*α* 联系 sin *α*, cos *α*。

4sin *α*cos *α* = (sin *α* + cos *α*)2 − (sin *α* − cos *α*)2
...
一角一函数，可以简单的理解为 *y* = *A*sin (*ω\*\*x* + *φ*) 的形式。
...
也可以结合弦化切，除以一个 sin2 + *c**o**s*2 的若干次方。
...
整体角，即将 *y* = *A*sin (*ω\*\*x* + *φ*) 中的 *ω\*\*x* + *φ* 设为单独的变量如 *t* 后进行解决的思想。

化为一角一函数后，用整体角结合三角函数性质进行快速解决。

根据整体角的范围，画出函数图像或者列出关于 *k*, *k* ∈ ℤ 的方程。
...
对于 *f*(*x*) = *A*sin (*ω\*\*x* + *φ*)，我们令 *t* = *ω\*\*x* + *φ*。
...
sin 2*x* + sin 2*y* + sin 2*z* = 4sin *x*sin *y*sin *z*
...
已知 tan *β* 有意义，且 $\\sin(\\alpha+\\beta)={1\\over2}$，$\\sin(\\alpha-\\beta)={1\\over3}$，求 $\\dfrac{\\tan\\alpha}{\\tan\\beta}$。
...
因为 *x* ∈ ℝ，所以 cos *θ* = 0，即： $\\displaystyle\\theta={\\pi\\over2}/{3\\pi\\over2}$。
