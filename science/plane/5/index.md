...
即知道 []{.arithmatex}$aCb$、[]{.arithmatex}$cBa$、[]{.arithmatex}$bAc$，才可以唯一确定一个两边一角的三角形。
...
在三角形中，[]{.arithmatex}$a=b\cos C+c\cos B$。

若 []{.arithmatex}$\sin^2A+\sin^2B=\sin^2C$，则该三角形是以 []{.arithmatex}$\angle C$ 为直角的 []{.arithmatex}$\mathrm{Rt\triangle}$。
...
特殊的，如果 []{.arithmatex}$\sin 2A=\sin 2B$，则有 []{.arithmatex}$2A=2B$ 或 []{.arithmatex}$2A+2B=\pi$，即 []{.arithmatex}$A=B$ 或 []{.arithmatex}$A+B=90^\circ$，即等腰或直角三角形。
...
如果直角三角形三边成等差数列，则变长一定为 []{.arithmatex}$3,4,5$。
...
做过 []{.arithmatex}$\angle A$ 的垂线，则该垂线长度 []{.arithmatex}$h$：
...
做出该三角形的外接圆，过圆心做 []{.arithmatex}$BC$ 的高，则，
...
即值 []{.arithmatex}$k$ 为三角形外接圆直径 []{.arithmatex}$2R$。
...
其中 []{.arithmatex}$\theta$ 为 []{.arithmatex}$c$ 的对角，即 []{.arithmatex}$\angle C$；第二步就是两边平方。
...
其中 []{.arithmatex}$\zeta$ 为 []{.arithmatex}$\triangle ABC$ 内切圆半径，[]{.arithmatex}$p$ 为三角形的半周长。
...
其中 []{.arithmatex}$S$ 表示三角形面积，[]{.arithmatex}$a,b,c$ 分别表示三边长。
...
|          编号           | 心的名称 |          定义          |
|:-----------------------:|:--------:|:----------------------:|
| []{.arithmatex}$X_1(I)$ |   内心   | 三条**角平分线**的交点 |
| []{.arithmatex}$X_2(G)$ |   重心   |   三条**中线**的交点   |
| []{.arithmatex}$X_3(O)$ |   外心   |  三条**中垂线**的交点  |
| []{.arithmatex}$X_4(H)$ |   垂心   |   三条**高线**的交点   |
...
- 设 []{.arithmatex}$I$ 为内心，则 []{.arithmatex}$a\cdot\overrightarrow{IA}+b\cdot\overrightarrow{IB}+c\cdot\overrightarrow{IC}=\vec0$.

- 设 []{.arithmatex}$H$ 为垂心，则 []{.arithmatex}$\tan A\cdot\overrightarrow{HA}+\tan B\cdot\overrightarrow{HB}+\tan C\cdot\overrightarrow{HC}=\vec0$.

- 设 []{.arithmatex}$O$ 为外心，则 []{.arithmatex}$\sin2A\cdot\overrightarrow{OA}+\sin2B\cdot\overrightarrow{OB}+\sin2C\cdot\overrightarrow{OC}=\vec0$.

### 欧拉线定理 {#_15}

- 欧拉定理：[]{.arithmatex}$O,I$ 分别为外接圆、内切圆圆心，则有 []{.arithmatex}$OI^2=R^2-2Rr$.

- 欧拉线定理：三角形的外心 []{.arithmatex}$O$，垂心 []{.arithmatex}$H$，重心 []{.arithmatex}$G$ 依次位于同一直线上，且重心到外心的距离是重心到垂心的距离的一半，即
...
- 已知平面上非零向量 []{.arithmatex}$\boldsymbol{a}$ 与 []{.arithmatex}$\boldsymbol{b}$，则 []{.arithmatex}$\boldsymbol{a} \cdot \boldsymbol{b} = \dfrac{1}{4}(|\boldsymbol{a} + \boldsymbol{b}|^2 - |\boldsymbol{a} - \boldsymbol{b}|^2)$。

- 在 []{.arithmatex}$\triangle ABC$ 中，若 []{.arithmatex}$M$ 是 []{.arithmatex}$BC$ 的中点，则 []{.arithmatex}$\overrightarrow{AB} \cdot \overrightarrow{AC} = |\overrightarrow{AM}|^2 - \dfrac{1}{4}|\overrightarrow{BC}|^2$。

在 []{.arithmatex}$\triangle ABC$ 中，对于共起点的数量积 []{.arithmatex}$\overrightarrow{AB} \cdot \overrightarrow{AC}$ 的求解问题，我们首先想到的是找出 []{.arithmatex}$BC$ 的中点 []{.arithmatex}$M$，则
...
化简整理便可得到如下结论：在 []{.arithmatex}$\triangle ABC$ 中，
...
任何事物都是由特殊再到一般，我们研究平面几何问题更多的是在研究三角形，因为三角形是我们接触最多也是最熟悉的，然后由三角形再延伸到四边形问题。那么对于向量余弦式是否也可以延伸到四边形呢？我们知道四边形通过对角线是可以分割成三角形的，下面一起来探讨这个问题。在平面四边形 []{.arithmatex}$ABCD$ 中，它可以由 []{.arithmatex}$\triangle ABC$ 与 []{.arithmatex}$\triangle ACD$ 组成，则在 []{.arithmatex}$\triangle ABC$ 中，由向量余弦式可得
...
那么在 []{.arithmatex}$\triangle ACD$ 中，则向量余弦式可得
...
重心：三角形三边中线交点，在平面直角坐标系中，重心的坐标是顶点坐标的算术平均，也就是说对于平面内任意一点 []{.arithmatex}$P$：
...
根据是中线的性质，做 []{.arithmatex}$AG$ 并延长交 []{.arithmatex}$BC$ 于点 []{.arithmatex}$H$，则：
...
锐角三角形外心到三边距离之和等于 []{.arithmatex}$R+r$。
...
而垂心到三角形一顶点距离等于此三角形外心到此顶点对边距离的 []{.arithmatex}$2$ 倍。

三角形外心 []{.arithmatex}$O$、重心 []{.arithmatex}$G$、垂心 []{.arithmatex}$H$ 三点共线且 []{.arithmatex}$OG:GH=1:2$ 此直线称为三角形的欧拉线。

## 三角形三线 {#_22}

### 中线长定理 {#_23}

在 []{.arithmatex}$\triangle ABC$ 中，[]{.arithmatex}$BC$ 的中点为 []{.arithmatex}$M$，对于中线 []{.arithmatex}$AM$，有：
...
在 []{.arithmatex}$\triangle ABC$ 中，[]{.arithmatex}$BC$ 上有一点 []{.arithmatex}$M$，则：
...
在 []{.arithmatex}$\triangle ABC$ 中，[]{.arithmatex}$\angle A$ 的平分线 []{.arithmatex}$AM$，有：
...
在 []{.arithmatex}$\triangle ABC$ 中 []{.arithmatex}$BC$ 上的高为 []{.arithmatex}$AD$，则：
...
又译斯台沃特定理，在 []{.arithmatex}$\triangle ABC$ 边 []{.arithmatex}$BC$ 上任意一点 []{.arithmatex}$D$，
...
或者设 []{.arithmatex}$x$ 表示两条对角线中点所连线段的长度：
...
注意到平行四边形对角线互相平分，即 []{.arithmatex}$x=0$，可得上面的第一个恒等式。
...
一直线与 []{.arithmatex}$\triangle ABC$ 的三边 []{.arithmatex}$AB,BC,AC$ 或他们的延长线分别交于 []{.arithmatex}$X,Y,Z$ 三点，则：
...
梅涅劳斯定理的逆定理表示为，满足上述式子，则 []{.arithmatex}$X,Y,Z$ 三点共线。
...
其中 []{.arithmatex}$p$ 为四边形的半周长，[]{.arithmatex}$\alpha,\beta$ 为其中二个对角。

布雷特施奈德公式可视为婆罗摩笈多公式之推广。

### 婆罗摩笈多公式 {#_36}

注意到圆内接四边形对角互补，其半角余弦值为零，[]{.arithmatex}$\cos90^\circ=0$ 则圆内接四边形面积可以简化为：
