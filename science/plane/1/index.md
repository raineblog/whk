...
一个有向线段由其两个端点表示，记为 $\\overrightarrow{AB}$ 或 **a**，同时我们记其长度，称为**向量的模**。
...
对于一个向量 *a⃗*，有向线段 *a⃗* 的长度称为向量的模，即为这个向量的大小。

符号表示为 |**a**| 或 $|\\overrightarrow{AB}|$ ，根据勾股定理，我们知道 $|\\vec a|=|(x,y)|=\\sqrt{x^2+y^2}$。
...
- 数量积：(*x*1′, *y*1′) ⋅ (*x*2′, *y*2′) = *x*1*x*2 + *y*1*y*2 + (*x*1*y*2 + *x*2*y*1)cos *θ*。
...
我们规定实数 *λ* 与向量 **a** 的积为一个向量，称为向量的数乘运算，记作 *λ\*\*a⃗*。

我们定义 *λ***a** = *λ*(*x*, *y*) = (*λ\*\*x*, *λ\*\*y*)。据此，我们可以得出以下向量数乘常用的结论：
...
设 *A*(*x*1, *y*1)、*B*(*x*2, *y*2) 为一平面内两点。

设 *C*(*x*, *y*) 为线段 *A\*\*B* 中点，则 $\\overrightarrow{AC}=\\overrightarrow{CB}$：
...
设 *A*(*x*1, *y*1)、*B*(*x*2, *y*2) 为一平面内两点，则，
...
两个非零向量 **a** 与 **b** 共线，等价于，有唯一实数 *λ*，使得 **b** = *λ***a**。

由数乘的定义知，对于非零向量 **a**，如果存在实数 *λ*，使得 **b** = *λ***a**，那么 **a** ∥ **b**。

数值上，有判别式 **a** = *λ***b** ⇔ |**a** ⋅ **b**| = |**a**||**b**|。

### 极化恒等式

对于任意平面向量 *a⃗*, *b⃗* 有：

4*a⃗* ⋅ *b⃗* = (*a⃗* + *b⃗*)2 − (*a⃗* − *b⃗*)2
...
若基底 **e**1, **e**2 不共线，则对于平面内任一向量 **a**，存在唯一实数 *x*, *y* 使得 **a** = *x***e**1 + *y***e**2。

形式化的，平面内的任一向量都可以唯一的表示为两个不共线向量的线性组合。

存在性的证明：感性理解（雾

唯一性的证明，反证：假设存在两组实数 ⟨*x*1, *y*1⟩、⟨*x*2, *y*2⟩，

$$ \\bm a=x_1\\bm e_1+y_1\\bm e_2=x_2\\bm e_1+y_2\\bm e_2\\ \\bm e_1(x_1-x_2)=\\bm e_2(y_2-y_1) $$

因为 *x*1 ≠ *x*2, *y*1 ≠ *y*2，则 **e**1, **e**2 共线，与条件不符，因此存在唯一性。
...
设，**a** = *x***e**1 + *y***e**2，用 *a*1, *a*2, *θ* 表示 **a**。
...
而坐标表示，(*a*, *b*) 与 (*c*, *d*) 平行，等价于 *a\*\*d* = *b\*\*c*。
