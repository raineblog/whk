...
符号表示即：\$\overrightarrow{AB}+\overrightarrow{BC}=\overrightarrow{[AC]{.abbr title="Alternating Current (交流电)"}}\$，其实这个也就是三角形法则所表述的。
...
设 \$C(x,y)\$ 为线段 \$AB\$ 中点，则 \$\overrightarrow{[AC]{.abbr title="Alternating Current (交流电)"}}=\overrightarrow{CB}\$：
