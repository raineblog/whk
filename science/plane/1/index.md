...
一个有向线段由其两个端点表示，记为 [\$\overrightarrow{AB}\$]{.arithmatex} 或 [\$\bm{a}\$]{.arithmatex}，同时我们记其长度，称为**向量的模**。
...
我们取这个向量在横、纵坐标上延伸的长度作为两个元素，将向量记为 [\$(a,b)\$]{.arithmatex}。

那么我们就得出了向量的几何意义，即向量 [\$(a,b)\$]{.arithmatex} 表示向右走 [\$a\$]{.arithmatex}、向上走 [\$b\$]{.arithmatex} 的位移。

已知两点 [\$A(a,b),B(c,d)\$]{.arithmatex}，易证 [\$\overrightarrow{AB}=(c-a,d-b)\$]{.arithmatex}。

### 向量的模 {#_4}

对于一个向量 [\$\vec a\$]{.arithmatex}，有向线段 [\$\vec a\$]{.arithmatex} 的长度称为向量的模，即为这个向量的大小。

符号表示为 [\$\|\bm a\|\$]{.arithmatex} 或 [\$\|\overrightarrow{AB}\|\$]{.arithmatex} ，根据勾股定理，我们知道 [\$\|\vec a\|=\|(x,y)\|=\sqrt{x\^2+y\^2}\$]{.arithmatex}。

### 特殊的向量 {#_5}

**零向量**：模为 [\$0\$]{.arithmatex} 的向量，零向量的方向任意（不过其实是无意义）。一般记为：[\$\vec 0\$]{.arithmatex}。

**单位向量**：模为 [\$1\$]{.arithmatex} 的向量称为单位向量。一般记为 [\$\bm e\$]{.arithmatex}，最常见的单位向量就是基向量。

**基向量**：[\$\bm i=(1,0)\$]{.arithmatex} 表示 [\$x\$]{.arithmatex} 方向的单位向量，[\$\bm j=(0,1)\$]{.arithmatex} 表示 [\$y\$]{.arithmatex} 方向的单位向量。

**平行向量**：方向相同或相反的两个非零向量，规定零向量与任何向量平行。记作： [\$\bm x\parallel\bm y\$]{.arithmatex}。
...
我们称 [\$x\$]{.arithmatex} 轴与 [\$y\$]{.arithmatex} 轴的角度为 [\$\theta(\theta\neq\pi/2)\$]{.arithmatex} 的坐标系。

定义平面直角坐标系中的点 [\$P(x,y)\$]{.arithmatex}，将 [\$P\$]{.arithmatex} 转移到斜坐标系中变成 [\$P\'(x\',y\')\$]{.arithmatex} 满足：
...
- 数量积：[\$(x_1\',y_1\')\cdot(x_2\',y_2\')=x_1x_2+y_1y_2+(x_1y_2+x_2y_1)\cos\theta\$]{.arithmatex}。

- 模长：[\$\overrightarrow{a}=(x\',y\'),\|\overrightarrow{a}\|=\sqrt{x\^2+y\^2+2xy\cos\theta}\$]{.arithmatex}。

- 夹角：[\$\displaystyle\overrightarrow{a}=(x_1\',y_1\'),\overrightarrow{b}=(x_2\',y_2\'),\cos\gamma=\frac{\overrightarrow{a}\cdot\overrightarrow{b}}{\|\overrightarrow{a}\|\|\overrightarrow{b}\|}\$]{.arithmatex}。

## 向量的线性运算 {#_7}

### 向量数乘 {#_8}

我们规定实数 [\$\lambda\$]{.arithmatex} 与向量 [\$\bm a\$]{.arithmatex} 的积为一个向量，称为向量的数乘运算，记作 [\$\lambda\vec a\$]{.arithmatex}。

我们定义 [\$\lambda\bm a=\lambda(x,y)=(\lambda x,\lambda y)\$]{.arithmatex}。据此，我们可以得出以下向量数乘常用的结论：

- [\$\|\lambda\bm a\|=\|\lambda\|\|\bm a\|\$]{.arithmatex}；
- 当 [\$\lambda \>0\$]{.arithmatex} 时，[\$\lambda\bm a\$]{.arithmatex} 与 [\$\bm a\$]{.arithmatex} 同向；
- 当 [\$\lambda =0\$]{.arithmatex} 时，[\$\lambda \bm a=\vec 0\$]{.arithmatex}；
- 当 [\$\lambda\<0\$]{.arithmatex} 时，[\$\lambda \bm a\$]{.arithmatex} 与 [\$\bm a\$]{.arithmatex} 方向相反。
...
类比物理中的位移，从 [\$A\$]{.arithmatex} 经 [\$B\$]{.arithmatex} 到 [\$C\$]{.arithmatex}，那么经过的位移等价于直接从 [\$A\$]{.arithmatex} 到 [\$C\$]{.arithmatex}。

符号表示即：[\$\overrightarrow{AB}+\overrightarrow{BC}=\overrightarrow{AC}\$]{.arithmatex}，其实这个也就是三角形法则所表述的。

向量减法类似：[\$\overrightarrow{OA}-\overrightarrow{OB}=\overrightarrow{OA}+\overrightarrow{BO}=\overrightarrow{BA}\$]{.arithmatex}，后面用到了交换律。
...
设 [\$A(x_1,y_1)\$]{.arithmatex}、[\$B(x_2,y_2)\$]{.arithmatex} 为一平面内两点。

设 [\$C(x,y)\$]{.arithmatex} 为线段 [\$AB\$]{.arithmatex} 中点，则 [\$\overrightarrow{AC}=\overrightarrow{CB}\$]{.arithmatex}：
...
设 [\$A(x_1,y_1)\$]{.arithmatex}、[\$B(x_2,y_2)\$]{.arithmatex} 为一平面内两点，则，
...
已知两个向量 [\$\bm a,\bm b\$]{.arithmatex} ，它们的夹角为 [\$\theta\$]{.arithmatex}，那么这两个向量的点积为：

::: {.arithmatex}
\$\$ \bm a \cdot \bm b=\|\bm a\|\|\bm b\|\cos \theta \$\$
:::

- 其中，我们称 [\$\|\bm a\|\cos \theta\$]{.arithmatex} 为 [\$\bm a\$]{.arithmatex} 在 [\$\bm b\$]{.arithmatex} 方向上的投影。

  ::: {.arithmatex}
  \$\$ \|\bm a\|\cos\theta=\dfrac{\bm a\cdot\bm b}{\|\bm b\|} \$\$
  :::

  而投影向量需要再乘上 [\$\bm b\$]{.arithmatex} 方向的单位向量：

  ::: {.arithmatex}
  \$\$ \dfrac{\bm a\cdot\bm b}{\|\bm b\|\^2}\bm b \$\$
  :::

- 其中，[\$\theta\in\[0,\pi\]\$]{.arithmatex}，但是直线的夹角 [\$\theta\in\[0,\pi)\$]{.arithmatex}，因为相反向量是完全不同的。

点积的几何意义即为：点积 [\$\bm a \cdot \bm b\$]{.arithmatex} 等于 [\$\bm a\$]{.arithmatex} 的模与 [\$\bm b\$]{.arithmatex} 在 [\$\bm a\$]{.arithmatex} 方向上的投影的乘积。
...
向量与零向量点积，结果为 [\$\vec0\$]{.arithmatex}。在不引起混淆的情况下，点积的点号可以省略不写。
...
可以得到，若 [\$\bm b\neq\bm c\$]{.arithmatex}，[\$\bm a\cdot\bm b=\bm a\cdot\bm c\$]{.arithmatex} 的充要条件是 [\$\bm a\perp(\bm b-\bm c)\$]{.arithmatex}，
...
根据 [\$\cos 90\^\circ=0\$]{.arithmatex}，[\$\bm a \perp \bm b \iff \bm a\cdot \bm b=0\$]{.arithmatex}。
...
两个非零向量 [\$\bm a\$]{.arithmatex} 与 [\$\bm b\$]{.arithmatex} 共线，等价于，有唯一实数 [\$\lambda\$]{.arithmatex}，使得 [\$\bm b=\lambda \bm a\$]{.arithmatex}。

由数乘的定义知，对于非零向量 [\$\bm a\$]{.arithmatex}，如果存在实数 [\$\lambda\$]{.arithmatex}，使得 [\$\bm b=\lambda \bm a\$]{.arithmatex}，那么 [\$\bm a \parallel \bm b\$]{.arithmatex}。

数值上，有判别式 [\$\bm a = \lambda \bm b \iff \|\bm a\cdot \bm b\|=\|\bm a\|\|\bm b\|\$]{.arithmatex}。

### 极化恒等式 {#_17}

对于任意平面向量 [\$\vec a,\vec b\$]{.arithmatex} 有：
...
其中 [\$M\$]{.arithmatex} 为 [\$AB\$]{.arithmatex} 中点。

## 基底分解 {#_18}

### 平面向量基本定理 {#_19}

若基底 [\$\bm e_1,\bm e_2\$]{.arithmatex} 不共线，则对于平面内任一向量 [\$\bm a\$]{.arithmatex}，存在唯一实数 [\$x,y\$]{.arithmatex} 使得 [\$\bm a=x\bm e_1+y\bm e_2\$]{.arithmatex}。

形式化的，平面内的任一向量都可以唯一的表示为两个不共线向量的线性组合。

存在性的证明：感性理解（雾

唯一性的证明，反证：假设存在两组实数 [\$\langle x_1,y_1\rangle\$]{.arithmatex}、[\$\langle x_2,y_2\rangle\$]{.arithmatex}，

::: {.arithmatex}
\$\$ \bm a=x_1\bm e_1+y_1\bm e_2=x_2\bm e_1+y_2\bm e_2\\ \bm e_1(x_1-x_2)=\bm e_2(y_2-y_1) \$\$
:::

因为 [\$x_1\neq x_2,y_1\neq y_2\$]{.arithmatex}，则 [\$\bm e_1,\bm e_2\$]{.arithmatex} 共线，与条件不符，因此存在唯一性。
...
设，[\$\bm a=x\bm e_1+y\bm e_2\$]{.arithmatex}，用 [\$a_1,a_2,\theta\$]{.arithmatex} 表示 [\$\bm a\$]{.arithmatex}。
...
对于线段 [\$AB\$]{.arithmatex} 及一点 [\$Q\$]{.arithmatex}，若存在一点 [\$O\$]{.arithmatex} 满足，

::: {.arithmatex}
\$\$ \overrightarrow{OQ}=\lambda\overrightarrow{OA}+(1-\lambda)\overrightarrow{OB} \$\$
:::

则 [\$Q\$]{.arithmatex} 在直线 [\$AB\$]{.arithmatex} 上，即 [\$A,B,Q\$]{.arithmatex} 三点共线。
...
类似的，若 [\$C\$]{.arithmatex} 是线段 [\$AB\$]{.arithmatex} 中点，则对于任意一点 [\$O\$]{.arithmatex}，有，

::: {.arithmatex}
\$\$ \overrightarrow{OC}={1\over2}\overrightarrow{OA}+{1\over2}\overrightarrow{OB} \$\$
:::

而坐标表示，[\$(a,b)\$]{.arithmatex} 与 [\$(c,d)\$]{.arithmatex} 平行，等价于 [\$ad=bc\$]{.arithmatex}。
