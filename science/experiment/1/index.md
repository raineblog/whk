...
- 干燥：连续两次干燥，质量差值小于 [\$\pu{0.1g}\$]{.arithmatex}。

### 升华 {#_6}

- 固固混合，其一易升华。

- 例如 [\$\ce{NH4Cl}\$]{.arithmatex} 受热分解，随即化合重新生成。
...
- 常见滴定误差分析：
  ...
  - [\$\ce{I-}\$]{.arithmatex} 会被空气中的氧气氧化，滴定 [\$\ce{I2}\$]{.arithmatex} 偏高。
...
- 密度大于 [\$1\$]{.arithmatex} 的溶剂（水在上层）：四氯化碳、氯仿、溴苯、二硫化碳等。

- 密度小于 [\$1\$]{.arithmatex} 的溶剂（水在下层）：液态的饱和烃、直馏汽油、苯及其同系物、液态环烷烃、液态饱和酯。
...
离心法是一种利用离心力分离悬浮液中颗粒的技术，其核心公式为离心力 [\$F=m\omega\^2r\$]{.arithmatex}。
...
沉降系数 [\$s\$]{.arithmatex} 定义为颗粒在单位离心场中的沉降速度，公式为

::: {.arithmatex}
\$\$ s=\dfrac{v}{\omega\^2r} \$\$
:::

单位为斯维德伯格 [\$\pu{1S}=10\^{-13}\$]{.arithmatex} 秒。

反映颗粒特性：与颗粒密度、大小及形状相关。

应用场景：通过测定可推断分子量或颗粒结构，如 [DNA]{.abbr title="Deoxyribonucleic Acid (脱氧核糖核酸)"} 超螺旋构象的沉降系数差异。
...
- 容量瓶一定要指定容量，例如 [\$\pu{1000mL}\$]{.arithmatex} 容量瓶。
- 定容至刻度线下 [\$\pu{1\sim2cm}\$]{.arithmatex} 后用胶头滴管滴加。
...
通常来说，滴定管的规格主要是 [\$\pu{25mL}\$]{.arithmatex} 和 [\$\pu{50mL}\$]{.arithmatex}，分度值为 [\$\pu{0.1mL}\$]{.arithmatex}，读数时需要估读一位。与量筒不同的是，滴定管的零刻度线在最上方，读数时需仔细注意。另外注意，滴定管的最大刻度线位于滴定管底部，但不是滴定管的最后一块容积，也就是说，如果现在的刻度是 [\$\pu{10mL}\$]{.arithmatex}，规格是 [\$\pu{25mL}\$]{.arithmatex}，那么将滴定管内的所有液体放干净，放出来的液体体积是至少有，或者说大鱼 [\$\pu{15mL}\$]{.arithmatex} 的。

此外，滴定管通常来说，可以从任何刻度开始读数，但是其读数一定要滴定前后各读数一次。有的时候，我们通常会在滴定前标定到 [\$\pu{0.00mL}\$]{.arithmatex} 刻度线处，这样既可以方便读数，有的时候题目要求分析这样做可以让结果更准确的原因，可能是因为刻度线如果不均匀，从同一刻度开始滴定，会稍微准确一些。

洗涤和润洗：滴定管在使用前需要先用蒸馏水清洗干净，称为洗涤，然后用待测液润洗 [\$2\sim 3\$]{.arithmatex} 次，避免残留的蒸馏水使滴定管内盛放的液体稀释，或者挂壁。此外，有一个容易出坑的点是，滴定管内盛放的液体，虽然通常是标准液，但是如果题目明确指出或者暗示，是将待测液放入滴定管，那么需要按照这个来。常见的说法有，用已知浓度的氢氧化钠溶液滴定未知浓度的盐酸溶液，用酸式滴定管。
...
- 常见 [\$\pH\$]{.arithmatex} 试剂的变色范围。

  ![alt text](../image.avif){width="80%"}

- 广泛 [\$\pH\$]{.arithmatex} 试纸：

  - 试纸不能用水湿润，即使是测量蒸馏水的 [\$\pH\$]{.arithmatex}。

  - 比色卡对照，只能读整数，有小数比色卡称为精密 [\$\pH\$]{.arithmatex} 试纸。
...
通过记录电位（[\$E\$]{.arithmatex}）随滴定剂体积（[\$V\$]{.arithmatex}）的变化，找出电位突变最大的点，即为滴定终点。根据反应类型的不同，需要选择不同的**指示电极**：

| 滴定反应类型 | 指示电极 | 参比电极 | 备注 |
|:---|:---|:---|:---|
| **酸碱滴定** | 玻璃电极 (Glass Electrode) | 饱和甘汞电极 (SCE) 或 Ag / AgCl | 响应 [\$\ce{H+}\$]{.arithmatex} 浓度 |
| **氧化还原滴定** | 铂电极 (Pt Electrode) | 饱和甘汞电极 (SCE) | 铂作为惰性导体传递电子 |
| **沉淀滴定** | 银电极 (Ag) 或 离子选择性电极 (ISE) | 双液接甘汞电极 | 防止参比电极液漏出干扰测定（如 [\$\ce{Cl-}\$]{.arithmatex}） |
...
1.  酸碱滴定 (Acid-Base Titration)
    ...
    - 最常见，使用 pH 玻璃电极。
    - **特长：** 可以分辨多元酸（如磷酸 [\$H_3PO_4\$]{.arithmatex}）的多个解离步骤，曲线上会出现多个突跃点。也可以滴定极弱的酸或碱（非水溶液滴定）。

2.  氧化还原滴定 (Redox Titration)

    - 使用铂电极。
    - 例子：用 [\$KMnO_4\$]{.arithmatex} 滴定 [\$Fe\^{2+}\$]{.arithmatex}，或者碘量法。电位取决于溶液中氧化态和还原态物质浓度的比值。

3.  沉淀滴定 (Precipitation Titration)

    - 通常涉及银离子，使用银电极。
    - 例子：用 [\$AgNO_3\$]{.arithmatex} 滴定卤素离子 ([\$Cl\^-, Br\^-, I\^-\$]{.arithmatex})。可以同时测定混合卤素离子（因为 [\$K\_{sp}\$]{.arithmatex} 不同，会出现分步沉淀和多个突跃）。

4.  络合滴定 (Complexometric Titration)

    - 使用离子选择性电极（ISE）。
    - 例子：EDTA 滴定金属离子（如 [\$Ca\^{2+}, Mg\^{2+}\$]{.arithmatex}）。
...
| 气体 | 方程式 |
|---:|:---|
| [\$\ce{H2}\$]{.arithmatex} | [\$\ce{Zn + H2SO4 -\> H2 \^ + ZnSO4}\$]{.arithmatex} |
| [\$\ce{CO2}\$]{.arithmatex} | [\$\ce{CaCO3 + 2HCl -\> CaCl2 + H2O + CO2 \^}\$]{.arithmatex} |
|  | [\$\ce{Na2CO3 + H2SO4 -\> Na2SO4 + H2O + CO2 \^}\$]{.arithmatex} |
| [\$\ce{O2}\$]{.arithmatex} | [\$\ce{2H2O2 -\>\[MnO2\] 2H2O + O2 \^}\$]{.arithmatex} |
|  | [\$\ce{2KClO3 -\>\[MnO2\] 2KCl + 3O2 \^}\$]{.arithmatex} |
|  | [\$\ce{2KMnO4 -\>\[\triangle\] K2MnO4 + MnO2 + O2 \^}\$]{.arithmatex} |
| [\$\ce{HCl}\$]{.arithmatex} | [\$\ce{2NaCl + H2SO4(浓) -\>\[\triangle\] Na2SO4 + 2HCl}\$]{.arithmatex} |
|  | 浓硫酸滴到浓盐酸里（吸水放热、溶剂减少） |
| [\$\ce{HX}\$]{.arithmatex}（[\$\ce{Br/I}\$]{.arithmatex}） | [\$\ce{3NaX + H3PO4(浓) -\>\[\triangle\] Na3PO4 + 3HX}\$]{.arithmatex} |
...
![量气](../tech/%E9%87%8F%E6%B0%94%E8%A3%85%E7%BD%AE.svg){width="100%"}
...
![防倒吸装置](../tech/%E5%AE%89%E5%85%A8%E7%93%B6.avif){width="100%"}

### 尾气处理 {#_32}

燃烧式处理：

- 燃着的酒精灯：[\$\ce{O2,H2}\$]{.arithmatex}。

吸收式处理（注意防倒吸）：

- 碱性溶液（[\$\ce{NaOH}\$]{.arithmatex} 溶液）：[\$\ce{SO2,Cl2}\$]{.arithmatex}。

- 酸性溶液（稀硫酸）：[\$\ce{NH3}\$]{.arithmatex}。

- 水（严格注意防倒吸）：极易溶于水的 [\$\ce{NH3,HCl}\$]{.arithmatex}。
...
| 气体 | 检验方法 |
|---:|:---|
| [\$\ce{CO2}\$]{.arithmatex} | 使澄清石灰水变浑浊 |
| [\$\ce{H2O}\$]{.arithmatex} | 使无水硫酸铜变蓝 |
| [\$\ce{CO}\$]{.arithmatex} | 还原氧化铜后，检验二氧化碳 |
| [\$\ce{H2}\$]{.arithmatex} | 还原氧化铜后，检验水 |
| [\$\ce{HCl}\$]{.arithmatex} | 使蓝色石蕊试纸变红，使硝酸银溶液产生沉淀 |
| [\$\ce{NH3}\$]{.arithmatex} | 使红色石蕊试纸变红，有刺激性气味 |
| [\$\ce{SO2}\$]{.arithmatex} | 使澄清石灰水变浑浊，使品红溶液褪色且可恢复，使酸性高锰酸钾溶液褪色 |
| [\$\ce{Cl2}\$]{.arithmatex} | 黄绿色，使湿润的淀粉碘化钾试纸变蓝，使品红溶液褪色且不可恢复 |
...
|  | 除杂方法 |
|---:|:---|
| [\$\ce{CO2(HCl)}\$]{.arithmatex} | 饱和 [\$\ce{NaHCO3}\$]{.arithmatex} |
| [\$\ce{CO2(SO2)}\$]{.arithmatex} | 饱和 [\$\ce{NaHCO3}\$]{.arithmatex}，酸性高锰酸钾 |
| [\$\ce{H2(HCl)}\$]{.arithmatex} | [\$\ce{NaOH}\$]{.arithmatex} 或水 |
| [\$\ce{HCl(Cl2)}\$]{.arithmatex} | 灼热铜网，生成 [\$\ce{CuCl2}\$]{.arithmatex} 后除杂 |
| [\$\ce{Cl2(HCl)}\$]{.arithmatex} | 饱和 [\$\ce{NaCl}\$]{.arithmatex} |
| [\$\ce{N2(O2)}\$]{.arithmatex} | 灼热铜网 |
| [\$\ce{NH3(CO2)}\$]{.arithmatex} | 碱石灰 |
| [\$\ce{NO(NO2)}\$]{.arithmatex} | 水 |
