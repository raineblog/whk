...
取一个微小锥形角 d*Ω*，在球壳上两个面元 d*S*1, d*S*2，距离点 *P* 距离分别为 *r*1, *r*2。
...
代数运算：常取 *G* ≈ 20/3 × 10−11（忽略单位），*π*2 ≈ 10。
...
两式做比：*m**A**r**O**A* = *m**B**r**O**B*。

两式做和：

*G\*\*M* = *G*(*m\*\*A* + *m\*\*B*) = *ω*2*L*2(*r**O**A* + *r**O**B*) = *ω*2*L*3
...
- 其中 *r*1, *r*2 分别是原本圆轨道与目标圆轨道的半径。

以提高轨道为例：

- 于 *v*1 点火后 *v\*\*p*，最高轨道 *v\*\*q* 点火加速到 *v*2 圆周。
