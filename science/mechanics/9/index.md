...
这是热机效率的理论极限。它告诉我们，要提高发动机效率，要么提高燃烧温度（提高 T\_{\\text{H}}），要么优化散热（降低 T\_{\\text{L}}）。
...
- **经典误区**：利用河水能量逆水航行。这在能量上是守恒的，但因为试图从单一热源吸热全部做功（而不产生其他影响），违背了第二定律。
...
**功的计算**：在 P-V 图上，气体对外做功或外界对气体做功，可以通过曲线下的面积来表示。根据热力学第一定律中功的定义 W = - \\int p \\mathrm{d}V：
...
\\delta W = p\_{\\text{ext}} \\mathrm{d}V

其中 p\_{\\text{ext}} 是外界施加在系统边界上的压强，而非系统内部的压强。功的本质是“系统克服外部阻力推动边界位移”。只有在准静态（可逆）过程中，外压与内压始终相差无穷小，才能用内部压强 p 代替 p\_{\\text{ext}} 来积分。

这里有一个容易混淆的关键点：**膨胀不等于做功**。系统体积增大是膨胀，但做功要求膨胀时必须克服外界阻力。例如气体向真空自由膨胀：

- 体积变大了
- 但右边是真空，p\_{\\text{ext}} = 0
- 因此 W = \\int 0 \\cdot \\mathrm{d}V = 0

这说明“体积增大”不是做功的充分条件。同样，有压强也不等于在做功——刚性容器里加热气体，压强升高、温度升高，但体积不变（\\mathrm{d}V = 0），因此 W = 0。必须同时有压强和边界位移，才有体积功。

不同物态的做功特性也有显著差异：

- **气体**：容易压缩和膨胀，体积变化大，最适合用 p \\mathrm{dV} 语言描述做功。内燃机、蒸汽机等正是利用气体的大幅膨胀功来驱动机械。
- **固体**：体积变化通常很小，但可以通过弹性形变（拉伸、压缩、弯曲、扭转）做功。弹簧释放推动物体就是典型的固体做功，但不是靠体积膨胀。
- **液体**：几乎不可压缩，普通情况下的 p \\mathrm{dV} 膨胀功很小。但在相变（如水沸腾变为蒸汽）时体积暴涨，做功可显著增大。此外液体还有表面张力做功 \\delta W = \\gamma \\mathrm{d}A。

### 热机与循环

17 \\sim 18 世纪，许多人致力于制造一种机器，它不需要任何动力或燃料，却能不断地对外做功，史称“第一类永动机”。任何动力机械的作用都是把其他形式的能转化为机械能。内燃机把燃料的化学能转化为燃气的内能然后再转化为机械能，电动机把电能转化为机械能……如果没有燃料、电流或其他动力的输入，能量从哪里来呢！永动机的思想违背了能量守恒定律，所以是不可能制成的。
...
- **等容摩尔热容（C\_{V,m}）**：在体积不变的过程中，\\pu{1 mol} 物质温度升高 \\pu{1 K} 所吸收的热量。对于单原子理想气体，C\_{V,m} = \\frac{3}{2}R。
- **等压摩尔热容（C\_{p,m}）**：在压力不变的过程中，\\pu{1 mol} 物质温度升高 \\pu{1 K} 所吸收的热量。
...
其中 \\gamma = C_p / C\_{V} 称为**绝热指数**。这一公式解释了为什么喷漆罐在使用时会变冷，以及柴油机通过快速压缩空气来点火的原理。
...
如果算出来是 $，我们就说氧气的体积分数是 20%。所以，**在恒容容器中说体积分数，其实是在描述一个“假设状态”（Hypothetical State）**，而不是描述气体当前在容器里的空间分布。

**核心桥梁是理想气体状态方程**。为什么大家依然喜欢用“体积分数”这个词？因为对于**理想气体**，它有一个完美的数学等价关系：

PV = nRT

- **体积分数** \\varphi = V_i / V\_{total}
- **摩尔分数** x = n_i / n\_{total}
- **压强分数**（即分压比总压） P_i / P\_{total}
...
密闭容器中理想气体在**单位时间内、单位面积上与容器壁的碰撞次数**（在物理学中通常记为碰撞频率 Z 或 \\Gamma），与你提到的**压强、温度、分子密度都有关**，此外还与**气体分子的质量**有关。不过，由于理想气体状态方程（P = nkT）将压强、温度和分子密度联系在了一起，因此这三个量并不是完全独立的。
...
而分子的平均速率 \\bar{v} 与**温度 T** 和 **分子质量 m** 有关：
...
- 根据热力学第一定律（\\Delta U = W + Q），这团气体的**内能是不变的**（\\Delta U = 0）。
...
### 实验：探究一定质量气体压强、体积、温度的关系

实验探究一定质量气体的压强（p）、体积（V）和温度（T）之间的关系，是热学研究的基础。在物理学中，由于这三个变量相互关联，通常采用**控制变量法**，即在保持其中一个物理量不变的情况下，研究另外两个量之间的定量关系。

探究气体等温变化的规律（玻意耳定律）：

1. 实验目的与原理
   - **目的**：探究一定质量的气体在温度不变时，压强与体积的关系。
   - **原理**：在保证气体的质量和温度不变的条件下，改变密闭气体的体积，测量对应的压强值，寻找 p 与 V 的数学关系。
1. 实验装置与操作
   - **器材**：注射器、压力表（或压强传感器）、刻度尺。
   - **步骤**：
     - 将注射器活塞移动到中间位置，用橡胶套堵住小孔，封闭一定质量的空气。
     - **缓慢**推拉活塞以改变空气柱的体积（通过注射器刻度读取 V），待示数稳定后由压力表读出压强 p。
     - 重复多次实验，记录多组 p 和 V 的数据。
1. 数据处理与结论
   - **图线分析**：作出 p-V 图像，得到一条双曲线（等温线）。为了更直观地验证反比关系，通常作出 **p-1/V 图像**，若图线是**过原点的直线**，则说明压强与体积成反比。
   - **结论（玻意耳定律）**：一定质量的某种气体，在温度不变的情况下，压强 p 与体积 V 成反比，即 **pV = C** 或 **p_1V_1 = p_2V_2**。
1. 注意事项与误差分析
   - **保持等温**：活塞移动要缓慢，且**不能用手握住注射器**有气体的部分，以防手温改变气体的温度。
   - **保证质量**：在活塞上涂抹润滑油可以起到密封作用，防止漏气。
   - **误差来源**：主要来自空气柱长度的测量误差，读数时视线应与柱塞底面平行。

探究气体等容变化的规律（查理定律）：

1. 实验目的与原理
   - **目的**：探究一定质量的气体在体积不变时，压强与温度的关系。
   - **原理**：利用水浴加热改变封闭气体的温度，记录不同温度下的压强。
1. 实验装置与步骤
   - **器材**：烧瓶、压强传感器（或压强计）、温度传感器（或温度计）、烧杯、热水。
   - **步骤**：
     - 将烧瓶密封，浸入盛水的烧杯中，使烧瓶内气体的体积保持不变。
     - 通过加热烧杯中的水改变温度，待示数稳定（热平衡）后，记录对应的温度 T 和压强 p。
1. 数据处理与结论
   - **图线分析**：在 p-t 图像中，图线是不过原点的直线；在 **p-T 图像**（热力学温标）中，图线是**过原点的倾斜直线**（等容线）。
   - **结论（查理定律）**：一定质量的某种气体，在体积不变的情况下，压强 p 与热力学温度 T 成正比，即 **p/T = C** 或 **p_1/T_1 = p_2/T_2**。

探究气体等压变化的规律（盖-吕萨克定律）：

1. 实验目的与原理
   - **目的**：探究一定质量的气体在压强不变时，体积与温度的关系。
   - **原理**：通过水浴加热，观察气体体积随温度的变化，且利用活塞或液柱受力平衡保持压强恒定。
1. 实验装置与结论
   - **实验过程**：将封闭气体的细玻璃管（内有可移动液柱）浸入水中，改变水温，测量空气柱的长度（即体积 V）和对应的温度 T。
   - **结论（盖-吕萨克定律）**：一定质量的某种气体，在压强不变的情况下，体积 V 与热力学温度 T 成正比，即 **V/T = C** 或 **V_1/T_1 = V_2/T_2**。
   - **图线**：**V-T 图像**是一条过坐标原点的倾斜直线（等压线）。

综合规律与微观解释：

1. **理想气体状态方程**：将三个实验定律综合，可以得出一定质量的理想气体的状态方程：**pV/T = C**（常量）。这说明对于一定质量的气体，压强和体积的乘积与热力学温度的比值是不变的。
1. **微观解释**：
   - **等温过程**：温度不变则分子平均动能不变，体积减小使**分子密集程度**增大，单位时间内撞击单位面积器壁的分子数增多，故压强增大。
   - **等容过程**：体积不变则分子密集程度不变，温度升高使**分子平均动能**增大，分子撞击器壁更剧烈且频繁，故压强增大。
   - **等压过程**：温度升高使分子平均动能增大（有增压趋势），必须增大体积以减小分子密集程度（产生减压效果），从而保持压强不变。
1. **理想气体模型**：实验定律在压强不太大、温度不太低时近似成立。物理学中将严格遵从这些定律的气体称为**理想气体**。
...
对于汞液柱问题，通常用 \\pu{cmHg} 作为压强的单位，因为这样可以把 mgh 直接写成 h（液面的高度差），与 \\pu{Pa} 不统一。
...
**例题**：一竖直放置的气缸由两个粗细不同的圆柱形筒组成，气缸中活塞Ⅰ和活塞Ⅱ之间封闭有一定量的理想气体，两活塞用一轻质弹簧连接，气缸连接处有小卡销，活塞Ⅱ不能通过连接处。活塞Ⅰ、Ⅱ的质量分别为 2m、m，面积分别为 2S、S，弹簧原长为 l。初始时系统处于平衡状态，此时弹簧的伸长量为 0.1l，活塞Ⅰ、Ⅱ到气缸连接处的距离相等，两活塞间气体的温度为 T_0。已知活塞外大气压强为 p_0，忽略活塞与缸壁间的摩擦，气缸无漏气，不计弹簧的体积。
...
- 向下的力：活塞Ⅰ上方大气压力 p_0 \\cdot 2S，活塞Ⅰ重力 2mg，活塞Ⅱ重力 mg。
- 向上的力：活塞Ⅱ下方大气压力 p_0 \\cdot S。
- 气体对机械系统的等效作用力：气体对活塞Ⅰ向上的力为 p \\cdot 2S，对活塞Ⅱ向下的力为 p \\cdot S，气体对系统总的等效作用力为向上 p \\cdot 2S - p \\cdot S = p \\cdot S。
...
可见压强 p 并非 p_0，但它是**由常数（重力、大气压）决定的固定值**。只要活塞没有卡住，在缓慢加热过程中，这个压强始终不变——这就是许多参考答案说“气体压强不变”的由来。
...
1. 活塞Ⅰ对气体的压力：向下，大小为 p \\cdot 2S。
1. 活塞Ⅱ对气体的压力：向上，大小为 p \\cdot S。
1. 气缸台阶面对气体的支持力：向上！台阶面积恰好为 2S - S = S，气体压在台阶上，台阶给气体一个向上的反作用力，大小为 p \\cdot S。
...
上述台阶面的分析可以推广到任意形状的容器。以一个上窄下宽的圆台形容器装满水为例：水深为 h，底面积为 S\_{\\text{底}}，水的总重力为 G\_{\\text{水}}。液体压强给出容器底部的压力：
...
1. 重力：G\_{\\text{水}}（向下）。
1. 底部支持力：F\_{\\text{底}} = \\rho g h \\cdot S\_{\\text{底}}（向上）。
1. 侧壁的压力：因为容器上窄下宽，水往外顶侧壁，根据牛顿第三定律，侧壁给水一个向内且斜向下的反作用力。将所有侧壁反作用力的竖直向下分力之和记为 F\_{\\text{侧压}}。

平衡条件给出：

F\_{\\text{底}} = G\_{\\text{水}} + F\_{\\text{侧压}}
...
**充气问题**：设想一个容器，初始状态压强为 p_1、体积为 V、温度为 T_1，充入同种气体后，末状态压强为 p_2、体积仍为 V、温度为 T_2。

设充入气体的物质的量为 \\Delta n，这些气体在充入前处于某个状态（比如大气压 p_0、温度 T_0、体积 V_0）。根据物质的量守恒：
...
这就是充气问题的基本方程。如果充入的是大气（p_0 = p\_{\\text{atm}}，T_0 = T\_{\\text{环境}}），则只需确定充入气体的体积 V_0 即可。

**放气问题**：类似地，容器放气时，设放出气体的物质的量为 \\Delta n：
...
其中 p_0、T_0、V_0 是放出气体在环境中的状态参量。

**一个实用的技巧**：如果充气或放气过程中温度保持不变（T_1 = T_2 = T），上述方程可以简化为：
...
**打气问题的特例**：用打气筒给容器打气，每次打入压强为 p_0、体积为 V_0 的气体，共打 n 次。设容器体积为 V，最终压强为 p，温度不变。
...
**抽气问题的特例**：用抽气机从容器中抽气，每次抽出体积为 V_0 的气体。设容器体积为 V，初始压强为 p_0，抽 n 次后压强为 p_n，温度不变。

每次抽气时，容器内气体膨胀到体积 V + V_0（假设抽气机与容器连通），然后抽气机将气体排出。根据玻意耳定律，第一次抽气后：
...
例如，一个钢瓶装有压强为 p_0 的气体，每次放出气体的物质的量为 \\Delta n（在标准状态下体积为 V\_{\\text{std}}），共放 n 次，求最终压强。
...
来源：[175.题 6 水银溢出稳定问题（PV=nRT 物理学难题集萃）](https://www.bilibili.com/video/BV1MV4y1p7kG/?share_source=copy_web&vd_source=a2a4227b4b9a68841a8404acff2b8dc9)。
...
不妨记试管总长度 L，设温度为 T_x 时，某一平衡点水银柱剩下 x，那么气柱的总长度就是 L-x，在这个状态下：

\\dfrac{(p_0+x)(L-x)}{T_x}=C

注意到，我们把它化为整式，就是一个二次方程，即：

x^2+(L-p_0)x+CT_x-p_0L=0

我们发现，如果这个方程无解，那么说明不存在平衡点，也就是说只要大于等于这个温度，水银就会被气体本身的能力彻底推出试管，这个温度就是：

\\Delta=(L-p_0)^2-4(CT_x-p_0L)=0

即：

T_x=\\dfrac{(L-p_0)^2+4p_0L}{4C}

注意，不是所有情况都会有这种结果，一定要对着数分析。

关于 L 形试管的补充：高中物理中常见的 L 形试管模型，水银封在水平细管中，很多人会疑惑，现实中水银不会横着流过去吗？这种 L 形装置真的存在吗？

**结论是：在满足一定条件下，这种装置是真实存在的，水银确实不会横着流散。** 原因可从宏观和微观两个层面理解。

**宏观层面：压强平衡。** 水银停在水平管中不动，是因为左右两侧的力完全平衡。以一个典型场景为例，L 形拐角处的压强来自上方开口处的大气压（如 P_0 = \\pu{75 cmHg}）加上竖直水银柱（如 \\pu{5 cm}）产生的压强，即水银柱右侧受到的向左压强为 \\pu{80 cmHg}。左端封闭气体的压强 P_A 恰好等于 \\pu{80 cmHg}，维持平衡。
...
**如果换成水**：有两个致命弱点。其一，水“浸润”玻璃（附着力大于内聚力），会沿管壁蔓延爬升，难以维持边缘清晰的“水柱活塞”；其二，水的表面张力只有水银的约七分之一，极易被气体从中间“顶破”。此外，水的密度远小于水银，要产生同样的大气压平衡，竖直水柱需要约 \\pu{10 m} 高，在普通实验室中根本无法实现。
...
已知水在 \\pu{14 ^\\circ C} 时的饱和蒸气压为 \\pu{12.0 mmHg}，试近似计算在 \\pu{100 ^\\circ C} 和 \\pu{14 ^\\circ C} 时，单位时间内通过单位面积水面的蒸发变为水蒸气的分子数之比 n\_{100}:n\_{14}。
...
1. **内能变化量**：两球完全相同，初始温度相同，都升高了相同的温度 \\Delta T，且膨胀后的体积相等。材质、初态和末态的温度与体积完全一致，因此两球的内能变化量相等，即 \\Delta U_A = \\Delta U_B。
1. **对外做功量**：小球在液体中膨胀，需要推开周围的液体，克服液体的压强做功。液体内部的压强为 p = p_0 + \\rho g h，其中 p_0 为大气压，\\rho 为液体密度，h 为深度。由于水银的密度远大于水的密度（\\rho\_{\\text{水银}} \\gg \\rho\_{\\text{水}}），两球被固定在同一深度 h，因此在整个膨胀过程中，水银中小球所承受的外部压强始终大于水中小球的外部压强。两球膨胀增加的体积相同，在压强更大的水银中膨胀需要做更多的功，即 W_B > W_A。
1. **结论**：将上述两点代入热力学第一定律公式。由于 \\Delta U 相同且 W_B > W_A，因此 Q_B > Q_A。即**水银中的小球吸收的热量更多**——为了达到相同的最终状态，水银中的小球需要额外吸收更多的热量来克服更大的外部压强做功。

#### 例题八

一根两端封闭的粗细不均匀玻璃管，上粗下细，竖直放置。管内有一段水银柱将理想气体分为 A（上方）和 B（下方）两部分。现缓慢升高温度，试判断水银柱的移动方向，并比较压强变化量 \\Delta p_A 与 \\Delta p_B、压力变化量 \\Delta F_A 与 \\Delta F_B 的大小关系。
...
\\dfrac{P}{T} = C \\Rightarrow \\Delta P = P \\cdot \\dfrac{\\Delta T}{T}

由于两部分气体初始温度 T 相同、升高的温度 \\Delta T 相同，压强增加量 \\Delta P 与初始压强 P 成正比。

根据初始状态的受力平衡，下方气体的压强需要支撑上方气体压强加上水银柱压强，即 P_B = P_A + \\rho g h，因此初始压强 P_B > P_A。

若体积不变，下方气体压强增加量更大（\\Delta P_B' > \\Delta P_A'），水银柱受到向上的净力，将**向上移动**。
...
因为 h' < h，所以 P_B' - P_A' < P_B - P_A，移项得 \\Delta P_B < \\Delta P_A，即上方气体压强增加量更大。

**第三步：比较压力变化量（本题难点）。**

上方气体对液面压力（向下）F_A = P_A \\cdot S_A，下方气体对液面压力（向上）F_B = P_B \\cdot S_B。
...
1. 水银柱自身重力 G（向下）
1. 下方气体压力 F_B（向上）
1. 管壁对水银的弹力在竖直方向的分力 F\_{\\text{壁}}

由于管子上粗下细（类似漏斗状），倾斜管壁对水银产生垂直于管壁斜向上的弹力，其竖直分量 F\_{\\text{壁}} 向上。平衡条件为：
...
取变化量，重力 G 不变（\\Delta G = 0），因此：

\\Delta F_A - \\Delta F_B = \\Delta F\_{\\text{壁}}

温度升高后，管内气体压强增大，水银向外挤压管壁的力变大，根据牛顿第三定律，管壁的托力 F\_{\\text{壁}} 也变大，即 \\Delta F\_{\\text{壁}} > 0。

代回等式得 \\Delta F_A - \\Delta F_B > 0，即 \\Delta F_A > \\Delta F_B。
