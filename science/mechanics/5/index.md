...
![alt text](../m-image-9.avif){width="40%"}
...
我们只需要求 [\$x=x_1+x_2\$]{.arithmatex} 的表达式即可，但是这通常是个相当复杂的问题，其中最简单的是在同一直线上两个相同频率的简谐振动的合成（[\$\omega_1=\omega_2\$]{.arithmatex}）。

此时我们发现两个式子都是关于 [\$\omega t\$]{.arithmatex} 的，我们不妨先将 [\$\cos\$]{.arithmatex} 的括号展开，然后直接对 [\$\cos\omega t\$]{.arithmatex} 和 [\$\sin\omega t\$]{.arithmatex} 合并同类项，然后应用辅助角公式即可得到 [\$x=A\cos(\omega t+\varphi)\$]{.arithmatex}，其中
...
![alt text](../m-image-14.avif){width="40%"}
...
简谐运动是最基本的振动，我们先对简谐运动下一个定义：如果物体的位移与时间的关系遵从正弦函数的规律，即它的振动图像（[\$x-t\$]{.arithmatex} 图像）是一条正弦曲线，这样的振动是一种简谐运动（**运动学定义**）。

在定义中已经描述了简谐运动的一个重要性质 [\$x-t\$]{.arithmatex} 图像，我们知道对正弦曲线求导还是正弦曲线，因此 [\$v-t\$]{.arithmatex} 图像、[\$a-t\$]{.arithmatex} 图像的性质很容易得出。
...
- [\$A\$]{.arithmatex} 是振幅，即最大位移。

- [\$\omega\$]{.arithmatex} 是角速度，代表振动的快慢，而完成一次全振动所需要的时间称为周期。
  ...
  对于仅受到劲度系数为 [\$k\$]{.arithmatex} 的理想弹簧的弹力的简谐运动，容易推导

  ::: {.arithmatex}
  \$\$ \omega=\sqrt{\dfrac{k}{m}} \$\$
  :::

- [\$\omega t+\varphi\$]{.arithmatex} 是相位，反应了 [\$t\$]{.arithmatex} 时刻的空间状态，若 [\$t=0\$]{.arithmatex} 则为初始状态，称为初相位。

  我们称一对频率相同的简谐运动的相位差为其相位之差的绝对值 [\$\Delta\varphi=\|\varphi_1-\varphi_2\|\$]{.arithmatex}。
...
通常来说，如果要求我们证明一个运动是简谐运动，通常来说只需要证明 [\$F\$]{.arithmatex} 与到某一点位移成正比即可，这个点即为平衡点，例如在天体中我们常常探讨的一个问题。
...
我们可以认为，简谐运动可以表示为匀速圆周运动在一个方向上的投影，而 [\$\omega\$]{.arithmatex} 正好对应圆周运动的角速度，我们似乎在数学中讨论了这个问题。

容易证明，简谐运动中的 [\$F=kx\$]{.arithmatex} 与圆周运动的关系式
...
多物体临界问题：通常来说，最大振幅 [\$x\_{\max}\$]{.arithmatex} 正比于最大拉力

::: {.arithmatex}
\$\$ x\_{\max}=\dfrac{F\_{\max}}{k} \$\$
:::

也就是 [\$F(x\_{\max})=F\_{\max}\$]{.arithmatex}。
...
根据圆的思想，有 [\$a=\omega\^2r\$]{.arithmatex}，其中 [\$r=x\$]{.arithmatex}，因此
...
具体的，我们带入 [\$v=0,x=A\$]{.arithmatex}，因此
...
不考虑一切**阻力**，一个**刚性轻绳**拴着一个**小球**做**角度很小**（一般来说小于 [\$5\^\circ\$]{.arithmatex} 左右，在这个角度内，我们可以取 [\$\sin x=x\$]{.arithmatex} 的近似）的摆动，这样的装置就叫作单摆。单摆是实际摆的理想化模型。
...
根据弧度制的定义，[\$\theta\$]{.arithmatex} 就是弧长除以绳长 [\$\ell\$]{.arithmatex}，而弧长可以近似看作位移，
...
注意：我们这里称绳长 [\$\ell\$]{.arithmatex} 是小球直径远小于绳长，因此在近似的情况下，[\$\ell\$]{.arithmatex} 应当取绳子悬挂点到下面物体的重心的距离，下面漏斗模型是非常经典的问题。

漏斗漏沙子模型：一条细绳拴着一个装满沙子的漏斗，做单摆运动，沙子缓慢均匀漏下，下方垫一块匀速运动的毯子，则沙子在毯子上留下的轨迹近似为一个正弦图像。

![alt text](../m-image-13.avif){width="60%"}

一些性质：

- 沙子不断漏下，因此整体的重心先下降再上升，因此 [\$\ell\$]{.arithmatex} 先增大后减小，周期也同样先增大后减小。

- 漏斗摆动的速度 [\$Q\$]{.arithmatex} 点大于 [\$P\$]{.arithmatex} 点，而单位时间内流下的沙子近似相等，因此 [\$P\$]{.arithmatex} 点积累的沙子的量比 [\$Q\$]{.arithmatex} 点多。
...
我们测的了 [\$T\$]{.arithmatex} 和 [\$\ell\$]{.arithmatex}，则利用公式

::: {.arithmatex}
\$\$ g=\dfrac{4\pi\^2}{T\^2}\cdot\ell \$\$
:::

或者多次测量作出 [\$T\^2-\ell\$]{.arithmatex} 图求斜率，因为图像斜率与 [\$\ell\$]{.arithmatex} 误差关系较小。
...
::: {.grid}
![alt text](../m-image-7.avif)

![alt text](../m-image-8.avif)
:::
...
在波的传播方向上，振动相位总是相同的两个相邻质点间的距离，叫作波长，通常用 [\$\lambda\$]{.arithmatex} 表示。
...
有自然定义频率 [\$f=\dfrac{1}{T}\$]{.arithmatex}，则波速的定义式：
...
其中 [\$\rho\$]{.arithmatex} 为单位长度绳的质量，[\$T\$]{.arithmatex} 为绳中的张力。因此对于一定长度的绳，我们可以通过调节绳的张力，改变声波的速度。

- 声音的远传现象：夜间声音传得更远。夜间地面温度低、空中温度高，声速下层慢、上层快，声波向下折射，不易扩散到高空。

- 海洋中的声道效应：海水中存在温度和压力梯度，形成声速最小的层面。声波被约束在该层传播，可用于远距离水下通信。

另外，声速还与温度有关，在 [\$\pu{0\^oC}\$]{.arithmatex} 的空气中约为 [\$\pu{330m/s}\$]{.arithmatex}，常温下的空气中约为 [\$\pu{340m/s}\$]{.arithmatex}，在水中约为 [\$\pu{1500m/s}\$]{.arithmatex}，在钢铁铝铜玻璃等固体中通常能达到 [\$\pu{5000m/s}\$]{.arithmatex} 甚至更高。

### 波的函数 {#_16}

简谐波：如果波的图像是正弦曲线，这样的波叫作正弦波，也叫简谐波。介质中有正弦波传播时，介质的质点在做简谐运动。

![alt text](../m-image-10.avif){width="80%"}

简谐波的波形图与质点的振动图像都是正弦曲线，但它们的意义是不同的。

- 如左，**振动图像**表示介质中"某一质点"在"各个时刻"的位移（[\$u-t\$]{.arithmatex}）。

- 如右，**波形图**表示介质中的"各个质点"在某"一时刻"的位移（[\$u-x\$]{.arithmatex}）。

我们做一些公式的推导，考虑以 [\$O\$]{.arithmatex} 为波源，设该处质点做简谐振动，其位移 [\$u\$]{.arithmatex} 与世界 [\$t\$]{.arithmatex} 的关系为：

::: {.arithmatex}
\$\$ u=A\cos\omega t \$\$
:::

考虑波速 [\$v\$]{.arithmatex} 传播到与 [\$O\$]{.arithmatex} 点距离为 [\$x\$]{.arithmatex} 的 [\$P\$]{.arithmatex} 点处，此处的相位与 [\$O\$]{.arithmatex} 相比落后了 [\$x/v\$]{.arithmatex} 的时间，又假设振幅相同，也就是说在 [\$P\$]{.arithmatex} 点的质点在 [\$t\$]{.arithmatex} 时刻位移为：

::: {.arithmatex}
\$\$ u\'=A\cos\omega\paren{t-\dfrac{x}{v}} \$\$
:::

不妨令 [\$k=\dfrac{2\pi}{\lambda}\$]{.arithmatex}，称为角波数（单位为 [\$\pu{rad/m}\$]{.arithmatex}），得到：
...
1.  **模型建立：** 假设波通过一个宽度为 [\$a\$]{.arithmatex} 的狭缝。
...
2.  **极小值条件：** 根据菲涅耳衍射理论，单缝衍射的第一级暗纹（能量极小值）出现的位置满足公式：
    ...
    - [\$a\$]{.arithmatex} 是缝宽。
    - [\$\lambda\$]{.arithmatex} 是波长。
    - [\$\theta\$]{.arithmatex} 是第一级暗纹相对于中心轴线的衍射角。

3.  **推导：**我们关注衍射角 [\$\theta\$]{.arithmatex}：

    ::: {.arithmatex}
    \$\$ \sin \theta = \frac{\lambda}{a} \$\$
    :::

    - 当缝宽 [\$a\$]{.arithmatex} 保持不变时（"经过同样的缝隙"），[\$\lambda\$]{.arithmatex} **增大**。
    - 因为 [\$\sin \theta\$]{.arithmatex} 与 [\$\lambda\$]{.arithmatex} 成正比，所以 [\$\sin \theta\$]{.arithmatex} 变大。
    - 在 [\$0\$]{.arithmatex} 到 [\$90\^\circ\$]{.arithmatex} 范围内，正弦函数是单调递增的，所以 [\$\theta\$]{.arithmatex} **变大**。

4.  **结论：** [\$\theta\$]{.arithmatex} 变大意味着波穿过缝隙后，向两侧扩散的角度更广，能量不再集中在直线方向，这就是"衍射现象更明显"。

#### 视角二：基于"惠更斯-菲涅耳原理"的相位干涉分析（微观机制视角） {#-}

**构想：** 把缝隙 [\$a\$]{.arithmatex} 想象成由无数个并排的"点波源"组成的队伍。

1.  **破坏性干涉（由亮变暗）：**波之所以会形成定向的束（不明显衍射），或者在某个角度消失（暗纹），是因为缝隙不同位置发出的子波在远处相互抵消了（反相干涉）。

    最简单的抵消条件是：缝隙**最上端**发出的波与**中心**发出的波，到达某点的路程差刚好是半个波长（[\$\lambda/2\$]{.arithmatex}）。此时一正一负，刚好抵消。

2.  **波长 [\$\lambda\$]{.arithmatex} 的尺子效应：**

    - **如果波长很短（[\$\lambda \ll a\$]{.arithmatex}）：** 只需要稍微偏离一点点角度，缝隙边缘和中心的路径差就能达到 [\$\lambda/2\$]{.arithmatex}。这意味着波刚想往旁边跑，就立刻被自己抵消掉了。所以波只能乖乖走直线。

    - **如果波长很长（[\$\lambda \approx a\$]{.arithmatex} 或 [\$\lambda \> a\$]{.arithmatex}）：** 波长太长了，即便你偏转了很大的角度（甚至偏转到 [\$90\^\circ\$]{.arithmatex}），缝隙边缘和中心的路径差可能还凑不够半个波长（[\$\Delta L \< \lambda/2\$]{.arithmatex}）。

    - **关键点：** 如果凑不够半个波长，就无法形成有效的"相消干涉"。

3.  **结论：** 因为波长太长，缝隙内的子波们无法通过相互抵消来限制波的扩散。既然"没人拦得住"（无法相消），波就会肆无忌惮地向各个方向传播，导致衍射极其明显。

![alt text](../e9057a15a2bbb76c_c2b0c7e.svg){width="50%"}
...
1.  **解释反射定律**：想象一束平面波（波面为直线）以入射角 [\$\theta_1\$]{.arithmatex} 射向界面。在波面 [\$AA\'\$]{.arithmatex} 上的各点，当 [\$A\$]{.arithmatex} 点到达界面时，[\$A\'\$]{.arithmatex} 点还未到达。经过一段时间 [\$\Delta t\$]{.arithmatex}，[\$A\$]{.arithmatex} 点发出的子波已经以半径 [\$v_1 \Delta t\$]{.arithmatex} 在介质 1 中传播，而 [\$A\'\$]{.arithmatex} 点刚好到达界面 [\$B\$]{.arithmatex} 点。连接所有子波的包络面（即过 [\$B\$]{.arithmatex} 点作 [\$A\$]{.arithmatex} 点子波的切线），就得到了新的反射波面。通过简单的几何关系，可以严格证明反射角 [\$\theta\' = \text{入射角 } \theta_1\$]{.arithmatex}。

2.  **解释折射定律**：同样的情景，但波进入了介质 2。当 [\$A\$]{.arithmatex} 点到达界面后，它发出的子波在介质 2 中以速度 [\$v_2\$]{.arithmatex} 传播。经过 [\$\Delta t\$]{.arithmatex}，[\$A\'\$]{.arithmatex} 点到达界面 [\$B\$]{.arithmatex} 点，此时 [\$A\$]{.arithmatex} 点在介质 2 中的子波半径为 [\$v_2 \Delta t\$]{.arithmatex}。连接包络面，通过几何关系可以证明折射定律。

::: {.grid}
![alt text](../997a0e28b6108d82_6fa4db1-56c4028.avif)

![alt text](../786ede037f970bf8_e2317df.svg)
:::
...
公式： [\$\Delta x \cdot \Delta p_y \approx h\$]{.arithmatex} （这里 [\$h\$]{.arithmatex} 代表某种波动常数，类比普朗克常数）。

1.  **位置限制（[\$\Delta x\$]{.arithmatex}）：**波通过缝隙，说明波在垂直方向上的位置被限制在缝宽 [\$a\$]{.arithmatex} 的范围内。所以，位置不确定度 [\$\Delta x \approx a\$]{.arithmatex}。

2.  **动量分散（[\$\Delta p_y\$]{.arithmatex}）：**根据不确定性原理，位置限制得越死，动量的不确定度就越大。
...
3.  **引入波长：**在波动学中，动量 [\$p\$]{.arithmatex} 与波长 [\$\lambda\$]{.arithmatex} 成反比（德布罗意关系 [\$p = h/\lambda\$]{.arithmatex}）。衍射角 [\$\theta\$]{.arithmatex} 大约等于横向动量与总动量的比值：
...
4.  **结论：**看最终公式 [\$\theta \approx \lambda/a\$]{.arithmatex}。很明显，**[\$\lambda\$]{.arithmatex} 越大，[\$\theta\$]{.arithmatex} 越大**。
...
![alt text](../m-image-3.avif){width="60%"}
...
![alt text](../m-image-4.avif){width="100%"}
...
如果起振方向相同，对于平面内一点 [\$P\$]{.arithmatex}，则比较 [\$\|\|PS_1\|-\|PS_2\|\|\$]{.arithmatex}（到两波源的距离之差）是 [\$\lambda/2\$]{.arithmatex} 的倍数的奇偶性即可，根据振动合成的公式可以推导得出两个特殊的位置：
...
- **振动加强点（线）**
  ...
  其中 [\$k\in\Z\$]{.arithmatex}，即

  ::: {.arithmatex}
  \$\$ \|\|PS_1\|-\|PS_2\|\|=k\lambda \$\$
  :::

  则 [\$P\$]{.arithmatex} 点为振动加强点。
...
![alt text](../m-image-16.avif){width="60%"}
...
我们发现，入射波的反射点 [\$b\$]{.arithmatex} 是固定不动的，所以必为波节。这表明入射波与反射波在 [\$b\$]{.arithmatex} 点的相位一定始终相反，即入射波在固定端发生反射时，相位发生了 [\$\pi\$]{.arithmatex} 的突变。由于在波的传播中，相距半个波长的两质元的相位差为 [\$\pi\$]{.arithmatex}，所以相位为 [\$\pi\$]{.arithmatex} 的突变，就相当于"损失"（或"附加"）了半个波长，因此通常又把这种现象称为**半波损失**。当然，这只是一种形象的说法，并非是波在实际上少传播或多传播了半个波长的距离。

通过一些推导，我们可以知道相邻波腹或波节之间的距离为 [\$\dfrac{\lambda}{2}\$]{.arithmatex}，振动的最大振幅是 [\$2A\$]{.arithmatex}。
...
在弦乐器中，弦的两端被固定，所以弦的两端必然是波节，因此长度为 [\$L\$]{.arithmatex} 上的弦形成的驻波的波长与 [\$L\$]{.arithmatex} 应有一定的关系，而并非所有的波长都能形成驻波。我们发现，弦长恰好为半波长的整数倍，即
...
- [\$f\'\$]{.arithmatex} 为观察者观察到的频率。

- [\$f\$]{.arithmatex} 为波源于该介质中的原始发射频率。

- [\$v\$]{.arithmatex} 为波在该介质中的传播速度。

- [\$v\_{\text{o}}\$]{.arithmatex}（Observer）为观察者相对于介质的移动速度，若接近发射源则前方运算符号为 [\$+\$]{.arithmatex} 号，反之则为 [\$−\$]{.arithmatex} 号；

- [\$v\_{\text{s}}\$]{.arithmatex}（Source）为发射源相对于介质的移动速度，若接近观察者则前方运算符号为 [\$−\$]{.arithmatex} 号，反之则为 [\$+\$]{.arithmatex} 号。
...
当波源以超波速运动时，即 [\$v\_{\text{s}}\>v\$]{.arithmatex}，因为在这种情况下，在任意时刻波源本身都将超过它此前发出的波前，所以在波源前方不可能有任何波动产生。

![alt text](../m-image-15.avif){width="40%"}

如图，波源在经过 [\$S\$]{.arithmatex} 点时发出的波，在其后 [\$t\$]{.arithmatex} 时刻的波阵面为半径为 [\$vt\$]{.arithmatex} 都球面，波源次此时已经前进了 [\$v\_{\text{s}}t\$]{.arithmatex} 的距离到达 [\$S\'\$]{.arithmatex} 点，在整个 [\$t\$]{.arithmatex} 时间内，波源发出的波所到达的前沿形成了一个圆锥面。这个锥形称为马赫锥，马赫锥是波的前缘，其外没有波动产生，这种形式的波动叫做**艏波**。这个锥形的顶角 [\$\alpha\$]{.arithmatex} 有公式
