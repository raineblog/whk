...
我们只需要求 *x* = *x*1 + *x*2 的表达式即可，但是这通常是个相当复杂的问题，其中最简单的是在同一直线上两个相同频率的简谐振动的合成（*ω*1 = *ω*2）。

此时我们发现两个式子都是关于 *ω\*\*t* 的，我们不妨先将 cos  的括号展开，然后直接对 cos *ω\*\*t* 和 sin *ω\*\*t* 合并同类项，然后应用辅助角公式即可得到 *x* = *A*cos (*ω\*\*t* + *φ*)，其中
...
- *ω\*\*t* + *φ* 是相位，反应了 *t* 时刻的空间状态，若 *t* = 0 则为初始状态，称为初相位。

  我们称一对频率相同的简谐运动的相位差为其相位之差的绝对值 *Δ\*\*φ* = |*φ*1 − *φ*2|。
...
多物体临界问题：通常来说，最大振幅 *x*max 正比于最大拉力
...
不考虑一切**阻力**，一个**刚性轻绳**拴着一个**小球**做**角度很小**（一般来说小于 5∘ 左右，在这个角度内，我们可以取 sin *x* = *x* 的近似）的摆动，这样的装置就叫作单摆。单摆是实际摆的理想化模型。
...
或者多次测量作出 *T*2 − *ℓ* 图求斜率，因为图像斜率与 *ℓ* 误差关系较小。
...
1. **波长 *λ* 的尺子效应：**
   ...
   - **如果波长很长（*λ* ≈ *a* 或 *λ* > *a*）：** 波长太长了，即便你偏转了很大的角度（甚至偏转到 90∘），缝隙边缘和中心的路径差可能还凑不够半个波长（*Δ\*\*L* \< *λ*/2）。
...
1. **解释反射定律**：想象一束平面波（波面为直线）以入射角 *θ*1 射向界面。在波面 *A\*\*A*′ 上的各点，当 *A* 点到达界面时，*A*′ 点还未到达。经过一段时间 *Δ\*\*t*，*A* 点发出的子波已经以半径 *v*1*Δ\*\*t* 在介质 1 中传播，而 *A*′ 点刚好到达界面 *B* 点。连接所有子波的包络面（即过 *B* 点作 *A* 点子波的切线），就得到了新的反射波面。通过简单的几何关系，可以严格证明反射角 *θ*′ = 入射角 *θ*1。
1. **解释折射定律**：同样的情景，但波进入了介质 2。当 *A* 点到达界面后，它发出的子波在介质 2 中以速度 *v*2 传播。经过 *Δ\*\*t*，*A*′ 点到达界面 *B* 点，此时 *A* 点在介质 2 中的子波半径为 *v*2*Δ\*\*t*。连接包络面，通过几何关系可以证明折射定律。
...
公式： *Δ\*\*x* ⋅ *Δ**p**y* ≈ *h* （这里 *h* 代表某种波动常数，类比普朗克常数）。

1. \*\*位置限制（*Δ\*\*x*）：\*\*波通过缝隙，说明波在垂直方向上的位置被限制在缝宽 *a* 的范围内。所以，位置不确定度 *Δ\*\*x* ≈ *a*。

1. \*\*动量分散（*Δ**p**y*）：\*\*根据不确定性原理，位置限制得越死，动量的不确定度就越大。
...
如果起振方向相同，对于平面内一点 *P*，则比较 ||*P\*\*S*1|−|*P\*\*S*2||（到两波源的距离之差）是 *λ*/2 的倍数的奇偶性即可，根据振动合成的公式可以推导得出两个特殊的位置：
...
通过一些推导，我们可以知道相邻波腹或波节之间的距离为 $\\dfrac{\\lambda}{2}$，振动的最大振幅是 2*A*。
...
- *f* 为波源于该介质中的原始发射频率。
- *v* 为波在该介质中的传播速度。
- *v*o（Observer）为观察者相对于介质的移动速度，若接近发射源则前方运算符号为
  - 号，反之则为 − 号；
- *v*s（Source）为发射源相对于介质的移动速度，若接近观察者则前方运算符号为 − 号，反之则为 + 号。
...
当波源以超波速运动时，即 *v*s > *v*，因为在这种情况下，在任意时刻波源本身都将超过它此前发出的波前，所以在波源前方不可能有任何波动产生。

如图，波源在经过 *S* 点时发出的波，在其后 *t* 时刻的波阵面为半径为 *v\*\*t* 都球面，波源次此时已经前进了 *v*s*t* 的距离到达 *S*′ 点，在整个 *t* 时间内，波源发出的波所到达的前沿形成了一个圆锥面。这个锥形称为马赫锥，马赫锥是波的前缘，其外没有波动产生，这种形式的波动叫做**艏波**。这个锥形的顶角 *α* 有公式
