...
![alt text](/static/image/m-image-9.57f4e3a4dc.avif){width="40%"}
...
![alt text](/static/image/m-image-14.3cee308a51.avif){width="40%"}
振动系统能量衰减的方式通常有两种。
...
根据弧度制的定义，$theta$ 就是弧长除以绳长 $ell$，而弧长可以近似看作位移，
...
![alt text](/static/image/m-image-13.704bf3501d.avif){width="60%"}
一些性质：
...
![alt text](/static/image/m-image-1.91469f49ea.avif){width="40%"}
...
![alt text](/static/image/m-image-10.221d3ef189.avif){width="80%"}
...
![alt text](/static/image/m-image-2.65373ece43.avif){width="100%"}
实验表明，只有缝、孔的宽度或障碍物的尺寸跟波长相差不多，或者**比波长更小**时，才能观察到明显的衍射现象。注意，障碍物越小，衍射现象越明显，但是缝、孔的宽度过小，也会阻挡波，因此不能说缝、孔的宽度越小，衍射现象越明显。

![alt text](/static/image/m-image-12.85340622ec.avif){width="80%"}
...
3. \*\*推导：\*\*我们关注衍射角 $theta$：
   ...
   - 在 $0$ 到 $90^\circ$ 范围内，正弦函数是单调递增的，所以 $theta$ **变大**。

4. **结论：** $theta$ 变大意味着波穿过缝隙后，向两侧扩散的角度更广，能量不再集中在直线方向，这就是"衍射现象更明显"。
...
![alt text](/static/svg/e9057a15a2bbb76c_c2b0c7e.fd53db4a85.svg){width="50%"}
...
1. **解释反射定律**：想象一束平面波（波面为直线）以入射角 $theta\_1$ 射向界面。在波面 $AA'$ 上的各点，当 $A$ 点到达界面时，$A'$ 点还未到达。经过一段时间 $Delta t$，$A$ 点发出的子波已经以半径 $v\_1 \Delta t$ 在介质 1 中传播，而 $A'$ 点刚好到达界面 $B$ 点。连接所有子波的包络面（即过 $B$ 点作 $A$ 点子波的切线），就得到了新的反射波面。通过简单的几何关系，可以严格证明反射角 $theta' = \text\{入射角 } \theta\_1$。
...
3. \*\*引入波长：\*\*在波动学中，动量 $p$ 与波长 $lambda$ 成反比（德布罗意关系 $p = h/\lambda$）。衍射角 $theta$ 大约等于横向动量与总动量的比值：
...
4. **结论：**看最终公式 $theta \approx \lambda/a$。很明显，**$lambda$ 越大，$theta$ 越大**。
...
![alt text](/static/image/m-image-3.9ab6295a0f.avif){width="60%"}
...
![alt text](/static/image/m-image-4.21e217eeb6.avif){width="100%"}
...
![alt text](/static/image/m-image-16.9913067dc8.avif){width="60%"}
...
![alt text](/static/image/m-image-17.3e4409f15b.avif){width="60%"}
...
![alt text](/static/image/m-image-15.ea716c3bcb.avif){width="40%"}
