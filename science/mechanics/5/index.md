...
{x1​x2​​=A1​cos(ω1​t+φ1​)=A2​cos(ω2​t+φ2​)​

我们只需要求 x=x1​+x2​ 的表达式即可，但是这通常是个相当复杂的问题，其中最简单的是在同一直线上两个相同频率的简谐振动的合成（ω1​=ω2​）。

此时我们发现两个式子都是关于 ωt 的，我们不妨先将 cos 的括号展开，然后直接对 cosωt 和 sinωt 合并同类项，然后应用辅助角公式即可得到 x=Acos(ωt+φ)，其中
...
xv=x˙a=x¨​=Asin(ωt+φ)=Aωcos(ωt+φ)=−Aω2sin(ωt+φ)​
...
- ω 是角速度，代表振动的快慢，而完成一次全振动所需要的时间称为周期。
...
- ωt+φ 是相位，反应了 t 时刻的空间状态，若 t=0 则为初始状态，称为初相位。

  我们称一对频率相同的简谐运动的相位差为其相位之差的绝对值 Δφ=∣φ1​−φ2​∣。
...
我们可以认为，简谐运动可以表示为匀速圆周运动在一个方向上的投影，而 ω 正好对应圆周运动的角速度，我们似乎在数学中讨论了这个问题。
...
多物体临界问题：通常来说，最大振幅 xmax​ 正比于最大拉力
...
根据圆的思想，有 a=ω2r，其中 r=x，因此

ω=mk​​

因此

E=21​mv2+21​kx2=Const.
...
不考虑一切**阻力**，一个**刚性轻绳**拴着一个**小球**做**角度很小**（一般来说小于 5∘ 左右，在这个角度内，我们可以取 sinx=x 的近似）的摆动，这样的装置就叫作单摆。单摆是实际摆的理想化模型。

我们尝试写出单摆的回复力，我们知道在单摆中，回复力就是切线方向上的分力，即

F=mgsinθ≈mgθ

根据弧度制的定义，θ 就是弧长除以绳长 ℓ，而弧长可以近似看作位移，
...
注意：我们这里称绳长 ℓ 是小球直径远小于绳长，因此在近似的情况下，ℓ 应当取绳子悬挂点到下面物体的重心的距离，下面漏斗模型是非常经典的问题。

漏斗漏沙子模型：一条细绳拴着一个装满沙子的漏斗，做单摆运动，沙子缓慢均匀漏下，下方垫一块匀速运动的毯子，则沙子在毯子上留下的轨迹近似为一个正弦图像。

一些性质：

- 沙子不断漏下，因此整体的重心先下降再上升，因此 ℓ 先增大后减小，周期也同样先增大后减小。
...
或者多次测量作出 T2−ℓ 图求斜率，因为图像斜率与 ℓ 误差关系较小。
...
其中 ρ 为单位长度绳的质量，T 为绳中的张力。因此对于一定长度的绳，我们可以通过调节绳的张力，改变声波的速度。

- 声音的远传现象：夜间声音传得更远。夜间地面温度低、空中温度高，声速下层慢、上层快，声波向下折射，不易扩散到高空。
- 海洋中的声道效应：海水中存在温度和压力梯度，形成声速最小的层面。声波被约束在该层传播，可用于远距离水下通信。

另外，声速还与温度有关，在 \\pu0oC 的空气中约为 \\pu330m/s，常温下的空气中约为 \\pu340m/s，在水中约为 \\pu1500m/s，在钢铁铝铜玻璃等固体中通常能达到 \\pu5000m/s 甚至更高。
...
不妨令 k=λ2π​，称为角波数（单位为 \\purad/m），得到：

u′=Acos(ωt−kx)

经过推导，我们还可以得出一个常用的公式：

u′=Acos2π(Tt​−λx​)=Acos2π(ft−λx​)
...
1. **极小值条件：** 根据菲涅耳衍射理论，单缝衍射的第一级暗纹（能量极小值）出现的位置满足公式：
...
1. \*\*推导：\*\*我们关注衍射角 θ：
   ...
   - 当缝宽 *a* 保持不变时（“经过同样的缝隙”），*λ* **增大**。
   - 因为 sin *θ* 与 *λ* 成正比，所以 sin *θ* 变大。
   - 在 0 到 90∘ 范围内，正弦函数是单调递增的，所以 *θ* **变大**。

1. **结论：** θ 变大意味着波穿过缝隙后，向两侧扩散的角度更广，能量不再集中在直线方向，这就是“衍射现象更明显”。
...
1. \*\*破坏性干涉（由亮变暗）：\*\*波之所以会形成定向的束（不明显衍射），或者在某个角度消失（暗纹），是因为缝隙不同位置发出的子波在远处相互抵消了（反相干涉）。
   ...
   最简单的抵消条件是：缝隙**最上端**发出的波与**中心**发出的波，到达某点的路程差刚好是半个波长（λ/2）。此时一正一负，刚好抵消。

1. **波长 *λ* 的尺子效应：**

   - **如果波长很短（*λ* ≪ *a*）：** 只需要稍微偏离一点点角度，缝隙边缘和中心的路径差就能达到 λ/2。这意味着波刚想往旁边跑，就立刻被自己抵消掉了。所以波只能乖乖走直线。
   - **如果波长很长（*λ* ≈ *a* 或 *λ* > *a*）：** 波长太长了，即便你偏转了很大的角度（甚至偏转到 90∘），缝隙边缘和中心的路径差可能还凑不够半个波长（ΔL\<λ/2）。
   - **关键点：** 如果凑不够半个波长，就无法形成有效的“相消干涉”。

1. **结论：** 因为波长太长，缝隙内的子波们无法通过相互抵消来限制波的扩散。既然“没人拦得住”（无法相消），波就会肆无忌惮地向各个方向传播，导致衍射极其明显。
...
1. **解释反射定律**：想象一束平面波（波面为直线）以入射角 θ1​ 射向界面。在波面 AA′ 上的各点，当 A 点到达界面时，A′ 点还未到达。经过一段时间 Δt，A 点发出的子波已经以半径 v1​Δt 在介质 1 中传播，而 A′ 点刚好到达界面 B 点。连接所有子波的包络面（即过 B 点作 A 点子波的切线），就得到了新的反射波面。通过简单的几何关系，可以严格证明反射角 θ′=入射角 θ1​。
1. **解释折射定律**：同样的情景，但波进入了介质 2。当 A 点到达界面后，它发出的子波在介质 2 中以速度 v2​ 传播。经过 Δt，A′ 点到达界面 B 点，此时 A 点在介质 2 中的子波半径为 v2​Δt。连接包络面，通过几何关系可以证明折射定律。
...
公式： Δx⋅Δpy​≈h （这里 h 代表某种波动常数，类比普朗克常数）。

1. \*\*位置限制（Δx）：\*\*波通过缝隙，说明波在垂直方向上的位置被限制在缝宽 a 的范围内。所以，位置不确定度 Δx≈a。

1. \*\*动量分散（Δpy​）：\*\*根据不确定性原理，位置限制得越死，动量的不确定度就越大。

   Δpy​≈ah​

   这意味着波（或粒子）在垂直方向上获得了一个随机的动量分量，导致它不再单纯沿水平方向飞，而是有了横向速度，这就是衍射（扩散）。

1. \*\*引入波长：\*\*在波动学中，动量 p 与波长 λ 成反比（德布罗意关系 p=h/λ）。衍射角 θ 大约等于横向动量与总动量的比值：
...
1. \*\*结论：\*\*看最终公式 *θ* ≈ *λ*/*a*。很明显，***λ* 越大，*θ* 越大**。
...
如果起振方向相同，对于平面内一点 P，则比较 ∣∣PS1​∣−∣PS2​∣∣（到两波源的距离之差）是 λ/2 的倍数的奇偶性即可，根据振动合成的公式可以推导得出两个特殊的位置：
...
主动降噪技术：声音是一种波，通过波的干涉能够起到消音的作用。这一原理可以使我们主动出击，通过发出与噪声振幅、频率相同，相位相反的声波来抵消噪声。简单来说，在耳机内设有麦克风，用来收集周围环境中的噪声信号，在此基础上，耳机的处理器能够预测下一时刻噪声的情况，并产生相应的抵消声波。
...
我们发现，入射波的反射点 b 是固定不动的，所以必为波节。这表明入射波与反射波在 b 点的相位一定始终相反，即入射波在固定端发生反射时，相位发生了 π 的突变。由于在波的传播中，相距半个波长的两质元的相位差为 π，所以相位为 π 的突变，就相当于“损失”（或“附加”）了半个波长，因此通常又把这种现象称为**半波损失**。当然，这只是一种形象的说法，并非是波在实际上少传播或多传播了半个波长的距离。

通过一些推导，我们可以知道相邻波腹或波节之间的距离为 2λ​，振动的最大振幅是 2A。
...
- f 为波源于该介质中的原始发射频率。
- v 为波在该介质中的传播速度。
- vo​（Observer）为观察者相对于介质的移动速度，若接近发射源则前方运算符号为 + 号，反之则为 − 号；
- vs​（Source）为发射源相对于介质的移动速度，若接近观察者则前方运算符号为 − 号，反之则为 + 号。
...
当波源以超波速运动时，即 vs​>v，因为在这种情况下，在任意时刻波源本身都将超过它此前发出的波前，所以在波源前方不可能有任何波动产生。

如图，波源在经过 S 点时发出的波，在其后 t 时刻的波阵面为半径为 vt 都球面，波源次此时已经前进了 vs​t 的距离到达 S′ 点，在整个 t 时间内，波源发出的波所到达的前沿形成了一个圆锥面。这个锥形称为马赫锥，马赫锥是波的前缘，其外没有波动产生，这种形式的波动叫做**艏波**。这个锥形的顶角 α 有公式
