...
在这条定律的数学表达式中，力失 **F** 是物体所受的合外力；并认定，质量具有不变性，是一个常数；力与加速度之间的关系是瞬时关系，即某一时刻的力决定了这一时刻的加速度。

多物体牛二：在 **F** = *m***a** 中，左侧 **F** 表示合外力，右侧等号应为 *m***a** 的矢量和即可，证明略。
...
设参考系 *S*′ 相对于惯性参考系 *S* 以不变的加速度 **a**0 运动。

质量为 *m* 的质点相对于参考系 *S*′、*S* 的加速度分别为 **a**′、**a**，他们满足关系：
...
其中 **F***惯* = −*m***a**0，称为惯性力，可以理解为受到与加速度方向相反的大小为 *m\*\*a*0 的力。
...
这个原理推广了惯性系中的情况。比如，一个关在正自由落体的电梯中的观察者，会发现只要不知道电梯外的情况，自己就是个有效的惯性系，即使此时他正受引力作用加速下落（注：当然此时的 *g* = *g*0 − *a*，其中 *g*0 表示原来的重力加速度，*a* 表示加速下落的加速度）。
...
*W* = *F⃗\*\*x⃗* = *F\*\*x*cos *θ* = *F**x**F*

其中 *θ* 表示力与位移的夹角，*x\*\*F* 表示在力 *F* 的方向上的位移。

一般来说（对于简单题），直线运动用 *W* = *F\*\*x*cos *θ*，曲线运动用 *W* = *F**x**F*。

做功正负：正负号取决于 cos *θ* 的大小，即
...
一定注意到功率的变化量是与 *F*、*v*、*θ* 都有关的。

## 能量

### 动能

物体因为运动而具有的能量，单位为焦耳 J。
...
机械能是指宏观物质所表现出的势能 *E\*\*p* 与动能 *E\*\*k* 的总和，即：
...
杆子和绳子这种同步转动的，关联速度 *v* = *r\*\*ω*（同一物体 *ω* 相同）。
...
1. 弹簧压缩拉伸，形变量不变，做功为零。
1. 弹簧产生的阻力与动力相等时，为加速度反向的点，注意不一定产生弹力即反向加速。
