...
一、一本书稳定地置放在桌子上，书的重量 [\$G\$]{.arithmatex} 与桌子施加于书本的力 [\$N\$]{.arithmatex} 是一对反作用力。

重力本质是由万有引力（近似）提供的，因此 [\$G\$]{.arithmatex} 的反作用力应该是书本对地球的吸引。桌子对书的力 [\$N\$]{.arithmatex} 的反作用力是，书对桌子的压力 [\$N\$]{.arithmatex}。
...
在这条定律的数学表达式中，力失 [\$\bm F\$]{.arithmatex} 是物体所受的合外力；并认定，质量具有不变性，是一个常数；力与加速度之间的关系是瞬时关系，即某一时刻的力决定了这一时刻的加速度。

多物体牛二：在 [\$\bm{F}=m\bm{a}\$]{.arithmatex} 中，左侧 [\$\bm{F}\$]{.arithmatex} 表示合外力，右侧等号应为 [\$m\bm{a}\$]{.arithmatex} 的矢量和即可，证明略。
...
设参考系 [\$S\'\$]{.arithmatex} 相对于惯性参考系 [\$S\$]{.arithmatex} 以不变的加速度 [\$\bm{a}\_0\$]{.arithmatex} 运动。

质量为 [\$m\$]{.arithmatex} 的质点相对于参考系 [\$S\'\$]{.arithmatex}、[\$S\$]{.arithmatex} 的加速度分别为 [\$\bm a\'\$]{.arithmatex}、[\$\bm a\$]{.arithmatex}，他们满足关系：

::: {.arithmatex}
\$\$ \bm{a}=\bm{a}\'+\bm{a}\_0 \$\$
:::

在惯性系 [\$S\$]{.arithmatex}，牛顿第二定律成立，

::: {.arithmatex}
\$\$ \bm{F}=m\bm{a}=m\bm{a}\'+m\bm{a}\_0 \$\$
:::

在非惯性系 [\$S\'\$]{.arithmatex}，有动力学方程，

::: {.arithmatex}
\$\$ m\bm{a}\'=\bm{F}-m\bm{a}\_0=\bm{F}+\bm{F}\_{惯} \$\$
:::

其中 [\$\bm{F}\_{惯}=-m\bm{a}\_0\$]{.arithmatex}，称为惯性力，可以理解为受到与加速度方向相反的大小为 [\$ma_0\$]{.arithmatex} 的力。

这个方程说明，如果把等式右边看成是质点在平动加速参照系 [\$S\'\$]{.arithmatex} 中受到的合力，那么在 [\$S\'\$]{.arithmatex} 系中仍能在形式上运用牛顿第二定律求解动力学问题。等式右边第一项 [\$F\$]{.arithmatex} 为质点所受真实力的合力，第二项则是惯性力。
...
这个原理推广了惯性系中的情况。比如，一个关在正自由落体的电梯中的观察者，会发现只要不知道电梯外的情况，自己就是个有效的惯性系，即使此时他正受引力作用加速下落（注：当然此时的 [\$g=g_0-a\$]{.arithmatex}，其中 [\$g_0\$]{.arithmatex} 表示原来的重力加速度，[\$a\$]{.arithmatex} 表示加速下落的加速度）。
...
**惯性质量**：一个物体的惯性质量决定它受力时的加速度。根据牛顿运动第二定律，假设一个物体受到一个力 [\$F\$]{.arithmatex}，其加速度为 [\$a\$]{.arithmatex}，则其惯性质量。
...
其中，[\$G\$]{.arithmatex} 表示万有引力常数，这个质量常称为引力质量。
...
其中 [\$\theta\$]{.arithmatex} 表示力与位移的夹角，[\$x_F\$]{.arithmatex} 表示在力 [\$F\$]{.arithmatex} 的方向上的位移。

一般来说（对于简单题），直线运动用 [\$W=Fx\cos\theta\$]{.arithmatex}，曲线运动用 [\$W=Fx_F\$]{.arithmatex}。

做功正负：正负号取决于 [\$\cos\theta\$]{.arithmatex} 的大小，即
...
约束力决定了系统中对象的位移，将其限制在范围内。它消除了在该方向上所有的位移，即物体平行此力的速度被约束为 [\$0\$]{.arithmatex}，因此约束力不对系统做功。

例如：用一根绳子系上一个小球做匀速圆周运动，小球会受到来自绳子，方向指向圆心的一个向心力。这个力的方向和球速度的方向垂直，所以这个力不做功（[\$W=0\$]{.arithmatex}）。
...
一定注意到功率的变化量是与 [\$F\$]{.arithmatex}、[\$v\$]{.arithmatex}、[\$\theta\$]{.arithmatex} 都有关的。

## 能量 {#_17}

### 动能 {#_18}

物体因为运动而具有的能量，单位为焦耳 [\$\mathrm{J}\$]{.arithmatex}。

::: {.arithmatex}
\$\$ E_k=\frac{1}{2}mv\^2 \$\$
:::

其中 [\$v\$]{.arithmatex} 表示速率，即瞬时速度。
...
理想情况下，一对平衡力做总功 [\$0\$]{.arithmatex}，除了摩擦力，因为产生的热量属于损耗。
...
机械能是指宏观物质所表现出的势能 [\$E_p\$]{.arithmatex} 与动能 [\$E_k\$]{.arithmatex} 的总和，即：
...
- 问一个点的信息：一个 [\$v\$]{.arithmatex} 已知的点、这个点。
- 问一个段的信息：两个 [\$v\$]{.arithmatex} 已知的点。
...
轻绳、轻杆（[\$m=0\$]{.arithmatex}）没有重力势能、动能。

因此其无法具有能量，总功 [\$W=0\$]{.arithmatex}，只起到传递能量的作用。

据此，我们可以整体的分析，对多个物体同时分析。

轻杆轻绳的力做功直接忽略不计，列功动能定理，列关联速度。

杆子和绳子这种同步转动的，关联速度 [\$v=r\omega\$]{.arithmatex}（同一物体 [\$\omega\$]{.arithmatex} 相同）。
...
本质：势能（[\$E_p\$]{.arithmatex}）和动能（[\$E_k\$]{.arithmatex}）的相互转化。
...
其中 [\$v_i\$]{.arithmatex} 为相对质心（[\$v_c\$]{.arithmatex}）的速度。
