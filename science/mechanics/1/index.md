...
其中 [\$A\$]{.arithmatex}、[\$B\$]{.arithmatex}、[\$C\$]{.arithmatex} 为作用在一点（假设为 [\$P\$]{.arithmatex}）力的大小。

而 [\$\alpha\$]{.arithmatex}、[\$\beta\$]{.arithmatex}、[\$\gamma\$]{.arithmatex} 则为在 [\$A\$]{.arithmatex}、[\$B\$]{.arithmatex}、[\$C\$]{.arithmatex} 力相对角的角度。
...
5.  质心的定义表达式：
    ...
    这是在某个确定的坐标下写出的，其中 [\$r_i\$]{.arithmatex} 为第 [\$i\$]{.arithmatex} 个质点在该坐标系下的位置，[\$m_i\$]{.arithmatex} 为第 [\$i\$]{.arithmatex} 个质点的质量，[\$M\$]{.arithmatex} 为总质量。
...
- 公式：[\$\bm F=-k\bm x\$]{.arithmatex}，其中 [\$k\$]{.arithmatex} 称为劲度系数，单位 [\$\pu{N/m}\$]{.arithmatex}。
...
- 主动力，可以写成一个近似的表达式：
  ...
  其中 [\$N\$]{.arithmatex} 为两个相互接触表面的法向力（即正压力），[\$\mu_k\$]{.arithmatex} 为滑动摩擦系数。
...
- 被动力，有近似上限：
  ...
  一般静摩擦系数 [\$\mu_s\$]{.arithmatex} 稍大于滑动摩擦系数 [\$\mu_k\$]{.arithmatex}。

  如果没有特殊说明，可以认为 [\$\mu_s=\mu_k\$]{.arithmatex}，统称为摩擦系数 [\$\mu\$]{.arithmatex}。

- 静摩擦力常作为一种通过接触面作用的未知的切向力，它的大小取决于周围其它物体施加的作用力和该物体的运动情况。只有在临界状态，即该物体在接触面处将产生相对滑动、又未产生滑动的临界运动状态时，静摩擦力等于最大静摩擦，[\$f_s=\mu_sN\$]{.arithmatex}。
...
- 在滑动摩擦下，称 [\$\varphi=\arctan(f_k/N)\$]{.arithmatex} 为滑动摩擦角，因此 [\$\mu=\tan\varphi\$]{.arithmatex}。

- 我们将滑动摩擦力和支持力合并为一个效果力，称为全反力 [\$F_R\$]{.arithmatex}。全反力和支持力的夹角即为摩擦角 [\$\varphi\$]{.arithmatex}，只要物体发生滑动摩擦力，无论支持力是多少，摩擦角都是不变的。

- 一物置于倾角为 [\$\theta\$]{.arithmatex} 的粗糙斜面上，[\$\theta\<\varphi\$]{.arithmatex} 则物体不会自行滑下，这一现象称为自锁。

另外，在静摩擦下，称 [\$\varphi=\arctan(f\_{sm}/N)\$]{.arithmatex} 为静摩擦角，[\$f\_{sm}\$]{.arithmatex} 为最大静摩擦力。

### 摩擦力做功 {#_9}

我们知道摩擦力做功通常可以表示为摩擦力乘上相对路程，也可以表示为一对摩擦力做的总功。

一对相互作用力做功之和​：考虑两个物体 [\$A\$]{.arithmatex} 和 [\$B\$]{.arithmatex}，[\$A\$]{.arithmatex} 受到 [\$F_A\$]{.arithmatex}，[\$B\$]{.arithmatex} 受到 [\$F_B\$]{.arithmatex}，[\$F_A\$]{.arithmatex} 和 [\$F_B\$]{.arithmatex} 是一对相互作用力。那么虽然 [\$F_A\$]{.arithmatex} 和 [\$F_B\$]{.arithmatex} 可能是变力，但由于他们时刻相等，可以考虑微分，设两个物体经过微小的位移，此时可以认为它们为大小不变的恒力。
...
设 [\$A\$]{.arithmatex} 经过微小位移 [\$\mathrm d \overrightarrow{x_A}\$]{.arithmatex}，[\$B\$]{.arithmatex} 经过微小位移 [\$\mathrm d \overrightarrow{x_B}\$]{.arithmatex}，则两力做功之和为
...
设 [\$A\$]{.arithmatex} 相对于 [\$B\$]{.arithmatex} 的运动路径是 [\$L\$]{.arithmatex}，对上式进行第二类曲线积分可得

::: {.arithmatex}
\$\$ W_A + W_B = \int_L \overrightarrow{F_A} \cdot \mathrm d l \$\$
:::

即一对相互作用力做功之和，只与力和 [\$A\$]{.arithmatex} 相对 [\$B\$]{.arithmatex} 运动的路径有关。注意到 [\$A\$]{.arithmatex} 相对 [\$B\$]{.arithmatex} 运动的路径是与选取的参考系无关的，因此 一对相互作用力做功之和与参考系无关。这又是一个非常特殊，经典且重要的一个做功与参考系无关的特例------两个与参考系都有关的力，其做功之和可能与参考系无关。

上式也相当于令 [\$B\$]{.arithmatex} 为参考系的特殊情形，此时由于 [\$B\$]{.arithmatex} 静止，[\$F_B\$]{.arithmatex} 不做功，这对相互作用力的做功之和就是 [\$F_A\$]{.arithmatex} 的做功。这提示我们计算一对相互作用力做功之和，可以考虑换系，以其中一个物体为参考系，则要计算的和简化为一个功。
...
两个磁铁相互吸引，令两磁铁静止，相互作用力做功之和为零。因此，[\$A\$]{.arithmatex} 对 [\$B\$]{.arithmatex} 做功 [\$W\$]{.arithmatex}，则 [\$B\$]{.arithmatex} 对 [\$A\$]{.arithmatex} 做功 [\$-W\$]{.arithmatex} 这种说法是完全错误的。
...
考虑一辆以 [\$v_车 = \pu{100 m/s}\$]{.arithmatex} 的速度匀速运行的火车上，一个人以 [\$\pu{1N}\$]{.arithmatex} 的力 [\$F\$]{.arithmatex} 推动原先静止的，质量为 [\$m = \pu{2kg}\$]{.arithmatex} 的箱子，使得箱子加速运动到 [\$v = \pu{1 m/s}\$]{.arithmatex}，火车地面光滑。

- 火车参考系上，箱子的动能变化量 [\$\Delta E\_{\mathrm k} = \dfrac 1 2 m v\^2 = \pu{1J}\$]{.arithmatex}。

- 地面参考系上，箱子的动能变化量为 [\$\Delta {E\_{\mathrm k}}\' = \dfrac 1 2 m (v + v_车)\^2 - \dfrac 1 2 m {v_车}\^2 = \pu{201J}\$]{.arithmatex}。

人推动箱子时，火车参考系上人的化学能似乎做功了 [\$\pu{1J}\$]{.arithmatex}，但地面参考系上人的化学能似乎做功了 [\$\pu{201 J}\$]{.arithmatex}。同一个人推动物体，在不同参考系上耗费化学能看似不同甚至相差如此巨大，但化学能怎么可能随参考系改变而改变？这么多多余的能量是哪来的？
...
人的动能变化量恒为 [\$\bm 0\$]{.arithmatex}，与参考系无关？确实，虽然其受到的斥力与摩擦力做功与参考系都有关，但这是一对平衡力，平衡力做功恒为 [\$0\$]{.arithmatex}，与参考系无关。
...
- 笛卡尔和牛顿认为应该将 [\$mv\$]{.arithmatex} 作为运动的度量（动量）。

- 莱布尼茨认为应该将 [\$mv\^2\$]{.arithmatex} 作为运动的度量（能量）。
...
用符号 [\$\bm{I}\$]{.arithmatex} 表示，也可用 [\$\bm{J}\$]{.arithmatex} 或 Imp 表示冲量，公式：

::: {.arithmatex}
\$\$ \bm{I}=\bm{F}t \$\$
:::

没有特殊的单位，其单位为 [\$F\$]{.arithmatex} 和 [\$t\$]{.arithmatex} 单位的合成，即 [\$\mathrm{N\cdot s}\$]{.arithmatex}（牛秒）。
...
如果 [\$I=0\$]{.arithmatex}，那么 [\$F=0\$]{.arithmatex}，因为 [\$t\$]{.arithmatex} 时间为 [\$0\$]{.arithmatex} 没有意义。

如果 [\$W=0\$]{.arithmatex}，那么是 [\$F=0\$]{.arithmatex} 或 [\$x=0\$]{.arithmatex} 或 [\$\cos\theta=0\$]{.arithmatex}。
...
用符号 [\$\bm{p}\$]{.arithmatex} 表示，公式：

::: {.arithmatex}
\$\$ \bm{p}=m\bm{v} \$\$
:::

没有特殊的单位，其单位为 [\$m\$]{.arithmatex} 和 [\$v\$]{.arithmatex} 单位的合成，即 [\$\mathrm{kg\cdot m/s}\$]{.arithmatex}（牛秒）。
...
即 [\$E_k=\frac{p\^2}{2m}\$]{.arithmatex}，但是这个结论适用范围很窄，一定要考虑方向的问题。
...
其中 [\$\bm{v}\$]{.arithmatex} 表示速度，[\$v\$]{.arithmatex} 表示速率。

在 [\$m\$]{.arithmatex} 不改变的情况下：

若 [\$p\$]{.arithmatex} 改变，[\$v\$]{.arithmatex} 的方向和大小一定有一个改变。

若 [\$E_k\$]{.arithmatex} 改变，只可能是 [\$v\$]{.arithmatex} 的大小改变。
...
其中 [\$I\$]{.arithmatex} 表示物体所受冲量，[\$m\$]{.arithmatex} 表示物体的质量，[\$F\$]{.arithmatex} 表示物体所受的合外力（恒力），[\$\Delta t\$]{.arithmatex} 表示力的作用时间，[\$\Delta v\$]{.arithmatex} 表示速度的变化量，[\$\Delta p\$]{.arithmatex} 是动量变化量。

理解方式：物体摔到地上，[\$m\Delta v\$]{.arithmatex} 是定值，比较软的地面会产生形变，时间 [\$t\$]{.arithmatex} 更长、合外力更小。
...
比如一个重力为 [\$G\$]{.arithmatex} 的小球落到地上，对地面的压力为 [\$N\$]{.arithmatex}，弹起用时为 [\$t\$]{.arithmatex}：

::: {.arithmatex}
\$\$ (N-G)t=\Delta p=m(v_t-v_0) \$\$
:::

其中 [\$v_0\$]{.arithmatex}、[\$v_t\$]{.arithmatex} 可以用匀变直求解，带入可解得未知量。
...
我们考虑功动能定理 [\$(1)\$]{.arithmatex} 和动量定理 [\$(2)\$]{.arithmatex} 存在什么区别和联系。

相同点：都有速度变化量 [\$\Delta v\$]{.arithmatex} 和质量；不同点：一个是位移 [\$x\$]{.arithmatex} 一个是时间 [\$t\$]{.arithmatex}。
