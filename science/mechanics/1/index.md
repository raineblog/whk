...
1. **结论**：\\theta 变大意味着波穿过缝隙后，向两侧扩散的角度更广，能量不再集中在直线方向，这就是“衍射现象更明显”。
...
1. **解释折射定律**：同样的情景，但波进入了介质 2。当 A 点到达界面后，它发出的子波在介质 2 中以速度 v_2 传播。经过 \\Delta t，A' 点到达界面 B 点，此时 A 点在介质 2 中的子波半径为 v_2 \\Delta t。连接包络面，通过几何关系可以证明折射定律。
...
1. \*\*动量分散（\\Delta p_y）：\*\*根据不确定性原理，位置限制得越死，动量的不确定度就越大。

   \\Delta p_y \\approx \\frac{h}{a}

   这意味着波（或粒子）在垂直方向上获得了一个随机的动量分量，导致它不再单纯沿水平方向飞，而是有了横向速度，这就是衍射（扩散）。

1. \*\*引入波长：\*\*在波动学中，动量 p 与波长 \\lambda 成反比（德布罗意关系 p = h/\\lambda）。衍射角 \\theta 大约等于横向动量与总动量的比值：
...
1. **结论：**看最终公式 \\theta \\approx \\lambda/a。很明显，**\\lambda 越大，\\theta 越大**。
