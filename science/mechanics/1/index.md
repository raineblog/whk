...
其中 []{.arithmatex}$A$、[]{.arithmatex}$B$、[]{.arithmatex}$C$ 为作用在一点（假设为 []{.arithmatex}$P$）力的大小。

而 []{.arithmatex}$\alpha$、[]{.arithmatex}$\beta$、[]{.arithmatex}$\gamma$ 则为在 []{.arithmatex}$A$、[]{.arithmatex}$B$、[]{.arithmatex}$C$ 力相对角的角度。
...
5.  质心的定义表达式：
    ...
    这是在某个确定的坐标下写出的，其中 []{.arithmatex}$r_i$ 为第 []{.arithmatex}$i$ 个质点在该坐标系下的位置，[]{.arithmatex}$m_i$ 为第 []{.arithmatex}$i$ 个质点的质量，[]{.arithmatex}$M$ 为总质量。
...
- 公式：[]{.arithmatex}$\bm F=-k\bm x$，其中 []{.arithmatex}$k$ 称为劲度系数，单位 []{.arithmatex}$\pu{N/m}$。
...
- 主动力，可以写成一个近似的表达式：
  ...
  其中 []{.arithmatex}$N$ 为两个相互接触表面的法向力（即正压力），[]{.arithmatex}$\mu_k$ 为滑动摩擦系数。
...
- 被动力，有近似上限：
  ...
  一般静摩擦系数 []{.arithmatex}$\mu_s$ 稍大于滑动摩擦系数 []{.arithmatex}$\mu_k$。

  如果没有特殊说明，可以认为 []{.arithmatex}$\mu_s=\mu_k$，统称为摩擦系数 []{.arithmatex}$\mu$。

- 静摩擦力常作为一种通过接触面作用的未知的切向力，它的大小取决于周围其它物体施加的作用力和该物体的运动情况。只有在临界状态，即该物体在接触面处将产生相对滑动、又未产生滑动的临界运动状态时，静摩擦力等于最大静摩擦，[]{.arithmatex}$f_s=\mu_sN$。
...
![全反力](../%E5%85%A8%E5%8F%8D%E5%8A%9B.avif){align="right" width="35%"}

- 在滑动摩擦下，称 []{.arithmatex}$\varphi=\arctan(f_k/N)$ 为滑动摩擦角，因此 []{.arithmatex}$\mu=\tan\varphi$。

- 我们将滑动摩擦力和支持力合并为一个效果力，称为全反力 []{.arithmatex}$F_R$。全反力和支持力的夹角即为摩擦角 []{.arithmatex}$\varphi$，只要物体发生滑动摩擦力，无论支持力是多少，摩擦角都是不变的。

- 一物置于倾角为 []{.arithmatex}$\theta$ 的粗糙斜面上，[]{.arithmatex}$\theta<\varphi$ 则物体不会自行滑下，这一现象称为自锁。

另外，在静摩擦下，称 []{.arithmatex}$\varphi=\arctan(f_{sm}/N)$ 为静摩擦角，[]{.arithmatex}$f_{sm}$ 为最大静摩擦力。

### 摩擦力做功 {#_9}

我们知道摩擦力做功通常可以表示为摩擦力乘上相对路程，也可以表示为一对摩擦力做的总功。

一对相互作用力做功之和​：考虑两个物体 []{.arithmatex}$A$ 和 []{.arithmatex}$B$，[]{.arithmatex}$A$ 受到 []{.arithmatex}$F_A$，[]{.arithmatex}$B$ 受到 []{.arithmatex}$F_B$，[]{.arithmatex}$F_A$ 和 []{.arithmatex}$F_B$ 是一对相互作用力。那么虽然 []{.arithmatex}$F_A$ 和 []{.arithmatex}$F_B$ 可能是变力，但由于他们时刻相等，可以考虑微分，设两个物体经过微小的位移，此时可以认为它们为大小不变的恒力。
...
设 []{.arithmatex}$A$ 经过微小位移 []{.arithmatex}$\mathrm d \overrightarrow{x_A}$，[]{.arithmatex}$B$ 经过微小位移 []{.arithmatex}$\mathrm d \overrightarrow{x_B}$，则两力做功之和为
...
设 []{.arithmatex}$A$ 相对于 []{.arithmatex}$B$ 的运动路径是 []{.arithmatex}$L$，对上式进行第二类曲线积分可得
...
即一对相互作用力做功之和，只与力和 []{.arithmatex}$A$ 相对 []{.arithmatex}$B$ 运动的路径有关。注意到 []{.arithmatex}$A$ 相对 []{.arithmatex}$B$ 运动的路径是与选取的参考系无关的，因此 一对相互作用力做功之和与参考系无关。这又是一个非常特殊，经典且重要的一个做功与参考系无关的特例------两个与参考系都有关的力，其做功之和可能与参考系无关。

上式也相当于令 []{.arithmatex}$B$ 为参考系的特殊情形，此时由于 []{.arithmatex}$B$ 静止，[]{.arithmatex}$F_B$ 不做功，这对相互作用力的做功之和就是 []{.arithmatex}$F_A$ 的做功。这提示我们计算一对相互作用力做功之和，可以考虑换系，以其中一个物体为参考系，则要计算的和简化为一个功。
...
两个磁铁相互吸引，令两磁铁静止，相互作用力做功之和为零。因此，[]{.arithmatex}$A$ 对 []{.arithmatex}$B$ 做功 []{.arithmatex}$W$，则 []{.arithmatex}$B$ 对 []{.arithmatex}$A$ 做功 []{.arithmatex}$-W$ 这种说法是完全错误的。
...
考虑一辆以 []{.arithmatex}$v_车 = \pu{100 m/s}$ 的速度匀速运行的火车上，一个人以 []{.arithmatex}$\pu{1N}$ 的力 []{.arithmatex}$F$ 推动原先静止的，质量为 []{.arithmatex}$m = \pu{2kg}$ 的箱子，使得箱子加速运动到 []{.arithmatex}$v = \pu{1 m/s}$，火车地面光滑。

- 火车参考系上，箱子的动能变化量 []{.arithmatex}$\Delta E_{\mathrm k} = \dfrac 1 2 m v^2 = \pu{1J}$。

- 地面参考系上，箱子的动能变化量为 []{.arithmatex}$\Delta {E_{\mathrm k}}' = \dfrac 1 2 m (v + v_车)^2 - \dfrac 1 2 m {v_车}^2 = \pu{201J}$。

人推动箱子时，火车参考系上人的化学能似乎做功了 []{.arithmatex}$\pu{1J}$，但地面参考系上人的化学能似乎做功了 []{.arithmatex}$\pu{201 J}$。同一个人推动物体，在不同参考系上耗费化学能看似不同甚至相差如此巨大，但化学能怎么可能随参考系改变而改变？这么多多余的能量是哪来的？
...
人的动能变化量恒为 []{.arithmatex}$\bm 0$，与参考系无关？确实，虽然其受到的斥力与摩擦力做功与参考系都有关，但这是一对平衡力，平衡力做功恒为 []{.arithmatex}$0$，与参考系无关。
...
1.  直角坐标系下的分量表示：
    ...
    :::: {.arithmatex}
    ::: {.MathJax_Preview}
    \begin{aligned} \bm F_1+\bm F_2+\dots=\sum \bm F_i=0\\ \sum F\_{ix}=0,\\\sum F\_{iy}=0,\\\sum F\_{iz}=0 \end{aligned}
    :::
...
- 笛卡尔和牛顿认为应该将 []{.arithmatex}$mv$ 作为运动的度量（动量）。

- 莱布尼茨认为应该将 []{.arithmatex}$mv^2$ 作为运动的度量（能量）。
...
用符号 []{.arithmatex}$\bm{I}$ 表示，也可用 []{.arithmatex}$\bm{J}$ 或 Imp 表示冲量，公式：
...
没有特殊的单位，其单位为 []{.arithmatex}$F$ 和 []{.arithmatex}$t$ 单位的合成，即 []{.arithmatex}$\mathrm{N\cdot s}$（牛秒）。
...
如果 []{.arithmatex}$I=0$，那么 []{.arithmatex}$F=0$，因为 []{.arithmatex}$t$ 时间为 []{.arithmatex}$0$ 没有意义。

如果 []{.arithmatex}$W=0$，那么是 []{.arithmatex}$F=0$ 或 []{.arithmatex}$x=0$ 或 []{.arithmatex}$\cos\theta=0$。

注意区分合外力（在瞬间，相当于恒力）的冲量和功：

:::: {.arithmatex}
::: {.MathJax_Preview}
\begin{aligned} \bm{I}\_{\text{合}}=\Sigma\bm{I}=t\Sigma\bm{F}=\bm{F}\_{\text{合}}t\\ W\_{\text{合}}=\Sigma(\bm{F}\bm{x})=\Sigma W \end{aligned}
:::
...
没有特殊的单位，其单位为 []{.arithmatex}$m$ 和 []{.arithmatex}$v$ 单位的合成，即 []{.arithmatex}$\mathrm{kg\cdot m/s}$（牛秒）。
...
即 []{.arithmatex}$E_k=\frac{p^2}{2m}$，但是这个结论适用范围很窄，一定要考虑方向的问题。
...
其中 []{.arithmatex}$\bm{v}$ 表示速度，[]{.arithmatex}$v$ 表示速率。

在 []{.arithmatex}$m$ 不改变的情况下：

若 []{.arithmatex}$p$ 改变，[]{.arithmatex}$v$ 的方向和大小一定有一个改变。

若 []{.arithmatex}$E_k$ 改变，只可能是 []{.arithmatex}$v$ 的大小改变。
...
其中 []{.arithmatex}$I$ 表示物体所受冲量，[]{.arithmatex}$m$ 表示物体的质量，[]{.arithmatex}$F$ 表示物体所受的合外力（恒力），[]{.arithmatex}$\Delta t$ 表示力的作用时间，[]{.arithmatex}$\Delta v$ 表示速度的变化量，[]{.arithmatex}$\Delta p$ 是动量变化量。

理解方式：物体摔到地上，[]{.arithmatex}$m\Delta v$ 是定值，比较软的地面会产生形变，时间 []{.arithmatex}$t$ 更长、合外力更小。
...
比如一个重力为 []{.arithmatex}$G$ 的小球落到地上，对地面的压力为 []{.arithmatex}$N$，弹起用时为 []{.arithmatex}$t$：
...
其中 []{.arithmatex}$v_0$、[]{.arithmatex}$v_t$ 可以用匀变直求解，带入可解得未知量。
...
:::: {.arithmatex}
::: {.MathJax_Preview}
\sum\_{i=1}\^n\bm{p}\_i=\mathit{const}
:::
...
:::: {.arithmatex}
::: {.MathJax_Preview}
\sum\_{i=1}\^nm_i\bm{v}\_i=\mathit{const}
:::
...
:::: {.arithmatex}
::: {.MathJax_Preview}
\begin{aligned} F\_{\text{合}}x=\frac{1}{2}mv_t\^2-\frac{1}{2}mv_0\^2&&(1)\\ F\_{\text{合}}t=mv_t-mv_0&&(2) \end{aligned}
:::
...
我们考虑功动能定理 []{.arithmatex}$(1)$ 和动量定理 []{.arithmatex}$(2)$ 存在什么区别和联系。

相同点：都有速度变化量 []{.arithmatex}$\Delta v$ 和质量；不同点：一个是位移 []{.arithmatex}$x$ 一个是时间 []{.arithmatex}$t$。
