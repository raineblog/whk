...
2.  位移关系
    ...
    可以用平均动量乘时间推出来（其中 [\$x\$]{.arithmatex} 为某一方向上的位移）：
...
质量为 [\$m=2\mathrm{kg}\$]{.arithmatex} 的小球，从 [\$A\$]{.arithmatex} 点以初速度 [\$v_0=6\mathrm{m/s}\$]{.arithmatex} 向右运动，小车质量 [\$M=4\mathrm{kg}\$]{.arithmatex}：

![题图](../uj7y6rn5.avif)

问：小球运动到点 [\$B\$]{.arithmatex} 后，能达到的最大高度是多少（水平共速模型）。
...
解得 [\$h=1.2\mathrm{m}\$]{.arithmatex}。

问：当小球返回 [\$A\$]{.arithmatex} 时，小球和小车的速度分别是多少（弹性模型）。
...
问：若水平轨道粗糙且长度为 [\$2\mathrm m\$]{.arithmatex}，小球回到远处后自由落体，问水平轨道的摩擦因数。
...
解得 [\$v_1=3\mathrm{m/s}\$]{.arithmatex}。
...
- 使用二元组表示，即 [\$(v_1,m_1)\$]{.arithmatex} 撞 [\$(0,m_2)\$]{.arithmatex}：

  ::: {.arithmatex}
  \$\$ \left\\\begin{aligned} v_1\'&=\frac{v_1(m_1-m_2)}{m_1+m_2}\\ v_2\'&=\frac{2m_1v_1}{m_1+m_2} \end{aligned}\right. \$\$
  :::

  注意到 [\$v_2\'\$]{.arithmatex} 永远是正的，但是 [\$v_1\'\$]{.arithmatex} 可能是负的。

- 具体的，如果 [\$m_1\<m_2\$]{.arithmatex}，[\$v_1\'\<0\$]{.arithmatex}，即物体反向。

- 特殊的，如果 [\$m_2\to\infty\$]{.arithmatex}，则 [\$v_1\'\to -v_1\$]{.arithmatex}，[\$v_2\'\to0\$]{.arithmatex}，类比到小球撞墙。

等重碰撞：

- 即 [\$(v_1,m)\$]{.arithmatex} 撞 [\$(v_2,m)\$]{.arithmatex}。易得 [\$v_1\'=v_2\$]{.arithmatex}，[\$v_2\'=v_1\$]{.arithmatex}，即交换速度。
...
质心的速度不变，可以列出初、末速度和质心速度 [\$v_f\$]{.arithmatex} 的关系：

::: {.arithmatex}
\$\$ \left\\\begin{aligned} v_1+v_1\'&=2v_f\\ v_2+v_2\'&=2v_f \end{aligned}\right. \$\$
:::

考虑求质心速度，等价于两物体撞到共速的速度 [\$v_f\$]{.arithmatex}：
...
非完全弹性碰撞后两物体速度相同，设为 [\$v_f\$]{.arithmatex}，可以列出动量守恒：
...
恢复系数通常在 [\$0\$]{.arithmatex} 与 [\$1\$]{.arithmatex} 之间，但是不是一定，我们记为 [\$C_r\$]{.arithmatex}：

- [\$C_r=1\$]{.arithmatex}，则此碰撞为弹性碰撞；
- [\$0\le C_r\<1\$]{.arithmatex}，则此碰撞为非弹性碰撞；
- [\$C_r=0\$]{.arithmatex}，则此碰撞为完全非弹性碰撞，两个物体黏贴在一起。
- [\$C_r\>1\$]{.arithmatex}，表示产生动能的碰撞。例如，当两个地雷碰撞引起爆炸。
- [\$C_r\<0\$]{.arithmatex}，表示其中一个物体会超过另外一个物体。例如，子弹穿过弹靶。
...
这个混合模型可以通过恢复系数 [\$C_r\$]{.arithmatex} 来表示，其中 [\$C_r\$]{.arithmatex} 是物体（材质）的特性。
...
2.  损失能量的范围：[\$\[0,\$]{.arithmatex} 完全非弹性碰撞损失的能量 [\$\]\$]{.arithmatex}，两边可以取等号。

3.  质量未知的动量变化范围：极限法，后物体撞前物体，

    1.  前物体质量极大速度极小，后物体近似原速返回，动量变化量为 [\$-2p_1\$]{.arithmatex}。

    2.  前物体和后物体速度接近，后物体近似速度不变，动量变化量为 [\$0\$]{.arithmatex}。

    故后物体动量变化范围为 [\$(-2p_1,0)\$]{.arithmatex}，不能取等号。
...
发现我们不知道 [\$m\$]{.arithmatex} 和 [\$t\$]{.arithmatex}，考虑进行处理，我们发现这里的质量只是打在接触面上的质量：
...
其中，[\$v\$]{.arithmatex} 是打在墙面上时的流体速度，[\$v_0\$]{.arithmatex} 为流体的初速度。

代数，[\$\rho\$]{.arithmatex} 为流体密度，[\$S\$]{.arithmatex} 为打在墙面上的接触面积，[\$F\$]{.arithmatex} 为墙面受到的平均压力。

其中，后半边式子成立的前提是粒子均匀密布在接触面上，如风和水管。

对于下雨一类的，需要带前半边的式子，用 [\$h\$]{.arithmatex} 表示 [\$t\$]{.arithmatex} 时间内的余量（积水高度）。

### 流和流量 {#_24}

一段时间内流经截面的某种物理量的总量就叫做**流量**。

流量是时间 [\$t\$]{.arithmatex} 的函数，流是流量关于时间的导数，流量是流在某段时间的定积分，反之。
...
把电荷 [\$q\$]{.arithmatex} 替换成其他物理量如质量、能量、粒子数，分别得到质量流、能流、粒子流。

### 能流密度 {#_25}

**流密度**可以用于描述某时刻流体在的空间流动的速率。

以水流为例，在一条河流或管道中，某时刻 [\$t\$]{.arithmatex} 在空间中任意一点 [\$\bm r\$]{.arithmatex} 处，都对应一个水流速度 [\$\bm v\$]{.arithmatex}，如果我们在该点放置一个与速度垂直的微小截面（通常叫面元），令其面积为 [\$\Delta S\$]{.arithmatex}，在一段微小时间 [\$\Delta t\$]{.arithmatex} 内流经截面的质量为 [\$\Delta m\$]{.arithmatex}，那么质量流密度可以用极限定义为，

::: {.arithmatex}
\$\$ \bm j(\bm r,t)=\hat{\bm n}\lim\_{\Delta S,\Delta t\to0}{\Delta m\over \Delta S\Delta t} \$\$
:::

其中 [\$\hat{\bm n}\$]{.arithmatex} 表示面元正方向法向量或者面元处流体的速度方向。
