...
1. 化学式相同，并不一定就是纯净物。
1. 高聚物，又称高分子化合物，由于其分子中的链节数目不同，相对分子质量也不同，因此高聚物一定是混合物。
1. 水合盐又称结晶水合物，组成确定、性质确定，属于纯净物。
1. 均匀不能成为判定是否为纯净物的依据，例如气体和互溶的液体。
1. 同一物质、不同状态的物质混在一起，仍然属于化学上的纯净物。
1. 除高聚物之外，凡是使用化学式表示的物质都是按照纯净物来对待和使用的。
1. 一般不认为光或热为能量，不属于物质，就像单质不属于电解质和非电解质范畴一样。
...
以上五大类十二小类基本覆盖了常见的置换反应。只要满足单质 + 化合物 ightarrow 单质 + 化合物的形式，就可以从周期表中选取相应元素，写出大量的置换反应方程式。
...
稀有气体（Noble Gases），又称惰性气体或钝气，位于元素周期表的**第18列（0族）**，包括氦（He）、氖（Ne）、氩（Ar）、氪（Kr）、氙（Xe）、氡（Rn）以及人工合成的（Og）。以下是对稀有气体的详细展开讲解：

稀有气体的发现过程是化学史上严谨求实的典范。

- **氩的发现**：1892年，物理学家瑞利发现从空气中分离出的氮气密度（1.2572 g/L）与从含氮物质分解得到的氮气密度（1.2508 g/L）存在微小差异。他与化学家拉姆齐合作，通过排除法并利用光谱分析，于1894年确认了空气中存在氩气。
- **氦的发现**：氦最初是在1868年通过观察太阳光谱中的特殊黄线发现的，取名自希腊语“太阳”。直到1895年，拉姆齐才在处理沥青铀矿产生的气体中证实了地球上也存在氦。
- **其他成员**：随后几年，拉姆齐利用分级液化空气的方法陆续发现了氪、氖和氙。氡则在1900年作为镭的放射性衰变产物被发现。

原子结构与物理性质：

- **电子排布与稳定性**：除氦的最外层为2电子（1s^2）外，其他稀有气体原子的最外层均为**8电子稳定结构**（ns^2np^6）。这种饱和的结构使其第一电离能极高，电子亲和能几乎为零，具有极强的化学惰性。
- **单原子分子**：稀有气体在常温常压下均为**单原子气体**，分子间仅存在微弱的伦敦色散力（范德华力）。
- **低熔沸点**：它们的熔沸点是所有元素中最低的，且随原子序数增大（电子数增多，色散力增强）而升高。例如，氦的沸点为 -268.9℃，是自然界中最难液化的气体。
- **原子半径**：稀有气体的原子半径通常指**范德华半径**，这与主族元素的共价半径定义不同，因此在同一周期内，其原子半径数值会呈现出“突然增大”的假象。

长期以来，稀有气体被认为绝对不发生化学反应。

- **重大转折**：1962年，英国化学家\*\*巴特利特（N. Bartlett）\*\*利用强氧化剂 PtF_6 氧化了氙，合成了第一个稀有气体化合物 XePtF_6（橙黄色固体）。这一发现证明了稀有气体“不惰”，因此更名为“稀有气体”。
- **氙的化学**：氙是稀有气体中化学活性最丰富的元素，能与电负性极强的氟、氧形成化合物。
  - **氟化物**：如 XeF_2（直线形）、XeF_4（平面正方形）和 XeF_6（变形八面体）。
  - **氧化物**：如 XeO_3（三角锥形，极易爆炸）和 XeO_4（正四面体形，极不稳定）。
- **成键模型**：现代化学常用\*\*三中心四电子键（3c-4e键）\*\*来解释 XeF_2 等超价分子的成键，而不再简单认为其动用了d轨道。

基于物理与化学特性：

- **保护气**：利用其化学性质极不活泼的特点，在金属焊接（氩弧焊）、白炽灯充气、半导体生产中充当保护气，防止物质被氧化。
- **电光源（霓虹灯）**：稀有气体在通电时能发出特征颜色的光（如氖发出**橙红色**，氩发出浅蓝色），广泛用于霓虹灯、航标灯、闪光灯等。
- **低温冷却剂**：液氦常用于超导技术和大型核磁共振（NMR）设备的冷却。
- **潜水呼吸气**：潜水员使用**氦-氧混合气**而非氮-氧气，是因为氦在血液中的溶解度低，可以预防减压症（潜水夫病）。
- **其他应用**：氙可用作麻醉剂；氪和氙的光谱接近日光，被称为“小太阳”光源。
- **氡（Rn）的危害**：氡是一种具有**天然放射性**的稀有气体，常从地壳岩石（如花岗岩）中释放并积聚在室内。由于氡衰变产生的 \\alpha 粒子会损伤呼吸道系统，它是导致肺癌的主要诱因之一。在建筑和装修中需注意通风以降低氡浓度。
...
1. **强者先行**：假设法，例如 \\ce{Cl2} 先氧化 \\ce{FeI2} 中的碘离子。
...
高考工业流程题通常以真实工业生产为背景，考察元素的性质、转化及分离提纯。掌握元素的“黑话”能快速破解题干信息。

### 工业流程的底层逻辑

1. **原料预处理**：粉碎（增大接触面积）、焙烧（改变价态或除去有机物）、酸浸/碱浸（溶解目标成分）。
1. **核心反应**：氧化还原调节（如 \\ce{Fe^2+ -> Fe^3+}）、络合溶解、沉淀转化。
1. **分离提纯**：调 pH 值沉淀杂质、萃取与反萃取、重结晶、洗涤（水洗、醇洗、酸洗）。
1. **产品获取**：蒸发浓缩、冷却结晶、过滤、干燥（恒温干燥、真空干燥）。

### 工业流程题元素“黑话”表

| 元素分类              | 核心考点（“黑话”内容）             | 工业应用场景                              |
| --------------------- | ---------------------------------- | ----------------------------------------- |
| **H / He**            | 还原剂、同位素（D、T）、低温冷却。 | 合成氨、水煤气、核聚变研究。              |
| **Li / Na / K**       | 焰色反应、强碱、储能材料。         | 锂电池、侯氏制碱、呼吸面具（\\ce{KO2}）。 |
| **Mg / Ca / Ba**      | 高熔点氧化物、硬水处理、显影检测。 | 耐火材料、钡餐（\\ce{BaSO4}）、漂白粉。   |
| **Al**                | **两性之王**、铝热反应、净水。     | 航空材料、明矾、焊铁轨。                  |
| **C / Si / Ge**       | 半导体材料、还原剂、高分子骨架。   | 芯片、光导纤维、高炉炼铁。                |
| **N / P / As**        | 硝酸强氧化性、化肥、剧毒。         | 炸药、磷肥、砒霜（\\ce{As2O3}）。         |
| **O / S / Se**        | 氧化剂、硫酸工业、光电感应。       | 漂白、硫化橡胶、硒鼓。                    |
| **F / Cl / Br / I**   | 极强氧化性、卤化剂、升华、萃取。   | 消毒剂、海水提溴/碘、有机氟化工。         |
| **Ti / V / Cr**       | 变价、催化剂、耐腐蚀。             | 钛白粉、接触法制硫酸、电镀。              |
| **Mn / Fe / Co / Ni** | 氧化态舞蹈、磁性、电池材料。       | 干电池、不锈钢、锂电正极。                |
| **Cu / Ag / Au / Pt** | 优良导体、惰性、催化。             | 电子器件、银镜反应、汽车尾气净化。        |
| **Zn / Cd / Hg**      | 牺牲阳极保护、电池、液态金属。     | 镀锌、汞齐、日光灯管。                    |
| **Pb / Sn / Sb / Bi** | 蓄电池、还原剂、低熔点合金。       | 焊锡、铅酸电池、阻燃剂。                  |
| **稀土/锕系**         | 工业维生素、核燃料。               | 永磁王（\\ce{Nd}）、荧光粉、核电站。      |

### 高考实战解题“心法”

- **看框图，找进出**：箭入为原料，箭出为产品或副产物，循环箭头利用率。
- **查价态，定氧还**：分析核心元素在各步骤中的价态变化，判断加入试剂的性质。
- **调 pH，控沉淀**：根据 K\_{sp} 计算沉淀 pH 范围，实现杂质的分步沉淀。
- **洗涤液，有讲究**：醇洗减少溶解损失、除去有机杂质；酸洗除去表面氧化膜或碱性杂质。
...
- **氟（F_2）**：淡黄绿色气体（剧毒，极强氧化性）。
- **氯（Cl_2）**：黄绿色气体。
- **溴（Br_2）**：深红棕色液体（易挥发）；其蒸气为**红棕色**。
- **碘（I_2）**：紫黑色固体，具有金属光泽；其蒸气为**紫色**。
- **硫（S）**：黄色或淡黄色固体（俗称硫磺）。
- **磷（P）**：
  - **白磷（黄磷）**：白色或微黄色蜡状固体（因微量红磷杂质而常呈黄色，剧毒，着火点低，在空气中易自燃）。
  - **红磷（赤磷）**：红褐色或赤褐色粉末（无毒，着火点高）。
  - **黑磷**：黑色有金属光泽的晶体（高压下制得）。
- **臭氧（O_3）**：淡蓝色气体（浓度高时或液态时为深蓝色）。
- **石墨/金刚石/无定形碳（C）**：石墨为灰黑色、不透明的晶体固体；金刚石纯净时为无色晶体；木炭、活性炭等表现为黑色。

金属单质：

- **铜（Cu）**：紫红色（或纯净时为亮红色）固体。
- **金（Au）**：金黄色。
- **铯（Cs）**：略带金色光泽的银白色金属（极活泼）。
- **铁（Fe）**：块状纯铁为**银白色**（工业铁因含碳杂质显银灰色）；**铁粉为黑色**（光学原因导致光线全吸收）。
- **其他常见金属（Li, Na, K, Mg, Al, Zn等）**：通常为**银白色**。

### 氧化物的颜色

铁的氧化物：

- **氧化亚铁（FeO）**：黑色粉末（不稳定，在空气中加热易被氧化）。
- **三氧化二铁（Fe_2O_3）**：**红棕色粉末**（俗称铁红，常作红色颜料、炼铁原料）。
- **四氧化三铁（Fe_3O_4）**：**黑色晶体**（具有磁性，俗称磁性氧化铁）。

铜的氧化物：

- **氧化亚铜（Cu_2O）**：**砖红色**固体（葡萄糖醛基检测的特征产物）。
- **氧化铜（CuO）**：**黑色**固体/粉末。

锰的氧化物：

- **二氧化锰（MnO_2）**：黑色或棕黑色粉末/过氧化氢分解的催化剂。
- **七氧化二锰（Mn_2O_7）**：暗绿色油状液体（高价金属氧化物，极易爆，强氧化性）。

其他重要氧化物：

- **二氧化氮（NO_2）**：**红棕色**、有刺激性气味的气体（注意：一氧化氮 NO 为无色气体，但在空气中极易转变为红棕色 NO_2）。
- **三氧化二铬（Cr_2O_3）**：**绿**色固体（俗称铬绿）。
- **三氧化铬（CrO_3）**：暗红色或紫色晶体（强氧化性，易潮解）。
- **五氧化二钒（V_2O_5）**：橙色（或橙黄色）固体。
- **氧化汞（HgO）**：红色或黄色（颜色差异由颗粒大小决定）。
- **氧化铅（PbO）**：黄色（密陀僧）。
- **四氧化三铅（Pb_3O_4）**：**鲜红色**或橙红色粉末（俗称铅丹）。
- **过氧化钠（Na_2O_2）**：**淡黄色**固体（常考非白色的碱金属氧化物）。

### 酸碱和指示剂的颜色

酸：

- **浓硝酸（HNO_3）**：纯净时无色。**久置或见光分解**产生的 NO_2 溶于其中会使溶液**显黄色**（高考常考：可通过通入氧气或空气使其恢复无色）。
- “次氯酸浓溶液为黄色”是不严谨的。次氯酸（HClO）是极弱的酸，只能存在于稀溶液中，无法制得浓溶液。日常见到的黄色/黄绿色液体实际是**新制氯水**（因溶有未反应的 Cl_2 分子而显浅黄绿色），纯 HClO 溶液是无色的。

碱（氢氧化物沉淀）：

- **氢氧化铁（Fe(OH)\_3）**：**红褐色**沉淀。
- **氢氧化亚铁（Fe(OH)\_2）**：**白色**絮状沉淀。在空气中极易被氧化，现象为：**白色 \\rightarrow 灰绿色 \\rightarrow 红褐色**。
- **氢氧化铜（Cu(OH)\_2）**：**蓝色**絮状沉淀。
- **氢氧化铬（Cr(OH)\_3）**：Cr(OH)\_3 通常表现为**灰绿色沉淀**（这与 Fe(OH)\_2 变质过程中的“灰绿色”容易混淆，需格外注意）。
- **氢氧化亚钴（Co(OH)\_2）**：具有多晶型，常表现为粉红色或蓝色沉淀。

常用指示剂变色范围（精准版）：

- **石蕊**：pH < 5.0 显红色；5.0 \\le pH \\le 8.0 显紫色；pH > 8.0 显蓝色。
- **酚酞**：pH < 8.2 显无色；8.2 \\le pH \\le 10.0 显粉红色；pH > 10.0 显红色（极强碱下会褪色）。
- **甲基橙**：pH < 3.1 显红色；3.1 \\le pH \\le 4.4 显**橙色**；pH > 4.4 显黄色。

### 盐类与特征沉淀的颜色

卤化银与常见难溶盐：

- **氯化银（AgCl）**：白色沉淀（不溶于稀硝酸）。
- **溴化银（AgBr）**：**淡黄色**沉淀（不溶于稀硝酸，用于感光胶片）。
- **碘化银（AgI）**：**黄色**沉淀（不溶于稀硝酸，用于人工降雨）。
- *补充：磷酸银（Ag_3PO_4）*：黄色沉淀（但**溶于**稀硝酸）。

含铜盐：

- **硫酸铜（CuSO_4）**：无水物为**白色粉末**；五水合物（CuSO_4 \\cdot 5H_2O，胆矾/蓝矾）为**蓝色晶体**。
- **碱式碳酸铜（Cu_2(OH)\_2CO_3）**：**翠绿色**固体（俗称铜绿）。
- **氯化铜（CuCl_2）**：无水物为棕黄色固体；其浓溶液因形成络合物常显**绿色**，稀溶液显**蓝色**。

含铁盐：

- **硫酸亚铁（FeSO_4 \\cdot 7H_2O）**：**浅绿色**晶体（俗称绿矾）。
- **氯化铁（FeCl_3）**：无水物为黑绿色/红棕色晶体；六水合物（FeCl_3 \\cdot 6H_2O）为黄色晶体；**其水溶液通常显棕黄色**（因 Fe^{3+} 水解）。
- **硫酸亚铁铵（(NH_4)\_2Fe(SO_4)\_2 \\cdot 6H_2O，莫尔盐）**：淡蓝绿色晶体（比普通亚铁盐更稳定，不易被空气氧化）。
- **硫化亚铁（FeS）**：黑色固体（常作实验室制取 H_2S 的原料）。

铬、锰及其他高频盐：

- **高锰酸钾（KMnO_4）**：**紫黑色**晶体；其水溶液显亮紫红色。
- **锰酸钾（K_2MnO_4）**：**墨绿色**晶体（在酸性或中性条件下易发生歧化反应）。
- **重铬酸钾（K_2Cr_2O_7）**：**橙红色**晶体（强氧化性，酸性溶液常用于检验酒精）。
- **铬酸钾（K_2CrO_4）**：**黄色**晶体。
- **硫化镉（CdS）**：黄色（俗称镉黄）。
- **铬酸铅（PbCrO_4）**：黄色（俗称铬黄）。
- **硫化铅（PbS）**、**硫化铜（CuS）**、**硫化银（Ag_2S）**：皆为高频考查的**黑色沉淀**（且不溶于稀盐酸/稀硫酸）。

### 离子与配合物的颜色

铁水合离子颜色（考卷中的标准溶液颜色）：

- **二价铜离子（[Cu(H_2O)\_4]^{2+}）**：**蓝色**。
- **二价铁离子（[Fe(H_2O)\_6]^{2+}）**：**浅绿色**。
- **三价铁离子（[Fe(H_2O)\_6]^{3+}）**：
- 纯粹的 [Fe(H_2O)\_6]^{3+} 在晶体中实际呈现出极浅的**淡紫色**（近乎无色）。但由于 Fe^{3+} 在水溶液中极易发生水解，生成 [Fe(H_2O)\_5(OH)]^{2+} 等配离子，从而在宏观上导致**高考考查的 Fe^{3+} 溶液始终呈现黄色（或棕黄色）**。

**钒（V）的特征离子颜色链**（常作为推断题的信息给予题）：
...
- **二价锰离子（Mn^{2+}）**：浅粉色（极稀时接近无色）。
- **三价铬离子（Cr^{3+}）**：根据配体不同，可显**紫色**或**绿色**。
- **二价镍离子（Ni^{2+}）**：**绿色**。
- **二价钴离子（Co^{2+}）**：粉红色。

特征配合物颜色（极高频颜色滴定/定性检验）：

- **四氨合铜离子（[Cu(NH_3)\_4]^{2+}）**：深蓝色（绛蓝色）溶液（由 Cu(OH)\_2 沉淀溶于过量氨水制得）。
- **硫氰合铁离子（[Fe(SCN)\_n]^{3-n}，通常写为 FeSCN^{2+}）**：**血红色**（不是沉淀！是可溶性复杂配离子，用于检验 Fe^{3+}）。
- **六氰合铁（II）酸根（[Fe(CN)\_6]^{4-}，亚铁氰根）**：淡黄色溶液。
- **六氰合铁（III）酸根（[Fe(CN)\_6]^{3-}，铁氰根）**：黄色或红棕色晶体（其稀溶液通常偏绿色或黄绿色）。
- **普鲁士蓝/滕氏蓝（Fe_4[Fe(CN)\_6]\_3 变体，或 KFe[Fe(CN)\_6]）**：
- Fe^{3+} 遇亚铁氰根 [Fe(CN)\_6]^{4-}，或者 Fe^{2+} 遇铁氰根 [Fe(CN)\_6]^{3-}，都会形成同种**深蓝色沉淀**（高考用于定性检验 Fe^{2+} 的专属特征反应）。
- **四氯合铜离子（[CuCl_4]^{2-}）**：**黄色**（这就是为什么浓氯化铜溶液呈现绿色的原因：黄色的 [CuCl_4]^{2-} 与蓝色的 [Cu(H_2O)\_4]^{2+} 混色）。

### 金属的焰色反应

> ⚠️ **性质重申**：焰色反应属于**物理变化**，是原子核外电子能级跃迁释放光子的过程。

- **锂（Li）**：**紫红色**（或深红色）。
- **钠（Na）**：**黄色**。
- **钾（K）**：**紫色**（**必须透过蓝色钴玻璃**观察，以滤去微量钠盐杂质产生的黄光干扰）。
- **钙（Ca）**：**砖红色**。
- **锶（Sr）**：**洋红色**。
- **钡（Ba）**：**黄绿色**。
- **铜（Cu）**：**绿色**或蓝绿色。
...
- **淀粉遇碘单质（I_2）**：变**蓝色**（注意：必须是 I_2 单质，如果是 I^- 离子则不显色）。
- **苯酚**：纯净时为无色晶体。**久置在空气中因被氧化而显粉红色**。
- **黄蛋白反应**：含有苯环的蛋白质分子遇到**浓硝酸**微热，会发生硝化反应呈现**黄色**。
- **卤素在有机溶剂（CCl_4、苯、己烷等）中的萃取显色**：
- **溴（Br_2）**：在水中为黄褐色/橙色；在有机溶剂中显**橙红色**（或深橙色）。
- **碘（I_2）**：在水中为淡黄色/棕黄色；在有机溶剂中显**紫色**（或紫红色）。
- **品红溶液**：
  - 通入二氧化硫（SO_2）显现其**漂白性**：红色褪去（生成不稳定的无色物质）。
  - **加热后**：溶液**恢复红色**（结合不稳固，高考常考实验鉴别点）。

2025-03-292026-07-03
