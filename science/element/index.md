...
经典问题，\\ceH2 和 \\ceHD 的“混合物“是纯净物，因为纯净物的定义中，不考虑原子核同位素等的不同。
...
- 特殊氧化物：与酸和碱反应不只生成一种盐的，例如 \\ceNa2O2,Fe3O4。
- 不成盐氧化物：不与酸和碱（通性）反应的，例如 \\ceNO,CO。
- 两性氧化物：两性金属对应的氧化物。
...
在高中里面，我们最喜欢讨论的是三种反应，第一是置换反应，因为单质种类繁多但分类清晰，第二是歧化和归中反应，他们是氧化还原反应的非常特殊的一类，第三就是多步反应，在多步反应中，通常蕴含着“歧化”和“归中”，以模型 \\ceA−>[X]B−>[X]C，\\ceA+C−>B 为例，B 就可以堪为 A,C 归中的结果，例如关于质子（即氢离子）的归中在原理部分很常见。

置换反应可以用通式 A+BC→AC+B 表示。所有置换反应都属于氧化还原反应，因为必有单质参与，化合价一定发生变化。我们不妨按照物质的种类和反应环境分类，按照金属、非金属置换金属、非金属（共 4 类），以及在水溶液中、非水溶液中的反应。高中阶段较为常见的包括铝热反应、镁在二氧化碳中的燃烧，单质碳还原二氧化硅、氯气氧化氨气（生成氮气和氯化铵）、铁和高温水蒸气生成四氧化三铁。

涉及稀有气体的置换：稀有气体极难发生反应，但较重的稀有气体（如 \\ceKr、\\ceXe）能形成氟化物和氧化物。在这些反应中，稀有气体通常作为“被置换”的元素出现，因为它们的化合物具有极强的氧化性和氟化能力。由于稀有气体单质极难夺取其他元素的电子或阴离子，因此不存在纯稀有气体去置换其他元素的反应。
...
溶液中的置换：活动性排在前面的金属（排除极活泼的 \\ceK、\\ceCa、\\ceNa，它们会直接和水反应）置换后面的金属。
...
金属置换氢气（从酸中）：排在氢之前的金属置换非氧化性酸（如 \\ceHCl、稀 \\ceH2SO4）中的氢。
...
以上五大类十二小类基本覆盖了常见的置换反应。只要满足单质 + 化合物 → 单质 + 化合物的形式，就可以从周期表中选取相应元素，写出大量的置换反应方程式。
...
- 导电能力排序：\\ceAg,Cu,Ag,Al，其中银一般仅用于要求极高的场景。
- 室内导线常用铜，室外常用铝（致密氧化膜，防止进一步氧化）。
- 密度在 \\pu4.5g/cm3 以上称为重金属，以下称为轻金属。
...
- \\ceK,Ca,Na,Mg：通常电解其熔融氯化物。
- \\ceAl：其氯化物不导电，通常电解氧化物（加入冰晶石）。
- \\ceZn,Fe,Cu：通常用 \\ceC,CO,H2 还原。
- \\ceHg：氧化物高温分解，单质与氧气加热化合。
- \\ceAu：自然界多游离态，富集。
...
1. 互补性：单质氧化性越强，其对应的离子还原性越弱。
   ...
   - （金属）活动性顺序：
     ...
     氧化性：\\ceAg+>Fe3+>Cu2+>H+>⋯>Fe2+>…

     还原性：\\ceAu\<Ag\<Cu\<(H)\<⋯\<Fe\<…

   - （非金属）活动性顺序：

     氧化性：\\ceF2>Cl2>Br2>I2>S。
...
1. 不是很不准确的（存疑）：
   ...
   - 通常价位变化越大越强（反例：硝酸）。
   - 通常浓度越大越强，酸性越强氧化性越强（对于 \\ceClO−,MnO4−,NO3− 等含氧酸）。
...
- 氧化性：\\ceMnO4−>Cl2>Br2>Fe3+>I2>SO2>S。
- 还原性：\\ceMn2+\<Cl−\<Br−\<Fe2+\<I−\<SO32−\<S2−。
