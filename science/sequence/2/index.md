...
对于 [\$k\$]{.arithmatex} 阶线性递推式，[\$a_n\$]{.arithmatex} 仅与 [\$n\$]{.arithmatex} 前面的 [\$k\$]{.arithmatex} 项有关。
...
其中 [\$P(x)\$]{.arithmatex} 是一个 [\$m\$]{.arithmatex} 次多项式。
...
其中，[\$q\_{1,2}\$]{.arithmatex} 称为原线性递推式的特征根。
...
的两个特征根分别为，[\$q_1,q_2\$]{.arithmatex}，那么

通项公式，[\$a_n\$]{.arithmatex} 一定可以表示为，

::: {.arithmatex}
\$\$ \boxed{a_n=\alpha q_1\^n+\beta q_2\^n} \$\$
:::

特殊的，如果 [\$q_1=q_2=q\$]{.arithmatex}，
...
对于更高阶的，把 [\$n\$]{.arithmatex} 前面多加几项 [\$n\^2,n\^3,\dots\$]{.arithmatex} 即可。
...
同时，容易发现 [\$k_1,k_2\$]{.arithmatex} 一定对于任意 [\$n\$]{.arithmatex} 成立，因此带入特殊值，

::: {.arithmatex}
\$\$ \boxed{\begin{aligned} a_1=\alpha q_1+\beta q_2\\ a_2=\alpha q_1\^2+\beta q_2\^2 \end{aligned}} \$\$
:::

容易发现，只有 [\$k_1,k_2\$]{.arithmatex} 为未知量，可以直接解出来，得到通项公式。
...
我们考虑最原始的算法，两边同时减去 [\$3a\_{n-1}\$]{.arithmatex}，
...
两边同时除以 [\$3\^n\$]{.arithmatex} 即可，暴力求解即可。
...
首先，注意到 [\$q=1\$]{.arithmatex} 为一个可行解，于是，
...
经检验，[\$a_1=1,a_2=2,a_3=6\$]{.arithmatex}​，符合题意，故，略。
...
已知数列 [\$\\a\\,\\b\\\$]{.arithmatex} 满足，

::: {.arithmatex}
\$\$ a_1=2,b_1=1\\ \begin{aligned} a\_{n+1}&=5a_n+3b_n+7\\ b\_{n+1}&=3a_n+5b_n \end{aligned} \$\$
:::

对于 [\$n\in\mathbb N\$]{.arithmatex}，求 [\$\\a\\\$]{.arithmatex} 解析式。

容易发现，我们可以利用上式，用 [\$a\_{n+1},a_n\$]{.arithmatex} 表示 [\$b_n\$]{.arithmatex}。

然后带入下式，即可求得 [\$a_n\$]{.arithmatex} 的递推公式。

但是这么做很复杂，考虑原递推公式有什么优秀的结构。

容易发现，[\$5,3,3,5\$]{.arithmatex} 存在有一种优美的形态，
...
A. 数列 [\$\\a\_{n+1}\^2-a_na\_{n+2}\\\$]{.arithmatex} 是常数数列。

B. 数列 [\$\\8a_na\_{n+1}-7\\\$]{.arithmatex} 的各项为平方数。

C. 数列 [\$\\4a_na\_{n+1}-7\\\$]{.arithmatex}​ 的各项为平方数。

D. 任意 [\$a_n\$]{.arithmatex} 除以 [\$9\$]{.arithmatex} 的余数为 [\$1\$]{.arithmatex} 或 [\$2\$]{.arithmatex}。

对于 A 选项，我们递推式两边同时乘上 [\$a_n\$]{.arithmatex}，
...
若函数 [\$f(x)\$]{.arithmatex} 在 [\$x_0\$]{.arithmatex} 附近有定义，且存在有极限 [\$L\$]{.arithmatex}，那么，

对于任意 [\$\varepsilon\>0\$]{.arithmatex}，一定存在 [\$\delta\>0\$]{.arithmatex}，使得当，
...
则称 [\$L\$]{.arithmatex} 是函数在 [\$x_0\$]{.arithmatex} 的**极限**，记为，

::: {.arithmatex}
\$\$ \lim\_{x\to x_0}f(x)=L \$\$
:::

特殊的，若对于 [\$x\>x_0\$]{.arithmatex}（[\$x-x_0\<\delta\$]{.arithmatex}）满足上式，

则称函数在 [\$x_0\$]{.arithmatex}​ 处存在**右极限**，表示为：

::: {.arithmatex}
\$\$ \lim\_{x\to x_0\^+}f(x)=L \$\$
:::

同样的，若对于 [\$x\<x_0\$]{.arithmatex}（[\$x_0-x\<\delta\$]{.arithmatex}）满足上式，

则称函数在 [\$x_0\$]{.arithmatex}​ 处存在**左极限**，表示为：
...
对于任意 [\$\varepsilon\>0\$]{.arithmatex}，都存在 [\$N\in\mathbb N\^\*\$]{.arithmatex}，使得对于任意 [\$n\>N\$]{.arithmatex}，

::: {.arithmatex}
\$\$ \|a_n-L\|\<\varepsilon \$\$
:::

则称数列 [\$a\$]{.arithmatex} **收敛于** [\$L\$]{.arithmatex}，记为，
...
直观的讲，即无论误差范围 [\$\varepsilon\$]{.arithmatex} 多小，从某项 [\$a_n\$]{.arithmatex} 开始，每一项与 [\$L\$]{.arithmatex} 的差距都小于 [\$\varepsilon\$]{.arithmatex}​。
...
特殊的，对于数列 [\$a_n=n\$]{.arithmatex}，

当 [\$n\to+\infty\$]{.arithmatex} 时，[\$a_n\to+\infty\$]{.arithmatex}，这种**无界数列**，一般说其不存在极限。
...
若 [\$\lim\limits\_{n\to\infty}a_n=\lim\limits\_{n\to\infty}b_n=L\$]{.arithmatex}，且 [\$a_n\le c_n\le b_n\$]{.arithmatex}，则 [\$\lim\limits\_{n\to\infty}c_n=L\$]{.arithmatex}​。
...
且左右极限都是 [\$0\$]{.arithmatex}，因此中间也收敛于 [\$0\$]{.arithmatex}。

### 极限的性质 {#_18}

**唯一性**：若数列 [\$\\a_n\\\_{n\in\mathbb N}\$]{.arithmatex} 存在极限，则极限是唯一的。

**有界性**：如果一个实数数列无界，则这个实数数列一定发散。

若数列 [\$\\a_n\\\_{n\in\mathbb N}\$]{.arithmatex} 存在极限，那么一定存在 [\$M\>0\$]{.arithmatex}，使得所有 [\$\|a_n\|\le M\$]{.arithmatex}。

注意到，并不是数列有界就一定存在极限，例如 [\$a_n=(-1)\^{n}\$]{.arithmatex}。

**保序性**：若两数列 [\$\\a_n\\\_{n\in\mathbb N},\\b_n\\\_{n\in\mathbb N}\$]{.arithmatex} 分别收敛于 [\$A,B\$]{.arithmatex}，则，
...
考虑到右面的 [\$n\$]{.arithmatex} 级别比较大，我们选用一三两项，
...
对于函数 [\$f\$]{.arithmatex} ，若 [\$x\$]{.arithmatex} 满足，

::: {.arithmatex}
\$\$ f(x)=x \$\$
:::

这个 [\$x\$]{.arithmatex}​ 称为这个函数的**不动点**，或定点，是被这个函数映射到其自身一个点。
...
即函数 [\$f\$]{.arithmatex} 的不动点为 [\$2\$]{.arithmatex}，因为 [\$f(2)=2\$]{.arithmatex}。

不是每一个函数都具有不动点，例如定义在实数上的函数 [\$f(x)=x+1\$]{.arithmatex} 就没有不动点。

因为对于任意的实数， [\$x\$]{.arithmatex} 永远不会等于 [\$x+1\$]{.arithmatex}。

用画图的话来说，不动点意味着点 [\$(x,f(x))\$]{.arithmatex} 在直线 [\$y=x\$]{.arithmatex} 上，即图像存在交点。

上例 [\$f(x)=x+1\$]{.arithmatex} 的情况是，这个函数的图像与那根直线是一对平行线。
...
那么，[\$f\$]{.arithmatex} 称为**迭代函数**（**生成函数**），则和上文一样，
...
当我们解出一个不动点 [\$x\$]{.arithmatex}，等式两边同时减去 [\$x\$]{.arithmatex}，
...
假设这个数列存在极限，记为 [\$a\$]{.arithmatex}，那么对两边同时取极限，

::: {.arithmatex}
\$\$ \lim\_{n\to\infty}a_n=\lim\_{n\to\infty}f(a\_{n-1}) \$\$
:::

一般默认 [\$f\$]{.arithmatex} 函数是光滑的，那么，
...
我们想要把数列 [\$a_n\to a\_{n+1}\$]{.arithmatex} 这个过程直观的表示出来，我们知道，

::: {.arithmatex}
\$\$ f(a_n)=a\_{n+1} \$\$
:::

容易想到，我们在平面内做出 [\$f(x)\$]{.arithmatex} 的图像，那么这上面的点，
...
- [\$f(a_n)\>a_n\$]{.arithmatex}，数列在此处递增，对应点在 [\$y=x\$]{.arithmatex} 图像上方；
- [\$f(a_n)\<a_n\$]{.arithmatex}，数列在此处递减，对应点在 [\$y=x\$]{.arithmatex} 图像下方。

于是，我们考虑在平面内再做出 [\$y=x\$]{.arithmatex} 的图像，那么数列的趋势符合上文。
...
过 [\$A_1\$]{.arithmatex} 做 [\$x\$]{.arithmatex} 轴平行线，交 [\$y=x\$]{.arithmatex} 于，

::: {.arithmatex}
\$\$ A_2(a_2,a_2) \$\$
:::

过 [\$A_2\$]{.arithmatex} 做 [\$y\$]{.arithmatex} 轴平行线，交 [\$y=f(x)\$]{.arithmatex}​ 于，
...
我们知道，按照顺序在 [\$y=f(x)\$]{.arithmatex}​ 图像上的点，对应原数列。

根据这个图像，我们还能知道不动点 [\$x=f(x)\$]{.arithmatex} 其实是这两个图像的交点。
...
若 [\$\|f\'(x_0)\|\<1\$]{.arithmatex}，不动点 [\$x_0\$]{.arithmatex}​ 称为吸引不动点，数列迭代过程中会靠近吸引不动点。

若 [\$\|f\'(x_0)\|\>1\$]{.arithmatex}，不动点 [\$x_0\$]{.arithmatex} 称为排斥不动点，数列迭代过程中会远离排斥不动点。
...
求 [\$a_n\$]{.arithmatex} 的通项公式。

求出不动点 [\$x\$]{.arithmatex}，满足，
...
我们把两个不动点 [\$2,1\$]{.arithmatex} 分别减到递推式两边，
...
适用于形如 [\$a\_{n+1}=f(a_n)\$]{.arithmatex}​，

求解通项公式部分，求解不动点 [\$x=f(x)\$]{.arithmatex} 后，
...
已知等差数列 [\$\\a_n\\\$]{.arithmatex} 满足，[\$a_3=7,a_5+a_7=26\$]{.arithmatex}，

- 求 [\$a_n\$]{.arithmatex} 及其前 [\$n\$]{.arithmatex} 项和 [\$S_n\$]{.arithmatex}；
- 令 [\$b_n=1/(a_n\^2-1)\$]{.arithmatex}，求其前 [\$n\$]{.arithmatex} 项和 [\$T_n\$]{.arithmatex}。
...
求证，对于任意 [\$n\in\mathbb N\^\*\$]{.arithmatex}，
...
发现如果把分母乘过去，[\$n\$]{.arithmatex} 的系数相同，会约去，因此，
...
对于 [\$a+b=3,a\ge b\$]{.arithmatex}，最自然的想法，直接取 [\$a=b=1.5\$]{.arithmatex}，
...
类似的，我们取 [\$a=2,b=1\$]{.arithmatex} 等也可以，
...
其中 [\$L\$]{.arithmatex} 大约是 [\$1.1217\$]{.arithmatex}，我们上面一个估算已经非常准确了。

#### 还是例题 {#_47}

已知数列 [\$\\a_n\\\$]{.arithmatex} 是公差不为零的等差数列，

且 [\$a_4\$]{.arithmatex} 是 [\$a_2,a_8\$]{.arithmatex} 等比中项，满足，

::: {.arithmatex}
\$\$ a_1+a_2+\dots+a_7=14 \$\$
:::

- 求 [\$a_n\$]{.arithmatex} 通项公式。
...
有数列 [\$\\b_n\\\$]{.arithmatex} 满足，

::: {.arithmatex}
\$\$ b_1=-1\\ b_n={n+1\over2\^{n-1}n(n-1)},n\ge2 \$\$
:::

- 求其前 [\$n\$]{.arithmatex} 项和 [\$T_n\$]{.arithmatex}。

观察到 [\$n(n+1)\$]{.arithmatex} 的形式，裂项，
...
注意到 [\$T_1=-1\$]{.arithmatex} 也成立，因此上式即为结果。

#### 又是例题 {#_49}

已知数列 [\$\\a_n\\\$]{.arithmatex} 是公差为 [\$d\neq0\$]{.arithmatex} 的等差数列，

且 [\$\\a\_{k_n}\\\$]{.arithmatex} 是等比数列，其中 [\$k_1=3,k_2=5,k_3=9\$]{.arithmatex}。

- 求 [\$k_1+k_2+\dots+k_n\$]{.arithmatex} 的值。
...
观察 [\$k_n\$]{.arithmatex} 的值，

我们发现 [\$3,5,9\$]{.arithmatex} 是经典的数列，考虑大胆猜测（雾
...
是一个公比为 [\$2\$]{.arithmatex} 的等比数列，符合条件。
...
对于数列 [\$\\b_n\\\$]{.arithmatex}，有，

::: {.arithmatex}
\$\$ b_n={n\over n+1}+\sqrt{n-1\over n+1} \$\$
:::

求证，对于 [\$n\in\mathbb N\^\*\$]{.arithmatex}，
...
- 求 [\$\lim\_{n\to\infty}a_n\$]{.arithmatex}。
...
我们知道 [\$\arctan\$]{.arithmatex} 的值域是 [\$(-\pi/2,\pi/2)\$]{.arithmatex}，因此，
...
考虑数学归纳法，对于 [\$n=1,2,3\$]{.arithmatex}，有 [\$a_n\>0\$]{.arithmatex}，

假设对于 [\$n\<k(k\>3)\$]{.arithmatex}，[\$a_n\>0\$]{.arithmatex}，考虑证明 [\$a_k\>0\$]{.arithmatex}。

::: {.arithmatex}
\$\$ a_k={2019+a\_{k-1}a\_{k-2}\over a\_{k-3}}\>0 \$\$
:::

成立，因此对于 [\$n\in\mathbb N\^\*\$]{.arithmatex}，[\$a_n\>0\$]{.arithmatex}。

然后整数，我们发现 [\$2019\$]{.arithmatex} 是我们不想要的，怎么办捏 QAQ
...
已知数列 [\$\\a_n\\\_{n\in\mathbb N\^\*}\$]{.arithmatex} 满足，

::: {.arithmatex}
\$\$ \forall k\in\mathbb N\^\*,a\_{k+1}+a_k=4k+3 \$\$
:::

- 求 [\$a_1+a\_{2020}\$]{.arithmatex}。
...
- 求其前 [\$n\$]{.arithmatex} 项和 [\$S_n\$]{.arithmatex} 的最大值。
...
注意到当 [\$n\ge3\$]{.arithmatex} 时，上条件成立，那么，
...
当 [\$n\$]{.arithmatex} 很大时，幂远大于线性，因此数列越来越小。
...
数列 [\$\\a_n\\\$]{.arithmatex} 满足，

::: {.arithmatex}
\$\$ a_1=3, a\_{n+1}=a_n\^2-3a_n+4 \$\$
:::

A. [\$\\a_n\\\$]{.arithmatex} 严格单调递增。  
B. [\$\\a_n\\\$]{.arithmatex} 无界。  
C. [\$a\_{100}=101\$]{.arithmatex}.  
D. [\$\displaystyle\lim\_{n\to\infty}\left({1\over a_1-1}+{1\over a_2-1}+\dots+{1\over a_n-1}\right)=1\$]{.arithmatex}.

容易发现，右侧系数 [\$134\$]{.arithmatex} 类似 [\$144\$]{.arithmatex} 的完全平方式，
...
后面不会了，注意到迭代形如 [\$a\_{n+1}=f(a_n)\$]{.arithmatex}，我们知道不动点是一个好工具，
...
注意到形如 [\$f(n)=g(n)-g(n\')\$]{.arithmatex} 的形式，裂项成功，
