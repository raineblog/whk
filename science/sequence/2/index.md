...
容易发现，我们可以利用上式，用 (a\_{n+1},a_n) 表示 (b_n)。
...
A. 数列 ({a\_{n+1}^2-a_na\_{n+2}}) 是常数数列。

B. 数列 ({8a_na\_{n+1}-7}) 的各项为平方数。

C. 数列 ({4a_na\_{n+1}-7})​ 的各项为平方数。
...
且 ({a\_{k_n}}) 是等比数列，其中 (k_1=3,k_2=5,k_3=9)。
...
后面不会了，注意到迭代形如 (a\_{n+1}=f(a_n))，我们知道不动点是一个好工具，
