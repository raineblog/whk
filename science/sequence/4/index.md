...
数学归纳法是证明某个命题对于所有满足 [\$n\ge n_0\$]{.arithmatex} 的整数 [\$n\$]{.arithmatex} 都成立的一般方法。首先我们在 [\$n\$]{.arithmatex} 取 最小值 [\$n_0\$]{.arithmatex} 时证明该命题，这一步骤成为**基础**。然后对 [\$n\>n_0\$]{.arithmatex}，假设该命题对 [\$n_0\sim n-1\$]{.arithmatex} 之间的所有值已经被证明，再证明该命题对 [\$n\$]{.arithmatex} 成立，这一步骤成为**归纳**。
...
1.  [\$0\$]{.arithmatex} 是自然数；
2.  每一个确定的自然数 [\$a\$]{.arithmatex}，都有一个确定的后继 [\$a\'\$]{.arithmatex}，[\$a\'\$]{.arithmatex} 也是自然数；
3.  对于每个自然数 [\$b,c\$]{.arithmatex}，[\$b=c\$]{.arithmatex} 当且仅当 [\$b\'=c\'\$]{.arithmatex}；
4.  [\$0\$]{.arithmatex} 不是任何自然数的后继；
5.  任意关于自然数的命题，如果证明：
    - 它对 [\$0\$]{.arithmatex} 成立，且假定它对自然数 [\$a\'\$]{.arithmatex} 为真时，
    - 可以证明它对 [\$a\'\$]{.arithmatex} 也成立。
    - 那么该命题对所有自然数都成立。

公理 [\$5\$]{.arithmatex} 保证了数学归纳法的正确性，从而被称为归纳法原理。

PS：在集合论和计算机科学领域中，认为 [\$0\$]{.arithmatex} 属于自然数。

但在数论领域中，认为 [\$0\$]{.arithmatex} 不属于自然数，因而按数论描述，自然数会同义于正整数。

因此，如果定义 [\$0\$]{.arithmatex} 不属于自然数，把上面的 [\$0\$]{.arithmatex} 改成 [\$1\$]{.arithmatex} 即可。

### 戴德金-皮亚诺结构 {#-}

戴德金-皮亚诺结构可以描述为满足所有以下条件的三元组 [\$(S,f,e)\$]{.arithmatex}：

1.  [\$(e\in S)\$]{.arithmatex}
2.  [\$(\forall a\in S)(f(a)\in S)\$]{.arithmatex}
3.  [\$(\forall b\in S)(\forall c\in S)((f(b)=f(c))\Rightarrow(b=c))\$]{.arithmatex}
4.  [\$(\forall a\in S)(f(a)\neq e)\$]{.arithmatex}
5.  [\$(\forall A\subseteq S)(((e\in A)\wedge(\forall a\in A)(f(a)\in A))\Rightarrow(A=S))\$]{.arithmatex}

一个形象化的例子就是最上面的，即三元组 [\$(\mathbb N,(f:\mathbb N\to\mathbb N\_+;\\x\mapsto(x+1)),0)\$]{.arithmatex}。
...
对于 [\$n\in\[0,5)\$]{.arithmatex}，易证。

假设对于 [\$n=5k+q,k\le t,k\in\mathbb Z,q\in\[0,5)\$]{.arithmatex} 成立。

证明对于 [\$n=5(k+1)+q\$]{.arithmatex} 也成立，以 [\$n=5(k+1)\$]{.arithmatex} 为例，

::: {.arithmatex}
\$\$ Q\_{5(k+1)}={1+Q\_{5(k+1)-1}\over Q\_{5(k+1)-2}}={1+Q\_{5k+4}\over Q\_{5k+3}}=\alpha \$\$
:::

对于 [\$q=2,3,4\$]{.arithmatex}，同理，略。

## 反向数学归纳法 {#_6}

反向数学归纳法，是从 [\$n\$]{.arithmatex} 到 [\$n-1\$]{.arithmatex} 来证明命题，而不是相反。
...
对于 [\$x_1,x_2\dots x_n\ge0\$]{.arithmatex}。
...
即，[\$P(1),P(2)\$]{.arithmatex} 成立。

### 性质一 {#_7}

若 [\$P(n)\$]{.arithmatex} 成立，则 [\$P(n-1)\$]{.arithmatex} 也成立。
...
若 [\$P(n)\$]{.arithmatex} 成立，则 [\$P(2n)\$]{.arithmatex} 成立。

我们记第一个 [\$P(n)\$]{.arithmatex} 为，

::: {.arithmatex}
\$\$ x_1\dots x_n\le\left({x_1+\dots+x_n\over n}\right)\^n \$\$
:::

同样的，记第二个 [\$P(n)\$]{.arithmatex} 为，

::: {.arithmatex}
\$\$ x\_{n+1}\dots x\_{2n}\le\left({x\_{n+1}+\dots+x\_{2n}\over n}\right)\^n \$\$
:::

我们知道 [\$P(2)\$]{.arithmatex} 是成立的，记

::: {.arithmatex}
\$\$ y_1=x_1\dots x_n\\ y_2=x\_{n+1}\dots x\_{2n} \$\$
:::

对 [\$y_1,y_2\$]{.arithmatex} 应用 [\$P(2)\$]{.arithmatex}，
...
我们可以知道，对于 [\$\forall n\in\mathbb N\^\*\$]{.arithmatex}，[\$P(n)\$]{.arithmatex} 成立。
