...
- 解一元一次不等式 *a\*\*x* > *b*，需要按照 *a* > 0, *a* = 0, *a* \< 0 分类讨论。
- 解一元一次不等式 *a\*\*x* > *b*，其中 *x* ∈ \[*m*, *n*\]，先按照 *a* > 0, *a* = 0, *a* \< 0 分类讨论，然后按照 $\\dfrac{b}{a}$ 是否落在区间 \[*m*, *n*\] 内。
...
- 指数化：指数化是利用指数性质，将式子或者参数转移到指数位置上，以方便放缩。例如：
  ...
  *x**e**x* = *e\*\*x* + ln *x*(*x* > 0)，*a\*\*x* = *e*ln *a\*\*x* = *e\*\*x*ln *a*。
...
证明：设 *f*(*x*) = *x* − sin *x*，那么 *f*′(*x*) = 1 − cos *x* ≥ 0。

所以 *f*(*x*) 在 (0, ∞) 上单调递增，*f*(*x*) > *f*(0) = 0 即 *x* > sin *x*。
...
所以 *f*(*x*) 在 (0, *π*/2) 上单调递增，*f*(*x*) > *f*(0) = 0 即 tan *x* > *x*。

我们由 ln *x* ≤ *x* − 1 可以推导出来一下几个基本不等式：
...
即 *f*(*x*) = *C\*\*e*−*P*(*x*)，而对于不等式即为 *e\*\*P*(*x*)*f*(*x*) 单调递增。

例题：已知函数 *f*(*x*) 是定义在 ℝ 上的函数，且满足 *f*′(*x*) + *f*(*x*) > 0，设 *a* = *f*(0)，*b* = 2*f*(ln 2)，*c* = *e\*\*f*(1)，则 *a*, *b*, *c* 的大小关系是？
...
将 *e*3, 3*e*, *e\*\*π*, *π\*\*e*, 3*π*, *π*3 按照从小到大都顺序排列。
...
- 单调函数 *f*(*x*) ≥ 0 在 \[*x*1, *x*2\] 上恒成立等价于 $\\begin{cases}f(x_1)&\\ge0\\ f(x_2)&\\ge0\\end{cases}$。
- 单调函数 *f*(*x*) ≤ 0 在 \[*x*1, *x*2\] 上恒成立等价于 $\\begin{cases}f(x_1)&\\le0\\ f(x_2)&\\le0\\end{cases}$。

对任何一个函数 *f*(*x*)，若它在区间上是先减后增，则其最大值必在端点处取得，同理可得若函数在区间上先增后减，其最小值必在区间端点处取得：

- 若 *a* > 0，则 *f*(*x*) = *a\*\*x*2 + *b\*\*x* + *c* ≤ 0 在 \[*x*1, *x*2\] 上恒成立等价于 $\\begin{cases}f(x_1)&\\le0\\ f(x_2)&\\le0\\end{cases}$。
- 若 *a* \< 0，则 *f*(*x*) = *a\*\*x*2 + *b\*\*x* + *c* ≥ 0 在 \[*x*1, *x*2\] 上恒成立等价于 $\\begin{cases}f(x_1)&\\ge0\\ f(x_2)&\\ge0\\end{cases}$。

二次函数恒正、恒负的等价条件：

1. 对一切实数 *x*，*a\*\*x*2 + *b\*\*x* + *c* ≥ 0 恒成立的条件是：
...
- 函数 *f*(*x*) = |*a\*\*x* + *b*| 在 \[*x*1, *x*2\] 上的最大值必在端点处取到。

- 对于内部不是一次函数的，可以设处二次函数为 *u*，利用换元法解决，例如：

  已知 *t* 为常数，函数 *y* = |*x*2 − 2*x* − *t*| 在区间 [0, 3] 上的最大值为 2，则 *t*=？

  令 *u* = *x*2 − 2*x* ∈ [−1, 3]，则问题转化为求 *t* 大取值范围使得 |*u* − *t*| 在 [−1, 3] 上的最大值为 2。
...
**例题**：已知 *y* = *x**e**x* + *x*2 + 2*x* + *a* 恰有两个不同的零点，则 *a* 的取值范围为。

**答案**：由题意方程 *x**e**x* + *x*2 + 2*x* + *a* = 0 有两个不同的实根，分离参数 −*a* = *x**e**x* + *x*2 + 2*x*。令 *f*(*x*) = *x**e**x* + *x*2 + 2*x*，则函数 *f*(*x*) 的定义域为 ℝ，求导
...
**例题**：已知函数 *f*(*x*) = *x*(ln *x* − *a\*\*x*) 有两个极值点，则实数 *a* 的取值范围为。

**答案**：函数 *f*(*x*) 的定义域为 *x* > 0，由题意 *f*′(*x*) = ln *x* − 2*a\*\*x* + 1 有两个零点，分离参数
...
当 *x* → +∞ 时，*g*(*x*) → 0；当 *x* → 0 时，*g*(*x*) → −∞。当 0 \< 2*a* \< 1，即 $0 < a < \\dfrac{1}{2}$ 时，直线 *y* = 2*a* 与 *y* = *g*(*x*) 的图像有两个交点。
...
若 *f*(*x*) = *e\*\*x* − *a\*\*x* + 2*a* > 0 恒成立，求 *a* 的取值范围。
...
- 当 *a* > 0 时：设 *g*(*x*) 与 *h*(*x*) 相切于点 (*x*0, *e\*\*x*0)，此时直线 *h*(*x*) 的斜率 *a*0 就应当等于曲线在该点的切线斜率
...
- 与指数相关的恒成立问题，如果很难求导数的零点，则优先考虑将函数变形为 *e**x**f*(*x*) 或 *e*−*x\*\*f*(*x*) 的形式。
...
1. 若 *f*(*x*) ≥ 0 在 \[*a*, *b*\] 上恒成立，则由端点效应可知 $\\begin{cases}f(a) \\ge 0\\ f(b) \\ge 0\\end{cases}$。
1. 若 *f*(*x*) > 0 在 \[*a*, *b*\] 上恒成立，则由端点效应可知 $\\begin{cases}f(a)
   > 0\\ f(b) > 0\\end{cases}$。
1. 若 *f*(*x*) ≥ 0 在 (*a*, *b*) 上恒成立，则由端点效应可知 $\\begin{cases}f(a) \\ge 0\\ f(b) \\ge 0\\end{cases}$。
1. 若 *f*(*x*) > 0 在 (*a*, *b*) 上恒成立，则由端点效应可知 $\\begin{cases}f(a) \\ge 0\\ f(b) \\ge 0\\end{cases}$。
...
通常来说，我们设 *x*0 为函数的零点，列出 *f*(*x*0) = 0 和要求的条件，组成方程组解方程即可。
...
函数不等式中含有 *x**e**x* 与 $\\frac{e^x}{x}$ 这类固定搭配，则考虑放缩如下：
...
当 *m* > *n* > 0 时，证明：*m**e**n* + *n* \< *n**e**m* + *m*。

要证 *m**e**n* + *n* \< *n**e**m* + *m*，即证
...
此不等式两边的结构形式一致，构造 $f(x) = \\dfrac{e^x}{x} - \\dfrac{1}{x}$，只需证明 *f*(*x*) 在 (0, +∞) 上单调递增即可。
...
以 *f*(*x*) = *x**e**x* − *x* − ln *x* 的最小值为例，对其变形
...
容易知道函数先单调递减，再单调递增，*f*′(*x*0) = 0 容易得到
...
例题：已知 *a*, *b*, *c* 均为正实数，*a*2 + *b*2 + 4*c*2 = 1，则 $ab+2ac+3\\sqrt{2}bc$ 的最大值为？

*φ*(*a*, *b*, *c*) = *a*2 + *b*2 + 4*c*2 − 1
...
我们在证明形如 *x*1 + *x*2 > *m* 或 *x*1*x*2 > *m* 的式子成立时，可以尝试将待证的不等式在形式上进行转化，转而证明转化后的不等式 *x*1 > *m* − *x*2 或 $x_1>\\dfrac{m}{x_2}$ 成立，之后利用函数的单调性，转化为函数值之间的关系，即 *f*(*x*1) 与 *f*(*m* − *x*2) 或 $f\\left(\\dfrac{m}{x_2}\\right)$ 进行比较。
...
例题：函数 *f*(*x*) = *x*2 − 2*x* + 1 + *a**e**x*。函数有两个极值点 *x*1、*x*2，求证 *x*1 + *x*2 > 4。

求出导函数 *f*′(*x*) = 2*x* − 2 + *a**e**x*，分离参数即 $a=\\dfrac{2-2x}{e^x}$ 有 2 解 *x*1、*x*2。

也就是说 *y* = *a* 与 $g=\\dfrac{2-2x}{e^x}$ 有 2 交点 *x*1、*x*2。

令 $g(x)=\\dfrac{2-2x}{e^x}$，则有 *g*(*x*1) = *g*(*x*2) = *a*，这就是一个经典的极值点偏移问题。

其中等于 *a* 是因为我们将 *a* 减到 *g*(*x*) 里面是等价的，因此也可以分出来常数忽略。

### 对称和模型

模型形如证明 *h*(*x*1, *x*2) > *C*，其中 *x*1, *x*2 可交换。

- 基础模型 *h*(*x*1, *x*2) = *x*1 + *x*2。有 *f*′(*x*) 在 (−∞, *C*/2) 恒正（函数单调递增），在 (*C*/2, +∞) 恒负（函数单调递减），且 *f*′(*C*/2) = 0（极大值点）。

  设 *x*1, *x*2 为 *f* 的两个零点且 *x*1 \< *x*2，满足 *f*(*x*1) = *f*(*x*2) = 0。
...
- 基础模型 *h*(*x*1, *x*2) = *x*1*x*2。有 *f*′(*x*) 在 (−∞, *C*) 恒负（函数单调递减），在 (*C*, +∞) 恒正（函数单调递增），且 *f*′(*C*) = 0（极小值点）。
...
经典例题：已知函数 *f*(*x*) = *e\*\*x* − *a\*\*x*(*a* ≠ 0) 有两个零点 *x*1, *x*2 且 *x*1 \< *x*2，证明 *x*1 + *x*2 > 2。
...
对于形如证明 *x*1*x*2 \< *C* 的，我们将在下面构造函数法中写。

### 构造函数法

例题：已知函数 $f(x)=\\dfrac{1}{2}x+m+\\dfrac{3}{2x}-\\ln x(m\\in\\R)$，若 *x*1, *x*2 是函数 *g*(*x*) = *x\*\*f*(*x*) 的两个极值点，且 *x*1 \< *x*2，证明 *x*1*x*2 \< 1。
...
当 0 \< *x* \< 1 时，*g*″(*x*) \< 0，*g*′(*x*) 单调递减；当 *x* > 1 时，*g*″(*x*) > 0，*g*′(*x*) 单调递增。因此 1 为 *g*′(*x*) 的极值点，因此 0 \< *x*1 \< 1 \< *x*2。

要证 *x*1*x*2 \< 1 即证

$$ x_2\<\\dfrac{1}{x_1} $$

因为 0 \< *x*1 \< 1 所以 $\\dfrac{1}{x_1}>1$，因此尝试证明

$$ g'(x_2)\<g'\\left(\\dfrac{1}{x_1}\\right) $$

又因为 *g*′(*x*1) = *g*′(*x*2) = 0，因此证明
...
我们知道常见的极值点偏移是给出 *f*(*x*1) = *f*(*x*2) = 0，而拐点偏移比较常见的是给出 *f*(*x*1) + *f*(*x*2) = *C* 这种，我们以例题。

例题：已知函数 *f*(*x*) = 2ln *x* + *x*2 − 1，若 *x*1, *x*2 是两个不相等的正数，且 *f*(*x*1) + *f*(*x*2) = 0，证明 *x*1 + *x*2 > 2。
...
因为 *x*2 > 2 的时候，显然 *x*1 + *x*2 > 2。考虑这个部分，即证
...
*g*(*x*) = *f*(*x*) + *f*(2 − *x*) = 2ln (2*x* − *x*2) − 2(2*x* − *x*2) + 2

其中 *x* ∈ (0, 1)，不妨令 *t* = 2*x* − *x*2 ∈ (0, 1)，由 ln *t* \< *x* − 1，不等式显然成立。
