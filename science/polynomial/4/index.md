...
所以 $f(x$ 在 $0,\pi/2$ 上单调递增，$f(x)>f(0)=0$ 即 $tan x>x$。
...
- 当 $a>0$ 时：设 $g(x$ 与 $h(x$ 相切于点 $x\_0,e^\{x\_0}$，此时直线 $h(x$
  的斜率 $a\_0$ 就应当等于曲线在该点的切线斜率
...
此不等式两边的结构形式一致，构造
$f(x) = \dfrac\{e^x}\{x} - \dfrac\{1}\{x}$，只需证明 $f(x$ 在 $0, +\infty$
上单调递增即可。
