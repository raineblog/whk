...
对于实数 *a*1, *a*2, *b*1, *b*2：

(*a*12 + *a*22)(*b*12 + *b*22) ≥ (*a*1*b*1 + *a*2*b*2)2
...
设其质量分别为 *m*1, *m*2, …, *m\*\*n*，给他们一个互异的初速度 *v*1, *v*2, …, *v\*\*n*。

根据能量守恒定律，经过有限的时间后，它们必定会趋于同一个速度，设为 *v\*\*f*。
...
∑(*m**i**v\*\*i*2)∑*m\*\*i* ≥ (∑*m**i**v\*\*i*)2

我们取 *m* → *a*2，*v* → *b*/*a*，即：

∑*a\*\*i*2∑*b\*\*i*2 ≥ (∑*a**i**b\*\*i*)2

取等条件为 *v\*\*i* = *v\*\*g*，即初始就共速，则：
...
示例：已知实数 *x*, *y* 满足 *x*2 + *y*2 = 3，求 5*x* + 2*y* 的最大值。根据柯西不等式：

(5*x* + 2*y*)2 = (*a\*\*c* + *b\*\*d*)2 ≤ (*a*2 + *b*2)(*c*2 + *d*2) = (52 + 22)(*x*2 + *y*2) = 87

即：

$$ (5x+2y)^2\\le 87\\Rightarrow 5x+2y\\le\\sqrt{87} $$

所以 5*x* + 2*y* 的最大值为 $\\sqrt{87}$。
...
*x*1*y*1 + … + *x**n**y\*\*n* ≥ *x\*\*σ*(1)*y*1 + … + *x\*\*σ*(*n*)*y\*\*n* ≥ *x**n**y*1 + … + *x*1*y\*\*n*
...
若 *x*1, *x*2…, *x\*\*n* ≥ −1 且 *x*2, *x*2, …, *x\*\*n* 同号，则

(1 + *x*1)(1 + *x*2)…(1 + *x\*\*n*) ≥ 1 + *x*1 + *x*2 + … + *x\*\*n*

### 权方和不等式

设 *a*1, *a*2, ⋯, *a\*\*n* > 0，*b*1, *b*2, ⋯, *b\*\*n* > 0，*r* ∈ ℝ，则
