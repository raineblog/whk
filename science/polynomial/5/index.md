...
对于实数 []{.arithmatex}$a_1,a_2,b_1,b_2$：
...
上式即拉格朗日恒等式，可知其 []{.arithmatex}$\ge0$ 且取等条件为：
...
光滑桌子（[]{.arithmatex}$\mu=0$）上面放着若干个质量不一的薄板，其中间的摩擦因数不为零（[]{.arithmatex}$\mu\neq0$）。

设其质量分别为 []{.arithmatex}$m_1,m_2,\dots,m_n$，给他们一个互异的初速度 []{.arithmatex}$v_1,v_2,\dots,v_n$。

根据能量守恒定律，经过有限的时间后，它们必定会趋于同一个速度，设为 []{.arithmatex}$v_f$。
...
我们取 []{.arithmatex}$m\to a^2$，[]{.arithmatex}$v\to b/a$，即：
...
取等条件为 []{.arithmatex}$v_i=v_g$，即初始就共速，则：
...
柯西-施瓦茨不等式​：对于正实数 []{.arithmatex}$a,b,x,y$，有
...
当且仅当 []{.arithmatex}$ad=bc$ 时，等号成立。

示例：已知实数 []{.arithmatex}$x,y$ 满足 []{.arithmatex}$x^2+y^2=3$，求 []{.arithmatex}$5x+2y$ 的最大值。根据柯西不等式：
...
所以 []{.arithmatex}$5x+2y$ 的最大值为 []{.arithmatex}$\sqrt{87}$。
...
示例：已知实数 []{.arithmatex}$x,y>0$ 满足 []{.arithmatex}$\frac{1}{x}+\frac{4}{y}=1$，求 []{.arithmatex}$x+y$ 的最小值。根据权方和不等式：
...
所以 []{.arithmatex}$x+y$ 的最小值为 []{.arithmatex}$9$。

## （三）排序不等式 {#_5}

### 基本形式 {#_6}

对于非严格单调递增（或递减）的实数序列 []{.arithmatex}$x,y$，另 []{.arithmatex}$\sigma(i)$ 表示 []{.arithmatex}$1\sim n$ 的任意一个排列，有，
...
顺序和不小于乱序和，乱序和不小于逆序和；取等为 []{.arithmatex}$x,y$ 分别两两相等。
...
若 []{.arithmatex}$x_1,x_2\dots,x_n\ge-1$ 且 []{.arithmatex}$x_2,x_2,\dots,x_n$ 同号，则
...
设 []{.arithmatex}$a_1,a_2,\cdots,a_n>0，b_1,b_2,\cdots,b_n>0，r\in\mathbb{R}$，则

- 当 []{.arithmatex}$r\ge0$，或 []{.arithmatex}$r\le-1$ 时，有
...
若 []{.arithmatex}$a,b>0，p>1，\dfrac{1}{p}+\dfrac{1}{q}=1$，则有
