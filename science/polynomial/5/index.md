...
对于实数 [\$a_1,a_2,b_1,b_2\$]{.arithmatex}：
...
上式即拉格朗日恒等式，可知其 [\$\ge0\$]{.arithmatex} 且取等条件为：
...
光滑桌子（[\$\mu=0\$]{.arithmatex}）上面放着若干个质量不一的薄板，其中间的摩擦因数不为零（[\$\mu\neq0\$]{.arithmatex}）。

设其质量分别为 [\$m_1,m_2,\dots,m_n\$]{.arithmatex}，给他们一个互异的初速度 [\$v_1,v_2,\dots,v_n\$]{.arithmatex}。

根据能量守恒定律，经过有限的时间后，它们必定会趋于同一个速度，设为 [\$v_f\$]{.arithmatex}。
...
我们取 [\$m\to a\^2\$]{.arithmatex}，[\$v\to b/a\$]{.arithmatex}，即：

::: {.arithmatex}
\$\$ \sum{a_i\^2}\sum{b_i\^2}\ge(\sum{a_ib_i})\^2 \$\$
:::

取等条件为 [\$v_i=v_g\$]{.arithmatex}，即初始就共速，则：
...
柯西-施瓦茨不等式​：对于正实数 [\$a,b,x,y\$]{.arithmatex}，有

::: {.arithmatex}
\$\$ (a\^2+b\^2)(c\^2+d\^2)\ge(ac+bd)\^2 \$\$
:::

当且仅当 [\$ad=bc\$]{.arithmatex} 时，等号成立。

示例：已知实数 [\$x,y\$]{.arithmatex} 满足 [\$x\^2+y\^2=3\$]{.arithmatex}，求 [\$5x+2y\$]{.arithmatex} 的最大值。根据柯西不等式：
...
所以 [\$5x+2y\$]{.arithmatex} 的最大值为 [\$\sqrt{87}\$]{.arithmatex}。
...
示例：已知实数 [\$x,y\>0\$]{.arithmatex} 满足 [\$\frac{1}{x}+\frac{4}{y}=1\$]{.arithmatex}，求 [\$x+y\$]{.arithmatex} 的最小值。根据权方和不等式：
...
所以 [\$x+y\$]{.arithmatex} 的最小值为 [\$9\$]{.arithmatex}。

## （三）排序不等式 {#_5}

### 基本形式 {#_6}

对于非严格单调递增（或递减）的实数序列 [\$x,y\$]{.arithmatex}，另 [\$\sigma(i)\$]{.arithmatex} 表示 [\$1\sim n\$]{.arithmatex} 的任意一个排列，有，

::: {.arithmatex}
\$\$ x_1y_1+\dots+x_ny_n\ge x\_{\sigma(1)}y_1+\dots+x\_{\sigma(n)}y_n\ge x_ny_1+\dots+x_1y_n \$\$
:::

顺序和不小于乱序和，乱序和不小于逆序和；取等为 [\$x,y\$]{.arithmatex} 分别两两相等。
...
若 [\$x_1,x_2\dots,x_n\ge-1\$]{.arithmatex} 且 [\$x_2,x_2,\dots,x_n\$]{.arithmatex} 同号，则
...
设 [\$a_1,a_2,\cdots,a_n\>0，b_1,b_2,\cdots,b_n\>0，r\in\mathbb{R}\$]{.arithmatex}，则

- 当 [\$r\ge0\$]{.arithmatex}，或 [\$r\le-1\$]{.arithmatex} 时，有

  ::: {.arithmatex}
  \$\$ \frac{a_1\^{r+1}}{b_1\^{r}}+\frac{a_2\^{r+1}}{b_2\^{r}}+\cdots+\frac{a_n\^{r+1}}{b_n\^{r}}\ge\frac{(a_1+a_2+\cdots+a_n)\^{r+1}}{(b_1+b_2+\cdots+b_n)\^{r}}。 \$\$
  :::

- 当 [\$-1\<r\<0\$]{.arithmatex} 时，有
...
若 [\$a,b\>0，p\>1，\dfrac{1}{p}+\dfrac{1}{q}=1\$]{.arithmatex}，则有
