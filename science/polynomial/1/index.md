...
- 可记为 {a_i}_{i\\in\\mathbb N} 或 (a_i)_{i\\in\\mathbb N} 或 \\langle a_i\\rangle\_{i\\in\\mathbb N}。
- 一个数列 a 的第 i 项，通常记为 a(i)，简记为 a_i。

有限数列：

- 若 I_n={1,2,\\dots,n}，则一个 (a:I_n\\to\\mathbb C) 的函数被称为**有限数列**。
- 可记为 {a_i}_{i=1}^n 或 (a_i)_{i=1}^n 或 \\langle a_i\\rangle\_{i=1}^n。
...
**通项公式**：表示 n 和 a_n 的关系的公式，叫做 a 的**通项公式**。
...
**递推公式**：表示 a_n 和 a_n 的前一或前几项的关系的公式，叫做 a 的**递推公式**。
...
- s_0=a_0，
- s_n=s\_{n-1}+a_n(\\forall n\\in\\mathbb Z^\*)
...
- 戴德金-皮亚诺结构：
  ...
  1. (\\forall b\\in S)(\\forall c\\in S)((f(b)=f(c))\\Rightarrow(b=c))
  1. (\\forall a\\in S)(f(a)\\neq e)
  1. (\\forall A\\subseteq S)(((e\\in A)\\wedge(\\forall a\\in A)(f(a)\\in A))\\Rightarrow(A=S))
...
对于 a\_{n-k},a_n,a\_{n+k} 有意义。据此，同理，
...
对于等差数列的前 n 项和，也可以将其构造为等差数列。等差数列 {a_n}，设公差为 d，若前 n 项和为 S_n，则 {S_m,S\_{2m}-S_m,S\_{3m}-S\_{2m}} 仍构成等差数列，公差为 m^2d。

如果数列 {a_n} 为等差数列，{a_n} 的前 n 项和 S_n 有如下结论：

- 若 a_1 < 0, d > 0，且此时 n 满足 \\begin{cases} a_n \\le 0 \\ a\_{n+1} \\ge 0 \\end{cases}，则 S_n 有最小值；
- 若 a_1 > 0, d < 0，且此时 n 满足 \\begin{cases} a_n \\ge 0 \\ a\_{n+1} \\le 0 \\end{cases}，则 S_n 有最大值。
...
- 数列 {a_n} 的前 n 项和为 S_n，若有如下项：f(n)S_n，f(a_n)S_n(即 S_n 的系数跟 n 有关)，则我们将递推式中的 a_n 改写为 S_n - S\_{n-1}。
- 若数列 {a_n} 的递推式形如 a\_{n+1} = a_n + f(n)，则可采用累加法求通项公式。
- 若数列 {a_n} 的递推式形如 a\_{n+1} = a_n \\cdot f(n)，则可采用累乘法求通项公式。
...
- 对于 a\_{n+1} = p a_n^r (p > 0, p \\ne 1, r \\ne 1) 型，两边同时取以 p 为底的对数，于是可得 \\log_p a\_{n+1} = r \\log_p a_n + 1，构造等比数列 {\\log_p a_n + x}，其中 x = \\dfrac{1}{r-1}。
...
对于 a\_{n+1} = \\dfrac{a a_n}{b a_n + c}，abc \\ne 0，取倒数得 \\dfrac{1}{a\_{n+1}} = \\dfrac{b a_n + c}{a a_n} = \\dfrac{c}{a} \\cdot \\dfrac{1}{a_n} + \\dfrac{b}{a}。

- 当 a=c 时，\\dfrac{1}{a\_{n+1}} = \\dfrac{1}{a_n} + \\dfrac{c}{a}，则 {\\dfrac{1}{a_n}} 为等差数列；
- 当 a \\ne c 时，\\dfrac{1}{a\_{n+1}} = \\dfrac{c}{a} \\cdot \\dfrac{1}{a_n} + \\dfrac{b}{a}，则 {\\dfrac{1}{a_n} + x} 为等比数列，x = \\dfrac{b}{c-a}。

### 变形累加

容易得出，下面的式子不断乘二，

\\begin{aligned} a_n=2a\_{n-1}+1\\ 2a\_{n-1}=4a\_{n-2}+2\\ 4a\_{n-2}=8a\_{n-3}+4\\ \\dots\\ 2^{n-3}a_3=2^{n-2}a_2+2^{n-3}\\ 2^{n-2}a_2=2^{n-1}a_1+2^{n-2} \\end{aligned}
...
- 定义法（用于证明）：对于 n \\ge 2 的任意正整数，验证 a_n - a\_{n-1} 或 \\dfrac{a_n}{a\_{n-1}} 为同一常数。
...
- 中项公式法（用于证明）：
  ...
  若 2a_n = a\_{n-1} + a\_{n+1}(n \\ge 2, n \\in \\mathbb{N}^\*)，则 {a_n} 为等差数列；

  若 a_n^2 = a\_{n-1}a\_{n+1}(n \\ge 2, n \\in \\mathbb{N}^\*)，则 {a_n} 为等比数列。
...
- 若 a\_{n+1}\\ge a_n，那么称 a 为**单调递增**数列。
- 若 a\_{n+1}>a_n，那么称 a 为**严格单调递增**数列。
- 若 a\_{n+1}\\le a_n，那么称 a 为**单调递减**数列。
- 若 a\_{n+1}\<a_n，那么称 a 为**严格单调递减**数列。

高中阶段一般认为单调即严格单调。

- 对于迭代数列 a\_{n+1} = f(a_n)，其中 f'(x) > 0。若 a_1 < a_2，则数列 {a_n} 单调递增；若 a_1 > a_2，则数列 {a_n} 单调递减；若 a_1 = a_2，则数列 {a_n} 是常数列。
