...
- 如果一个多项式函数 f(x)=a_n x^n + a\_{n-1}x^{n-1}+\\dots+a_1 x+a_0 具有中心对称性，那么它的对称中心为

  \\left(-\\frac{a\_{n-1}}{na_n},f\\left(-\\frac{a\_{n-1}}{na_n}\\right)\\right)

- 如果一个多项式函数 f(x)=a_n x^n + a\_{n-1}x^{n-1}+\\dots+a_1 x+a_0 具有轴对称性，那么它的对称轴为

  x=-\\frac{a\_{n-1}}{na_n}

- f(x)=ax^3+bx^2+cx+d 的对称中心为
...
- 对任意的 x\\in\\R，若 f(x+a)\\cdot f(x)=c (c \\neq 0)，则 T=2|a|。
...
对于一元二次方程 ax^2+bx+c=0 (a\\neq0)，其根的判别式 \\Delta=b^2-4ac。当 \\Delta \\ge 0 时，记 x_1,x_2 为方程的两个实根，则有韦达定理：
...
同时，一定情况下也可以用参变分离解决，例如方程 ax^2+2x+1=0 至少有一个正根，则：
...
如果要求方程 f(x)=ax^2+bx+c=0 (a\\neq0) 有两个大于 r 的实根，容易想到，构造函数 g(x)=f(x+r)，对 g(x) 应用两个实根大于 0 的结论，但是这样比较复杂，我们进行一次推导：
...
二次方程 f(x) = ax^2 + bx + c = 0 (a \\neq 0) 在区间 (p,q) 和 (m,n) 内各有一个实数根，若 a 的正负不确定，则其等价条件为
...
> 三次函数 f(x)=ax^3+bx^2+cx+d (a\\neq 0) 的图像中心对称，且其对称中心为 \\left(-\\dfrac{b}{3a},f\\left(-\\dfrac{b}{3a}\\right)\\right)。
...
因此，我们只需要判断 \\Delta 的符号，即可判断函数 f(x) 的极值点个数了。除了极值点，单调性也是可以比较快得出的，另外还有比较朴素的零点解法，就是解出两个极值点（横坐标）进行分类讨论。但是这种方法比较麻烦，而且一般来说不便于含参讨论，因此我们引入下面的方法。
...
> 如果在考场上，题目给定了一个具体的外点 P(2, 5) 和一个复杂的函数 f(x)=x^3-3x^2+2x+1，你先求拐点，再写拐点切线，然后再去列两个不等式组判断点 P 到底在哪个区域……这不仅计算量没有减少，反而增加了极高的出错率。这时候，这套方法确实显得“useless”（毫无用处），直接用“设切点求根法”反而最稳。
...
记住一个原则：**已知点求条数，用代数法；已知条数求范围，用平面划分法。**在大学解析几何里，遇到复杂的二次曲线或者三次曲线，第一件事就是**把坐标原点平移到对称中心（即拐点）**。\*\*判断一个点 P(X_0, Y_0) 在哪个区域，代数上怎么判断？\*\*你甚至不需要画图了，只需要算两个数：

1. Y_0 - (aX_0^3 + kX_0)（代表点在曲线的上方还是下方）
1. Y_0 - kX_0（代表点在切线的上方还是下方）

- 如果 **值 1 × 值 2 < 0** （一正一负，说明夹在曲线和切线中间），**必定有 3 条 切线**！
- 如果 **值 1 × 值 2 > 0** （同正或同负，说明在外面），**必定有 1 条 切线**！
- 如果 **值 1 × 值 2 = 0** （在边界上），除原点外是 **2 条** 切线。

**评价**：这个不等式 (Y_0 - 曲线)(Y_0 - 切线) < 0 将你所担心的“计算几何”完全代数化、傻瓜化了！有时压轴题的第一问让你证明：过曲线某一点的切线，与曲线有且仅有另一个交点。第二问让你证明，在某条线段上的任意一点只能做一条切线。如果你不知道平面划分法，你会在卷子上写满求导符号。如果你知道，你一眼就能看出题目给的那条线段，其实完全落在了“1 条切线区”（即曲线与拐点切线外部）。虽然你不能在高考卷子上直接写“由平面划分法可知”，但**你已经知道了答案和方向，你在构建不等式的时候，就是朝着证明“点在切线上方且在曲线上方”去凑的**。

考场策略：

1. **正向计算题**：坚决用“设切点，列方程，解重根”的代数法。
1. **含参取值范围题（尤其是点在直线上动）**：立刻画图，标出曲线和拐点切线，看动点轨迹穿过了哪个区域，找边界交点。
1. **遇到含有参数，代数极难处理的条数判断题**：把坐标系原点平移到拐点，用 (Y - aX^3 - kX)(Y - kX) 的正负号直接暴力破解！
...
1. 从 g(x) = k 中反解出 x，记为 x = h(k)，即 k \\le f[h(k)] < k+1。
...
1. **补项**：观察缺失的“线性项”。本题中，左边有 e^{x-k} 缺少 (x-k)，右边有 \\ln x 缺少 x。于是**同时加上 x**，刚好补齐了左右两边的线性部分。
1. **换元**：尝试将 \\ln x 看作一个整体 u，那么 x 就变成了 e^u。此时 \\ln x + x 变成了 u + e^u，瞬间与左边的 e^{x-k} + (x-k) 结构对称。
...
**题目**：已知 f(x)f'(x) + x > 1 对所有 x \\in \\mathbb{R} 成立，讨论 [f(1)]^2 与 [f(-1)]^2 的关系。
...
由已知条件，g'(x) = f(x)f'(x) + x - 1 > 0，因此 g(x) 在 \\mathbb{R} 上严格单调递增。
...
\\int\_{-1}^{1} \\left( f(x)f'(x) + x \\right) \\mathrm{d}x > \\int\_{-1}^{1} 1 \\mathrm{d}x
...
[f(1)]^2 + 0 > [f(-1)]^2 + 4 \\implies \\boxed{[f(1)]^2 - [f(-1)]^2 > 4}
...
1. 构造 g(x) = e^{2x}f(x)。
1. 由 f'(x) + 2f(x) > 0 两边乘 e^{2x} 得 [e^{2x}f(x)]' > 0。
1. g(x) 单调递增。g(x) > g(0) \\implies e^{2x}f(x) > 1 \\implies f(x) > e^{-2x}。
1. 解集为 (0, +\\infty)。
...
1. 构造 g(x) = x^2 f(x)。
1. 验证：g'(x) = 2xf(x) + x^2 f'(x) = x(2f(x) + xf'(x))。在 x>0 时，符号与原条件一致。
...
1. g(x) 单调递增。
1. g(1) > g(0) \\implies e^{1/2}f(1) > e^0 \\cdot f(0) = 1。
1. f(1) > e^{-1/2}。
...
1. **判定“严格单调递增”的充要条件（终极法则）**：f(x) 在 I 上严格单调递增 \\iff 在 I 的内部，**f'(x) \\ge 0，且 f'(x) 在任何子区间上都不恒为 0**。
...
- 直线 x = x\_{0} 是曲线 y = f(x) 的垂直渐近线，当且仅当
