...
- 函数 \$f(x)=a^x+a^\$ 是偶函数，函数 \$g(x)=a^x-a^\$ 是单调的奇函数。
...
- 如果一个多项式函数 \$f(x)=a_n x\^n + a\_{n-1}x\^{n-1}+\dots+a_1 x+a_0\$ 具有轴对称性，那么它的对称轴
...
\$\$ \begin{cases} \Delta\>0\\ x_1+x_2\>2r\\ af®\>0 \end{cases} \$\$
...
- 除法法则的逆向：
  ...
  \$\$ \dfrac{f\'(x) \cdot e\^x - f(x) \cdot (e^x)\'}{e^ \> 0 \$\$}
...
5.  \$g(x)\$ 单调递增。
6.  \$g(1) \> g(0) \implies e\^{½}f(1) \> e\^0 \cdot f(0) = 1\$。
7.  \$f(1) \> e\^{-½}\$。
...
- **经典例子**：\$f(x) = \sqrt\[3\]{x}\$ （即 \$x\^{⅓}\$）它在 \$x=0\$ 处不可导（切线垂直于 x 轴，\$f\'(0)\$ 为无穷大）。但在 \$(-\infty, 0)\$ 和 \$(0, +\infty)\$ 上，\$f\'(x) \> 0\$。因为 \$f(x)\$ 在 \$x=0\$ 处连续，所以它在整个 \$(-\infty, +\infty)\$ 上严格单调递增，不需要在 0 处断开。
...
\$\$ a+1=\dfrac{ax_0e^{x_0}}{2x_0-1}=\dfrac{e^ \$\$}(x_0+1)}{e\^{x_0}(x_0+1)-2

所以：

\$\$ a=\dfrac{e^{x_0}(x_0+1)}{e^ \$\$}(x_0+1)-2}\cdot\dfrac{2x_0-1}{x_0e\^{x_0}
...
- 任意的 \$x_1 \in A\$，任意的 \$x_2 \in B\$ 使得 \$f(x_1) \le g(x_2)\$，则 \$f(x)*{\max} \le g(x)*\$；

- 任意的 \$x_1 \in A\$，任意的 \$x_2 \in B\$ 使得 \$f(x_1) \ge g(x_2)\$，则 \$f(x)*{\min} \ge g(x)*\$。

- 对任意的 \$x_1 \in A\$，存在 \$x_2 \in B\$ 使得 \$f(x_1) \le g(x_2)\$，则 \$f(x)*{\max} \le g(x)*\$；

- 对任意的 \$x_1 \in A\$，存在 \$x_2 \in B\$ 使得 \$f(x_1) \ge g(x_2)\$，则 \$f(x)*{\min} \ge g(x)*\$。

- 存在 \$x_1 \in A\$，存在 \$x_2 \in B\$ 使得 \$f(x_1) \le g(x_2)\$，则 \$f(x)*{\min} \le g(x)*\$；

- 存在 \$x_1 \in A\$，存在 \$x_2 \in B\$ 使得 \$f(x_1) \ge g(x_2)\$，则 \$f(x)*{\max} \ge g(x)*\$。

绝对值问题：

- 对任意的 \$x_1 \in A\$，任意的 \$x_2 \in B\$ 有 \$\|f(x_1) - g(x_2)\| \le a\$，等价于 \$g(x)*{\max} - f(x)* \le a\$ 且 \$f(x)*{\max} - g(x)* \le a\$。

- 对任意的 \$x_1 \in A\$，任意的 \$x_2 \in B\$ 有 \$\|f(x_1) - g(x_2)\| \ge a\$，等价于 \$g(x)*{\min} - f(x)* \ge a\$ 或 \$f(x)*{\min} - g(x)* \ge a\$。

- 若存在 \$x_1 \in A\$，存在 \$x_2 \in B\$ 使得 \$\|f(x_1) - g(x_2)\| \le a\$，等价于 \$g(x)*{\min} - f(x)* \le a\$ 且 \$f(x)*{\min} - g(x)* \le a\$。

- 若存在 \$x_1 \in A\$，存在 \$x_2 \in B\$ 使得 \$\|f(x_1) - g(x_2)\| \ge a\$，等价于 \$g(x)*{\max} - f(x)* \ge a\$ 或 \$f(x)*{\max} - g(x)* \ge a\$。

- 若存在 \$x_1, x_2 \in A\$，有 \$\|f(x_1) - f(x_2)\| \ge a\$，等价于 \$f(x)*{\max} - f(x)* \ge a\$。

- 对任意 \$x_1 \in A\$，存在 \$x_2 \in B\$ 使得 \$\|f(x_1) - g(x_2)\| \le a\$，等价于 \$g(x)*{\min} - a \le f(x)*\$ 且 \$f(x)*{\max} \le a + g(x)*\$。

- 对任意 \$x_1 \in A\$，存在 \$x_2 \in B\$ 使得 \$\|f(x_1) - g(x_2)\| \ge a\$，等价于 \$g(x)*{\max} - f(x)* \ge a\$ 或 \$f(x)*{\min} - g(x)* \ge a\$。
