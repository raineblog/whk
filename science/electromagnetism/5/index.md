...
![alt text](../image-31.avif){align="right" width="40%"}

为了形象地描绘磁场的方向和强弱，与电场线类似，可以绘制磁场线即磁感应线，简称磁感线。一方面，磁感线上每一点的切线方向都跟这点的磁感应强度 []{.arithmatex}$B$ 的方向一致；另一方面，为了比较 []{.arithmatex}$B$ 的大小，使单位面积的磁感线数目与 []{.arithmatex}$B$ 的大小成正比，即 []{.arithmatex}$B$ 较大处磁感线密集，[]{.arithmatex}$B$ 较小处稀疏。

为了研究问题的方便，人们还引入一个叫做磁通量的物理量。设在磁感应强度为 []{.arithmatex}$B$ 的匀强磁场中或在一个非匀强磁场中可以视为匀强的小区域中，有一个与磁场方向垂直的平面，面积为 []{.arithmatex}$S$，则 []{.arithmatex}$B$ 和 []{.arithmatex}$S$ 的乘积就叫做穿过这个面积的磁通量，简称磁通。用字母 []{.arithmatex}$\Phi$ 表示，即：
...
如果磁场方向不与我们研究的平面垂直，而是有一个夹角 []{.arithmatex}$\alpha$，我们可用这个面在垂直于磁场方向的投影面积 []{.arithmatex}$S'$ 与 []{.arithmatex}$B$ 的乘积表示磁通量，即：
...
在国际单位制中，磁通量的单位是韦伯，符号是 []{.arithmatex}$\text{Wb}$，[]{.arithmatex}$1\,\text{Wb} = 1\,\text{T}\cdot\text{m}^2$。

![alt text](../image-32.avif){width="80%"}

**特别强调**：虽然磁通量是标量但是有正负，这里符号并非矢量中的方向的意思，而代表穿入或穿出，规定一个方向为正，正负可以相互抵消。如图所示，套在条形磁铁外的三个线圈，其面积 []{.arithmatex}$S_1>S_2=S_3$，设各线圈中的磁通量**记为** []{.arithmatex}$\Phi_1,\Phi_2,\Phi_3$，则它们的大小关系是？

![alt text](../image-24.avif){width="40%"}

容易知道，每个圈内，磁铁内的磁通量是相同的，因此只需要考虑磁铁外、圈内的与磁铁内的抵消。容易发现，外面的磁通量 []{.arithmatex}$\Phi_1'>\Phi_2'>\Phi_3'$。因此总的磁通量 []{.arithmatex}$\Phi_1<\Phi_2<\Phi_3$。

### 安培环路定律 {#_4}

我们把磁感强度沿着某一条线段的分量乘以这段线段的长度，叫做这条线段的**磁通**。如果是一个闭合的回路，那么回路上所有线段的磁通加起来，叫做这个闭合回路磁感强度的环流。

安培环路定律指出，**任何一个包围长直载流导线的闭合回路，若有一根电流为 []{.arithmatex}**$I$ 的长直导线垂直于回路所在平面，则该回路的环流是 []{.arithmatex}$\mu_0 I$。推而广之，如果有多根电流通过闭合回路，那么考虑电流就是所有电流的代数和，环路所包围电流的正负应服从右手定则。

:::: {.arithmatex}
::: {.MathJax_Preview}
\oint_C \mathbf{B} \cdot \mathrm{d}\boldsymbol{\ell} = \mu_0 I\_{\text{enc}}
:::

$$\oint_C \mathbf{B} \cdot \mathrm{d}\boldsymbol{\ell} = \mu_0 I_{\text{enc}}$$
::::

我们常常**联立方程**，可以用于求 []{.arithmatex}$B$。
...
![alt text](../image-25.avif){width="100%"}
...
![alt text](../image-41.avif){width="100%"}
...
此定律预测磁场如何与电路相互作用以产生电动势，这种现象称为电磁感应，其方向可以由楞次定律判断。在试题中，通过这个方法算出来的电动势通常只是关于时间的平均值；如果能证明电动势是定值，那么才可以得出其为电动势的准确值；如果取极限 []{.arithmatex}$\Delta t\to0$ 则也可以说是某一时刻的瞬时电动势，我们展开 []{.arithmatex}$\Delta\Phi=\Delta(BS)=B\Delta S+S\Delta B+\Delta B\Delta S$ 也可以理解这个说法。

重点辨析：电磁感应定律给出的是电动势，而不是"电势差"，实际我们用电动势除以总电阻得到电流，而电流与电阻乘积才能得到电势差，这与电池内阻类似。如果我们将导体棒看作电源，那么电磁感应定律得出的结果就是电动势，电源内阻就是导体棒的阻值。

对于一个匝数为 []{.arithmatex}$n$ 的线圈，可以看作 []{.arithmatex}$n$ 个线圈串联，因此：

:::: {.arithmatex}
::: {.MathJax_Preview}
E\_{感}=n\cdot\dfrac{\Delta\Phi}{\Delta t}
:::

$$E_{感}=n\cdot\dfrac{\Delta\Phi}{\Delta t}$$
::::

特殊的，如果给出一个线圈，问磁通量变化，我们是不需要乘 []{.arithmatex}$n$ 的，因为 []{.arithmatex}$n$ 在这个公式里面是在 []{.arithmatex}$\Delta\Phi$ 外面的，也就是说我们默认问线圈的磁通量变化，是考虑线圈"看起来"的面积。
...
![alt text](../image-26.avif){width="40%"}
...
其中 []{.arithmatex}$B,\ell,v$ 两两垂直，[]{.arithmatex}$\ell$ 为有效长度，即首尾相连的直线距离、并在 []{.arithmatex}$\bm v$ 方向上的投影。
...
如果面积不变（[]{.arithmatex}$\Delta S=0$）则推导公式：
...
![alt text](../image-39.avif){width="70%"}
...
![alt text](../image-27.avif){width="30%"}
...
式中 []{.arithmatex}$L$ 是比例系数，叫作**自感系数**，简称自感或电感，单位是亨利，简称亨，符号是 \$\pu{H}。不同的线圈，电感大小一般不同。它与线圈的大小、形状、匝数，以及**是否有铁芯**等因素有关。

![alt text](../image-42.avif){width="40%"}

开关断开后，灯泡的发光还能维持一小段时间，有时甚至会比开关断开之前更亮。有时自感电动势会大于原来电路中的电源电动势。

- 若开始时 []{.arithmatex}$I_L>I_A$，即当线圈的直流电阻 []{.arithmatex}$R_L<R_A$ 时，会出现灯 []{.arithmatex}$A$ 会闪亮一下的现象。

- 若开始时 []{.arithmatex}$I_L\le I_A$，即当线圈的直流电阻 []{.arithmatex}$R_L\ge R_A$ 时，不会出现灯 []{.arithmatex}$A$ 会闪亮一下的现象。
...
图 15.36 是日光灯的电路图，它主要由灯管、镇流器和启动器组成。日光灯不是靠它的灯丝发光的，在灯管内充有稀薄的汞汽，当汞汽导电时，汞原子受激发而产生紫外线，涂在管壁上的荧光粉吸收紫外线，发出可见光。各种荧光粉发出不同颜色的光，混合起来使人有白光的感觉。激发汞汽导电所需要的电压比 []{.arithmatex}$220\text{ V}$ 的电源电压高得多，而在汞汽一旦被激发发光后，灯管内的电阻立刻变小，此时只需较低的电压就能维持汞汽导电发光的状态，这个电压比电源电压小得多。这两方面的要求都是利用跟灯管串联的镇流器来完成的，镇流器是一个自感系数很大的有铁心的线圈。启动器是一个充有氖气的装置，通电后能很快地自动断电。开关闭合后，灯管内气体的电阻极大，相当于断路，所以电流通过串联的镇流器、灯丝和启动器，但启动器很快就突然断电，使镇流器产生很大的自感电动势，和电源的电压一起加在灯管两端的灯丝上，使汞汽被激发导电，灯管发光。灯管正常发光后，灯管和镇流器成串联状态，镇流器对交流电产生很大的阻碍作用（下一章我们将研究这个问题），分掉一部分电压，使灯管两端的电压降低到维持正常发光所需的电压，这样就保证了日光灯的正常工作。

::: {.grid markdown=""}
![alt text](../image-43.avif){width="80%"}

![alt text](../image-44.avif){width="60%"}
:::

自感现象也有不利的一面。例如，在自感系数很大而电流有很强的电路（如大型电动机中产生磁场的电磁铁电路）中，在切断电路的瞬间，会产生很高的自感电动势，使开关的闸刀和固定夹片之间的空气被激发产生电离而导电，形成电弧。这样会把开关烧毁，甚至造成其他灾害。为了防止这类现象，最好是把开关放在绝缘性能良好的油中。又如，电阻箱中的电阻通常都是用电阻线绕制而成的，为了避免绕线的自感影响电路，制作时采用双线绕法，如图 15.37 所示。
...
:::: {.arithmatex}
::: {.MathJax_Preview}
\begin{aligned} F\_{安}=BIL\\ I=\dfrac{E}{R+r}\\ E=B\ell v \end{aligned}
:::
...
:::: {.arithmatex}
::: {.MathJax_Preview}
F\_{安}=\dfrac{BL\ell v}{R+r}
:::

$$F_{安}=\dfrac{BL\ell v}{R+r}$$
::::

如果 []{.arithmatex}$L=\ell$（导线垂直于速度），那么可以写 []{.arithmatex}$L^2$。

容易知道，导线棒受到一个与恒力 []{.arithmatex}$F$ 反向的安培力，那么她会做一个加速度减小的加速运动。
...
通电导体棒在磁场中运动，我们取一个极小的时间 []{.arithmatex}$\Delta t$，则

:::: {.arithmatex}
::: {.MathJax_Preview}
\begin{aligned} F\_{安}\cdot\Delta t&=m\Delta v\\ BIL\cdot\Delta t&=m\Delta v\\ BLq&=m\Delta v \end{aligned}
:::
...
我们用积分的思想，就可以得到通过导体棒对电荷量即为 []{.arithmatex}$\dfrac{m(v_t-v_0)}{BL}$。

我们总结一下求电荷量的方式：

:::: {.arithmatex}
::: {.MathJax_Preview}
\begin{aligned} q&=N\dfrac{\Delta\Phi}{R+r}\\ q&=\dfrac{m(v_t-v_0)}{BL} \end{aligned}
:::
...
已知前进的位移 []{.arithmatex}$\Delta x$ 就可以用这个方式。

同时我们还可以将 []{.arithmatex}$q$ 联立，得到
...
一根长度为 []{.arithmatex}$\ell$ 的导线棒在磁场强度为 []{.arithmatex}$B$ 的匀强磁场中、垂直于磁感线的平面上绕着其一个端点旋转，角速度为 []{.arithmatex}$\omega$，则这根导线棒的电动势为
...
证明：取一时间 []{.arithmatex}$\Delta t$，则导线棒转过的扇形面积：
...
![alt text](../image-40.avif){width="40%"}
...
:::: {.arithmatex}
::: {.MathJax_Preview}
\begin{aligned} W_电&=Q+W_机\\ UIt&=I\^2Rt+W_机 \end{aligned}
:::
...
- 我们声称 []{.arithmatex}$R$ 为电动机线圈的电阻，而 []{.arithmatex}$U,I$ 是根据实际测量得到的。
...
![alt text](../image-4x.avif){width="80%"}
