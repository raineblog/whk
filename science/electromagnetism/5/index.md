...
![alt text](/static/image/image-31.12f3f2555a.avif){align="right" width="40%"}
...
在国际单位制中，磁通量的单位是韦伯，符号是 $text\{Wb}$，$1,\text\{Wb} = 1,\text\{T}\cdot\text\{m}^2$。

![alt text](/static/image/image-32.3d20cbb698.avif){width="80%"}
**特别强调**：虽然磁通量是标量但是有正负，这里符号并非矢量中的方向的意思，而代表穿入或穿出，规定一个方向为正，正负可以相互抵消。如图所示，套在条形磁铁外的三个线圈，其面积 $S\_1>S\_2=S\_3$，设各线圈中的磁通量**记为** $Phi\_1,\Phi\_2,\Phi\_3$，则它们的大小关系是？

![alt text](/static/image/image-24.9554355500.avif){width="40%"}
容易知道，每个圈内，磁铁内的磁通量是相同的，因此只需要考虑磁铁外、圈内的与磁铁内的抵消。容易发现，外面的磁通量 $Phi\_1'>\Phi\_2'>\Phi\_3'$。因此总的磁通量 $Phi\_1\<\Phi\_2\<\Phi\_3$。

### 安培环路定律 \{#\_4}

我们把磁感强度沿着某一条线段的分量乘以这段线段的长度，叫做这条线段的**磁通**。如果是一个闭合的回路，那么回路上所有线段的磁通加起来，叫做这个闭合回路磁感强度的环流。

安培环路定律指出，**任何一个包围长直载流导线的闭合回路，若有一根电流为 $I$ 的长直导线垂直于回路所在平面，则该回路的环流是 $mu\_0 I$**。推而广之，如果有多根电流通过闭合回路，那么考虑电流就是所有电流的代数和，环路所包围电流的正负应服从右手定则。

$$
\oint\_C \mathbf\{B} \cdot \mathrm\{d}\boldsymbol\{\ell} = \mu\_0 I\_\{\text\{enc}}
$$
...
![](/static/svg/Faraday_emf_experiment.b26eeb9d12.svg){width="80%"}
...
![alt text](/static/image/image-25.0c225181ad.avif){width="100%"}
...
![alt text](/static/image/image-41.645d1d414e.avif){width="100%"}
注意事项：
...
![alt text](/static/image/image-26.e52a347559.avif){width="40%"}
...
![alt text](/static/image/image-39.a4b7e3d8fb.avif){width="70%"}
...
![alt text](/static/image/image-27.ecd6078ef3.avif){width="30%"}
...
![alt text](/static/image/image-42.4f8b05592d.avif){width="40%"}
开关断开后，灯泡的发光还能维持一小段时间，有时甚至会比开关断开之前更亮。有时自感电动势会大于原来电路中的电源电动势。

- 若开始时 $I\_L>I\_A$，即当线圈的直流电阻 $R\_L\<R\_A$ 时，会出现灯 $A$ 会闪亮一下的现象。

- 若开始时 $I\_L\le I\_A$，即当线圈的直流电阻 $R\_L\ge R\_A$ 时，不会出现灯 $A$ 会闪亮一下的现象。
...
图 15.36 是日光灯的电路图，它主要由灯管、镇流器和启动器组成。日光灯不是靠它的灯丝发光的，在灯管内充有稀薄的汞汽，当汞汽导电时，汞原子受激发而产生紫外线，涂在管壁上的荧光粉吸收紫外线，发出可见光。各种荧光粉发出不同颜色的光，混合起来使人有白光的感觉。激发汞汽导电所需要的电压比 $220\text\{ V}$ 的电源电压高得多，而在汞汽一旦被激发发光后，灯管内的电阻立刻变小，此时只需较低的电压就能维持汞汽导电发光的状态，这个电压比电源电压小得多。这两方面的要求都是利用跟灯管串联的镇流器来完成的，镇流器是一个自感系数很大的有铁心的线圈。启动器是一个充有氖气的装置，通电后能很快地自动断电。开关闭合后，灯管内气体的电阻极大，相当于断路，所以电流通过串联的镇流器、灯丝和启动器，但启动器很快就突然断电，使镇流器产生很大的自感电动势，和电源的电压一起加在灯管两端的灯丝上，使汞汽被激发导电，灯管发光。灯管正常发光后，灯管和镇流器成串联状态，镇流器对交流电产生很大的阻碍作用（下一章我们将研究这个问题），分掉一部分电压，使灯管两端的电压降低到维持正常发光所需的电压，这样就保证了日光灯的正常工作。

![alt text](/static/image/image-43.982c36c423.avif){width="80%"}
![alt text](/static/image/image-44.814e5ef113.avif){width="60%"}
...
我们用积分的思想，就可以得到通过导体棒对电荷量即为 $dfrac\{m(v\_t-v\_0)}\{BL}$。

我们总结一下求电荷量的方式：

$$
\begin\{aligned} q&=N\dfrac\{\Delta\Phi}\{R+r}\ q&=\dfrac\{m(v\_t-v\_0)}\{BL} \end\{aligned}
$$
...
![alt text](/static/image/image-40.02172c80e7.avif){width="40%"}
...
$$
\begin\{aligned} W\_电&=Q+W\_机\ UIt&=I^2Rt+W\_机 \end\{aligned}
$$
...
![alt text](/static/image/image-4x.9f5c3e4329.avif){width="80%"}
所以，电路方程是：
