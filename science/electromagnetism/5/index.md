...
为了研究问题的方便，人们还引入一个叫做磁通量的物理量。设在磁感应强度为 *B* 的匀强磁场中或在一个非匀强磁场中可以视为匀强的小区域中，有一个与磁场方向垂直的平面，面积为 *S*，则 *B* 和 *S* 的乘积就叫做穿过这个面积的磁通量，简称磁通。用字母 *Φ* 表示，即：

*Φ* = *B\*\*S*

如果磁场方向不与我们研究的平面垂直，而是有一个夹角 *α*，我们可用这个面在垂直于磁场方向的投影面积 *S*′ 与 *B* 的乘积表示磁通量，即：

*Φ* = *B\*\*S*′ = *B\*\*S*cos *α*

在国际单位制中，磁通量的单位是韦伯，符号是 Wb，1Wb = 1T ⋅ m2。

**特别强调**：虽然磁通量是标量但是有正负，这里符号并非矢量中的方向的意思，而代表穿入或穿出，规定一个方向为正，正负可以相互抵消。如图所示，套在条形磁铁外的三个线圈，其面积 *S*1 > *S*2 = *S*3，设各线圈中的磁通量**记为** *Φ*1, *Φ*2, *Φ*3，则它们的大小关系是？

容易知道，每个圈内，磁铁内的磁通量是相同的，因此只需要考虑磁铁外、圈内的与磁铁内的抵消。容易发现，外面的磁通量 *Φ*1′ > *Φ*2′ > *Φ*3′。因此总的磁通量 *Φ*1 \< *Φ*2 \< *Φ*3。

### 安培环路定律

我们把磁感强度沿着某一条线段的分量乘以这段线段的长度，叫做这条线段的**磁通**。如果是一个闭合的回路，那么回路上所有线段的磁通加起来，叫做这个闭合回路磁感强度的环流。

安培环路定律指出，**任何一个包围长直载流导线的闭合回路，若有一根电流为 *I* 的长直导线垂直于回路所在平面，则该回路的环流是 *μ*0*I***。推而广之，如果有多根电流通过闭合回路，那么考虑电流就是所有电流的代数和，环路所包围电流的正负应服从右手定则。
...
此定律预测磁场如何与电路相互作用以产生电动势，这种现象称为电磁感应，其方向可以由楞次定律判断。在试题中，通过这个方法算出来的电动势通常只是关于时间的平均值；如果能证明电动势是定值，那么才可以得出其为电动势的准确值；如果取极限 *Δ\*\*t* → 0 则也可以说是某一时刻的瞬时电动势，我们展开 *Δ\*\*Φ* = *Δ*(*B\*\*S*) = *B**Δ**S* + *S**Δ**B* + *Δ**B**Δ\*\*S* 也可以理解这个说法。
...
特殊的，如果给出一个线圈，问磁通量变化，我们是不需要乘 *n* 的，因为 *n* 在这个公式里面是在 *Δ\*\*Φ* 外面的，也就是说我们默认问线圈的磁通量变化，是考虑线圈“看起来”的面积。
...
其中 *B*, *ℓ*, *v* 两两垂直，*ℓ* 为有效长度，即首尾相连的直线距离、并在 **v** 方向上的投影。
...
- 若开始时 *I\*\*L* > *I\*\*A*，即当线圈的直流电阻 *R\*\*L* \< *R\*\*A* 时，会出现灯 *A* 会闪亮一下的现象。
- 若开始时 *I\*\*L* ≤ *I\*\*A*，即当线圈的直流电阻 *R\*\*L* ≥ *R\*\*A* 时，不会出现灯 *A* 会闪亮一下的现象。
...
图 15.36 是日光灯的电路图，它主要由灯管、镇流器和启动器组成。日光灯不是靠它的灯丝发光的，在灯管内充有稀薄的汞汽，当汞汽导电时，汞原子受激发而产生紫外线，涂在管壁上的荧光粉吸收紫外线，发出可见光。各种荧光粉发出不同颜色的光，混合起来使人有白光的感觉。激发汞汽导电所需要的电压比 220 V 的电源电压高得多，而在汞汽一旦被激发发光后，灯管内的电阻立刻变小，此时只需较低的电压就能维持汞汽导电发光的状态，这个电压比电源电压小得多。这两方面的要求都是利用跟灯管串联的镇流器来完成的，镇流器是一个自感系数很大的有铁心的线圈。启动器是一个充有氖气的装置，通电后能很快地自动断电。开关闭合后，灯管内气体的电阻极大，相当于断路，所以电流通过串联的镇流器、灯丝和启动器，但启动器很快就突然断电，使镇流器产生很大的自感电动势，和电源的电压一起加在灯管两端的灯丝上，使汞汽被激发导电，灯管发光。灯管正常发光后，灯管和镇流器成串联状态，镇流器对交流电产生很大的阻碍作用（下一章我们将研究这个问题），分掉一部分电压，使灯管两端的电压降低到维持正常发光所需的电压，这样就保证了日光灯的正常工作。
...
一根长度为 *ℓ* 的导线棒在磁场强度为 *B* 的匀强磁场中、垂直于磁感线的平面上绕着其一个端点旋转，角速度为 *ω*，则这根导线棒的电动势为

$$ E=\\dfrac{1}{2}B\\ell^2\\omega $$

证明：取一时间 *Δ\*\*t*，则导线棒转过的扇形面积：
