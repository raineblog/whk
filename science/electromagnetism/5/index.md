...
为了形象地描绘磁场的方向和强弱，与电场线类似，可以绘制磁场线即磁感应线，简称磁感线。一方面，磁感线上每一点的切线方向都跟这点的磁感应强度 [\$B\$]{.arithmatex} 的方向一致；另一方面，为了比较 [\$B\$]{.arithmatex} 的大小，使单位面积的磁感线数目与 [\$B\$]{.arithmatex} 的大小成正比，即 [\$B\$]{.arithmatex} 较大处磁感线密集，[\$B\$]{.arithmatex} 较小处稀疏。

为了研究问题的方便，人们还引入一个叫做磁通量的物理量。设在磁感应强度为 [\$B\$]{.arithmatex} 的匀强磁场中或在一个非匀强磁场中可以视为匀强的小区域中，有一个与磁场方向垂直的平面，面积为 [\$S\$]{.arithmatex}，则 [\$B\$]{.arithmatex} 和 [\$S\$]{.arithmatex} 的乘积就叫做穿过这个面积的磁通量，简称磁通。用字母 [\$\Phi\$]{.arithmatex} 表示，即：

::: {.arithmatex}
\$\$ \Phi = BS \$\$
:::

如果磁场方向不与我们研究的平面垂直，而是有一个夹角 [\$\alpha\$]{.arithmatex}，我们可用这个面在垂直于磁场方向的投影面积 [\$S\'\$]{.arithmatex} 与 [\$B\$]{.arithmatex} 的乘积表示磁通量，即：

::: {.arithmatex}
\$\$ \Phi = BS\' = BS\cos\alpha \$\$
:::

在国际单位制中，磁通量的单位是韦伯，符号是 [\$\text{Wb}\$]{.arithmatex}，[\$1\\\text{Wb} = 1\\\text{T}\cdot\text{m}\^2\$]{.arithmatex}。

![alt text](../image-32.avif){width="80%"}

**特别强调**：虽然磁通量是标量但是有正负，这里符号并非矢量中的方向的意思，而代表穿入或穿出，规定一个方向为正，正负可以相互抵消。如图所示，套在条形磁铁外的三个线圈，其面积 [\$S_1\>S_2=S_3\$]{.arithmatex}，设各线圈中的磁通量**记为** [\$\Phi_1,\Phi_2,\Phi_3\$]{.arithmatex}，则它们的大小关系是？

![alt text](../image-24.avif){width="40%"}

容易知道，每个圈内，磁铁内的磁通量是相同的，因此只需要考虑磁铁外、圈内的与磁铁内的抵消。容易发现，外面的磁通量 [\$\Phi_1\'\>\Phi_2\'\>\Phi_3\'\$]{.arithmatex}。因此总的磁通量 [\$\Phi_1\<\Phi_2\<\Phi_3\$]{.arithmatex}。

### 安培环路定律 {#_4}

我们把磁感强度沿着某一条线段的分量乘以这段线段的长度，叫做这条线段的**磁通**。如果是一个闭合的回路，那么回路上所有线段的磁通加起来，叫做这个闭合回路磁感强度的环流。

安培环路定律指出，**任何一个包围长直载流导线的闭合回路，若有一根电流为 [\$I\$]{.arithmatex} 的长直导线垂直于回路所在平面，则该回路的环流是 [\$\mu_0 I\$]{.arithmatex}**。推而广之，如果有多根电流通过闭合回路，那么考虑电流就是所有电流的代数和，环路所包围电流的正负应服从右手定则。

::: {.arithmatex}
\$\$ \oint_C \mathbf{B} \cdot \mathrm{d}\boldsymbol{\ell} = \mu_0 I\_{\text{enc}} \$\$
:::

我们常常**联立方程**，可以用于求 [\$B\$]{.arithmatex}。
...
此定律预测磁场如何与电路相互作用以产生电动势，这种现象称为电磁感应，其方向可以由楞次定律判断。在试题中，通过这个方法算出来的电动势通常只是关于时间的平均值；如果能证明电动势是定值，那么才可以得出其为电动势的准确值；如果取极限 [\$\Delta t\to0\$]{.arithmatex} 则也可以说是某一时刻的瞬时电动势，我们展开 [\$\Delta\Phi=\Delta(BS)=B\Delta S+S\Delta B+\Delta B\Delta S\$]{.arithmatex} 也可以理解这个说法。

重点辨析：电磁感应定律给出的是电动势，而不是"电势差"，实际我们用电动势除以总电阻得到电流，而电流与电阻乘积才能得到电势差，这与电池内阻类似。如果我们将导体棒看作电源，那么电磁感应定律得出的结果就是电动势，电源内阻就是导体棒的阻值。

对于一个匝数为 [\$n\$]{.arithmatex} 的线圈，可以看作 [\$n\$]{.arithmatex} 个线圈串联，因此：

::: {.arithmatex}
\$\$ E\_{感}=n\cdot\dfrac{\Delta\Phi}{\Delta t} \$\$
:::

特殊的，如果给出一个线圈，问磁通量变化，我们是不需要乘 [\$n\$]{.arithmatex} 的，因为 [\$n\$]{.arithmatex} 在这个公式里面是在 [\$\Delta\Phi\$]{.arithmatex} 外面的，也就是说我们默认问线圈的磁通量变化，是考虑线圈"看起来"的面积。
...
其中 [\$B,\ell,v\$]{.arithmatex} 两两垂直，[\$\ell\$]{.arithmatex} 为有效长度，即首尾相连的直线距离、并在 [\$\bm v\$]{.arithmatex} 方向上的投影。
...
如果面积不变（[\$\Delta S=0\$]{.arithmatex}）则推导公式：
...
式中 [\$L\$]{.arithmatex} 是比例系数，叫作**自感系数**，简称自感或电感，单位是亨利，简称亨，符号是 \$\pu{H}。不同的线圈，电感大小一般不同。它与线圈的大小、形状、匝数，以及**是否有铁芯**等因素有关。

![alt text](../image-42.avif){width="40%"}

开关断开后，灯泡的发光还能维持一小段时间，有时甚至会比开关断开之前更亮。有时自感电动势会大于原来电路中的电源电动势。

- 若开始时 [\$I_L\>I_A\$]{.arithmatex}，即当线圈的直流电阻 [\$R_L\<R_A\$]{.arithmatex} 时，会出现灯 [\$A\$]{.arithmatex} 会闪亮一下的现象。

- 若开始时 [\$I_L\le I_A\$]{.arithmatex}，即当线圈的直流电阻 [\$R_L\ge R_A\$]{.arithmatex} 时，不会出现灯 [\$A\$]{.arithmatex} 会闪亮一下的现象。
...
图 15.36 是日光灯的电路图，它主要由灯管、镇流器和启动器组成。日光灯不是靠它的灯丝发光的，在灯管内充有稀薄的汞汽，当汞汽导电时，汞原子受激发而产生紫外线，涂在管壁上的荧光粉吸收紫外线，发出可见光。各种荧光粉发出不同颜色的光，混合起来使人有白光的感觉。激发汞汽导电所需要的电压比 [\$220\text{ V}\$]{.arithmatex} 的电源电压高得多，而在汞汽一旦被激发发光后，灯管内的电阻立刻变小，此时只需较低的电压就能维持汞汽导电发光的状态，这个电压比电源电压小得多。这两方面的要求都是利用跟灯管串联的镇流器来完成的，镇流器是一个自感系数很大的有铁心的线圈。启动器是一个充有氖气的装置，通电后能很快地自动断电。开关闭合后，灯管内气体的电阻极大，相当于断路，所以电流通过串联的镇流器、灯丝和启动器，但启动器很快就突然断电，使镇流器产生很大的自感电动势，和电源的电压一起加在灯管两端的灯丝上，使汞汽被激发导电，灯管发光。灯管正常发光后，灯管和镇流器成串联状态，镇流器对交流电产生很大的阻碍作用（下一章我们将研究这个问题），分掉一部分电压，使灯管两端的电压降低到维持正常发光所需的电压，这样就保证了日光灯的正常工作。
...
如果 [\$L=\ell\$]{.arithmatex}（导线垂直于速度），那么可以写 [\$L\^2\$]{.arithmatex}。

容易知道，导线棒受到一个与恒力 [\$F\$]{.arithmatex} 反向的安培力，那么她会做一个加速度减小的加速运动。
...
通电导体棒在磁场中运动，我们取一个极小的时间 [\$\Delta t\$]{.arithmatex}，则

::: {.arithmatex}
\$\$ \begin{aligned} F\_{安}\cdot\Delta t&=m\Delta v\\ BIL\cdot\Delta t&=m\Delta v\\ BLq&=m\Delta v \end{aligned} \$\$
:::

我们用积分的思想，就可以得到通过导体棒对电荷量即为 [\$\dfrac{m(v_t-v_0)}{BL}\$]{.arithmatex}。
...
我们假设匝数 [\$N=1\$]{.arithmatex}，然后化简上式

::: {.arithmatex}
\$\$ q=\dfrac{BL\Delta x}{R+r} \$\$
:::

已知前进的位移 [\$\Delta x\$]{.arithmatex} 就可以用这个方式。

同时我们还可以将 [\$q\$]{.arithmatex} 联立，得到
...
一根长度为 [\$\ell\$]{.arithmatex} 的导线棒在磁场强度为 [\$B\$]{.arithmatex} 的匀强磁场中、垂直于磁感线的平面上绕着其一个端点旋转，角速度为 [\$\omega\$]{.arithmatex}，则这根导线棒的电动势为

::: {.arithmatex}
\$\$ E=\dfrac{1}{2}B\ell\^2\omega \$\$
:::

证明：取一时间 [\$\Delta t\$]{.arithmatex}，则导线棒转过的扇形面积：
...
- 我们声称 [\$R\$]{.arithmatex} 为电动机线圈的电阻，而 [\$U,I\$]{.arithmatex} 是根据实际测量得到的。
