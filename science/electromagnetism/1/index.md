...
其中 *c*0 表示光速，*ε*0 为介电常数，因此：

*k\*\*e* ≈ 9 × 109 Nm2C−2

这个数是很大的。

介质极化后，束缚电荷也要产生电场，该电场与外电场的方向相反，两电场叠加的结果是介质内部的电场减弱。在真空中场强为 *E*0 的区域内充满电介质后，场强会减小到 *E*，其比值就叫这种电介质的介电常数（亦称相对介电常数），用 *ε* 表示：

$$ \\varepsilon=\\dfrac{E_0}{E} $$

显然，真空的介电常数 *ε*0 = 1，而空气的非常接近 1（可以看做真空），而水中可以达到约 80。
...
公式表达：一条直线上一次有电荷量为 *q*1, *q*2, *q*3 的点电荷，其间距分别为 *ℓ*1, *ℓ*2，若三点电荷平衡：
...
即 *q*1 + *q*2 → (*q*1 + *q*2)/2 + (*q*1 + *q*2)/2 的公式表达。
