...
![](../%E9%AA%8C%E7%94%B5%E5%99%A8%E4%B8%8E%E9%9D%99%E7%94%B5%E8%AE%A1.avif){align="right" width="30%"}
...
电荷量的国际单位是库仑（[\$\text C\$]{.arithmatex}），通常用符号 [\$Q\$]{.arithmatex} 表示。

**元电荷**（也成基本电荷）：基本电荷 [\$e\$]{.arithmatex} 是一个质子所带的**电荷量**，或一个电子所带的负**电荷量**。其中 [\$e\approx 1.602\times 10\^{-19}\text C\$]{.arithmatex}，一个电子携带的电荷量为 [\$-e\$]{.arithmatex}。

为什么是小数？历史上，安培（电流强度）是先于库仑定义的，最初的库伦定义就是**安培秒**，即 [\$1\$]{.arithmatex} 库仑是 [\$1\$]{.arithmatex} 安培电流在 [\$1\$]{.arithmatex} 秒钟内传递的电荷，表示为 [\$1\text C=1\mathrm{A\cdot s}\$]{.arithmatex} 或 [\$1\text C=1\text{A}\times1\text{s}\$]{.arithmatex}，而旧的安培定义为「在相距 [\$1\$]{.arithmatex} 米的两条平行导线间，产生每米 [\$0.2\$]{.arithmatex} 微牛顿的磁力所需的电流」。

比荷、比电荷（又称荷质比）：即带电物体所带电荷比上其质量，电子比荷约为 [\$1.758820024(11)\times 10\^{11}\mathrm{C/kg}\$]{.arithmatex}，质子的比荷约为 [\$9.578309\times 10\^7\mathrm{C/kg}\$]{.arithmatex}。
...
电荷必须依赖载流子存在，一个载流子的电量绝对值为 [\$e\$]{.arithmatex}，因此任何物体（夸克的分数电荷作为例外）的电量都是 [\$e\$]{.arithmatex} 的整数倍。
...
![alt text](../fimage-5.avif){width="40%"}
...
理论上 [\$Q\$]{.arithmatex} 应该带正负号，但是如果列标量方程，一般不带入正负号，表示库仑力的大小。
...
当两带电体之间的距离相对于两带电体的半径（考虑近似成球体）很大时，我们才能将两带电体看作点电荷：设两带电体距离为 [\$r\$]{.arithmatex}，两带电体半径分别为 [\$R\$]{.arithmatex}，必须有 [\$r\gg R\$]{.arithmatex}。

一般情况下微观粒子的大小都远远小于彼此之间的距离，因此微观粒子在计算静电力时一般都可以看做点电荷。

正因如此，[\$r\to0\$]{.arithmatex} 时静电力趋于无穷大这种说法是错误的，两带电体不能看成点电荷（两球体之间相距过近时，两球内部的电量分布会影响静电力的大小），此时库仑定律已不再适用。
...
其中 [\$c_0\$]{.arithmatex} 表示光速，[\$\varepsilon_0\$]{.arithmatex} 为介电常数，因此：
...
介质极化后，束缚电荷也要产生电场，该电场与外电场的方向相反，两电场叠加的结果是介质内部的电场减弱。在真空中场强为 [\$E_0\$]{.arithmatex} 的区域内充满电介质后，场强会减小到 [\$E\$]{.arithmatex}，其比值就叫这种电介质的介电常数（亦称相对介电常数），用 [\$\varepsilon\$]{.arithmatex} 表示：

::: {.arithmatex}
\$\$ \varepsilon=\dfrac{E_0}{E} \$\$
:::

显然，真空的介电常数 [\$\varepsilon_0=1\$]{.arithmatex}，而空气的非常接近 [\$1\$]{.arithmatex}（可以看做真空），而水中可以达到约 [\$80\$]{.arithmatex}。
...
- 设 [\$F_0\$]{.arithmatex} 表示球心等价点电荷的库仑力。
- 若两球电性相同，则 [\$F\<F_0\$]{.arithmatex}；
- 若两球电性不同，则 [\$F\>F_0\$]{.arithmatex}。

理解方式，本来提供库仑力的电荷因为感应，存在有相互远离或靠近的趋势。不考虑球内电荷的相互作用力变化，根据库仑力与 [\$r\$]{.arithmatex} 的平方成反比，可以得出上述结论。
...
公式表达：一条直线上一次有电荷量为 [\$q_1,q_2,q_3\$]{.arithmatex} 的点电荷，其间距分别为 [\$\ell_1,\ell_2\$]{.arithmatex}，若三点电荷平衡：
...
形成等边三角形后，按照 [\$1:1:2\$]{.arithmatex} 的关系，库仑力等大同向，此时给一个匀强电场即可平衡。

证明：暴力展开。

### 原子核问题 {#_16}

已知氢核（质子）的质量为 [\$1.67\times10\^{-27}\pu{kg}\$]{.arithmatex}，电子的质量为 [\$9.1\times10\^{-31}\pu{kg}\$]{.arithmatex}，在氢原子内它们之间的最短距离为 [\$5.3\times10\^{-11}\pu{m}\$]{.arithmatex}。试比较氢核和核外电子之间的库仑力和万有引力。

氢核和电子所带的电量都是 [\$1.6\times10\^{-19}\pu{C}\$]{.arithmatex}。
...
![摩擦生电](../%E6%91%A9%E6%93%A6%E7%94%9F%E7%94%B5.avif)
...
即 [\$q_1+q_2\to(q_1+q_2)/2+(q_1+q_2)/2\$]{.arithmatex} 的公式表达。

注意到这是符合电荷守恒定律的。

### 感应起电 {#_20}

![感应起电](../Electroscope_showing_induction.avif){align="right" width="50%"}
...
感应起电接地模型：基础模型题目描述形如，带电物体 [\$\text C\$]{.arithmatex} 靠近枕形导体 [\$\text{AB}\$]{.arithmatex} 的 [\$\text A\$]{.arithmatex} 端，将导体 [\$\mathrm{A/B}\$]{.arithmatex} 端接地（或者用手触碰一下），然后拿开或不拿开 [\$\text{C}\$]{.arithmatex}，问导体 [\$\mathrm{A},\text{B}\$]{.arithmatex} 两端分别带什么电荷。

性质：导体任意一端接地，效果相同。  
解释：因为是导体，所以电荷自由移动，完全不需要考虑导体的两端的差异。

性质：如果拿开 [\$\text{C}\$]{.arithmatex}，则导体两端电荷相同。  
解释：显然。
...
<th style="text-align: center;">导体 <span class="arithmatex">$\text A$</span> 端</th>
<th style="text-align: center;">导体 <span class="arithmatex">$\text B$</span> 端</th>
</tr>
</thead>
<tbody>
<tr>
<td style="text-align: center;">移开 <span class="arithmatex">$\text{C}$</span> 物体</td>
<td style="text-align: center;">负电荷</td>
<td style="text-align: center;">负电荷</td>
</tr>
<tr>
<td style="text-align: center;">不移开 <span class="arithmatex">$\text{C}$</span> 物体</td>
<td style="text-align: center;">负电荷</td>
<td style="text-align: center;">不带电</td>
</tr>
</tbody>
</table>
<figcaption><p>假设 <span class="arithmatex">${C}$</span> 带正点，且移开物体 <span class="arithmatex">${C}$</span> 前已经停止接地</p></figcaption>
</figure>
...
![空球壳模型](../%E7%A9%BA%E7%90%83%E5%A3%B3%E6%A8%A1%E5%9E%8B.svg){width="100%"}
...
![静电屏蔽](../%E9%9D%99%E7%94%B5%E5%B1%8F%E8%94%BD.svg){width="100%"}
...
![alt text](../fimage-6.avif){width="80%"}
...
宇宙中充斥着各种各样的等离子体，是其最常见的物质相态，亦可以经由对处于其他相态的物质的加工取得。等离子体是宇宙重子物质最常见的形态，其中大部分存在于稀薄的星系际空间（特别是星系团内介质）和恒星之中。地球大气离地表 [\$300\$]{.arithmatex} 公里的电离层也是处于等离子态，电离层是地球大气较外层的气体吸收了太阳辐射能量，发生光致游离而形成。
...
|:--:|:--:|:--:|
| 行走在地毯上 | [\$\pu{35,000V}\$]{.arithmatex} | [\$\pu{1,500V}\$]{.arithmatex} |
| 行走在塑料地板 | [\$\pu{12,000V}\$]{.arithmatex} | [\$\pu{250V}\$]{.arithmatex} |
| 在工作台边工作 | [\$\pu{6,000V}\$]{.arithmatex} | [\$\pu{100V}\$]{.arithmatex} |
| 从工作台拿起塑料袋 | [\$\pu{20,000V}\$]{.arithmatex} | [\$\pu{1,200V}\$]{.arithmatex} |
| 发泡胶椅 | [\$\pu{18,000V}\$]{.arithmatex} | [\$\pu{1,500V}\$]{.arithmatex} |
...
![alt text](../fimage-4.avif){width="40%"}
