...
电场线用于描述电场，是不存在的，有场的方向（用箭头表示）和大小（用疏密表示）两个要素。场的方向与试探点电荷（正电荷）在该点所受电场力的方向相同，与该点电场强度 []{.arithmatex}$E$ 方向相同。
...
电场强度是用来表示电场的强弱和方向的物理量，常用 []{.arithmatex}$E$ 表示。
...
国际单位为**伏特每米** []{.arithmatex}$\mathrm{V/m}$ 或**牛顿每库仑** []{.arithmatex}$\mathrm{N/C}$（这两个单位实际上相等），常用单位有伏特每厘米 []{.arithmatex}$\mathrm{V/cm}$。
...
其中 []{.arithmatex}$Q$ 表示的是场源电荷。

### 高斯定理 {#_8}

**电通量**：穿过电场 []{.arithmatex}$E$ 中某一截面 []{.arithmatex}$\Delta S$ 的电通量 []{.arithmatex}$\Delta\varphi_e$ 被定义为：
...
式中 []{.arithmatex}$\theta$ 为截面 []{.arithmatex}$\Delta S$ 的法线与电场线间的夹角，它实际上可理解为穿过 []{.arithmatex}$\Delta S$ 的电场线的条数。
...
当电场线穿过一个有限大小曲面 []{.arithmatex}$S$ 时，其电通量等于其上无限多个小面元上的电通量的代数和： 其中 []{.arithmatex}$E$ 为闭合面 []{.arithmatex}$S$ 上每一个面元 []{.arithmatex}$\Delta S$ 处的电场强度，这个 []{.arithmatex}$E$ 是由所有形成静电场的电荷（包括 []{.arithmatex}$S$ 面内外的电荷）在该面元 []{.arithmatex}$\Delta S$ 处激发产生的电场强度。而等号右边的电荷代数和，仅指 []{.arithmatex}$S$ 面内的电荷。

**高斯定理**：静电场中穿过任意闭合曲面（称为高斯面，例如一个球壳）[]{.arithmatex}$S$ 的电通量可表示为：
...
式中 []{.arithmatex}$k$ 是静电力常量，[]{.arithmatex}$Q=\sum q$ 为闭合曲面内全部电荷的代数和。
...
设有半径为 []{.arithmatex}$R$ 的均匀带电球壳 []{.arithmatex}$A$，带电量为 []{.arithmatex}$Q$，研究 []{.arithmatex}$A$ 球壳内部的电场强度，可以建立一个半径为 []{.arithmatex}$r(r<R)$ 的 []{.arithmatex}$B$ 球壳作为高斯面（[]{.arithmatex}$B$ 球壳和 []{.arithmatex}$A$ 球壳同心）。因为 []{.arithmatex}$B$ 球壳内部的净电荷为零，又因为 []{.arithmatex}$A$、[]{.arithmatex}$B$ 两个球壳高度对称，[]{.arithmatex}$B$ 球壳处如果有电场线一定是垂直于 []{.arithmatex}$B$ 球壳的，所以 []{.arithmatex}$A$ 球壳内部的所有地方电场强度都为零。即
...
研究 []{.arithmatex}$A$ 球壳外部的电场强度，可以建立一个半径为 []{.arithmatex}$r(r>R)$ 的 []{.arithmatex}$B$ 球壳作为高斯面（[]{.arithmatex}$B$ 球壳和 []{.arithmatex}$A$ 球壳同心）。因为 []{.arithmatex}$B$ 球壳内部的净电荷为 []{.arithmatex}$Q$，而且 []{.arithmatex}$A$、[]{.arithmatex}$B$ 两个球壳高度对称，[]{.arithmatex}$B$ 球壳处的电场线处处垂直于 []{.arithmatex}$B$ 球壳，所以有：
...
这是很类似重力势能 []{.arithmatex}$W_G=mgH$ 的。
...
**电势差**：用字母 []{.arithmatex}$U$ 表示，单位为伏特 []{.arithmatex}$\text V$，有表达式
...
如以离场源电荷无穷远处（即 []{.arithmatex}$r_b\to\infty$）电荷的电势能为零，则电荷 []{.arithmatex}$q$ 在离场源电荷 []{.arithmatex}$r(r_a=r)$ 处的电势能就是：
...
根据电势的定义，很容易得出：当以无穷远处电势为零时，点电荷电场中离场源电荷为 []{.arithmatex}$r$ 处的各点的电势为：
...
可见，点电荷电场中某点的电势与场源电荷的电量 []{.arithmatex}$Q$ 成正比，与该点到场源电荷的距离 []{.arithmatex}$r$ 成反比。
...
- 假设有正电荷 []{.arithmatex}$q$，沿着电场线移动，则 []{.arithmatex}$W=Fx=Eqx$ 为正功，电势能 []{.arithmatex}$E_p=\varphi q$ 减小，因为 []{.arithmatex}$q$ 是正数，所以 []{.arithmatex}$\varphi$ 减小。
- 假设有负电荷 []{.arithmatex}$q$，沿着电场线移动，则 []{.arithmatex}$W=Fx=Eqx$ 为负功，电势能 []{.arithmatex}$E_p=\varphi q$ 增大，因为 []{.arithmatex}$q$ 是负数，所以 []{.arithmatex}$\varphi$ 减小。
...
比较电势能就是比较电势，因为 []{.arithmatex}$E_p=\varphi q$，其中 []{.arithmatex}$q$ 为定值。

回忆：比较电场强度直接放试探点电荷，注意电场强度是矢量，要比较方向。

### 图像问题 {#_17}

做题方法：在 []{.arithmatex}$x$ 轴上画出电场线

- []{.arithmatex}$E-x$ 图像：注意到 []{.arithmatex}$E$ 的正负就表示电场线方向，因此只需要找曲线零点分段即可。
- []{.arithmatex}$\varphi-x$ 图像：注意到 []{.arithmatex}$\varphi$ 递增的方向就是电场线的方向，因此只需要分段找单调区间即可。

特殊性质：在 []{.arithmatex}$\varphi-x$ 图像中，一点的曲线斜率大小即为该点的场强大小。

证明：根据 []{.arithmatex}$\varphi_A-\varphi_B=U_{AB}=Ed$ 显然。

## 电场的描述 {#_18}

### 电偶极子 {#_19}

真空中一对相距为 []{.arithmatex}$l$，带等量异号电荷的点电荷系统 []{.arithmatex}$(-q,+q)$，且 []{.arithmatex}$l$ 远小于讨论中所涉及的距离，这样的电荷体系称为电偶极子，连接两电荷的直线称为电偶极子的轴线。

将电偶极子中电荷的电量 []{.arithmatex}$q$ 与两点电荷间距 []{.arithmatex}$l$ 的乘积定义为电偶极矩，即：
...
电偶极矩是矢量，方向与 []{.arithmatex}$l$ 的方向一致，由 []{.arithmatex}$-q$ 指向 []{.arithmatex}$+q$。

### 微元电场叠加 {#_20}

根据力的合成原理，如果空间内一点受到不止一个电荷的影响，则他们所形成的电场可以叠加，因为电场是一个矢量，所以需要用矢量叠加原理来叠加。

通常去一个 []{.arithmatex}$\Delta l$，然后当成点电荷用库仑定律处理。
...
- 根据 []{.arithmatex}$\bm E_{总}=\bm0=\bm E_1+\bm E_{n-1}$，得出 []{.arithmatex}$\bm E_1=-\bm E_{n-1}$。
...
如图，如果从 []{.arithmatex}$A$ 点静止释放，在 []{.arithmatex}$B$ 点恰好速度为零，则 []{.arithmatex}$C$ 点一定是动能最高的点，[]{.arithmatex}$OC$ 方向即为等效重力场方向；根据对称性，[]{.arithmatex}$OC$ 一定是角平分线。
