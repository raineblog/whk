...
国际单位为**伏特每米** V/m 或**牛顿每库仑** N/C（这两个单位实际上相等），常用单位有伏特每厘米 V/cm。
...
**电通量**：穿过电场 *E* 中某一截面 *Δ\*\*S* 的电通量 *Δ**φ**e* 被定义为：

*Δ**φ**e* = **E** ⋅ *Δ***S** = *E**Δ**S*cos *θ*

式中 *θ* 为截面 *Δ\*\*S* 的法线与电场线间的夹角，它实际上可理解为穿过 *Δ\*\*S* 的电场线的条数。

*φ\*\*e* = ∑*Δ**S**E**i**Δ**S**i*cos *θ*

当电场线穿过一个有限大小曲面 *S* 时，其电通量等于其上无限多个小面元上的电通量的代数和： 其中 *E* 为闭合面 *S* 上每一个面元 *Δ\*\*S* 处的电场强度，这个 *E* 是由所有形成静电场的电荷（包括 *S* 面内外的电荷）在该面元 *Δ\*\*S* 处激发产生的电场强度。而等号右边的电荷代数和，仅指 *S* 面内的电荷。

**高斯定理**：静电场中穿过任意闭合曲面（称为高斯面，例如一个球壳）*S* 的电通量可表示为：

$$ \\varphi_e=4\\pi k\\sum q=\\dfrac{Q}{\\varepsilon_0} $$

式中 *k* 是静电力常量，*Q* = ∑*q* 为闭合曲面内全部电荷的代数和。
...
*W\*\*电* = **F***电* ⋅ **x** = *F**电**x*cos *θ* = *E**q**H*

这是很类似重力势能 *W\*\*G* = *m**g**H* 的。
...
如以离场源电荷无穷远处（即 *r\*\*b* → ∞）电荷的电势能为零，则电荷 *q* 在离场源电荷 *r*(*r\*\*a* = *r*) 处的电势能就是：
...
- 假设有正电荷 *q*，沿着电场线移动，则 *W* = *F\*\*x* = *E**q**x* 为正功，电势能 *E\*\*p* = *φ\*\*q* 减小，因为 *q* 是正数，所以 *φ* 减小。
- 假设有负电荷 *q*，沿着电场线移动，则 *W* = *F\*\*x* = *E**q**x* 为负功，电势能 *E\*\*p* = *φ\*\*q* 增大，因为 *q* 是负数，所以 *φ* 减小。
...
比较电势能就是比较电势，因为 *E\*\*p* = *φ\*\*q*，其中 *q* 为定值。
...
- *φ* − *x* 图像：注意到 *φ* 递增的方向就是电场线的方向，因此只需要分段找单调区间即可。

特殊性质：在 *φ* − *x* 图像中，一点的曲线斜率大小即为该点的场强大小。

证明：根据 *φ\*\*A* − *φ\*\*B* = *U**A**B* = *E\*\*d* 显然。
...
/// caption 图甲等价于金属板左侧对称点有一电量大小相等的相反电荷如图乙 ///
...
- 根据 **E***总* = **0** = **E**1 + **E***n* − 1，得出 **E**1 = −**E***n* − 1。
...
如图，如果从 *A* 点静止释放，在 *B* 点恰好速度为零，则 *C* 点一定是动能最高的点，*O\*\*C* 方向即为等效重力场方向；根据对称性，*O\*\*C* 一定是角平分线。
