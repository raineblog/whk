...
def submit_to_google_indexing(tasks: list[dict]) -> list[dict]:
    ...
    if not creds_json:
        print("[Google Indexing] ⚠️ 未配置 GOOGLE_INDEXING_CREDENTIALS 环境变量，跳过 Google Indexing 提交。")
        return tasks
    ...
    try:
        ...
        print(f"[Google Indexing] 正在使用 Google API 官方 Batch 方式发送 {len(tasks)} 个 URL 任务...")
        batch.execute()
